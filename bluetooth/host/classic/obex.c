...
static struct bt_buf *obex_alloc_buf(struct bt_obex *obex)
{
	struct bt_buf *buf;
...
static int obex_send(struct bt_obex *obex, uint16_t mopl, struct bt_buf *buf)
{
...
bool bt_obex_has_header(struct bt_buf *buf, uint8_t id)
{
	struct bt_obex_has_header data;
	int err;
...
struct server_handler {
	uint8_t opcode;
	uint16_t min_len;
	int (*handler)(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf);
};

static int obex_server_connect(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf)
{
	...
	if (bt_atomic_get(&server->_state) != BT_OBEX_DISCONNECTED) {
	...
	if (!bt_atomic_cas(&server->_opcode, 0, BT_OBEX_OPCODE_CONNECT)) {
		LOG_WRN("Unexpected conn request");
		rsp_code = BT_OBEX_RSP_CODE_FORBIDDEN;
		goto failed;
	}

	version = bt_buf_pull_u8(buf);
	flags = bt_buf_pull_u8(buf);
	mopl = bt_buf_pull_be16(buf);

	bt_atomic_set_bit_to(&server->_flags, BT_OBEX_HAS_TARGET,
			  bt_obex_has_header(buf, BT_OBEX_HEADER_ID_TARGET));

	err = bt_obex_get_header_target(buf, &target_len, &target_data);
	if (err != 0 && bt_atomic_test_bit(&server->_flags, BT_OBEX_HAS_TARGET)) {
	...
	bt_atomic_set(&server->_state, BT_OBEX_CONNECTING);
...
failed:
	...
	rsp_hdr = bt_buf_add(buf, sizeof(*rsp_hdr));
	rsp_hdr->code = rsp_code;
	rsp_conn_hdr = bt_buf_add(buf, sizeof(*rsp_conn_hdr));
...
static int obex_server_disconn(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf)
{
	struct bt_obex_rsp_hdr *rsp_hdr;
	uint8_t rsp_code;
	int err;
	...
	if (bt_atomic_get(&server->_state) != BT_OBEX_CONNECTED) {
	...
	if (!bt_atomic_cas(&server->_opcode, 0, BT_OBEX_OPCODE_DISCONN)) {
		LOG_WRN("Unexpected disconn request");
		rsp_code = BT_OBEX_RSP_CODE_FORBIDDEN;
		goto failed;
	}

	bt_atomic_set(&server->_state, BT_OBEX_DISCONNECTING);
				  ...
				  struct bt_buf *buf)
{
	...
	if (!bt_atomic_cas(&server->_opcode, 0, req_code)) {
		opcode = bt_atomic_get(&server->_opcode);
		...
		if ((opcode != req_code) && !bt_atomic_cas(&server->_opcode, opcode, req_code)) {
			LOG_WRN("OP code mismatch %u != %u", (uint8_t)bt_atomic_get(&server->_opcode),
				opcode);
...
static int obex_server_put(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf)
{
	return obex_server_put_common(server, false, len, buf);
}

static int obex_server_put_final(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf)
{
...
static int obex_server_get(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf)
{
	return obex_server_get_common(server, false, len, buf);
}

static int obex_server_get_final(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf)
{
	return obex_server_get_common(server, true, len, buf);
}

static int obex_server_setpath(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf)
{
	struct bt_obex_rsp_hdr *rsp_hdr;
	...
	if (!bt_atomic_cas(&server->_opcode, 0, BT_OBEX_OPCODE_SETPATH)) {
		LOG_WRN("Unexpected setpath request");
		rsp_code = BT_OBEX_RSP_CODE_FORBIDDEN;
		goto failed;
	}

	req_hdr = bt_buf_pull_mem(buf, sizeof(*req_hdr));
...
static int obex_server_action(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf)
{
...
static int obex_server_session(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf)
{
	struct bt_obex_rsp_hdr *rsp_hdr;
	uint8_t rsp_code;
	int err;
...
static int obex_server_abort(struct bt_obex_server *server, uint16_t len, struct bt_buf *buf)
{
	struct bt_obex_rsp_hdr *rsp_hdr;
	uint8_t rsp_code;
	int err;
	...
	bt_atomic_set(&server->_opcode, BT_OBEX_OPCODE_ABORT);
...
static struct bt_obex_server *obex_get_server_from_conn_id(struct bt_obex *obex, uint32_t conn_id)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&obex->_servers, server, next, _node) {
							 ...
							 enum bt_obex_state state)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&obex->_servers, server, next, _node) {
		if (server->uuid == NULL && bt_atomic_get(&server->_state) == state) {
			return server;
		}
	}
	return NULL;
...
static int obex_server_recv(struct bt_obex *obex, struct bt_buf *buf)
{
	...
	if (hdr->code == BT_OBEX_OPCODE_CONNECT) {
		...
		} else if (bt_atomic_ptr_get(&obex->_active_server) != NULL) {
			server = bt_atomic_ptr_get(&obex->_active_server);
		} else if (bt_slist_len(&obex->_servers) == 1) {
			/* Only if the count of servers is 1, use it as default. */
			server = OBEX_SERVER(bt_slist_peek_head(&obex->_servers));
		}
	}
	...
	ARRAY_FOR_EACH(server_handler, i) {
		if (server_handler[i].opcode == hdr->code) {
			...
			if (err != 0) {
				bt_atomic_ptr_clear(&obex->_active_server);
				LOG_WRN("Handler err %d", err);
			} else {
				/* Save the server */
				bt_atomic_ptr_set(&obex->_active_server, server);
			}
			return err;
		}
	}
...
failed:
	...
	rsp_hdr = (void *)bt_buf_add(buf, sizeof(*rsp_hdr));
	rsp_hdr->code = rsp_code;
	if (hdr != NULL && hdr->code == BT_OBEX_OPCODE_CONNECT) {
		struct bt_obex_conn_rsp_hdr *conn_rsp_hdr;

		conn_rsp_hdr = (void *)bt_buf_add(buf, sizeof(*conn_rsp_hdr));
			       ...
			       struct bt_buf *buf)
{
	...
	bt_atomic_clear(&client->_opcode);
	bt_atomic_ptr_clear(&client->obex->_active_client);

	if (bt_atomic_get(&client->_state) != BT_OBEX_CONNECTING) {
		LOG_WRN("Invalid connection state %u", (uint8_t)bt_atomic_get(&client->_state));
		goto failed;
	}
	...
	if (bt_atomic_test_bit(&client->_flags, BT_OBEX_HAS_TARGET) &&
	...
	bt_atomic_set(&client->_state,
		   rsp_code == BT_OBEX_RSP_CODE_SUCCESS ? BT_OBEX_CONNECTED : BT_OBEX_DISCONNECTED);

	if (bt_atomic_get(&client->_state) == BT_OBEX_DISCONNECTED) {
		bt_slist_find_and_remove(&client->obex->_clients, &client->_node);
	}

	bt_atomic_set(&client->_pre_opcode, BT_OBEX_OPCODE_CONNECT);
	bt_atomic_ptr_set(&client->obex->_last_client, client);
			       ...
			       struct bt_buf *buf)
{
	...
	if (bt_atomic_get(&client->_state) != BT_OBEX_DISCONNECTING) {
		return -EINVAL;
	}
	...
	bt_atomic_set(&client->_pre_opcode, BT_OBEX_OPCODE_DISCONN);
	bt_atomic_ptr_set(&client->obex->_last_client, client);
				  ...
				  struct bt_buf *buf)
{
	...
	if (bt_atomic_get(&client->_state) != BT_OBEX_CONNECTED) {
		LOG_WRN("Invalid state, connect is not established");
		return -EINVAL;
	}
	...
	if (rsp_code != BT_OBEX_RSP_CODE_CONTINUE) {
		bt_atomic_set(&client->_pre_opcode, bt_atomic_get(&client->_opcode));
		bt_atomic_ptr_set(&client->obex->_last_client, client);
		bt_atomic_clear(&client->_opcode);
		bt_atomic_ptr_clear(&client->obex->_active_client);
	}
			     ...
			     struct bt_buf *buf)
{
	...
	if (len != 0) {
		...
		handler = obex_client_find_handler(bt_atomic_get(&client->_pre_opcode));
		if (handler != NULL) {
	...
	bt_atomic_clear(&client->_pre_opcode);
	bt_atomic_ptr_clear(&client->obex->_last_client);
	bt_atomic_clear(&client->_opcode);
	bt_atomic_ptr_clear(&client->obex->_active_client);
...
static struct bt_obex_client *obex_get_last_client(struct bt_obex *obex)
{
	...
	client = bt_atomic_ptr_get(&obex->_last_client);
	if (client == NULL) {
		LOG_WRN("Last client not found");
		return NULL;
	}

	if (!bt_atomic_cas(&client->_opcode, 0, BT_OBEX_OPCODE_ABORT)) {
		LOG_WRN("opcode cannot be set");
		return NULL;
	}

	if (!bt_atomic_cas(&client->_pre_opcode, BT_OBEX_OPCODE_ABORT, 0)) {
		LOG_WRN("Last opcode is not ABORT");
		(void)bt_atomic_cas(&client->_opcode, BT_OBEX_OPCODE_ABORT, 0);
		return NULL;
	}
...
static int obex_client_recv(struct bt_obex *obex, struct bt_buf *buf)
{
	struct client_handler *handler;
	struct bt_obex_rsp_hdr *hdr;
	struct bt_obex_client *client = bt_atomic_ptr_get(&obex->_active_client);
	uint16_t len;
	int err;
	...
	hdr = bt_buf_pull_mem(buf, sizeof(*hdr));
	len = sys_be16_to_cpu(hdr->len);
	handler = obex_client_find_handler(bt_atomic_get(&client->_opcode));
	if (handler) {
...
int bt_obex_transport_connected(struct bt_obex *obex)
{
	...
	bt_atomic_ptr_clear(&obex->_active_client);
	bt_atomic_ptr_clear(&obex->_last_client);
...
int bt_obex_transport_disconnected(struct bt_obex *obex)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&obex->_clients, client, cnext, _node) {
		bt_atomic_clear(&client->_opcode);
		bt_atomic_clear(&client->_pre_opcode);
		bt_atomic_set(&client->_state, BT_OBEX_DISCONNECTED);
	}

	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&obex->_servers, server, snext, _node) {
		bt_atomic_clear(&server->_opcode);
		bt_atomic_set(&server->_state, BT_OBEX_DISCONNECTED);
	}

	bt_slist_init(&obex->_clients);
	return 0;
}
...
int bt_obex_recv(struct bt_obex *obex, struct bt_buf *buf)
{
...
int bt_obex_server_register(struct bt_obex_server *server, const struct bt_uuid_128 *uuid)
{
	...
	if (bt_atomic_get(&server->_state) != BT_OBEX_DISCONNECTED) {
		LOG_ERR("Invalid state, connect is %u", (uint8_t)bt_atomic_get(&server->_state));
		return -EINPROGRESS;
	}

	if (bt_slist_find(&server->obex->_servers, &server->_node, NULL)) {
		LOG_ERR("server %p has been registered", server);
		return -EALREADY;
	}

	if (!bt_slist_is_empty(&server->obex->_servers) && server->uuid == NULL) {
	...
	bt_slist_append(&server->obex->_servers, &server->_node);
...
int bt_obex_server_unregister(struct bt_obex_server *server)
{
	...
	bt_slist_find_and_remove(&server->obex->_servers, &server->_node);

	return 0;
}

int bt_obex_connect(struct bt_obex_client *client, uint16_t mopl, struct bt_buf *buf)
{
	...
	if (bt_atomic_get(&client->_state) != BT_OBEX_DISCONNECTED) {
		LOG_WRN("Invalid state, connect is %u", (uint8_t)bt_atomic_get(&client->_state));
		return -EINPROGRESS;
	}

	if (!bt_slist_is_empty(&client->obex->_clients) &&
	    !bt_obex_has_header(buf, BT_OBEX_HEADER_ID_TARGET)) {
		LOG_ERR("The Header target should be added");
		return -EINVAL;
	}

	if (!bt_atomic_ptr_cas(&client->obex->_active_client, NULL, client)) {
		LOG_WRN("One OBEX request is executing");
		return -EBUSY;
	}

	if (!bt_atomic_cas(&client->_opcode, 0, BT_OBEX_OPCODE_CONNECT)) {
		LOG_WRN("Operation inprogress");
		return -EBUSY;
	}

	bt_atomic_set_bit_to(&client->_flags, BT_OBEX_HAS_TARGET,
			  bt_obex_has_header(buf, BT_OBEX_HEADER_ID_TARGET));
	...
	bt_atomic_set(&client->_state, BT_OBEX_CONNECTING);

	if (!bt_slist_find(&client->obex->_clients, &client->_node, NULL)) {
		bt_slist_append(&client->obex->_clients, &client->_node);
	}

	req_hdr = bt_buf_push(buf, sizeof(*req_hdr));
	req_hdr->flags = 0;
			...
			struct bt_buf *buf)
{
	...
	if (bt_atomic_get(&server->_state) != BT_OBEX_CONNECTING) {
		LOG_WRN("Invalid state");
		return -ENOTCONN;
	}

	if (bt_atomic_get(&server->_opcode) != BT_OBEX_OPCODE_CONNECT) {
	...
	if (rsp_code == BT_OBEX_RSP_CODE_SUCCESS) {
		...
		if (bt_atomic_test_bit(&server->_flags, BT_OBEX_HAS_TARGET) !=
		    bt_obex_has_header(buf, BT_OBEX_HEADER_ID_WHO)) {
	...
	server->rx.mopl = mopl;
	if (rsp_code == BT_OBEX_RSP_CODE_SUCCESS) {
		old_state = bt_atomic_set(&server->_state, BT_OBEX_CONNECTED);
	} else {
		old_state = bt_atomic_set(&server->_state, BT_OBEX_DISCONNECTED);
	}

	rsp_hdr = bt_buf_push(buf, sizeof(*rsp_hdr));
	rsp_hdr->flags = 0;
	...
	err = obex_send(server->obex, server->tx.mopl, buf);
	if (err) {
		bt_atomic_set(&server->_state, old_state);
		...
		bt_atomic_clear(&server->_opcode);
		bt_atomic_ptr_clear(&server->obex->_active_server);
	}
	return err;
}

int bt_obex_disconnect(struct bt_obex_client *client, struct bt_buf *buf)
{
	struct bt_obex_req_hdr *hdr;
	int err;
	bool allocated = false;
	...
	if (!bt_atomic_cas(&client->_opcode, 0, BT_OBEX_OPCODE_DISCONN)) {
		LOG_WRN("Operation inprogress");
		return -EBUSY;
	}
	...
	if (err) {
		...
		bt_atomic_set(&client->_state, BT_OBEX_DISCONNECTING);
	}
	return err;
}

int bt_obex_disconnect_rsp(struct bt_obex_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	struct bt_obex_rsp_hdr *hdr;
	int err;
	bt_atomic_val_t old_state;
	...
	if (bt_atomic_get(&server->_state) != BT_OBEX_DISCONNECTING) {
		LOG_WRN("Invalid state");
		return -EINVAL;
	}

	if (bt_atomic_get(&server->_opcode) != BT_OBEX_OPCODE_DISCONN) {
		LOG_WRN("Invalid response");
		return -EINVAL;
	}
...
int bt_obex_put(struct bt_obex_client *client, bool final, struct bt_buf *buf)
{
	struct bt_obex_req_hdr *hdr;
	...
	active_client = bt_atomic_ptr_get(&client->obex->_active_client);
	if (!bt_atomic_ptr_cas(&client->obex->_active_client, NULL, client) &&
	...
	opcode = bt_atomic_get(&client->_opcode);

	req_code = final ? BT_OBEX_OPCODE_PUT_F : BT_OBEX_OPCODE_PUT;
	if (!bt_atomic_cas(&client->_opcode, 0, req_code)) {
		if ((opcode != BT_OBEX_OPCODE_PUT_F) && (opcode != BT_OBEX_OPCODE_PUT)) {
			LOG_WRN("Operation inprogress");
			return -EBUSY;
		}
		...
		if ((opcode != req_code) && !bt_atomic_cas(&client->_opcode, opcode, req_code)) {
			LOG_WRN("OP code mismatch %u != %u", (uint8_t)bt_atomic_get(&client->_opcode),
				opcode);
			return -EINVAL;
		}
	}
	...
	err = obex_send(client->obex, client->tx.mopl, buf);
	if (err) {
		bt_atomic_set(&client->_opcode, opcode);
		bt_atomic_ptr_set(&client->obex->_active_client, active_client);
...
int bt_obex_put_rsp(struct bt_obex_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	struct bt_obex_rsp_hdr *hdr;
	int err;
	uint8_t opcode;
...
int bt_obex_get(struct bt_obex_client *client, bool final, struct bt_buf *buf)
{
	struct bt_obex_req_hdr *hdr;
	struct bt_obex_client *active_client;
	int err;
...
int bt_obex_get_rsp(struct bt_obex_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	struct bt_obex_rsp_hdr *hdr;
	int err;
	uint8_t opcode;
...
int bt_obex_abort(struct bt_obex_client *client, struct bt_buf *buf)
{
	struct bt_obex_req_hdr *hdr;
	struct bt_obex_client *active_client;
	int err;
	...
	if (!bt_atomic_get(&client->_opcode)) {
		LOG_WRN("No operation is inprogress");
		return -EINVAL;
	} else if (bt_atomic_get(&client->_opcode) == BT_OBEX_OPCODE_ABORT) {
		LOG_WRN("Abort is inprogress");
		return -EINPROGRESS;
	} else if (!obex_op_support_abort((uint8_t)bt_atomic_get(&client->_opcode))) {
		LOG_WRN("Opcode %02x cannot be aborted", (uint8_t)bt_atomic_get(&client->_opcode));
		return -ENOTSUP;
	}
	...
	opcode = bt_atomic_set(&client->_opcode, BT_OBEX_OPCODE_ABORT);
	bt_atomic_clear(&client->_pre_opcode);
	...
	if (err) {
		...
		bt_atomic_set(&client->_pre_opcode, opcode);
	}
	return err;
}

int bt_obex_abort_rsp(struct bt_obex_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	struct bt_obex_rsp_hdr *hdr;
	int err;
	bool allocated = false;
	...
	if (bt_atomic_get(&server->_opcode) != BT_OBEX_OPCODE_ABORT) {
		LOG_WRN("Invalid response");
		return -EINVAL;
	}
...
int bt_obex_setpath(struct bt_obex_client *client, uint8_t flags, struct bt_buf *buf)
{
	struct bt_obex_req_hdr *hdr;
	struct bt_obex_setpath_req_hdr *req_hdr;
	int err;
	...
	if (!bt_atomic_cas(&client->_opcode, 0, BT_OBEX_OPCODE_SETPATH)) {
		LOG_WRN("Operation inprogress");
		return -EINPROGRESS;
	}
...
int bt_obex_setpath_rsp(struct bt_obex_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	struct bt_obex_rsp_hdr *hdr;
	int err;
	bool allocated = false;
	...
	if (bt_atomic_get(&server->_opcode) != BT_OBEX_OPCODE_SETPATH) {
		LOG_WRN("Invalid response");
		return -EINVAL;
	}
...
int bt_obex_action(struct bt_obex_client *client, bool final, struct bt_buf *buf)
{
	struct bt_obex_req_hdr *hdr;
	struct bt_obex_client *active_client;
	int err;
...
int bt_obex_action_rsp(struct bt_obex_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	struct bt_obex_rsp_hdr *hdr;
	int err;
	uint8_t opcode;
...
int bt_obex_add_header_count(struct bt_buf *buf, uint32_t count)
{
	size_t total;
	...
	if (bt_buf_tailroom(buf) < total) {
		return -ENOMEM;
	}

	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_COUNT);
	bt_buf_add_be32(buf, count);
	return 0;
}

int bt_obex_add_header_name(struct bt_buf *buf, uint16_t len, const uint8_t *name)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_NAME);
	bt_buf_add_be16(buf, (uint16_t)total);
	if (len) {
		bt_buf_add_mem(buf, name, len);
	}
	return 0;
}

int bt_obex_add_header_type(struct bt_buf *buf, uint16_t len, const uint8_t *type)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_TYPE);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, type, len);
	return 0;
}

int bt_obex_add_header_len(struct bt_buf *buf, uint32_t len)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_LEN);
	bt_buf_add_be32(buf, len);
	return 0;
}

int bt_obex_add_header_time_iso_8601(struct bt_buf *buf, uint16_t len, const uint8_t *t)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_TIME_ISO_8601);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, t, len);
	return 0;
}

int bt_obex_add_header_time(struct bt_buf *buf, uint32_t t)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_TIME);
	bt_buf_add_be32(buf, t);
	return 0;
}

int bt_obex_add_header_description(struct bt_buf *buf, uint16_t len, const uint8_t *dec)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_DES);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, dec, len);
	return 0;
}

int bt_obex_add_header_target(struct bt_buf *buf, uint16_t len, const uint8_t *target)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_TARGET);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, target, len);
	return 0;
}

int bt_obex_add_header_http(struct bt_buf *buf, uint16_t len, const uint8_t *http)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_HTTP);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, http, len);
	return 0;
}

int bt_obex_add_header_body(struct bt_buf *buf, uint16_t len, const uint8_t *body)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_BODY);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, body, len);
	return 0;
}

int bt_obex_add_header_end_body(struct bt_buf *buf, uint16_t len, const uint8_t *body)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_END_BODY);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, body, len);
	return 0;
}

int bt_obex_add_header_who(struct bt_buf *buf, uint16_t len, const uint8_t *who)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_WHO);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, who, len);
	return 0;
}

int bt_obex_add_header_conn_id(struct bt_buf *buf, uint32_t conn_id)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_CONN_ID);
	bt_buf_add_be32(buf, conn_id);
	return 0;
}

int bt_obex_add_header_app_param(struct bt_buf *buf, size_t count, const struct bt_obex_tlv data[])
{
	size_t total;
	uint16_t len = 0;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_APP_PARAM);
	bt_buf_add_be16(buf, (uint16_t)total);
	for (size_t i = 0; i < count; i++) {
		len += data[i].data_len + sizeof(data[i].type) + sizeof(data[i].data_len);
		bt_buf_add_u8(buf, data[i].type);
		bt_buf_add_u8(buf, data[i].data_len);
		bt_buf_add_mem(buf, data[i].data, (size_t)data[i].data_len);
	}
	return 0;
}

int bt_obex_add_header_auth_challenge(struct bt_buf *buf, size_t count,
				      const struct bt_obex_tlv data[])
{
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_AUTH_CHALLENGE);
	bt_buf_add_be16(buf, (uint16_t)total);
	for (size_t i = 0; i < count; i++) {
		len += data[i].data_len + sizeof(data[i].type) + sizeof(data[i].data_len);
		bt_buf_add_u8(buf, data[i].type);
...
int bt_obex_add_header_auth_rsp(struct bt_buf *buf, size_t count, const struct bt_obex_tlv data[])
{
	size_t total;
	uint16_t len = 0;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_AUTH_RSP);
	bt_buf_add_be16(buf, (uint16_t)total);
	for (size_t i = 0; i < count; i++) {
		len += data[i].data_len + sizeof(data[i].type) + sizeof(data[i].data_len);
		bt_buf_add_u8(buf, data[i].type);
...
int bt_obex_add_header_creator_id(struct bt_buf *buf, uint32_t creator_id)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_CREATE_ID);
	bt_buf_add_be32(buf, creator_id);
	return 0;
}

int bt_obex_add_header_wan_uuid(struct bt_buf *buf, uint16_t len, const uint8_t *uuid)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_WAN_UUID);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, uuid, len);
	return 0;
}

int bt_obex_add_header_obj_class(struct bt_buf *buf, uint16_t len, const uint8_t *obj_class)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_OBJECT_CLASS);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, obj_class, len);
	return 0;
}

int bt_obex_add_header_session_param(struct bt_buf *buf, uint16_t len,
				     const uint8_t *session_param)
{
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_SESSION_PARAM);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, session_param, len);
	return 0;
}

int bt_obex_add_header_session_seq_number(struct bt_buf *buf, uint32_t session_seq_number)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_SESSION_SEQ_NUM);
	bt_buf_add_be32(buf, session_seq_number);
	return 0;
}

int bt_obex_add_header_action_id(struct bt_buf *buf, uint8_t action_id)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_ACTION_ID);
	bt_buf_add_u8(buf, action_id);
	return 0;
}

int bt_obex_add_header_dest_name(struct bt_buf *buf, uint16_t len, const uint8_t *dest_name)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_DEST_NAME);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, dest_name, len);
	return 0;
}

int bt_obex_add_header_perm(struct bt_buf *buf, uint32_t perm)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_PERM);
	bt_buf_add_be32(buf, perm);
	return 0;
}

int bt_obex_add_header_srm(struct bt_buf *buf, uint8_t srm)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_SRM);
	bt_buf_add_u8(buf, srm);
	return 0;
}

int bt_obex_add_header_srm_param(struct bt_buf *buf, uint8_t srm_param)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_OBEX_HEADER_ID_SRM_PARAM);
	bt_buf_add_u8(buf, srm_param);
	return 0;
}

int bt_obex_header_parse(struct bt_buf *buf,
...
int bt_obex_get_header_count(struct bt_buf *buf, uint32_t *count)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_name(struct bt_buf *buf, uint16_t *len, const uint8_t **name)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_type(struct bt_buf *buf, uint16_t *len, const uint8_t **type)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_len(struct bt_buf *buf, uint32_t *len)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_time_iso_8601(struct bt_buf *buf, uint16_t *len, const uint8_t **t)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_time(struct bt_buf *buf, uint32_t *t)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_description(struct bt_buf *buf, uint16_t *len, const uint8_t **dec)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_target(struct bt_buf *buf, uint16_t *len, const uint8_t **target)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_http(struct bt_buf *buf, uint16_t *len, const uint8_t **http)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_body(struct bt_buf *buf, uint16_t *len, const uint8_t **body)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_end_body(struct bt_buf *buf, uint16_t *len, const uint8_t **body)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_who(struct bt_buf *buf, uint16_t *len, const uint8_t **who)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_conn_id(struct bt_buf *buf, uint32_t *conn_id)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_app_param(struct bt_buf *buf, uint16_t *len, const uint8_t **app_param)
{
	struct bt_obex_find_header_data data;
	int err;
...
bool bt_obex_has_app_param(struct bt_buf *buf, uint8_t id)
{
...
int bt_obex_get_header_auth_challenge(struct bt_buf *buf, uint16_t *len, const uint8_t **auth)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_auth_rsp(struct bt_buf *buf, uint16_t *len, const uint8_t **auth)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_creator_id(struct bt_buf *buf, uint32_t *creator_id)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_wan_uuid(struct bt_buf *buf, uint16_t *len, const uint8_t **uuid)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_obj_class(struct bt_buf *buf, uint16_t *len, const uint8_t **obj_class)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_session_param(struct bt_buf *buf, uint16_t *len,
				     const uint8_t **session_param)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_session_seq_number(struct bt_buf *buf, uint32_t *session_seq_number)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_action_id(struct bt_buf *buf, uint8_t *action_id)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_dest_name(struct bt_buf *buf, uint16_t *len, const uint8_t **dest_name)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_perm(struct bt_buf *buf, uint32_t *perm)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_srm(struct bt_buf *buf, uint8_t *srm)
{
	struct bt_obex_find_header_data data;
	int err;
...
int bt_obex_get_header_srm_param(struct bt_buf *buf, uint8_t *srm_param)
{
	struct bt_obex_find_header_data data;
	int err;
