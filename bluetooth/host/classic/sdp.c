...
static int bt_sdp_get_attr(const struct bt_buf *buf,
...
static int sdp_get_u16_data(const struct bt_sdp_attr_item *attr, uint16_t *u16)
{
	...
	p = attr->val;
	__ASSERT_MSG(p != NULL, "attr->val cannot be NULL");
...
int bt_sdp_get_proto_param(const struct bt_buf *buf, enum bt_sdp_proto proto,
			   uint16_t *param)
{
...
int bt_sdp_get_addl_proto_param(const struct bt_buf *buf, enum bt_sdp_proto proto,
...
int bt_sdp_get_profile_version(const struct bt_buf *buf, uint16_t profile,
			       uint16_t *version)
{
	struct bt_sdp_attr_item attr;
	struct bt_sdp_uuid_desc pd;
