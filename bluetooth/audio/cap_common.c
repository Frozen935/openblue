...
OS_MUTEX_DEFINE(active_proc_mutex);
#define CAP_ACTIVE_PROC_MUTEX_TIMEOUT OS_MSEC(10000U)

struct bt_cap_common_proc *bt_cap_common_get_active_proc(void)
{
	__maybe_unused int err;

	err = os_mutex_lock(&active_proc_mutex, CAP_ACTIVE_PROC_MUTEX_TIMEOUT);
	__ASSERT(err == 0, "Failed to lock mutex: %d", err);

	LOG_DBG("Took active_proc_mutex");
...
void bt_cap_common_unlock_proc(void)
{
	...
	err = os_mutex_unlock(&active_proc_mutex);
