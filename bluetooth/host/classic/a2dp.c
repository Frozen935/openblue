...
static struct bt_buf *a2dp_alloc_buf(struct bt_avdtp *session)
{
	return NULL;
}
...
static int a2dp_get_capabilities_ind(struct bt_avdtp *session, struct bt_avdtp_sep *sep,
				     struct bt_buf *rsp_buf, bool get_all_caps, uint8_t *errcode)
{
	...
	*errcode = 0;
	...
	cap = bt_buf_add(rsp_buf, sizeof(*cap));
	...
	if (bt_buf_tailroom(rsp_buf) < ep->codec_cap->len + sizeof(*cap) + sizeof(*media_cap)) {
		goto set_err_and_return;
	}
...
static int a2dp_process_config_ind(struct bt_avdtp *session, struct bt_avdtp_sep *sep,
				   uint8_t int_seid, struct bt_buf *buf, uint8_t *errcode,
				   bool reconfig)
{
	struct bt_a2dp *a2dp = A2DP_AVDTP(session);
	struct bt_a2dp_ep *ep;
...
static void bt_a2dp_media_data_callback(struct bt_avdtp_sep *sep, struct bt_buf *buf)
{
	...
	stream = ep->stream;
	media_hdr = bt_buf_pull_mem(buf, sizeof(*media_hdr));
...
static int a2dp_get_config_ind(struct bt_avdtp *session, struct bt_avdtp_sep *sep,
			       struct bt_buf *rsp_buf, uint8_t *errcode)
{
	struct bt_a2dp_ep *ep;
	struct bt_avdtp_generic_service_cap *cap;
	struct bt_avdtp_media_codec_capabilities *media_cap;
	...
	*errcode = 0;
	...
	if (bt_buf_tailroom(rsp_buf) < ep->stream->codec_config.len + sizeof(*cap) +
	    sizeof(*media_cap)) {
		goto set_err_and_return;
	}
	...
	media_cap->media_code_type = ep->codec_type;
	/* Codec Info Element */
	bt_buf_add_mem(rsp_buf, &ep->stream->codec_config.codec_ie[0],
...
int a2dp_delay_report_ind(struct bt_avdtp *session, struct bt_avdtp_sep *sep, struct bt_buf *buf,
			  uint8_t *errcode)
{
	struct bt_a2dp_ep *ep;
	struct bt_a2dp_stream *stream;
...
static int bt_a2dp_set_config_cb(struct bt_avdtp_req *req, struct bt_buf *buf)
{
	uint8_t status;
	struct bt_a2dp_ep *ep;
	struct bt_a2dp_stream *stream;
...
static int bt_a2dp_get_capabilities_cb(struct bt_avdtp_req *req, struct bt_buf *buf)
{
	int err;
	uint8_t *codec_info_element;
...
static int bt_a2dp_discover_cb(struct bt_avdtp_req *req, struct bt_buf *buf)
{
...
static int bt_a2dp_open_cb(struct bt_avdtp_req *req, struct bt_buf *buf)
{
	struct bt_a2dp_ep *ep = CONTAINER_OF(CTRL_REQ(req)->sep, struct bt_a2dp_ep, sep);
...
static int bt_a2dp_start_cb(struct bt_avdtp_req *req, struct bt_buf *buf)
{
	struct bt_a2dp_ep *ep = CONTAINER_OF(CTRL_REQ(req)->sep, struct bt_a2dp_ep, sep);
...
static int bt_a2dp_suspend_cb(struct bt_avdtp_req *req, struct bt_buf *buf)
{
	struct bt_a2dp_ep *ep = CONTAINER_OF(CTRL_REQ(req)->sep, struct bt_a2dp_ep, sep);
...
static int bt_a2dp_close_cb(struct bt_avdtp_req *req, struct bt_buf *buf)
{
...
static int bt_a2dp_abort_cb(struct bt_avdtp_req *req, struct bt_buf *buf)
{
...
static int bt_a2dp_get_config_cb(struct bt_avdtp_req *req, struct bt_buf *buf)
{
	int err;
	bool delay_report;
	uint8_t status;
...
static int bt_a2dp_delay_report_cb(struct bt_avdtp_req *req, struct bt_buf *buf)
{
	uint8_t status;
...
struct bt_buf *bt_a2dp_stream_create_pdu(struct bt_buf_pool *pool, os_timeout_t timeout)
{
	...
	/* add AVDTP header */
	if (bt_buf_tailroom(buf) < sizeof(struct bt_avdtp_media_hdr)) {
		bt_buf_unref(buf);
		return NULL;
	}

	bt_buf_add(buf, sizeof(struct bt_avdtp_media_hdr));

	return buf;
}

int bt_a2dp_stream_send(struct bt_a2dp_stream *stream, struct bt_buf *buf, uint16_t seq_num,
			uint32_t ts)
{
	struct bt_avdtp_media_hdr *media_hdr;
...
int a2dp_endpoint_released(struct bt_avdtp_sep *sep)
{
	...
	if (ep->stream != NULL) {
		 ...
		 * all the related callbacks are in the same task context.
		 */
		ep->stream = NULL;
