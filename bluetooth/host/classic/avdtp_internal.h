...
typedef int (*bt_avdtp_func_t)(struct bt_avdtp_req *req, struct bt_buf *buf);
...
struct bt_avdtp_ops_cb {
	...
	struct bt_buf *(*alloc_buf)(struct bt_avdtp *session);
...
struct bt_avdtp {
	...
	struct bt_work _release_work;
	/* semaphore for lock/unlock */
	os_sem_t sem_lock;
	struct bt_buf *reasm_buf;
	uint8_t num_of_signal_pkts;
	uint8_t tid_sent;
};
