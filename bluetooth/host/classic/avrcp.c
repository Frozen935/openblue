...
struct bt_avrcp_ct {
	...
	struct bt_buf *reassembly_buf;	/**< Buffer for reassembling fragments */
...
struct bt_avrcp_tg {
	...
	bt_slist_t vd_rsp_tx_pending;

	/* Critical locker */
	os_sem_t lock;

	/* AVRCP vendor dependent response TX work */
	struct bt_work_delayable vd_rsp_tx_work;
...
struct avrcp_handler {
	...
	void (*func)(struct bt_avrcp *avrcp, uint8_t tid, struct bt_buf *buf);
};
...
struct avrcp_pdu_vendor_handler  {
	...
	int (*func)(struct bt_avrcp *avrcp, uint8_t tid, uint8_t result, struct bt_buf *buf);
};

BT_BUF_POOL_FIXED_DEFINE(avrcp_vd_rx_pool, CONFIG_BT_MAX_CONN,
...
struct avrcp_pdu_handler {
	bt_avrcp_pdu_id_t pdu_id;
	uint8_t min_len;
	int (*func)(struct bt_avrcp *avrcp, uint8_t tid, struct bt_buf *buf);
};
...
BT_BUF_POOL_FIXED_DEFINE(avctp_ctrl_rx_pool, CONFIG_BT_MAX_CONN, BT_AVRCP_FRAGMENT_SIZE,
			  CONFIG_BT_CONN_TX_USER_DATA_SIZE, NULL);

BT_BUF_POOL_FIXED_DEFINE(avctp_ctrl_tx_pool, CONFIG_BT_MAX_CONN,
			  BT_L2CAP_BUF_SIZE(CONFIG_BT_L2CAP_TX_MTU),
			  CONFIG_BT_CONN_TX_USER_DATA_SIZE, NULL);

BT_BUF_POOL_DEFINE(avctp_browsing_rx_pool, BT_BUF_ACL_RX_COUNT,
...
static void avrcp_tx_buf_destroy(struct bt_buf *buf)
{
	for (int i = 0; i < CONFIG_BT_MAX_CONN; i++) {
		struct bt_avrcp_tg *tg = &bt_avrcp_tg_pool[i];

		if ((tg->avrcp != NULL) && (tg->avrcp->acl_conn != NULL)) {
			(void)bt_work_reschedule(&tg->vd_rsp_tx_work, OS_TIMEOUT_NO_WAIT);
		}
	}
...
BT_BUF_POOL_FIXED_DEFINE(avrcp_vd_tx_pool, CONFIG_BT_MAX_CONN,
...
{
	os_sem_take(&tg->lock, OS_TIMEOUT_FOREVER);
}
...
static int bt_avrcp_rsp_to_status(uint8_t rsp, struct bt_buf *buf, uint8_t *status)
{
...
static struct bt_buf *avrcp_create_unit_pdu(struct bt_avrcp *avrcp, uint8_t ctype_or_rsp)
{
	struct bt_buf *buf;
...
static struct bt_buf *avrcp_create_subunit_pdu(struct bt_avrcp *avrcp, uint8_t ctype_or_rsp)
{
	struct bt_buf *buf;
	struct bt_avrcp_frame *cmd;
...
static struct bt_buf *avrcp_create_passthrough_pdu(struct bt_avrcp *avrcp, uint8_t ctype_or_rsp)
{
	struct bt_buf *buf;
	struct bt_avrcp_frame *cmd;
...
static struct bt_buf *avrcp_create_vendor_pdu(struct bt_avrcp *avrcp, uint8_t ctype_or_rsp)
{
	struct bt_buf *buf;
	struct bt_avrcp_frame *cmd;
...
static int avrcp_send(struct bt_avrcp *avrcp, struct bt_buf *buf, bt_avctp_cr_t cr, uint8_t tid)
{
	int err;
...
static struct bt_buf *avrcp_create_browsing_pdu(struct bt_avrcp *avrcp)
{
	return bt_avrcp_create_pdu(NULL);
}

static int avrcp_browsing_send(struct bt_avrcp *avrcp, struct bt_buf *buf, bt_avctp_cr_t cr,
			       uint8_t tid)
{
	int err;
...
static int bt_avrcp_send_subunit_info(struct bt_avrcp *avrcp, uint8_t tid, uint8_t rsp_type)
{
	...
	if (bt_buf_tailroom(buf) < sizeof(param)) {
		LOG_WRN("Not enough tailroom in buffer");
		bt_buf_unref(buf);
		return -ENOMEM;
	}
	bt_buf_add_mem(buf, param, sizeof(param));
				      ...
				      uint8_t rsp)
{
	...
	if (ct->reassembly_buf != NULL) {
		...
		bt_buf_unref(ct->reassembly_buf);
		ct->reassembly_buf = NULL;
	}

	/* Allocate reassembly buffer */
	ct->reassembly_buf = bt_buf_alloc(&avrcp_vd_rx_pool, OS_TIMEOUT_NO_WAIT);
	if (ct->reassembly_buf == NULL) {
	...
	ctx = bt_buf_user_data(ct->reassembly_buf);
...
static int add_fragment_data(struct bt_avrcp_ct *ct, const uint8_t *data, uint16_t data_len)
{
	...
	if (bt_buf_tailroom(ct->reassembly_buf) < data_len) {
		LOG_ERR("Insufficient space in reassembly buffer");
		return -ENOMEM;
	}

	/* Add fragment data to reassembly buffer */
	bt_buf_add_mem(ct->reassembly_buf, data, data_len);
	return 0;
}
...
static struct bt_buf *avrcp_prepare_vendor_pdu(struct bt_avrcp *avrcp,
						...
						uint8_t pdu_id, uint16_t param_len)
{
	...
	if (bt_buf_tailroom(buf) < sizeof(*pdu)) {
		LOG_WRN("Not enough tailroom: required");
		bt_buf_unref(buf);
		return NULL;
	}

	pdu = bt_buf_add(buf, sizeof(*pdu));
				  ...
				  struct bt_buf *buf)
{
	...
	if (bt_buf_headroom(buf) < (sizeof(*pdu) + sizeof(*hdr))) {
		LOG_WRN("Not enough headroom: for vendor dependent PDU");
		bt_buf_unref(buf);
		return -ENOMEM;
	}

	pdu = bt_buf_push(buf, sizeof(*pdu));
	sys_put_be24(BT_AVRCP_COMPANY_ID_BLUETOOTH_SIG, pdu->company_id);
	pdu->pdu_id = pdu_id;
	BT_AVRCP_AVC_PDU_SET_PACKET_TYPE(pdu, BT_AVRCP_PKT_TYPE_SINGLE);
	pdu->param_len = sys_cpu_to_be16(param_len);

	hdr = bt_buf_push(buf, sizeof(struct bt_avrcp_header));
					       ...
					       uint8_t op_pdu_id, uint8_t target_pdu_id)
{
	...
	bt_buf_add_u8(buf, target_pdu_id);
					       ...
					       uint8_t tid, struct bt_buf *buf)
{
	...
	node = bt_slist_peek_head(&tg->vd_rsp_tx_pending);
	if (!node) {
	...
	tx_buf = CONTAINER_OF(node, struct bt_buf, node);

	__ASSERT_NO_MSG(tx_buf->user_data_size >= sizeof(*tx));
	tx = bt_buf_user_data(tx_buf);
	...
	pdu_id = bt_buf_pull_u8(buf);

	/* Clear TX_ONGOING and update TID */
	bt_atomic_clear_bit(tx->flags, BT_AVRCP_TG_FLAG_TX_ONGOING);
	tx->tid = tid;

	if (abort_continuing) {
		/* ABORT_CONTINUING: set abort flag, clear sending flag */
		bt_atomic_set_bit(tx->flags, BT_AVRCP_TG_FLAG_ABORT_CONTINUING);
...
static void avrcp_tg_tx_remove(struct bt_avrcp_tg *tg, struct bt_buf *buf)
{
	avrcp_tg_lock(tg);
	bt_slist_find_and_remove(&tg->vd_rsp_tx_pending, &buf->node);
	avrcp_tg_unlock(tg);
}

static void bt_avrcp_tg_vendor_tx_work(struct bt_work *work)
{
	...
	buf = CONTAINER_OF(node, struct bt_buf, node);

	__ASSERT_NO_MSG(buf->user_data_size >= sizeof(*tx));
	tx = bt_buf_user_data(buf);

	if (bt_atomic_test_bit(tx->flags, BT_AVRCP_TG_FLAG_TX_ONGOING)) {
		return;
	}
	...
	if ((tx->sent_len == 0) && (buf->len <= max_payload_size)) {
		...
		bt_work_reschedule(&tg->vd_rsp_tx_work, OS_TIMEOUT_NO_WAIT);
		return;
	}
	...
	if (bt_atomic_test_and_clear_bit(tx->flags, BT_AVRCP_TG_FLAG_ABORT_CONTINUING) ||
	    (tx->rsp == BT_AVRCP_RSP_REJECTED)) {
		...
		if (tx->rsp == BT_AVRCP_RSP_REJECTED) {
			if (bt_buf_tailroom(rsp_buf) < sizeof(uint8_t)) {
				LOG_ERR("Insufficient space in response buffer");
				goto done;
			}
			bt_buf_add_u8(rsp_buf, error_code);
		}
	...
	bt_atomic_set_bit(tx->flags, BT_AVRCP_TG_FLAG_TX_ONGOING);
...
static int bt_avrcp_tg_send_vendor_rsp(struct bt_avrcp_tg *tg, uint8_t tid, uint8_t pdu_id,
				       uint8_t result, struct bt_buf *buf)
{
	struct bt_avrcp_tg_vd_rsp_tx *tx;
	...
	tx->sent_len = 0U;
	bt_atomic_clear_bit(tx->flags, BT_AVRCP_TG_FLAG_TX_ONGOING);
	bt_atomic_clear_bit(tx->flags, BT_AVRCP_TG_FLAG_ABORT_CONTINUING);

	LOG_DBG("Sending vendor dependent response: tid=%u, total_len=%u", tid, buf->len);
	avrcp_tg_lock(tg);
	bt_slist_append(&tg->vd_rsp_tx_pending, &buf->node);
	avrcp_tg_unlock(tg);
...
static int process_register_notification_rsp(struct bt_avrcp *avrcp, uint8_t tid,
					     uint8_t rsp_code, struct bt_buf *buf)
{
					   ...
					   uint8_t rsp_code, struct bt_buf *buf)
{
	...
	absolute_volume = bt_buf_pull_u8(buf);
					   ...
					   uint8_t pdu_id, uint8_t status)
{
	...
	if (rsp_code == BT_AVRCP_RSP_REJECTED) {
		if (bt_buf_tailroom(buf) < sizeof(status)) {
...
static int handle_vendor_pdu(struct bt_avrcp *avrcp, uint8_t tid, struct bt_buf *buf,
					       ...
					       struct bt_buf *buf)
{
	...
	avrcp_hdr = bt_buf_pull_mem(buf, sizeof(*avrcp_hdr));
	...
	pdu = bt_buf_pull_mem(buf, sizeof(*pdu));
...
static void avrcp_unit_info_rsp_handler(struct bt_avrcp *avrcp, uint8_t tid, struct bt_buf *buf)
{
	struct bt_avrcp_header *avrcp_hdr;
	...
	if ((avrcp_ct_cb != NULL) && (avrcp_ct_cb->unit_info_rsp != NULL)) {
		...
		bt_buf_pull_u8(buf); /* Always 0x07 */
		rsp.unit_type = FIELD_GET(GENMASK(7, 3), bt_buf_pull_u8(buf));
		rsp.company_id = bt_buf_pull_be24(buf);
...
static void avrcp_pass_through_rsp_handler(struct bt_avrcp *avrcp, uint8_t tid, struct bt_buf *buf)
{
	struct bt_avrcp_header *avrcp_hdr;
...
static void avrcp_unit_info_cmd_handler(struct bt_avrcp *avrcp, uint8_t tid, struct bt_buf *buf)
{
	struct bt_avrcp_header *avrcp_hdr;
	bt_avrcp_subunit_type_t subunit_type;
	bt_avrcp_subunit_id_t subunit_id;
...
static int avrcp_register_notification_cmd_handler(struct bt_avrcp *avrcp, uint8_t tid,
						   uint8_t ctype_or_rsp, struct bt_buf *buf)
{
				       ...
				       struct bt_buf *buf)
{
	...
	bt_work_reschedule(&get_avrcp_tg(avrcp)->vd_rsp_tx_work, OS_TIMEOUT_NO_WAIT);
	return BT_AVRCP_STATUS_OPERATION_COMPLETED;
}
						...
						uint8_t ctype_or_rsp, struct bt_buf *buf)
{
	...
	battery_status = bt_buf_pull_u8(buf);
					    ...
					    uint8_t ctype_or_rsp, struct bt_buf *buf)
{
	...
	player_id = bt_buf_pull_be16(buf);
...
static int avrcp_recv(struct bt_avctp *session, struct bt_buf *buf, bt_avctp_cr_t cr, uint8_t tid)
{
	struct bt_avrcp *avrcp = AVRCP_AVCTP(session);
	struct bt_avrcp_header *avrcp_hdr;
	bt_avrcp_rsp_t rsp;
...
static int avrcp_ct_handle_change_path(struct bt_avrcp *avrcp, uint8_t tid, struct bt_buf *buf)
{
	uint8_t status;
	...
	if (status == BT_AVRCP_STATUS_OPERATION_COMPLETED) {
		num_items = bt_buf_pull_be32(buf);
	}
...
static int avrcp_ct_handle_search(struct bt_avrcp *avrcp, uint8_t tid, struct bt_buf *buf)
{
...
static int handle_pdu(struct bt_avrcp *avrcp, uint8_t tid, struct bt_buf *buf,
...
static int browsing_avrcp_recv(struct bt_avctp *session, struct bt_buf *buf, bt_avctp_cr_t cr,
			       uint8_t tid)
{
	struct bt_avrcp *avrcp = AVRCP_BROW_AVCTP(session);
	...
	brow = bt_buf_pull_mem(buf, sizeof(struct bt_avrcp_brow_pdu));
...
static struct bt_buf *browsing_avrcp_l2cap_alloc_buf(struct bt_avctp *session)
{
	struct bt_buf *buf;

	buf = bt_buf_alloc(&avctp_browsing_rx_pool, OS_TIMEOUT_FOREVER);
	if (buf == NULL) {
...
void bt_avrcp_init(void)
{
	...
	ARRAY_FOR_EACH(avrcp_connection, i) {
		...
		if (!initialized) {
			/* Init delay work */
			bt_work_init_delayable(&bt_avrcp_tg_pool[i].vd_rsp_tx_work,
					      bt_avrcp_tg_vendor_tx_work);
			bt_slist_init(&bt_avrcp_tg_pool[i].vd_rsp_tx_pending);

			os_sem_init(&bt_avrcp_tg_pool[i].lock, 1, 1);
		}
...
struct bt_buf *bt_avrcp_create_pdu(struct bt_buf_pool *pool)
{
	return bt_conn_create_pdu(pool, BT_AVRCP_HEADROOM);
}

struct bt_buf *bt_avrcp_create_vendor_pdu(struct bt_buf_pool *pool)
{
	return bt_conn_create_pdu(pool, BT_AVRCP_HEADROOM +
				  sizeof(struct bt_avrcp_avc_vendor_pdu));
}
			    ...
			    const uint8_t *payload, uint8_t len)
{
	...
	bt_buf_add_u8(buf, FIELD_PREP(BIT(7), state) | FIELD_PREP(GENMASK(6, 0), opid));
	bt_buf_add_u8(buf, len);
	if (len) {
		bt_buf_add_mem(buf, payload, len);
	}
				      ...
				      uint32_t interval, bt_avrcp_notify_changed_cb_t cb)
{
	...
	/* Add event ID */
	bt_buf_add_u8(buf, event_id);

	/* Add playback interval */
	bt_buf_add_be32(buf, interval);
...
int bt_avrcp_ct_get_caps(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t cap_id)
{
	...
	if (bt_buf_tailroom(buf) < sizeof(cap_id)) {
		LOG_WRN("Not enough tailroom: for cap_id");
		bt_buf_unref(buf);
		return -ENOMEM;
	}
	bt_buf_add_u8(buf, cap_id);
...
int bt_avrcp_ct_list_player_app_setting_vals(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t attr_id)
{
	...
	if (bt_buf_tailroom(buf) < sizeof(attr_id)) {
		LOG_WRN("Not enough tailroom: for attr_id");
		bt_buf_unref(buf);
		return -ENOMEM;
	}

	bt_buf_add_u8(buf, attr_id);
...
int bt_avrcp_ct_set_player_app_setting_val(struct bt_avrcp_ct *ct, uint8_t tid, struct bt_buf *buf)
{
	int err;
					 ...
					 uint8_t battery_status)
{
	...
	if (bt_buf_tailroom(buf) < sizeof(battery_status)) {
		LOG_WRN("Not enough tailroom: for battery_status");
		bt_buf_unref(buf);
		return -ENOMEM;
	}

	bt_buf_add_u8(buf, battery_status);
...
int bt_avrcp_ct_set_absolute_volume(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t absolute_volume)
{
	...
	if (bt_buf_tailroom(buf) < sizeof(absolute_volume)) {
		LOG_WRN("Not enough tailroom: for absolute_volume");
		bt_buf_unref(buf);
		return -ENOMEM;
	}
	bt_buf_add_u8(buf, absolute_volume);
...
int bt_avrcp_ct_set_addressed_player(struct bt_avrcp_ct *ct, uint8_t tid, uint16_t player_id)
{
	...
	if (bt_buf_tailroom(buf) < sizeof(player_id)) {
		LOG_WRN("Not enough tailroom: for player_id");
		bt_buf_unref(buf);
		return -ENOMEM;
	}

	bt_buf_add_be16(buf, player_id);
...
int bt_avrcp_ct_play_item(struct bt_avrcp_ct *ct, uint8_t tid, struct bt_buf *buf)
{
	int err;
...
int bt_avrcp_ct_add_to_now_playing(struct bt_avrcp_ct *ct, uint8_t tid, struct bt_buf *buf)
{
	int err;
				   ...
				   struct bt_avrcp_unit_info_rsp *rsp)
{
	...
	bt_buf_add_u8(buf, 0x07);
	/* Add Unit Type info */
	bt_buf_add_u8(buf, FIELD_PREP(GENMASK(7, 3), (rsp->unit_type)));
	/* Company ID */
	bt_buf_add_be24(buf, (rsp->company_id));
...
static int bt_avrcp_browsing_send(struct bt_avrcp *avrcp, struct bt_buf *buf, uint8_t pdu_id,
				  uint8_t avctp_type, uint8_t tid)
{
	...
	if (bt_buf_headroom(buf) < sizeof(struct bt_avrcp_brow_pdu)) {
		LOG_ERR("Not enough headroom for browsing PDU header");
		return -ENOMEM;
	}

	hdr = bt_buf_push(buf, sizeof(struct bt_avrcp_brow_pdu));
...
#if defined(CONFIG_BT_AVRCP_BROWSING)
static inline int avrcp_ct_precheck(struct bt_avrcp_ct *ct, struct bt_buf *buf)
{
	if ((ct == NULL) || (ct->avrcp == NULL) || (buf == NULL)) {
		return -EINVAL;
	}
...
int bt_avrcp_ct_get_folder_items(struct bt_avrcp_ct *ct, uint8_t tid, struct bt_buf *buf)
{
	int err;
...
int bt_avrcp_ct_change_path(struct bt_avrcp_ct *ct, uint8_t tid, struct bt_buf *buf)
{
	int err;
...
int bt_avrcp_ct_get_item_attrs(struct bt_avrcp_ct *ct, uint8_t tid, struct bt_buf *buf)
{
	int err;
...
int bt_avrcp_ct_get_total_number_of_items(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t scope)
{
	...
	if (bt_buf_tailroom(buf) < sizeof(scope)) {
		LOG_ERR("Not enough tailroom for scope");
		bt_buf_unref(buf);
		return -ENOMEM;
	}
	bt_buf_add_u8(buf, scope);
...
int bt_avrcp_ct_search(struct bt_avrcp_ct *ct, uint8_t tid, struct bt_buf *buf)
{
	int err;
...
static inline int avrcp_tg_precheck(struct bt_avrcp_tg *tg, struct bt_buf *buf)
{
	if ((tg == NULL) || (tg->avrcp == NULL) || (buf == NULL)) {
		return -EINVAL;
	}
...
int bt_avrcp_tg_set_browsed_player(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
	int err;
...
int bt_avrcp_tg_get_folder_items(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
	int err;
...
int bt_avrcp_tg_get_item_attrs(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
	int err;
...
int bt_avrcp_tg_search(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
	int err;
				       ...
				       struct bt_buf *buf)
{
	...
	if (bt_buf_tailroom(buf) < param_len) {
		LOG_ERR("Not enough space in bt_buf");
		return -ENOMEM;
	}
	...
	case BT_AVRCP_EVT_PLAYBACK_STATUS_CHANGED:
		...
		bt_buf_add_u8(buf, data->play_status);
		break;
	case BT_AVRCP_EVT_TRACK_CHANGED: {
		uint64_t identifier = sys_get_be64(data->identifier);

		bt_buf_add_be64(buf, identifier);
		break;
	}
	case BT_AVRCP_EVT_PLAYBACK_POS_CHANGED:
		bt_buf_add_be32(buf, data->playback_pos);
		break;
	case BT_AVRCP_EVT_BATT_STATUS_CHANGED:
		...
		bt_buf_add_u8(buf, data->battery_status);
		break;
	case BT_AVRCP_EVT_SYSTEM_STATUS_CHANGED:
		...
		bt_buf_add_u8(buf, data->system_status);
		break;
	case BT_AVRCP_EVT_PLAYER_APP_SETTING_CHANGED:
		bt_buf_add_u8(buf, data->setting_changed.num_of_attr);
		bt_buf_add_mem(buf, data->setting_changed.attr_vals,
		data->setting_changed.num_of_attr * sizeof(struct bt_avrcp_app_setting_attr_val));
		break;
	case BT_AVRCP_EVT_ADDRESSED_PLAYER_CHANGED:
		bt_buf_add_be16(buf, data->addressed_player_changed.player_id);
		bt_buf_add_be16(buf, data->addressed_player_changed.uid_counter);
		break;
	case BT_AVRCP_EVT_UIDS_CHANGED:
		bt_buf_add_be16(buf, data->uid_counter);
		break;
	case BT_AVRCP_EVT_VOLUME_CHANGED:
		...
		bt_buf_add_u8(buf, data->absolute_volume);
		break;
	case BT_AVRCP_EVT_TRACK_REACHED_END:
	case BT_AVRCP_EVT_TRACK_REACHED_START:
	case BT_AVRCP_EVT_AVAILABLE_PLAYERS_CHANGED:
				     ...
				     struct bt_buf *buf)
{
	...
	if (bt_buf_headroom(buf) < sizeof(struct bt_avrcp_header)) {
		LOG_ERR("Not enough headroom in buffer for bt_avrcp_header");
		return -ENOMEM;
	}
	avrcp_hdr = bt_buf_push(buf, sizeof(struct bt_avrcp_header));
						 ...
						 , uint8_t status, struct bt_buf *buf)
{
	...
	if (rsp_code == BT_AVRCP_RSP_REJECTED) {
		if (bt_buf_tailroom(status_buf) < sizeof(status)) {
			LOG_ERR("Not enough space in status bt_buf");
			bt_buf_unref(status_buf);
			return -ENOMEM;
		}
		bt_buf_add_u8(status_buf, status);
	}
...
int bt_avrcp_tg_get_caps(struct bt_avrcp_tg *tg, uint8_t tid, uint8_t status, struct bt_buf *buf)
{
	int err;
