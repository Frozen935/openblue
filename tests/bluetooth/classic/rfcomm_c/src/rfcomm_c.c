/* rfcomm_c.c - Bluetooth classic rfcomm_c smoke test */
...
BT_BUF_POOL_FIXED_DEFINE(rfcomm_c_pool, 2, BT_RFCOMM_BUF_SIZE(DATA_MTU),
...
static int cmd_send(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	buf = bt_rfcomm_create_pdu(&rfcomm_c_pool);
	data_len = MIN(data_len, MIN(DATA_MTU, bt_buf_tailroom(buf) - 1));
	bt_buf_add_mem(buf, buf_data, data_len);
...
BT_SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	BT_SHELL_CMD_ARG(send, NULL, "<server channel> [data length]", cmd_send, 2, 1),
