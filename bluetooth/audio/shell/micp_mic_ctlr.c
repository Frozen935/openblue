				    ...
				    char **argv)
{
	...
	if (result != 0) {
		bt_shell_print("Failed to register callbacks: %d", result);
	}
...
BT_SHELL_SUBCMD_SET_CREATE(micp_mic_ctlr_cmds,
...
BT_SHELL_CMD_ARG_DEFINE(micp_mic_ctlr, &micp_mic_ctlr_cmds,
		       "Bluetooth Microphone Controller shell commands",
		       cmd_micp_mic_ctlr, 1, 1);

int bt_shell_cmd_micp_mic_ctlr_register(struct bt_shell *sh)
{
	return bt_shell_cmd_register(sh, &micp_mic_ctlr);
}
