...
static bool is_slot_committed(struct slot *slot_to_check)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&list, s, n) {
		if (s == slot_to_check) {
			return true;
		}
	}
...
int bt_mesh_dfu_slot_del(const struct bt_mesh_dfu_slot *dfu_slot)
{
	...
	if (!bt_slist_find_and_remove(&list, &slot->n)) {
		return -EINVAL;
	}
			  ...
			  bt_storage_read_cb read_cb, void *cb_arg)
{
	...
	if (!strncmp(prop, PROP_HEADER, len)) {
		if (read_cb(cb_arg, &slots[idx], HEADER_SIZE) > 0) {
			...
			if (prev == NULL) {
				bt_slist_prepend(&list, &slots[idx].n);
			} else {
				bt_slist_insert(&list, &prev->n, &slots[idx].n);
			}
	...
	if (!strncmp(prop, PROP_FWID, len)) {
		if (read_cb(cb_arg, &slots[idx].slot.fwid,
			...
			slots[idx].idx = 0;
			bt_slist_find_and_remove(&list, &slots[idx].n);
			return 0;
		}
