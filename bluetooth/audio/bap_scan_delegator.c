...
#define PAST_TIMEOUT              OS_SECONDS(10)

#define SCAN_DELEGATOR_BUF_SEM_TIMEOUT OS_MSEC(CONFIG_BT_BAP_SCAN_DELEGATOR_BUF_TIMEOUT)
BT_BUF_SIMPLE_DEFINE_STATIC(read_buf, BT_ATT_MAX_ATTRIBUTE_LEN);
...
struct bass_recv_state_internal {
	...
	struct bt_work_delayable notify_work;
};
...
static void set_receive_state_changed_cb(struct bt_conn *conn, void *data)
{
	 ...
	 */
	err = bt_work_schedule(&internal_state->notify_work, OS_TIMEOUT_NO_WAIT);
...
static void receive_state_notify_cb(struct bt_conn *conn, void *data)
{
	...
	if (flags->updated) {
		...
		err = bt_work_reschedule(&internal_state->notify_work,
					OS_USEC(BT_CONN_INTERVAL_TO_US(conn_info.le.interval)));
		__ASSERT_MSG(err >= 0, "Failed to reschedule work: %d", err);
	}
}

static void bt_buf_put_recv_state(const struct bass_recv_state_internal *recv_state)
{
	const struct bt_bap_scan_delegator_recv_state *state = &recv_state->state;

	bt_buf_simple_reset(&read_buf);

	__ASSERT_MSG(recv_state, "NULL receive state");
	...
	(void)bt_buf_simple_add_u8(&read_buf, state->src_id);
	(void)bt_buf_simple_add_u8(&read_buf, state->addr.type);
	(void)bt_buf_simple_add_mem(&read_buf, &state->addr.a,
				     sizeof(state->addr.a));
	(void)bt_buf_simple_add_u8(&read_buf, state->adv_sid);
	(void)bt_buf_simple_add_le24(&read_buf, state->broadcast_id);
	(void)bt_buf_simple_add_u8(&read_buf, state->pa_sync_state);
	(void)bt_buf_simple_add_u8(&read_buf, state->encrypt_state);
	if (state->encrypt_state == BT_BAP_BIG_ENC_STATE_BAD_CODE) {
		(void)bt_buf_simple_add_mem(&read_buf, &state->bad_code,
					     sizeof(state->bad_code));
	}
	(void)bt_buf_simple_add_u8(&read_buf, state->num_subgroups);
	for (int i = 0; i < state->num_subgroups; i++) {
		const struct bt_bap_bass_subgroup *subgroup = &state->subgroups[i];

		(void)bt_buf_simple_add_le32(&read_buf, subgroup->bis_sync);
		(void)bt_buf_simple_add_u8(&read_buf, subgroup->metadata_len);
		(void)bt_buf_simple_add_mem(&read_buf, subgroup->metadata,
					     subgroup->metadata_len);
	}
}
				  ...
				  struct bass_recv_state_internal *internal_state)
{
	if (IS_ENABLED(CONFIG_BT_BAP_SCAN_DELEGATOR_LOG_LEVEL_DBG)) {
		...
		err = os_mutex_lock(&internal_state->mutex, SCAN_DELEGATOR_BUF_SEM_TIMEOUT);
		if (err == 0) {
			bt_debug_dump_recv_state(internal_state);
			err = os_mutex_unlock(&internal_state->mutex);
...
static void notify_work_handler(struct bt_work *work)
{
	struct bass_recv_state_internal *internal_state = CONTAINER_OF(
		bt_work_delayable_from_work(work), struct bass_recv_state_internal, notify_work);
	int err;

	err = os_mutex_lock(&internal_state->mutex, SCAN_DELEGATOR_BUF_SEM_TIMEOUT);
	if (err != 0) {
		LOG_DBG("Failed to take mutex: %d", err);
		err = bt_work_reschedule(&internal_state->notify_work, OS_TIMEOUT_NO_WAIT);
		__ASSERT_MSG(err >= 0, "Failed to reschedule work: %d", err);
		return;
	}

	bt_buf_put_recv_state(internal_state);
							  ...
							  uint32_t broadcast_id)
{
	...
	ARRAY_FOR_EACH_PTR(scan_delegator.recv_states, recv_state_internal) {
		...
		err = os_mutex_lock(&recv_state_internal->mutex, SCAN_DELEGATOR_BUF_SEM_TIMEOUT);
		if (err != 0) {
			LOG_DBG("Failed to lock mutex: %d", err);
		...
		err = os_mutex_unlock(&recv_state_internal->mutex);
		__ASSERT_MSG(err == 0, "Failed to unlock mutex: %d", err);
				     ...
				     struct bt_buf_simple *buf)
{
	...
	addr = bt_buf_simple_pull_mem(buf, sizeof(*addr));
	if (addr->type > BT_ADDR_LE_RANDOM) {
		LOG_DBG("Invalid address type %u", addr->type);
		return BT_GATT_ERR(BT_ATT_ERR_VALUE_NOT_ALLOWED);
	}

	adv_sid = bt_buf_simple_pull_u8(buf);
	if (adv_sid > BT_GAP_SID_MAX) {
		LOG_DBG("Invalid adv SID %u", adv_sid);
		return BT_GATT_ERR(BT_ATT_ERR_VALUE_NOT_ALLOWED);
	}

	broadcast_id = bt_buf_simple_pull_le24(buf);
	...
	pa_sync = bt_buf_simple_pull_u8(buf);
	...
	pa_interval = bt_buf_simple_pull_le16(buf);

	state->num_subgroups = bt_buf_simple_pull_u8(buf);
	...
	for (int i = 0; i < state->num_subgroups; i++) {
		...
		internal_state->requested_bis_sync[i] = bt_buf_simple_pull_le32(buf);
		...
		subgroup->metadata_len = bt_buf_simple_pull_u8(buf);
		...
		metadata = bt_buf_simple_pull_mem(buf, subgroup->metadata_len);
				  ...
				  struct bt_buf_simple *buf)
{
	...
	src_id = bt_buf_simple_pull_u8(buf);
	...
	num_subgroups = bt_buf_simple_pull_u8(buf);
	...
	for (int i = 0; i < num_subgroups; i++) {
		...
		requested_bis_sync[i] = bt_buf_simple_pull_le32(buf);
					 ...
					 struct bt_buf_simple *buf)
{
	...
	broadcast_code = bt_buf_simple_pull_mem(buf, sizeof(internal_state->broadcast_code));
			       ...
			       uint16_t len, uint16_t offset)
{
	...
	if (recv_state->active) {
		...
		err = os_mutex_lock(&recv_state->mutex, SCAN_DELEGATOR_BUF_SEM_TIMEOUT);
		if (err != 0) {
			LOG_DBG("Failed to lock mutex: %d", err);
...
int bt_bap_scan_delegator_register(struct bt_bap_scan_delegator_cb *cb)
{
	...
	if (bt_atomic_test_and_set_bit(scan_delegator_flags,
				    SCAN_DELEGATOR_FLAG_REGISTERED_SCAN_DELIGATOR)) {
	...
	err = bass_register();
	if (err) {
		bt_atomic_clear_bit(scan_delegator_flags,
				 SCAN_DELEGATOR_FLAG_REGISTERED_SCAN_DELIGATOR);
		return err;
	}
	...
	for (size_t i = 0; i < ARRAY_SIZE(scan_delegator.recv_states); i++) {
		...
		err = os_mutex_init(&internal_state->mutex);
		__ASSERT_MSG(err == 0, "Failed to initialize mutex");

		bt_work_init_delayable(&internal_state->notify_work, notify_work_handler);
	}
...
int bt_bap_scan_delegator_unregister(void)
{
	...
	if (!bt_atomic_test_and_clear_bit(scan_delegator_flags,
				       SCAN_DELEGATOR_FLAG_REGISTERED_SCAN_DELIGATOR)) {
	...
	err = bass_unregister();
	if (err) {
		bt_atomic_set_bit(scan_delegator_flags,
			       SCAN_DELEGATOR_FLAG_REGISTERED_SCAN_DELIGATOR);
		return err;
	}
...
int bt_bap_scan_delegator_rem_src(uint8_t src_id)
{
	...
	bt_buf_simple_init_with_data(&buf, (void *)&src_id, sizeof(src_id));
