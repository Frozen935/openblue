...
static void proxy_send_beacons(struct bt_work *work);
...
static struct bt_mesh_proxy_client {
	...
	struct bt_work send_beacons;
#if defined(CONFIG_BT_MESH_PRIV_BEACONS)
	bool privacy;
#endif
} clients[CONFIG_BT_MAX_CONN] = {
	[0 ... (CONFIG_BT_MAX_CONN - 1)] = {
		.send_beacons = BT_WORK_INITIALIZER(proxy_send_beacons),
	},
};
			       ...
			       struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_reserve(buf, 10);

	bt_buf_simple_add_u8(buf, CFG_FILTER_STATUS);

	if (client->filter_type == ACCEPT) {
		bt_buf_simple_add_u8(buf, 0x00);
	} else {
		bt_buf_simple_add_u8(buf, 0x01);
	}
	...
	bt_buf_simple_add_be16(buf, filter_size);
...
static void proxy_filter_recv(struct bt_conn *conn,
			      struct bt_mesh_net_rx *rx, struct bt_buf_simple *buf)
{
	...
	case CFG_FILTER_ADD:
		while (buf->len >= 2) {
			...
			addr = bt_buf_simple_pull_be16(buf);
		       ...
		       struct bt_mesh_subnet *sub)
{
	...
	BT_BUF_SIMPLE_DEFINE(buf, 28);

	bt_buf_simple_reserve(&buf, 1);
...
void bt_mesh_proxy_addr_add(struct bt_buf_simple *buf, uint16_t addr)
{
	struct bt_mesh_proxy_client *client;
	struct bt_mesh_proxy_role *cli =
