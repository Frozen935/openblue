...
static bt_slist_t bap_broadcast_source_cbs = BT_SLIST_STATIC_INIT(&bap_broadcast_source_cbs);
...
static void broadcast_source_set_state(struct bt_bap_broadcast_source *source, uint8_t state)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&source->subgroups, subgroup, _node) {
		struct bt_bap_stream *stream;

		BT_SLIST_FOR_EACH_CONTAINER(&subgroup->streams, stream, _node) {
...
static struct bt_bap_broadcast_subgroup *broadcast_source_new_subgroup(uint8_t index)
{
	for (size_t i = 0; i < ARRAY_SIZE(broadcast_source_subgroups[index]); i++) {
		...
		if (bt_slist_is_empty(&subgroup->streams)) {
			return subgroup;
		}
	}
...
static bool encode_base_subgroup(struct bt_bap_broadcast_subgroup *subgroup,
				 ...
				 uint8_t *streams_encoded, struct bt_buf_simple *buf)
{
	struct bt_bap_stream *stream;
	...
	bt_buf_simple_add_u8(buf, stream_count);
	bt_buf_simple_add_u8(buf, codec_cfg->id);
	bt_buf_simple_add_le16(buf, codec_cfg->cid);
	bt_buf_simple_add_le16(buf, codec_cfg->vid);

	bt_buf_simple_add_u8(buf, codec_cfg->data_len);
	...
	bt_buf_simple_add_u8(buf, codec_cfg->meta_len);
	...
	bt_buf_simple_add_mem(buf, codec_cfg->meta, codec_cfg->meta_len);
	...
	for (uint8_t i = 0U; i < stream_count; i++) {
		...
		bt_buf_simple_add_u8(buf, bis_index);
		...
		bt_buf_simple_add_u8(buf, stream_data[i].data_len);
#if CONFIG_BT_AUDIO_CODEC_CFG_MAX_DATA_SIZE > 0
		...
		bt_buf_simple_add_mem(buf, stream_data[i].data, stream_data[i].data_len);
#endif /* CONFIG_BT_AUDIO_CODEC_CFG_MAX_DATA_SIZE > 0 */
...
static bool encode_base(struct bt_bap_broadcast_source *source, struct bt_buf_simple *buf)
{
	struct bt_bap_broadcast_subgroup *subgroup;
	...
	bt_buf_simple_add_le16(buf, BT_UUID_BASIC_AUDIO_VAL);

	bt_buf_simple_add_le24(buf, source->qos->pd);
	bt_buf_simple_add_u8(buf, subgroup_count);
...
static void broadcast_source_cleanup(struct bt_bap_broadcast_source *source)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&source->subgroups, subgroup, next_subgroup, _node) {
		struct bt_bap_stream *stream, *next_stream;

		BT_SLIST_FOR_EACH_CONTAINER_SAFE(&subgroup->streams, stream, next_stream, _node) {
			...
			bt_slist_remove(&subgroup->streams, NULL, &stream->_node);
		}
		bt_slist_remove(&source->subgroups, NULL, &subgroup->_node);
	}
			 ...
			 const struct bt_bap_broadcast_source_stream_param *stream_param)
{
	...
	/* Fake merge subgroup codec configuration with the BIS configuration to check the merged
	 * size.
	 * As per the BAP spec, if a value exist at level 2 (subgroup) and 3 (BIS), then it is
	 * the value at level 3 that shall be used
	 */
	if (subgroup_codec_cfg->id == BT_HCI_CODING_FORMAT_LC3) {
		struct bt_audio_codec_cfg merged_codec_cfg;
		int err;

		(void)memcpy(merged_codec_cfg.data, subgroup_codec_cfg->data,
			     subgroup_codec_cfg->data_len);
		merged_codec_cfg.data_len = subgroup_codec_cfg->data_len;

		err = bt_audio_data_parse(stream_param->data, stream_param->data_len,
					  merge_bis_and_subgroup_data_cb, &merged_codec_cfg);
		if (err != 0) {
		 ...
		 */
		if (subgroup_codec_cfg->data_len + stream_param->data_len >
		    sizeof(subgroup_codec_cfg->data)) {
			LOG_DBG("Could not merge BIS and subgroup config (%u > %u)",
				subgroup_codec_cfg->data_len + stream_param->data_len,
				sizeof(subgroup_codec_cfg->data));
	...
	const struct bt_bap_broadcast_source *source)
{
	...
	for (size_t i = 0U; i < subgroup_param->params_count; i++) {
		...
		if (!can_merge_codec_cfg_data(subgroup_param->codec_cfg, stream_param)) {
			LOG_DBG("Cannot merge subgroup and BIS subgroup codec_cfg data");
			return false;
		}
	}
#endif /* CONFIG_BT_AUDIO_CODEC_CFG_MAX_DATA_SIZE > 0 */
...
static enum bt_bap_ep_state broadcast_source_get_state(struct bt_bap_broadcast_source *source)
{
	...
	if (bt_slist_is_empty(&source->subgroups)) {
				   ...
				   struct bt_bap_broadcast_source **out_source)
{
	...
	*out_source = NULL;
	...
	for (index = 0; index < ARRAY_SIZE(broadcast_sources); index++) {
		if (bt_slist_is_empty(&broadcast_sources[index].subgroups)) { /* Find free entry */
	...
	for (size_t i = 0U; i < param->params_count; i++) {
		...
		bt_slist_append(&source->subgroups, &subgroup->_node);
		...
		for (size_t j = 0U; j < subgroup_param->params_count; j++) {
			...
			bt_slist_append(&subgroup->streams, &stream->_node);
			stream_count++;
		}
	}
...
static bool valid_broadcast_source_reconfig_param(const struct bt_bap_broadcast_source_param *param,
						  struct bt_bap_broadcast_source *source)
{
	struct bt_bap_broadcast_subgroup *subgroup;
	size_t subgroup_cnt = 0U;
	size_t bis_count = 0U;
	...
	BT_SLIST_FOR_EACH_CONTAINER(&source->subgroups, subgroup, _node) {
		...
		for (size_t i = 0U; i < subgroup_param->params_count; i++) {
			...
			BT_SLIST_FOR_EACH_CONTAINER(&subgroup->streams, subgroup_stream, _node) {
...
static void broadcast_source_reconfig_update_subgroup(
				     ...
				     struct bt_bap_broadcast_source_param *param)
{
	...
	if (!valid_broadcast_source_reconfig_param(param, source)) {
		return -EINVAL;
	}
	...
	for (size_t i = 0U; i < param->params_count; i++) {
		...
		if (i == 0) {
			subgroup =
				BT_SLIST_PEEK_HEAD_CONTAINER(&source->subgroups, subgroup, _node);
		} else {
			subgroup = BT_SLIST_PEEK_NEXT_CONTAINER(subgroup, _node);
		}

		subgroup_param = &param->params[i];

		broadcast_source_reconfig_update_subgroup(source, subgroup, subgroup_param);
	}
...
static void big_started_cb(struct bt_iso_big *big)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&bap_broadcast_source_cbs, listener, _node) {
...
int bt_bap_broadcast_source_register_cb(struct bt_bap_broadcast_source_cb *cb)
{
	...
	if (bt_slist_find(&bap_broadcast_source_cbs, &cb->_node, NULL)) {
	...
	bt_slist_append(&bap_broadcast_source_cbs, &cb->_node);
...
int bt_bap_broadcast_source_unregister_cb(struct bt_bap_broadcast_source_cb *cb)
{
	...
	if (!bt_slist_find_and_remove(&bap_broadcast_source_cbs, &cb->_node)) {
