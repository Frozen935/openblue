...
static struct bt_work_q settings_work_q;
...
void bt_mesh_settings_store_schedule(enum bt_mesh_settings_flag flag)
{
	...
	if (timeout_ms < remaining_ms) {
		if (IS_ENABLED(CONFIG_BT_MESH_SETTINGS_WORKQ)) {
			bt_work_reschedule_for_queue(&settings_work_q, &pending_store,
						    OS_MSEC(timeout_ms));
		} else {
			bt_work_reschedule(&pending_store, OS_MSEC(timeout_ms));
		}
	} else {
		if (IS_ENABLED(CONFIG_BT_MESH_SETTINGS_WORKQ)) {
			bt_work_schedule_for_queue(&settings_work_q, &pending_store,
						  OS_MSEC(timeout_ms));
		} else {
			bt_work_schedule(&pending_store, OS_MSEC(timeout_ms));
		}
	}
}
...
{
	(void)bt_work_cancel_delayable(&pending_store);
