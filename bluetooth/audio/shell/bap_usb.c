...
BT_MEM_POOL_DEFINE_STATIC(usb_in_buf_pool, ROUND_UP(USB_STEREO_FRAME_SIZE, UDC_BUF_GRANULARITY),
...
static void usb_data_request(const struct device *dev)
{
	...
	err = bt_mem_pool_alloc(&usb_in_buf_pool, &pcm_buf, OS_TIMEOUT_NO_WAIT);
	if (err != 0) {
	...
	if (err != 0) {
		...
		bt_mem_pool_free(&usb_in_buf_pool, pcm_buf);
	}
}
...
static void usb_buf_release_cb(const struct device *dev, uint8_t terminal, void *buf,
			       void *user_data)
{
	bt_mem_pool_free(&usb_in_buf_pool, buf);
}
 ...
 */
BT_MEM_POOL_DEFINE_STATIC(usb_out_buf_pool, USB_STEREO_FRAME_SIZE, 3, UDC_BUF_ALIGN);
				 ...
				 void *user_data)
{
	...
	__ASSERT_MSG(size <= USB_STEREO_FRAME_SIZE, "%u was not <= %d", size, USB_STEREO_FRAME_SIZE);

	ret = bt_mem_pool_alloc(&usb_out_buf_pool, &buf, OS_TIMEOUT_NO_WAIT);
	if (ret != 0) {
			     ...
			     void *user_data)
{
	...
	if (!out_terminal_enabled || buf == NULL || size == 0U) {
		bt_mem_pool_free(&usb_out_buf_pool, buf);
		return;
	}
...
int bap_usb_init(void)
{
	...
	if (!bt_hci_is_ready(uac2_headset)) {
