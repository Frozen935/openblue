...
{
	bt_buf_simple_init(msg, 0);

	switch (BT_MESH_MODEL_OP_LEN(opcode)) {
	case 1:
		bt_buf_simple_add_u8(msg, opcode);
		break;
	case 2:
		bt_buf_simple_add_be16(msg, opcode);
		break;
	case 3:
		bt_buf_simple_add_u8(msg, ((opcode >> 16) & 0xff));
		/* Using LE for the CID since the model layer is defined as
		 * little-endian in the mesh spec and using BT_MESH_MODEL_OP_3
		 * will declare the opcode in this way.
		 */
		bt_buf_simple_add_le16(msg, opcode & 0xffff);
		break;
	default:
...
int bt_mesh_msg_ack_ctx_wait(struct bt_mesh_msg_ack_ctx *ack, os_timeout_t timeout)
{
	int err;

	err = os_sem_take(&ack->sem, timeout);
		     ...
		     struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_reset(model->pub->msg);
	bt_buf_simple_add_mem(model->pub->msg, buf->data, buf->len);

	return bt_mesh_model_publish(model);
}

int bt_mesh_msg_ackd_send(const struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
			  struct bt_buf_simple *buf, const struct bt_mesh_msg_rsp_ctx *rsp)
{
	int err;
	...
	if (!err) {
		return bt_mesh_msg_ack_ctx_wait(rsp->ack, OS_MSEC(rsp->timeout));
	}
