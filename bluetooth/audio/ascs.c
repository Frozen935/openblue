...
struct bt_ascs_ase {
	...
	struct bt_work_delayable disconnect_work;
	struct bt_work_delayable state_transition_work;
...
static OS_SEM_DEFINE(ase_buf_sem, 1, 1);
BT_BUF_SIMPLE_DEFINE_STATIC(ase_buf, ASE_BUF_SIZE);

static int control_point_notify(struct bt_conn *conn, const void *data, uint16_t len);
static int ascs_ep_get_status(struct bt_bap_ep *ep, struct bt_buf_simple *buf);
...
static void ase_free(struct bt_ascs_ase *ase)
{
	...
	(void)bt_work_cancel_delayable(&ase->disconnect_work);
	(void)bt_work_cancel_delayable(&ase->state_transition_work);
}
...
static int ase_state_notify(struct bt_ascs_ase *ase)
{
	...
	err = os_sem_take(&ase_buf_sem, ASE_BUF_SEM_TIMEOUT);
	if (err != 0) {
	...
	os_sem_give(&ase_buf_sem);

	return err;
}

static void ascs_disconnect_stream_work_handler(struct bt_work *work)
{
	...
	if (pair_stream != NULL) {
		...
		(void)bt_work_cancel_delayable(&pair_ase->disconnect_work);
	}
...
static int ascs_disconnect_stream(struct bt_bap_stream *stream)
{
	...
	return bt_work_reschedule(&ase->disconnect_work,
				 OS_MSEC(CONFIG_BT_ASCS_ISO_DISCONNECT_DELAY));
}
...
static void state_transition_work_handler(struct bt_work *work)
{
	struct bt_work_delayable *d_work = bt_work_delayable_from_work(work);
...
int ascs_ep_set_state(struct bt_bap_ep *ep, enum bt_bap_ep_state state)
{
	...
	err = bt_work_schedule(&ase->state_transition_work, OS_TIMEOUT_NO_WAIT);
	if (err < 0) {
...
static void ascs_ep_get_status_config(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	struct bt_ascs_ase_status_config *cfg;
	struct bt_bap_qos_cfg_pref *pref = &ep->qos_pref;

	cfg = bt_buf_simple_add(buf, sizeof(*cfg));
	...
	bt_buf_simple_add_mem(buf, ep->codec_cfg.data, ep->codec_cfg.data_len);
}

static void ascs_ep_get_status_qos(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	struct bt_ascs_ase_status_qos *qos;

	qos = bt_buf_simple_add(buf, sizeof(*qos));
...
static void ascs_ep_get_status_enable(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	struct bt_ascs_ase_status_enable *enable;

	enable = bt_buf_simple_add(buf, sizeof(*enable));
	enable->cig_id = ep->cig_id;
	enable->cis_id = ep->cis_id;

	enable->metadata_len = ep->codec_cfg.meta_len;
	bt_buf_simple_add_mem(buf, ep->codec_cfg.meta, ep->codec_cfg.meta_len);
...
static int ascs_ep_get_status(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_reset(buf);

	(void)bt_buf_simple_add_u8(buf, ep->id);
	(void)bt_buf_simple_add_u8(buf, ep->state);
			  ...
			  struct bt_buf *buf)
{
	...
	if (IS_ENABLED(CONFIG_BT_BAP_DEBUG_STREAM_DATA)) {
		LOG_DBG("stream %p ep %p len %zu", stream, stream->ep, bt_buf_frags_len(buf));
	}
...
BT_BUF_SIMPLE_DEFINE_STATIC(cp_rsp_buf, CP_RSP_BUF_SIZE);

static void ascs_cp_rsp_init(uint8_t op)
{
	struct bt_ascs_cp_rsp *rsp;

	bt_buf_simple_reset(&cp_rsp_buf);

	rsp = bt_buf_simple_add(&cp_rsp_buf, sizeof(*rsp));
	rsp->op = op;
	rsp->num_ase = 0;
}
...
static void ascs_cp_rsp_add(uint8_t id, uint8_t code, uint8_t reason)
{
	...
	ase_rsp = bt_buf_simple_add(&cp_rsp_buf, sizeof(*ase_rsp));
	ase_rsp->id = id;
	ase_rsp->code = code;
...
static bool is_valid_config_len(struct bt_conn *conn, struct bt_buf_simple *buf)
{
	...
	op = bt_buf_simple_pull_mem(buf, sizeof(*op));
	...
	for (uint8_t i = 0U; i < op->num_ases; i++) {
		...
		config = bt_buf_simple_pull_mem(buf, sizeof(*config));
		if (buf->len < config->cc_len) {
			LOG_WRN("Malformed codec specific config");
			return false;
		}

		(void)bt_buf_simple_pull_mem(buf, config->cc_len);
	}
...
static ssize_t ascs_config(struct bt_conn *conn, struct bt_buf_simple *buf)
{
	...
	req = bt_buf_simple_pull_mem(buf, sizeof(*req));
	...
	for (uint8_t i = 0; i < req->num_ases; i++) {
		...
		(void)bt_buf_simple_pull(buf, cfg->cc_len);
...
static bool is_valid_qos_len(struct bt_conn *conn, struct bt_buf_simple *buf)
{
	...
	(void)bt_buf_simple_pull_mem(buf, params_size);
...
static ssize_t ascs_qos(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static bool is_valid_enable_len(struct bt_conn *conn, struct bt_buf_simple *buf)
{
	...
	for (uint8_t i = 0U; i < op->num_ases; i++) {
		...
		metadata = bt_buf_simple_pull_mem(buf, sizeof(*metadata));
		if (buf->len < metadata->len) {
			LOG_WRN("Malformed metadata");
			return false;
		}

		(void)bt_buf_simple_pull_mem(buf, metadata->len);
	}
...
static ssize_t ascs_enable(struct bt_conn *conn, struct bt_buf_simple *buf)
{
	...
	for (uint8_t i = 0U; i < req->num_ases; i++) {
		...
		(void)bt_buf_simple_pull(buf, meta->len);
...
static bool is_valid_start_len(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static ssize_t ascs_start(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static bool is_valid_disable_len(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static ssize_t ascs_disable(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static bool is_valid_stop_len(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static ssize_t ascs_stop(struct bt_conn *conn, struct bt_buf_simple *buf)
{
	const struct bt_ascs_start_op *req;
...
static bool is_valid_metadata_len(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static ssize_t ascs_metadata(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static bool is_valid_release_len(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static ssize_t ascs_release(struct bt_conn *conn, struct bt_buf_simple *buf)
{
			     ...
			     uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	bt_buf_simple_init_with_data(&buf, (void *) data, len);
...
#define BT_ASCS_ASE_SINKS \
	UTIL_LISTIFY(CONFIG_BT_ASCS_MAX_ASE_SNK_COUNT, BT_ASCS_ASE_SNK_DEFINE, (,)),
#else
#define BT_ASCS_ASE_SINKS
...
#define BT_ASCS_ASE_SOURCES \
	UTIL_LISTIFY(CONFIG_BT_ASCS_MAX_ASE_SRC_COUNT, BT_ASCS_ASE_SRC_DEFINE, (,)),
#else
#define BT_ASCS_ASE_SOURCES
