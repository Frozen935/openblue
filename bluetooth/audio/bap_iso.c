...
void bt_bap_iso_bind_ep(struct bt_bap_iso *iso, struct bt_bap_ep *ep)
{
	...
	__ASSERT_NO_MSG(iso != NULL);
	__ASSERT_MSG(ep->iso == NULL, "ep %p bound with iso %p already", ep, ep->iso);
	__ASSERT_MSG(ep->dir == BT_AUDIO_DIR_SINK || ep->dir == BT_AUDIO_DIR_SOURCE,
	...
	__ASSERT_MSG(iso_dir->ep == NULL, "iso %p bound with ep %p", iso, iso_dir);
	iso_dir->ep = ep;
	...
	if (paired_ep != NULL && paired_ep->stream != NULL && paired_ep->stream->conn != NULL &&
		...
		__ASSERT_MSG(paired_ep->stream->conn == ep->stream->conn,
...
void bt_bap_iso_unbind_ep(struct bt_bap_iso *iso, struct bt_bap_ep *ep)
{
	...
	__ASSERT_NO_MSG(ep != NULL);
	__ASSERT_NO_MSG(iso != NULL);
	__ASSERT_MSG(ep->iso == iso, "ep %p not bound with iso %p, was bound to %p",
		 ep, iso, ep->iso);
	__ASSERT_MSG(ep->dir == BT_AUDIO_DIR_SINK || ep->dir == BT_AUDIO_DIR_SOURCE,
		 "Invalid dir: %u", ep->dir);
	...
	iso_dir = bap_iso_get_iso_dir(is_unicast_client_ep(ep), iso, ep->dir);
	__ASSERT_MSG(iso_dir->ep == ep, "iso %p not bound with ep %p", iso, ep);
	iso_dir->ep = NULL;
				    ...
				    enum bt_audio_dir dir)
{
	...
	__ASSERT_MSG(dir == BT_AUDIO_DIR_SINK || dir == BT_AUDIO_DIR_SOURCE,
		 "invalid dir: %u", dir);
			    ...
			    enum bt_audio_dir dir)
{
	...
	__ASSERT_MSG(stream->iso == NULL, "stream %p bound with bap_iso %p already", stream,
	...
	__ASSERT_MSG(bap_iso_ep->stream == NULL, "bap_iso %p bound with stream %p", bap_iso,
		 bap_iso_ep->stream);
...
void bt_bap_iso_unbind_stream(struct bt_bap_stream *stream, enum bt_audio_dir dir)
{
	...
	__ASSERT_NO_MSG(stream != NULL);
	__ASSERT_MSG(stream->iso != NULL, "stream %p not bound with an bap_iso", stream);
	...
	__ASSERT_MSG(bap_iso_ep->stream == stream, "bap_iso %p (%p) not bound with stream %p (%p)",
