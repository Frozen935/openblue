...
static int data_status(const struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
		       struct bt_buf_simple *buf, uint32_t op,
				  ...
				  struct bt_mesh_large_comp_data_rsp *rsp))
{
	...
	offset = bt_buf_simple_pull_le16(buf);
	total_size = bt_buf_simple_pull_le16(buf);
	...
	if (bt_mesh_msg_ack_ctx_match(&cli->ack_ctx, op, ctx->addr, (void **)&rsp)) {
		...
		if (rsp->data) {
			bt_buf_simple_add_mem(rsp->data, buf->data,
					       MIN(bt_buf_simple_tailroom(rsp->data), buf->len));
		}
	     ...
	     size_t offset, struct bt_mesh_large_comp_data_rsp *rsp)
{
	...
	bt_buf_simple_add_le16(&msg, offset);
