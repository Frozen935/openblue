...
#ifndef __INCLUDE_LIST_DLIST_H__
#define __INCLUDE_LIST_DLIST_H__
...
struct _dnode {
	union {
		...
		struct _dnode *next; /* ptr to next node    (bt_dnode_t) */
	};
	union {
		struct _dnode *tail; /* ptr to tail of list (bt_dlist_t) */
		struct _dnode *prev; /* ptr to previous node (bt_dnode_t) */
	};
};
 ...
 */
typedef struct _dnode bt_dlist_t;
/**
 ...
 *
 *     BT_DLIST_FOR_EACH_NODE(l, n) {
 *         <user code>
 *     }
 *
 * This and other BT_DLIST_*() macros are not thread safe.
 *
 ...
 * User _MUST_ add the loop statement curly braces enclosing its own code:
 *
 *     BT_DLIST_ITERATE_FROM_NODE(l, n) {
 *         <user code>
 *     }
 *
 * Like BT_DLIST_FOR_EACH_NODE(), but __dn already contains a node in the list
 ...
 * User _MUST_ add the loop statement curly braces enclosing its own code:
 *
 *     BT_DLIST_FOR_EACH_NODE_SAFE(l, n, s) {
 *         <user code>
 *     }
 *
 * This and other BT_DLIST_*() macros are not thread safe.
 ...
 *
 * @param __dn A pointer on a bt_dnode_t to get its container
 ...
 *
 * @param __dl A pointer on a bt_dlist_t to peek
 * @param __cn Container struct type pointer
 * @param __n The field name of bt_dnode_t within the container struct
 */
 ...
 * User _MUST_ add the loop statement curly braces enclosing its own code:
 *
 *     BT_DLIST_FOR_EACH_CONTAINER(l, c, n) {
 *         <user code>
 *     }
 *
 * @param __dl A pointer on a bt_dlist_t to iterate on
 ...
 * User _MUST_ add the loop statement curly braces enclosing its own code:
 *
 *     BT_DLIST_FOR_EACH_CONTAINER_SAFE(l, c, cn, n) {
 *         <user code>
 *     }
 *
 * @param __dl A pointer on a bt_dlist_t to iterate on
...
static inline void bt_dlist_init(bt_dlist_t *list)
{
	list->head = (bt_dnode_t *)list;
	list->tail = (bt_dnode_t *)list;
}
...
static inline void bt_dnode_init(bt_dnode_t *node)
{
	node->next = NULL;
	node->prev = NULL;
}
...
static inline bool bt_dnode_is_linked(const bt_dnode_t *node)
{
...
static inline bool bt_dlist_is_head(const bt_dlist_t *list, const bt_dnode_t *node)
{
...
static inline bool bt_dlist_is_tail(const bt_dlist_t *list, const bt_dnode_t *node)
{
...
static inline bool bt_dlist_is_empty(const bt_dlist_t *list)
{
 ...
 *
 * This and other bt_dlist_*() functions are not thread safe.
 *
 * @param list the doubly-linked list to operate on
 *
...
static inline bool bt_dlist_has_multiple_nodes(const bt_dlist_t *list)
{
...
static inline bt_dnode_t *bt_dlist_peek_head(const bt_dlist_t *list)
{
	return bt_dlist_is_empty(list) ? NULL : list->head;
}
...
static inline bt_dnode_t *bt_dlist_peek_head_not_empty(const bt_dlist_t *list)
{
	return list->head;
}
 ...
 *
 * Faster than bt_dlist_peek_next() if node is known not to be NULL.
 *
 * @param list the doubly-linked list to operate on
 ...
 *
 * Faster than bt_dlist_peek_prev() if node is known not to be NULL.
 *
 * @param list the doubly-linked list to operate on
...
static inline bt_dnode_t *bt_dlist_peek_tail(const bt_dlist_t *list)
{
	return bt_dlist_is_empty(list) ? NULL : list->tail;
}
...
static inline void bt_dlist_append(bt_dlist_t *list, bt_dnode_t *node)
{
	bt_dnode_t *const tail = list->tail;
...
static inline void bt_dlist_prepend(bt_dlist_t *list, bt_dnode_t *node)
{
	bt_dnode_t *const head = list->head;
 ...
 */
static inline void bt_dlist_insert(bt_dnode_t *successor, bt_dnode_t *node)
{
	bt_dnode_t *const prev = successor->prev;
	...
	int (*cond)(bt_dnode_t *node, void *data), void *data)
{
	if (bt_dlist_is_empty(list)) {
		...
		bt_dnode_t *pos = bt_dlist_peek_head(list);

		while ((pos != NULL) && (cond(pos, data) == 0)) {
			pos = bt_dlist_peek_next(list, pos);
		}
		if (pos != NULL) {
			bt_dlist_insert(pos, node);
		} else {
			bt_dlist_append(list, node);
		}
	}
 ...
 *
 * Like :c:func:`bt_dlist_remove()`, this routine removes a specific node
 * from a list. However, unlike :c:func:`bt_dlist_remove()`, this routine
 * does not re-initialize the removed node. One significant implication of
 * this difference is that the function :c:func`bt_dnode_is_linked()` will
 ...
 */
static inline void bt_dlist_dequeue(bt_dnode_t *node)
{
	bt_dnode_t *const prev = node->prev;
	bt_dnode_t *const next = node->next;
...
static inline void bt_dlist_remove(bt_dnode_t *node)
{
	bt_dnode_t *const prev = node->prev;
	bt_dnode_t *const next = node->next;
...
static inline bt_dnode_t *bt_dlist_get(bt_dlist_t *list)
{
	bt_dnode_t *node = NULL;

	if (!bt_dlist_is_empty(list)) {
		node = list->head;
		bt_dlist_remove(node);
	}
 ...
 */
static inline size_t bt_dlist_len(const bt_dlist_t *list)
{
	size_t len = 0;
	bt_dnode_t *node = NULL;

	BT_DLIST_FOR_EACH_NODE(list, node) {
		len++;
	}
	return len;
}
...
#endif /* __INCLUDE_LIST_DLIST_H__ */
