...
struct bt_avctp_ops_cb {
	...
	int (*recv)(struct bt_avctp *session, struct bt_buf *buf, bt_avctp_cr_t cr, uint8_t tid);
	struct bt_buf *(*alloc_buf)(struct bt_avctp *session);
};
...
struct bt_avctp {
	...
	struct bt_buf_pool *tx_pool;
	struct bt_buf_pool *rx_pool;
	struct bt_buf *reassembly_buf;
};
