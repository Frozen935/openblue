...
#define L2CAP_CONN_TIMEOUT	OS_SECONDS(CONFIG_BT_L2CAP_CONN_RTX_TIMEOUT)
#define L2CAP_DISC_TIMEOUT	OS_SECONDS(2)
/** @brief Local L2CAP RTX (Response Timeout eXpired)
 *
 *  Specification-allowed range for the value of RTX is 1 to 60 seconds.
 */
#define L2CAP_RTX_TIMEOUT	OS_SECONDS(2)
 ...
 */
BT_BUF_POOL_FIXED_DEFINE(disc_pool, 1,
			  BT_L2CAP_BUF_SIZE(
...
__l2cap_lookup_ident(struct bt_conn *conn, uint16_t ident, uint16_t req_opcode, bool remove)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&conn->channels, chan, node) {
		if ((BT_L2CAP_LE_CHAN(chan)->ident == ident) &&
			...
			if (remove) {
				bt_slist_remove(&conn->channels, prev,
						 &chan->node);
			}
...
static void l2cap_chan_del(struct bt_l2cap_chan *chan)
{
	 ...
	 */
	while ((buf = bt_fifo_get(&le_chan->tx_queue, OS_TIMEOUT_NO_WAIT))) {
...
#if defined(CONFIG_BT_L2CAP_DYNAMIC_CHANNEL)
static void l2cap_rtx_timeout(struct bt_work *work)
{
...
static void l2cap_rx_process(struct bt_work *work)
{
	...
	while ((buf = bt_fifo_get(&ch->rx_queue, OS_TIMEOUT_NO_WAIT))) {
		       ...
		       bt_l2cap_chan_destroy_t destroy)
{
	...
	bt_slist_append(&conn->channels, &chan->node);
	chan->conn = conn;
			   ...
			   bt_l2cap_chan_destroy_t destroy)
{
	 ...
	 */
	bt_work_init_delayable(&le_chan->rtx_work, l2cap_rtx_timeout);

	if (L2CAP_LE_CID_IS_DYN(le_chan->rx.cid)) {
		bt_work_init(&le_chan->rx_work, l2cap_rx_process);
		bt_fifo_init(&le_chan->rx_queue);
...
static struct bt_buf *l2cap_create_le_sig_pdu(uint8_t code, uint8_t ident,
					       uint16_t len)
{
	struct bt_l2cap_sig_hdr *hdr;
	struct bt_buf_pool *pool = NULL;
	struct bt_buf *buf;
 ...
 * handler.
 */
static int l2cap_send_sig(struct bt_conn *conn, struct bt_buf *buf)
{
...
static void raise_data_ready(struct bt_l2cap_le_chan *le_chan)
{
	...
	if (!bt_slist_find(&le_chan->chan.conn->l2cap_data_ready,
				 &le_chan->_pdu_ready, NULL)) {
		bt_slist_append(&le_chan->chan.conn->l2cap_data_ready,
				 &le_chan->_pdu_ready);
		      ...
		      bt_conn_tx_cb_t cb, void *user_data)
{
	...
	bt_fifo_put(&le_chan->tx_queue, pdu);
...
static bool chan_has_data(struct bt_l2cap_le_chan *lechan)
{
	return !bt_fifo_is_empty(&lechan->tx_queue);
}
...
static struct bt_l2cap_le_chan *get_ready_chan(struct bt_conn *conn)
{
	...
	bt_snode_t *pdu_ready = bt_slist_peek_head(&conn->l2cap_data_ready);
	...
	BT_SLIST_FOR_EACH_CONTAINER(&conn->l2cap_data_ready, lechan, _pdu_ready) {
...
struct bt_buf *l2cap_data_pull(struct bt_conn *conn,
				size_t amount,
				size_t *length)
{
	 ...
	 */
	struct bt_buf *fifo_pdu = bt_fifo_peek_head(&lechan->tx_queue);
	...
	struct bt_buf *pdu = bt_buf_ref(fifo_pdu);
	...
	if (lechan->_pdu_remaining == 0) {
		...
		hdr = bt_buf_push(pdu, sizeof(*hdr));
	...
	if (last_frag && last_seg) {
		...
		fifo_pdu = bt_fifo_get(&lechan->tx_queue, OS_TIMEOUT_NO_WAIT);

		__ASSERT_NO_MSG(fifo_pdu == pdu);

		bt_buf_unref(fifo_pdu);
	}
...
struct bt_l2cap_server *bt_l2cap_server_lookup_psm(uint16_t psm)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&servers, server, node) {
		if (server->psm == psm) {
			return server;
		}
	}
...
int bt_l2cap_server_register(struct bt_l2cap_server *server)
{
	...
	bt_slist_append(&servers, &server->node);
...
static void l2cap_chan_destroy(struct bt_l2cap_chan *chan)
{
	 ...
	 */
	struct bt_work_q *rtx_work_queue = le_chan->rtx_work.queue;

	if (rtx_work_queue == NULL ||
	    os_thread_self() != bt_work_queue_thread_get(rtx_work_queue)) {
		bt_work_cancel_delayable_sync(&le_chan->rtx_work, &le_chan->rtx_sync);
	} else {
		bt_work_cancel_delayable(&le_chan->rtx_work);
	}

	/* Remove buffers on the SDU RX queue */
	while ((buf = bt_fifo_get(&le_chan->rx_queue, OS_TIMEOUT_NO_WAIT))) {
		bt_buf_unref(buf);
	}

	/* Destroy segmented SDU if it exists */
	if (le_chan->_sdu) {
		bt_buf_unref(le_chan->_sdu);
		le_chan->_sdu = NULL;
		le_chan->_sdu_len = 0U;
	}
}
...
response:
	...
	bt_buf_add_mem(buf, dcid, sizeof(scid) * req_cid_count);
				...
				struct bt_buf *buf)
{
	...
	while ((ch = l2cap_lookup_ident(conn, ident, BT_L2CAP_ECRED_RECONF_REQ))) {
		 ...
		 * which one it is.
		 */
		bt_work_cancel_delayable(&ch->rtx_work);
			      ...
			      struct bt_buf *buf)
{
	...
	case BT_L2CAP_LE_ERR_ENCRYPTION:
		while ((chan = l2cap_lookup_ident(conn, ident, BT_L2CAP_ECRED_CONN_REQ))) {
			...
			/* Cancel RTX work */
			bt_work_cancel_delayable(&chan->rtx_work);
 ...
 */
static bool bt_atomic_add_safe_u16(bt_atomic_t *target, uint16_t addition)
{
...
int bt_l2cap_chan_give_credits(struct bt_l2cap_chan *chan, uint16_t additional_credits)
{
	...
	if (bt_atomic_add_safe_u16(&le_chan->rx.credits, additional_credits)) {
...
int bt_l2cap_chan_recv_complete(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
	struct bt_l2cap_le_chan *le_chan = BT_L2CAP_LE_CHAN(chan);
...
static struct bt_buf *l2cap_alloc_frag(os_timeout_t timeout, void *user_data)
{
...
static void l2cap_chan_le_recv_sdu(struct bt_l2cap_le_chan *chan,
				   struct bt_buf *buf, uint16_t seg)
{
	int err;
				   ...
				   struct bt_buf *buf)
{
	...
	len = chan->_sdu->len;
	if (len) {
		memcpy(&seg, bt_buf_user_data(chan->_sdu), sizeof(seg));
	}
	...
	memcpy(bt_buf_user_data(chan->_sdu), &seg, sizeof(seg));

	LOG_DBG("chan %p seg %d len %u", chan, seg, buf->len);

	/* Append received segment to SDU */
	len = bt_buf_append_bytes(chan->_sdu, buf->len, buf->data, OS_TIMEOUT_NO_WAIT,
...
#if defined(CONFIG_BT_L2CAP_SEG_RECV)
static void l2cap_chan_le_recv_seg_direct(struct bt_l2cap_le_chan *chan, struct bt_buf *seg)
{
	uint16_t seg_offset;
	...
	if (chan->_sdu_len_done == chan->_sdu_len) {
		...
		chan->_sdu_len = bt_buf_pull_le16(seg);
		chan->_sdu_len_done = 0;
			       ...
			       struct bt_buf *buf)
{
	...
	if (chan->chan.ops->alloc_buf) {
		...
		uint16_t credits = DIV_ROUND_UP(
			MIN(sdu_len - buf->len, bt_buf_tailroom(chan->_sdu)),
			chan->rx.mps);
	...
	owned_ref = bt_buf_ref(buf);
	err = chan->chan.ops->recv(&chan->chan, owned_ref);
	if (err != -EINPROGRESS) {
		bt_buf_unref(owned_ref);
		owned_ref = NULL;
	}
				  ...
				  struct bt_buf *buf)
{
	...
	bt_fifo_put(&chan->rx_queue, buf);
	bt_work_submit(&chan->rx_work);
}
#endif /* CONFIG_BT_L2CAP_DYNAMIC_CHANNEL */

static void l2cap_chan_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf,
			    bool complete)
{
#if defined(CONFIG_BT_L2CAP_DYNAMIC_CHANNEL)
	struct bt_l2cap_le_chan *le_chan = BT_L2CAP_LE_CHAN(chan);
...
void bt_l2cap_recv(struct bt_conn *conn, struct bt_buf *buf, bool complete)
{
	struct bt_l2cap_hdr *hdr;
	struct bt_l2cap_chan *chan;
	uint16_t cid;
...
static void l2cap_disconnected(struct bt_l2cap_chan *chan)
{
	 ...
	 */
	(void)bt_work_cancel_delayable(&le_chan->rtx_work);
#endif /* CONFIG_BT_L2CAP_DYNAMIC_CHANNEL */
}
...
__maybe_unused static bool user_data_not_empty(const struct bt_buf *buf)
{
	size_t ud_len = sizeof(struct closure);
	const uint8_t *ud = bt_buf_user_data(buf);
...
static int bt_l2cap_dyn_chan_send(struct bt_l2cap_le_chan *le_chan, struct bt_buf *buf)
{
	...
	if (bt_buf_headroom(buf) < BT_L2CAP_SDU_CHAN_SEND_RESERVE) {
		/* Call `bt_buf_reserve(buf, BT_L2CAP_SDU_CHAN_SEND_RESERVE)`
	 ...
	 */
	bt_buf_push_le16(buf, sdu_len);

	/* Put buffer on TX queue */
	bt_fifo_put(&le_chan->tx_queue, buf);
...
int bt_l2cap_chan_send(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
	if (!buf || !chan) {
		return -EINVAL;
	}
