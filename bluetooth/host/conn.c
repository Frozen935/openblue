...
#else
	return bt_work_main_work_queue();
#endif /* CONFIG_BT_CONN_TX_NOTIFY_WQ */
}
		    ...
		    size_t len, bt_conn_tx_cb_t cb, void *ud)
{
	...
	if (!pkts || os_sem_take(pkts, OS_TIMEOUT_NO_WAIT)) {
...
static bool cannot_send_to_controller(struct bt_conn *conn)
{
	...
	return !pkts || os_sem_count_get(pkts) == 0;
}
...
#if defined(CONFIG_BT_CONN)
static os_sem_t pending_recycled_events;
...
void bt_conn_unref(struct bt_conn *conn)
{
	...
	if (IS_ARRAY_ELEMENT(acl_conns, conn)) {
		(void)os_sem_give(&pending_recycled_events);
...
#if defined(CONFIG_BT_CONN)
	(void)os_sem_init(&pending_recycled_events, 0, OS_SEM_MAX_LIMIT);
#endif /* CONFIG_BT_CONN */
