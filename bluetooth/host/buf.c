...
static void acl_in_pool_destroy(struct bt_buf *buf)
{
	bt_hci_host_num_completed_packets(buf);
	buf_rx_freed_notify(BT_BUF_ACL_IN);
}

static void evt_pool_destroy(struct bt_buf *buf)
{
	bt_buf_destroy(buf);
	buf_rx_freed_notify(BT_BUF_EVT);
}

BT_BUF_POOL_DEFINE(acl_in_pool, (BT_BUF_ACL_RX_COUNT_EXTRA + BT_BUF_HCI_ACL_RX_COUNT),
		    BT_BUF_ACL_SIZE(CONFIG_BT_BUF_ACL_RX_SIZE), sizeof(struct bt_conn_rx),
		    acl_in_pool_destroy);

BT_BUF_POOL_FIXED_DEFINE(evt_pool, CONFIG_BT_BUF_EVT_RX_COUNT, BT_BUF_EVT_RX_SIZE, 0,
			  evt_pool_destroy);
#else
static void hci_rx_pool_destroy(struct bt_buf *buf)
{
	bt_buf_destroy(buf);
...
BT_BUF_POOL_FIXED_DEFINE(hci_rx_pool, BT_BUF_RX_COUNT, BT_BUF_RX_SIZE,
...
#if defined(CONFIG_BT_HCI_ACL_FLOW_CONTROL)
	if (type == BT_BUF_EVT) {
		buf = bt_buf_alloc(&evt_pool, timeout);
	} else {
		buf = bt_buf_alloc(&acl_in_pool, timeout);
	}
#else
	buf = bt_buf_alloc(&hci_rx_pool, timeout);
#endif
	if (buf) {
		bt_buf_add_u8(buf, bt_buf_type_to_h4(type));
	}
			       ...
			       os_timeout_t timeout)
{
	...
	case BT_HCI_EVT_CMD_COMPLETE:
		buf = bt_buf_alloc(&sync_evt_pool, timeout);
		break;
	default:
		if (discardable) {
			/* Discardable, decided in Host-side HCI Transport driver. */
			buf = bt_buf_alloc(&discardable_pool, timeout);
		} else {
	...
	if (buf) {
		bt_buf_add_u8(buf, BT_HCI_H4_EVT);
	}
...
#if defined(CONFIG_BT_HCI_ACL_FLOW_CONTROL)
struct bt_buf_pool *bt_buf_get_evt_pool(void)
{
	return &evt_pool;
}

struct bt_buf_pool *bt_buf_get_acl_in_pool(void)
{
	return &acl_in_pool;
}
#else
struct bt_buf_pool *bt_buf_get_hci_rx_pool(void)
{
	return &hci_rx_pool;
}
#endif /* CONFIG_BT_HCI_ACL_FLOW_CONTROL */
...
#if defined(CONFIG_BT_CONN) || defined(CONFIG_BT_ISO)
struct bt_buf_pool *bt_buf_get_num_complete_pool(void)
{
...
struct bt_buf *bt_buf_make_view(struct bt_buf *view,
				 ...
				 struct bt_buf_view_meta *meta)
{
	 ...
	 */
	__ASSERT_NO_MSG(bt_buf_headroom(parent) > 0);

	__ASSERT_NO_MSG(!bt_buf_has_view(parent));

	LOG_DBG("make-view %p viewsize %zu meta %p", view, len, meta);

	bt_buf_simple_clone(&parent->b, &view->b);
	view->size = bt_buf_headroom(parent) + len;
	view->len = len;
	view->flags = BT_BUF_EXTERNAL_DATA;

	/* we have a view, eat `len`'s worth of data from the parent */
	(void)bt_buf_pull(parent, len);
...
void bt_buf_destroy_view(struct bt_buf *view, struct bt_buf_view_meta *meta)
{
	...
	bt_buf_unref(meta->parent);

	memset(meta, 0, sizeof(*meta));
	bt_buf_destroy(view);
}

bool bt_buf_has_view(const struct bt_buf *parent)
{
