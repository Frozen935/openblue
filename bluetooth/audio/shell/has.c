...
static int cmd_preset_reg(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err = 0;
	struct bt_has_preset_register_param param = {
		.index = bt_shell_strtoul(argv[1], 16, &err),
		.properties = bt_shell_strtoul(argv[2], 16, &err),
		.name = argv[3],
		.ops = &preset_ops,
	};
...
static int cmd_preset_unreg(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err = 0;
...
static int cmd_has_register(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err;
	struct bt_has_features_param param = {
		.type = BT_HAS_HEARING_AID_TYPE_MONAURAL,
...
static int cmd_preset_list(const struct bt_shell *sh, size_t argc, char **argv)
{
...
static int cmd_preset_avail(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err = 0;
	const uint8_t index = bt_shell_strtoul(argv[1], 16, &err);
...
static int cmd_preset_unavail(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err = 0;
	const uint8_t index = bt_shell_strtoul(argv[1], 16, &err);
...
static int cmd_preset_active_set(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err = 0;
	const uint8_t index = bt_shell_strtoul(argv[1], 16, &err);
...
static int cmd_preset_active_get(const struct bt_shell *sh, size_t argc, char **argv)
{
...
static int cmd_preset_active_clear(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err;
...
static int cmd_preset_name_set(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err = 0;
	const uint8_t index = bt_shell_strtoul(argv[1], 16, &err);
...
BT_SHELL_SUBCMD_SET_CREATE(has_cmds,
	...
	BT_SHELL_CMD_ARG(preset_list, NULL, "List all presets", cmd_preset_list, 1, 0),
	BT_SHELL_CMD_ARG(preset_set_avail, NULL, "Set preset as available <index>",
		      cmd_preset_avail, 2, 0),
	BT_SHELL_CMD_ARG(preset_set_unavail, NULL, "Set preset as unavailable <index>",
		      cmd_preset_unavail, 2, 0),
	BT_SHELL_CMD_ARG(preset_active_set, NULL, "Set active preset <index>",
		      cmd_preset_active_set, 2, 0),
	BT_SHELL_CMD_ARG(preset_active_get, NULL, "Get active preset", cmd_preset_active_get, 1, 0),
...
BT_SHELL_CMD_ARG_DEFINE(has, &has_cmds, "Bluetooth HAS shell commands", cmd_has, 1, 1);

int bt_shell_cmd_has_register(struct bt_shell *sh)
{
	return bt_shell_cmd_register(sh, &has);
}
