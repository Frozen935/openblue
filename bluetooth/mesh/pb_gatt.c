...
int bt_mesh_pb_gatt_recv(struct bt_conn *conn, struct bt_buf_simple *buf)
{
			  ...
			  const struct prov_bearer_cb *cb, void *cb_data)
{
	...
	bt_work_reschedule(&link.prot_timer, OS_SECONDS(timeout));
...
static int buf_send(struct bt_buf_simple *buf, prov_bearer_send_complete_t cb,
		    void *cb_data)
{
	if (!link.conn) {
		return -ENOTCONN;
