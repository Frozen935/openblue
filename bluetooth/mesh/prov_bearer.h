...
#define PROTOCOL_TIMEOUT     OS_SECONDS(PROTOCOL_TIMEOUT_SEC)
#define PROTOCOL_TIMEOUT_EXT OS_SECONDS(PROTOCOL_TIMEOUT_EXT_SEC)
...
struct prov_bearer {
	 ...
	 *  @return Zero on success, or (negative) error code otherwise.
	 */
	int (*send)(struct bt_buf_simple *buf, prov_bearer_send_complete_t cb,
		    void *cb_data);
