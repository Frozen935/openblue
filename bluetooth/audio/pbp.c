			    ...
			    struct bt_buf_simple *pba_data_buf)
{
	...
	bt_buf_simple_add_le16(pba_data_buf, BT_UUID_PBA_VAL);
	bt_buf_simple_add_u8(pba_data_buf, features);
	bt_buf_simple_add_u8(pba_data_buf, meta_len);
	bt_buf_simple_add_mem(pba_data_buf, meta, meta_len);
			      ...
			      uint8_t **meta)
{
	...
	bt_buf_simple_init_with_data(&buf, (void *)data->data, data->data_len);
	uuid = bt_buf_simple_pull_mem(&buf, BT_UUID_SIZE_16);
	...
	*features = bt_buf_simple_pull_u8(&buf);
	meta_len = bt_buf_simple_pull_u8(&buf);
	if (buf.len < meta_len) {
		return -EBADMSG;
	}

	*meta = (uint8_t *)bt_buf_simple_pull_mem(&buf, meta_len);
