...
{
	if (err != 0) {
		bt_shell_error("VCP discover failed (%d)", err);
	} else {
		bt_shell_print("VCP discover done with %u VOCS and %u AICS", vocs_count,
			       aics_count);

		if (bt_vcp_vol_ctlr_included_get(vol_ctlr, &vcp_included)) {
			bt_shell_error("Could not get VCP context");
		}
	}
}
...
static void vcs_vol_down_cb(struct bt_vcp_vol_ctlr *vol_ctlr, int err)
{
	if (err != 0) {
		...
		bt_shell_print("VCP vol_down done");
	}
}
...
static void vcs_vol_up_cb(struct bt_vcp_vol_ctlr *vol_ctlr, int err)
{
	if (err != 0) {
		...
		bt_shell_print("VCP vol_up done");
	}
}
...
static void vcs_unmute_cb(struct bt_vcp_vol_ctlr *vol_ctlr, int err)
{
	if (err != 0) {
		...
		bt_shell_print("VCP unmute done");
	}
}
...
static void vcs_vol_down_unmute_cb(struct bt_vcp_vol_ctlr *vol_ctlr, int err)
{
	if (err != 0) {
		...
		bt_shell_print("VCP vol_down_unmute done");
	}
}
...
static void vcs_vol_up_unmute_cb(struct bt_vcp_vol_ctlr *vol_ctlr, int err)
{
	if (err != 0) {
		...
		bt_shell_print("VCP vol_up_unmute done");
	}
}
...
static void vcs_vol_set_cb(struct bt_vcp_vol_ctlr *vol_ctlr, int err)
{
	if (err != 0) {
		...
		bt_shell_print("VCP vol_set done");
	}
}
...
{
	if (err != 0) {
		bt_shell_error("Set offset failed (%d) for inst %p", err, inst);
	} else {
		bt_shell_print("Offset set for inst %p", inst);
	}
}
...
static int cmd_vcp_vol_ctlr_discover(const struct bt_shell *sh, size_t argc,
				   char **argv)
{
	...
	if (!cb_registered) {
		...
		if (result != 0) {
			bt_shell_print(sh, "CB register failed: %d", result);
			return result;
		}
...
static int cmd_vcp_vol_ctlr_state_get(const struct bt_shell *sh, size_t argc,
				    char **argv)
{
	int result;
...
static int cmd_vcp_vol_ctlr_flags_get(const struct bt_shell *sh, size_t argc,
				    char **argv)
{
	int result;
...
static int cmd_vcp_vol_ctlr_volume_down(const struct bt_shell *sh, size_t argc,
				      char **argv)
{
	int result;
...
static int cmd_vcp_vol_ctlr_volume_up(const struct bt_shell *sh, size_t argc,
				    char **argv)
...
static int cmd_vcp_vol_ctlr_unmute_volume_down(const struct bt_shell *sh,
...
static int cmd_vcp_vol_ctlr_unmute_volume_up(const struct bt_shell *sh,
					   size_t argc, char **argv)
{
	int result;
...
static int cmd_vcp_vol_ctlr_volume_set(const struct bt_shell *sh, size_t argc,
				     char **argv)

{
	...
	if (result != 0) {
		bt_shell_error(sh, "Failed to parse volume: %d", result);

		return -ENOEXEC;
	}

	if (volume > UINT8_MAX) {
		bt_shell_error(sh, "Volume shall be 0-255, was %lu", volume);
...
static int cmd_vcp_vol_ctlr_unmute(const struct bt_shell *sh, size_t argc,
				 char **argv)
{
	int result;
...
static int cmd_vcp_vol_ctlr_mute(const struct bt_shell *sh, size_t argc,
				 char **argv)
{
	int result;
...
static int cmd_vcp_vol_ctlr_vocs_state_get(const struct bt_shell *sh, size_t argc,
					 char **argv)
{
	unsigned long index;
	int result = 0;
	...
	if (result != 0) {
		bt_shell_error(sh, "Failed to parse index: %d", result);

		return -ENOEXEC;
	}

	if (index >= vcp_included.vocs_cnt) {
		bt_shell_error(sh, "Index shall be less than %u, was %lu",
...
static int cmd_vcp_vol_ctlr_vocs_location_get(const struct bt_shell *sh,
					    size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_vocs_location_set(const struct bt_shell *sh,
					    size_t argc, char **argv)
{
	...
	if (result != 0) {
		bt_shell_error(sh, "Failed to parse location: %d", result);

		return -ENOEXEC;
	}

	if (location > UINT32_MAX) {
		bt_shell_error(sh, "Invalid location %lu", location);
		return -ENOEXEC;
	}
...
static int cmd_vcp_vol_ctlr_vocs_offset_set(const struct bt_shell *sh,
					  size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
	...
	if (result != 0) {
		bt_shell_error(sh, "Failed to parse offset: %d", result);

		return -ENOEXEC;
	}

	if (!IN_RANGE(offset, BT_VOCS_MIN_OFFSET, BT_VOCS_MAX_OFFSET)) {
		bt_shell_error(sh, "Offset shall be %d-%d, was %ld",
...
static int cmd_vcp_vol_ctlr_vocs_output_description_get(const struct bt_shell *sh,
						      size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_vocs_output_description_set(const struct bt_shell *sh,
						      size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_aics_input_state_get(const struct bt_shell *sh,
					       size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_aics_gain_setting_get(const struct bt_shell *sh,
						size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_aics_input_type_get(const struct bt_shell *sh,
					      size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_aics_input_status_get(const struct bt_shell *sh,
						size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_aics_input_unmute(const struct bt_shell *sh,
					    size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_aics_input_mute(const struct bt_shell *sh,
					  size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_aics_manual_input_gain_set(const struct bt_shell *sh,
						     size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_aics_auto_input_gain_set(const struct bt_shell *sh,
						   size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_aics_gain_set(const struct bt_shell *sh, size_t argc,
					char **argv)
{
	unsigned long index;
	int result = 0;
	...
	index = bt_shell_strtoul(argv[1], 0, &result);
	if (result != 0) {
		bt_shell_error(sh, "Could not parse index: %d", result);
		return -ENOEXEC;
	}
	...
	if (result != 0) {
		bt_shell_error(sh, "Could not parse gain: %d", result);
		return -ENOEXEC;
	}

	if (!IN_RANGE(gain, INT8_MIN, INT8_MAX)) {
		bt_shell_error(sh, "Gain shall be %d-%d, was %ld",
...
static int cmd_vcp_vol_ctlr_aics_input_description_get(const struct bt_shell *sh,
						     size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr_aics_input_description_set(const struct bt_shell *sh,
						     size_t argc, char **argv)
{
	unsigned long index;
	int result = 0;
...
static int cmd_vcp_vol_ctlr(const struct bt_shell *sh, size_t argc, char **argv)
{
	if (argc > 1) {
...
BT_SHELL_STATIC_SUBCMD_SET_CREATE(vcp_vol_ctlr_cmds,
...
BT_SHELL_CMD_ARG_REGISTER(vcp_vol_ctlr, &vcp_vol_ctlr_cmds,
