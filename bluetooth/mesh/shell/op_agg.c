...
static int cmd_seq_start(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	bt_shell_print("mesh dst set to 0x%04x", elem_addr);

	err = bt_mesh_op_agg_cli_seq_start(bt_mesh_shell_target_ctx.net_idx,
					   bt_mesh_shell_target_ctx.app_idx,
					   bt_mesh_shell_target_ctx.dst, elem_addr);
	if (err) {
		bt_shell_error("Failed to configure Opcodes Aggregator Context (err %d)", err);
	}

	return 0;
}

static int cmd_seq_send(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err;

	err = bt_mesh_op_agg_cli_seq_send();
	if (err) {
		bt_shell_error("Failed to send Opcodes Aggregator Sequence message (err %d)", err);
	}

	return 0;
}

static int cmd_seq_abort(const struct bt_shell *sh, size_t argc, char *argv[])
{
