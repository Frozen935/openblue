...
static bt_slist_t sdp_db = BT_SLIST_STATIC_INIT(&sdp_db);
static uint8_t num_services;

static struct bt_sdp bt_sdp_pool[CONFIG_BT_MAX_CONN];

/* Pool for outgoing SDP packets */
BT_BUF_POOL_FIXED_DEFINE(sdp_pool, CONFIG_BT_MAX_CONN, BT_L2CAP_BUF_SIZE(SDP_MTU),
 ...
 *  @param None
 *
 *  @return Pointer to the bt_buf buffer
 */
static struct bt_buf *bt_sdp_create_pdu(void)
{
 ...
 *  @return None
 */
static int bt_sdp_send(struct bt_l2cap_chan *chan, struct bt_buf *buf,
		       uint8_t op, uint16_t tid)
{
	...
	hdr = bt_buf_push(buf, sizeof(struct bt_sdp_hdr));
	hdr->op_code = op;
 ...
 */
static uint16_t parse_data_elem(struct bt_buf_simple *buf,
				struct bt_sdp_data_elem *data_elem)
{
	...
	data_elem->type = bt_buf_simple_pull_u8(buf);
	...
	case BT_SDP_URL_STR_UNSPEC:
		...
		case 1:
			data_elem->data_size = bt_buf_simple_pull_u8(buf);
			break;
		case 2:
			data_elem->data_size = bt_buf_simple_pull_be16(buf);
			break;
		case 4:
			data_elem->data_size = bt_buf_simple_pull_be32(buf);
			break;
		default:
...
static struct bt_sdp_record *bt_sdp_foreach_svc(bt_sdp_svc_func_t func, void *user_data)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&sdp_db, rec, next, node) {
 ...
 * @return 0 for success, or relevant error code
 */
static uint16_t parse_service_search_pattern(struct bt_buf *buf, struct bt_buf_simple *ssp)
{
	...
	bt_buf_simple_init_with_data(ssp, bt_buf_pull_mem(buf, data_elem.data_size),
				      data_elem.data_size);

	bt_buf_simple_save(ssp, &state);
	...
	while (ssp->len > 0) {
		...
		bt_buf_simple_pull(ssp, data_elem.data_size);
		uuid_count++;
	...
	bt_buf_simple_restore(ssp, &state);
 ...
 */
static bool service_search_pattern_matched(struct bt_buf_simple *buf, struct bt_sdp_record *record)
{
	struct bt_buf_simple_state state;
	struct bt_sdp_data_elem data_elem;
	uint16_t res;
	...
	while (buf->len > 0) {
		...
		if (data_elem.data_size == BT_UUID_SIZE_16) {
			u.uuid.type = BT_UUID_TYPE_16;
			u.u16.val = bt_buf_simple_pull_be16(buf);
		} else if (data_elem.data_size == BT_UUID_SIZE_32) {
			u.uuid.type = BT_UUID_TYPE_32;
			u.u32.val = bt_buf_simple_pull_be32(buf);
			...
			bt_buf_simple_pull(buf, data_elem.data_size);
		} else {
 ...
 * @return 0 for success, or relevant error code
 */
static uint16_t sdp_svc_search_req(struct bt_sdp *sdp, struct bt_buf *buf, uint16_t tid)
{
	struct bt_sdp_svc_rsp *rsp;
	struct bt_buf *resp_buf;
	...
	max_rec_count = bt_buf_pull_be16(buf);
	cont_state_size = bt_buf_pull_u8(buf);
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&sdp_db, record, next, node) {
		...
		if (pkt_full) {
			...
			bt_buf_add_u8(resp_buf, SDP_SS_CONT_STATE_SIZE);
			bt_buf_add_u8(resp_buf, matched_recs);
		...
		bt_buf_add_be32(resp_buf, record->handle);
		current_recs++;
	}
	 ...
	 */
	if (!pkt_full) {
		bt_buf_add_u8(resp_buf, 0);
	} else {
		bt_buf_add_be16(resp_buf, total_recs);
	}
 ...
 */
static uint16_t copy_attribute_from_buf(struct bt_buf *buf, const uint8_t *data, uint32_t length,
					uint32_t *offset, uint32_t *index, uint16_t *space)
{
	...
	*offset = end;
	...
	if (*index < end) {
		len = end - *index;
		len = MIN(len, *space);
		bt_buf_add_mem(buf, &data[*index - start], len);
		*space -= len;
		*index += len;
		return len;
	}
 ...
 *  (used in recursion)
 */
static uint32_t copy_attribute(const struct bt_sdp_data_elem *elem, struct bt_buf *buf,
			       uint32_t *offset, uint32_t *index, uint16_t *space,
			       uint8_t nest_level)
{
	...
	BT_BUF_SIMPLE_DEFINE(attr_buf, 21); /* 1 + 4 + 16 bytes */
	...
	/* Copy the header */
	bt_buf_simple_reset(&attr_buf);

	bt_buf_simple_add_u8(&attr_buf, elem->type);

	switch (total_size - (seq_size + 1U)) {
	case 1:
		bt_buf_simple_add_u8(&attr_buf, elem->data_size);
		break;
	case 2:
		bt_buf_simple_add_be16(&attr_buf, elem->data_size);
		break;
	case 4:
		bt_buf_simple_add_be32(&attr_buf, elem->data_size);
		break;
	}
	...
	if ((elem->type & BT_SDP_TYPE_DESC_MASK) == BT_SDP_SEQ_UNSPEC ||
		...
		if (seq_size == 1U) {
			bt_buf_simple_add_u8(&attr_buf, *((const uint8_t *)elem->data));
		} else if (seq_size == 2U) {
			bt_buf_simple_add_be16(&attr_buf, *((const uint16_t *)elem->data));
		} else if (seq_size == 4U) {
			bt_buf_simple_add_be32(&attr_buf, *((const uint32_t *)elem->data));
		} else if (seq_size == 8U) {
			...
			bt_buf_simple_add_mem(&attr_buf, val, seq_size);
		}
			 ...
			 void *user_data)
{
	...
	for (idx_filter = 0U; idx_filter < sad->num_filters; idx_filter++) {
		...
		if (!sad->state->pkt_full && sad->rsp_buf) {
			...
			if (!sad->seq &&
			    (sad->state->last_att_index == 0) && (sad->state->last_att == 0)) {
				sad->seq = bt_buf_add(sad->rsp_buf,
						       sizeof(*sad->seq));
			...
			if (attr_data_index == 0) {
				/* Add attribute ID */
				bt_buf_add_u8(sad->rsp_buf, BT_SDP_UINT16);
				bt_buf_add_be16(sad->rsp_buf, att_id_cur);
 ...
 *  @return 0 for success, or relevant error code
 */
static uint16_t get_att_search_list(struct bt_buf *buf, uint32_t *filter, size_t max_filters,
				    size_t *num_filters)
{
	struct bt_sdp_data_elem data_elem;
	uint16_t res;
	...
	while (size) {
		...
		if (data_elem.data_size == 2U) {
			filter[(*num_filters)++] = 0xffff0000 |
							bt_buf_pull_be16(buf);
		}

		/* This is an attribute ID range */
		if (data_elem.data_size == 4U) {
			filter[(*num_filters)++] = bt_buf_pull_be32(buf);
		}
 ...
 *  @return 0 for success, or relevant error code
 */
static uint16_t sdp_svc_att_req(struct bt_sdp *sdp, struct bt_buf *buf, uint16_t tid)
{
	...
	svc_rec_hdl = bt_buf_pull_be32(buf);
	max_att_len = bt_buf_pull_be16(buf);
	...
	if (cont_state_size) {
		...
		state.last_att = bt_buf_pull_u8(buf);
		state.last_att_index = bt_buf_pull_be32(buf);
	}
	...
	if (!att_list_len) {
		...
		bt_buf_add_u8(rsp_buf, BT_SDP_SEQ8);
		bt_buf_add_u8(rsp_buf, 0);
		att_list_len = 2U;
	}
	...
	if (state.pkt_full) {
		...
		bt_buf_add_u8(rsp_buf, SDP_SA_CONT_STATE_SIZE);
		bt_buf_add_u8(rsp_buf, state.last_att);
		bt_buf_add_be32(rsp_buf, state.last_att_index);
	} else {
		bt_buf_add_u8(rsp_buf, 0);
	}
 ...
 *  @return 0 for success, or relevant error code
 */
static uint16_t sdp_svc_search_att_req(struct bt_sdp *sdp, struct bt_buf *buf, uint16_t tid)
{
	uint32_t filter[MAX_NUM_ATT_ID_FILTER];
	struct search_state state = {
		.att_list_size = 0,
	...
	struct bt_buf *rsp_buf, *rsp_buf_cpy;
	struct bt_sdp_record *record, *next;
	struct bt_sdp_att_rsp *rsp;
	...
	if (cont_state_size) {
		...
		state.current_svc = bt_buf_pull_u8(buf);
		state.last_att = bt_buf_pull_u8(buf);
		state.last_att_index = bt_buf_pull_be32(buf);
	...
	if (!cont_state_size) {
		seq = bt_buf_add(rsp_buf, sizeof(*seq));
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&sdp_db, record, next, node) {
		...
		if (state.pkt_full && !dry_run) {
			...
			if (state.current_svc < num_services) {
				...
				/* Add continuation state */
				bt_buf_add_u8(rsp_buf, SDP_SSA_CONT_STATE_SIZE);
				bt_buf_add_u8(rsp_buf, state.current_svc);
				bt_buf_add_u8(rsp_buf, state.last_att);
				bt_buf_add_be32(rsp_buf, state.last_att_index);
			}
...
static const struct {
	uint8_t  op_code;
	uint16_t  (*func)(struct bt_sdp *sdp, struct bt_buf *buf, uint16_t tid);
} handlers[] = {
 ...
 *  @return None
 */
static int bt_sdp_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
	struct bt_l2cap_br_chan *ch = CONTAINER_OF(chan,
			struct bt_l2cap_br_chan, chan);
...
void bt_sdp_init(void)
{
	...
	ARRAY_FOR_EACH(bt_sdp_client_pool, i) {
		...
		os_sem_init(&bt_sdp_client_pool[i].sem_lock, 1, 1);
	}
...
int bt_sdp_register_service(struct bt_sdp_record *service)
{
	...
	if (bt_slist_find(&sdp_db, &service->node, NULL)) {
		LOG_ERR("Service already registered");
		return -EEXIST;
	}

	if (!bt_slist_is_empty(&sdp_db)) {
		struct bt_sdp_record *last;

		last = CONTAINER_OF(bt_slist_peek_tail(&sdp_db), struct bt_sdp_record, node);
		index = last->index + 1;
	...
	*((uint32_t *)(service->attrs[0].val.data)) = service->handle;
	...
	bt_slist_append(&sdp_db, &service->node);
...
static void sdp_client_params_iterator(struct bt_sdp_client *session)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&session->reqs, param, tmp, _node) {
		...
		bt_slist_remove(&session->reqs, NULL, &param->_node);
		/* Invalidate cached param in context */
		session->param = NULL;
		if (session->rec_buf != NULL) {
			bt_buf_unref(session->rec_buf);
		...
		if (!bt_slist_is_empty(&session->reqs)) {
			os_sem_give(&session->sem_lock);
...
static uint16_t sdp_client_get_total(struct bt_sdp_client *session,
				  struct bt_buf *buf, uint16_t *total)
{
	uint16_t pulled;
	uint8_t seq;
	...
	if (session->cstate.length == 0U) {
		seq = bt_buf_pull_u8(buf);
		pulled = 1U;
		switch (seq) {
		case BT_SDP_SEQ8:
			*total = bt_buf_pull_u8(buf);
			pulled += 1U;
			break;
		case BT_SDP_SEQ16:
			*total = bt_buf_pull_be16(buf);
			pulled += 2U;
			break;
		case BT_SDP_SEQ32:
			*total = bt_buf_pull_be32(buf);
			pulled += 4U;
			break;
		default:
...
static uint16_t get_ss_record_len(struct bt_buf *buf)
{
...
static uint16_t get_ssa_record_len(struct bt_buf *buf)
{
	uint16_t len;
	uint8_t seq;
	...
	switch (seq) {
	case BT_SDP_SEQ8:
		len = bt_buf_pull_u8(buf);
		break;
	case BT_SDP_SEQ16:
		len = bt_buf_pull_be16(buf);
		break;
	case BT_SDP_SEQ32:
		len = bt_buf_pull_be32(buf);
		break;
	default:
		LOG_WRN("Sequence type 0x%02x not handled", seq);
		len = 0U;
				    ...
				    enum uuid_state state)
{
	...
	while (session->rec_buf->len) {
		struct bt_buf_simple_state buf_state;

		bt_buf_simple_save(&session->rec_buf->b, &buf_state);
		...
		if (session->rec_buf->len > rec_len) {
			...
			if (state == UUID_PARTIAL_RESOLVED) {
				...
				bt_buf_simple_restore(&session->rec_buf->b, &buf_state);
				 ...
				 */
				buf = bt_buf_alloc(session->param->pool, OS_TIMEOUT_NO_WAIT);
				if (buf != NULL) {
					if (bt_buf_tailroom(buf) < len) {
						goto no_more_space;
					}

					bt_buf_add_mem(buf, src, len);
					if (bt_buf_tailroom(buf) <=
					    bt_buf_tailroom(session->rec_buf)) {
						goto no_more_space;
					}
...
no_more_space:
				...
				bt_buf_reset(session->rec_buf);
				dst = bt_buf_add(session->rec_buf, len);
				if (dst == src) {
		 ...
		 * send to user so far
		 */
		bt_buf_pull(session->rec_buf, rec_len);
				...
				const struct bt_sdp_discover_params *param)
{
	...
	bt_buf_add_u8(buf, BT_SDP_SEQ8);

	switch (param->uuid->type) {
	case BT_UUID_TYPE_16:
		/* Seq length */
		bt_buf_add_u8(buf, sizeof(uint8_t) + BT_UUID_SIZE_16);
		/* Seq type */
		bt_buf_add_u8(buf, BT_SDP_UUID16);
		/* Seq value */
		bt_buf_add_be16(buf, BT_UUID_16(param->uuid)->val);
		break;
	case BT_UUID_TYPE_32:
		bt_buf_add_u8(buf, sizeof(uint8_t) + BT_UUID_SIZE_32);
		bt_buf_add_u8(buf, BT_SDP_UUID32);
		bt_buf_add_be32(buf, BT_UUID_32(param->uuid)->val);
		break;
	case BT_UUID_TYPE_128:
		bt_buf_add_u8(buf, sizeof(uint8_t) + BT_UUID_SIZE_128);
		bt_buf_add_u8(buf, BT_SDP_UUID128);
		sys_memcpy_swap(uuid128, BT_UUID_128(param->uuid)->val, sizeof(uuid128));
		bt_buf_add_mem(buf, uuid128, sizeof(uuid128));
		break;
	default:
	...
	bt_buf_add_be16(buf, bt_buf_tailroom(session->rec_buf) / SDP_RECORD_HANDLE_SIZE);
	/*
	 ...
	 */
	if (session->cstate.length == 0U) {
		bt_buf_add_u8(buf, 0x00);
	} else {
		bt_buf_add_u8(buf, session->cstate.length);
		bt_buf_add_mem(buf, session->cstate.data, session->cstate.length);
	}
...
static void sdp_client_add_attribute_id(struct bt_buf *buf, struct bt_sdp_attribute_id_list *ids)
{
	uint16_t len;
	 ...
	 */
	if (len > UINT8_MAX) {
		bt_buf_add_u8(buf, BT_SDP_SEQ16);
		bt_buf_add_be16(buf, len);
	} else {
		bt_buf_add_u8(buf, BT_SDP_SEQ8);
		bt_buf_add_u8(buf, len);
	}

	if (ids == NULL || ids->count == 0) {
		/* Data element definition for two following 16bits range elements */
		bt_buf_add_u8(buf, BT_SDP_UINT32);
		/* Get all attributes. It enables filter out wanted only attributes */
		bt_buf_add_be16(buf, 0x0000);
		bt_buf_add_be16(buf, 0xffff);
		return;
	}
	...
	for (size_t i = 0; i < ids->count; i++) {
		if (ids->ranges[i].beginning == ids->ranges[i].ending) {
			...
			bt_buf_add_u8(buf, BT_SDP_UINT16);
			/* Get all attributes. It enables filter out wanted only attributes */
			bt_buf_add_be16(buf, ids->ranges[i].beginning);
		} else {
			/* Data element definition for two following 16bits range elements */
			bt_buf_add_u8(buf, BT_SDP_UINT32);
			/* Get all attributes. It enables filter out wanted only attributes */
			bt_buf_add_be16(buf, ids->ranges[i].beginning);
			bt_buf_add_be16(buf, ids->ranges[i].ending);
		}
	}
}
				...
				const struct bt_sdp_discover_params *param)
{
	...
	len = bt_buf_tailroom(session->rec_buf);
	...
	/* Add service record handle  */
	bt_buf_add_be32(buf, param->handle);
				 ...
				 const struct bt_sdp_discover_params *param)
{
	...
	if (session->cstate.length == 0U) {
		...
		bt_buf_add_u8(buf, session->cstate.length);
		bt_buf_add_mem(buf, session->cstate.data,
...
static int sdp_client_discover(struct bt_sdp_client *session)
{
	...
	os_sem_take(&session->sem_lock, OS_TIMEOUT_FOREVER);
	if (!session->param) {
		param = GET_PARAM(bt_slist_peek_head(&session->reqs));
	} else {
		param = session->param;
	}

	if (param != NULL && session->rec_buf == NULL) {
		session->rec_buf = bt_buf_alloc(param->pool, OS_TIMEOUT_FOREVER);
	}
...
static int sdp_client_receive_ss(struct bt_sdp_client *session, struct bt_buf *buf)
{
	struct bt_sdp_pdu_cstate *cstate;
	...
	total_count = bt_buf_pull_be16(buf);
	...
	current_count = bt_buf_pull_be16(buf);
	...
	if (record_len > bt_buf_tailroom(session->rec_buf)) {
		LOG_WRN("Not enough room for getting records data");
		return -EINVAL;
	}

	bt_buf_add_mem(session->rec_buf, buf->data, record_len);
	bt_buf_pull(buf, record_len);
	...
	if (cstate->length) {
		...
		bt_buf_pull(buf, cstate->length + sizeof(cstate->length));
	...
	bt_buf_pull(buf, sizeof(cstate->length));
...
static int sdp_client_receive_ssa_sa(struct bt_sdp_client *session, struct bt_buf *buf)
{
	struct bt_sdp_pdu_cstate *cstate;
	uint16_t frame_len;
	uint16_t total;
	...
	frame_len = bt_buf_pull_be16(buf);
	...
	if (frame_len > bt_buf_tailroom(session->rec_buf)) {
		LOG_WRN("Not enough room for getting records data");
		return -EINVAL;
	}

	bt_buf_add_mem(session->rec_buf, buf->data, frame_len);
	bt_buf_pull(buf, frame_len);
...
static int sdp_client_receive(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
...
static struct bt_buf *sdp_client_alloc_buf(struct bt_l2cap_chan *chan)
{
	struct bt_sdp_client *session = SDP_CLIENT_CHAN(chan);
	struct bt_buf *buf;
	...
	session->param = GET_PARAM(bt_slist_peek_head(&session->reqs));

	buf = bt_buf_alloc(session->param->pool, OS_TIMEOUT_FOREVER);
	__ASSERT_NO_MSG(buf);
...
static void sdp_client_clean_after_release(struct bt_sdp_client *session)
{
	...
	bt_slist_init(&session->reqs_next);
...
static void sdp_client_disconnected(struct bt_l2cap_chan *chan)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&session->reqs, param, tmp, _node) {
		...
		bt_slist_find_and_remove(&session->reqs, &param->_node);
	}
...
void sdp_client_released(struct bt_l2cap_chan *chan)
{
	...
	os_sem_take(&session->sem_lock, OS_TIMEOUT_FOREVER);
	if (!bt_slist_is_empty(&session->reqs_next)) {
		/* put the reqs_next to reqs */
		BT_SLIST_FOR_EACH_CONTAINER_SAFE(&session->reqs_next, param, tmp, _node) {
			bt_slist_append(&session->reqs, &param->_node);
			/* Remove already processed node */
			bt_slist_remove(&session->reqs_next, NULL, &param->_node);
		}
		...
		if (err) {
			bt_slist_init(&cb_reqs);
			BT_SLIST_FOR_EACH_CONTAINER_SAFE(&session->reqs, param, tmp, _node) {
				bt_slist_append(&cb_reqs, &param->_node);
			}
		...
		if (err) {
			...
			BT_SLIST_FOR_EACH_CONTAINER_SAFE(&cb_reqs, param, tmp, _node) {
				      ...
				      struct bt_sdp_discover_params *params)
{
	...
	if (session->state == SDP_CLIENT_CONNECTING ||
		...
		bt_slist_append(&session->reqs, &params->_node);
		os_sem_give(&session->sem_lock);
		return 0;
	}
	...
	if (session->state == SDP_CLIENT_DISCONNECTING) {
		bt_slist_append(&session->reqs_next, &params->_node);
		os_sem_give(&session->sem_lock);
		return 0;
	}
...
int bt_sdp_get_proto_param(const struct bt_buf *buf, uint16_t proto, uint16_t *param)
{
	struct bt_sdp_attribute attr;
...
int bt_sdp_get_addl_proto_param(const struct bt_buf *buf, uint16_t proto, uint8_t index,
				uint16_t *param)
{
	struct bt_sdp_attribute attr;
	struct bt_sdp_attr_value value;
...
int bt_sdp_get_profile_version(const struct bt_buf *buf, uint16_t profile, uint16_t *version)
{
	struct bt_sdp_attribute attr;
	struct bt_sdp_attr_value value;
	int err;
...
int bt_sdp_get_features(const struct bt_buf *buf, uint16_t *features)
{
	struct bt_sdp_attribute attr;
	struct bt_sdp_attr_value value;
	int err;
...
int bt_sdp_get_vendor_id(const struct bt_buf *buf, uint16_t *vendor_id)
{
	struct bt_sdp_attribute attr;
	struct bt_sdp_attr_value value;
	int err;
...
int bt_sdp_get_product_id(const struct bt_buf *buf, uint16_t *product_id)
{
	struct bt_sdp_attribute attr;
	struct bt_sdp_attr_value value;
	int err;
...
static int sdp_attr_pull_len(uint8_t size, struct bt_buf_simple *buf, uint32_t *len)
{
	switch (size) {
	case sizeof(uint8_t):
		*len = bt_buf_simple_pull_u8(buf);
		break;
	case sizeof(uint16_t):
		*len = bt_buf_simple_pull_be16(buf);
		break;
	case sizeof(uint32_t):
		*len = bt_buf_simple_pull_be32(buf);
		break;
	default:
...
static int sdp_attr_get_len(uint8_t type, struct bt_buf_simple *buf, uint32_t *len)
{
...
static int bt_sdp_parse_attribute(struct bt_buf_simple *buf, struct bt_sdp_attribute *attr)
{
	uint8_t type;
	uint32_t len;
	int err;
	...
	if (bt_buf_simple_pull_u8(buf) != BT_SDP_UINT16) {
		LOG_WRN("Invalid attribute");
		return -EINVAL;
	}

	/* Parse attribute ID */
	attr->id = bt_buf_simple_pull_be16(buf);

	attr->val.data = buf->data;
	src = buf->data;

	type = bt_buf_simple_pull_u8(buf);
...
int bt_sdp_record_parse(const struct bt_buf *buf,
			bool (*func)(const struct bt_sdp_attribute *attr, void *user_data),
			void *user_data)
{
	struct bt_buf_simple sbuf;
	struct bt_sdp_attribute attr;
	int err = -ENODATA;
	bool stop;
	...
	bt_buf_simple_init_with_data(&sbuf, buf->data, buf->len);
	while (sbuf.len > 0) {
...
bool bt_sdp_has_attr(const struct bt_buf *buf, uint16_t attr_id)
{
	struct bt_sdp_has_attr_data data;
	int err;
...
int bt_sdp_get_attr(const struct bt_buf *buf, uint16_t attr_id, struct bt_sdp_attribute *attr)
{
	struct bt_sdp_get_attr_data data;
	int err;
...
static int sdp_attr_get_uint(uint8_t size, struct bt_buf_simple *buf,
			     struct bt_sdp_attr_value_uint *uint)
{
	switch (size) {
	case sizeof(uint8_t):
		uint->u8 = bt_buf_simple_pull_u8(buf);
		break;
	case sizeof(uint16_t):
		uint->u16 = bt_buf_simple_pull_be16(buf);
		break;
	case sizeof(uint32_t):
		uint->u32 = bt_buf_simple_pull_be32(buf);
		break;
	case sizeof(uint64_t):
		uint->u64 = bt_buf_simple_pull_be64(buf);
		break;
	case BIT(BT_SDP_UINT128 & BT_SDP_SIZE_DESC_MASK):
		memcpy(uint->u128, bt_buf_simple_pull_mem(buf, size), size);
		break;
	default:
...
static int sdp_attr_get_sint(uint8_t size, struct bt_buf_simple *buf,
			     struct bt_sdp_attr_value_int *sint)
{
	switch (size) {
	case sizeof(int8_t):
		sint->s8 = (int8_t)bt_buf_simple_pull_u8(buf);
		break;
	case sizeof(int16_t):
		sint->s16 = (int16_t)bt_buf_simple_pull_be16(buf);
		break;
	case sizeof(int32_t):
		sint->s32 = (int32_t)bt_buf_simple_pull_be32(buf);
		break;
	case sizeof(int64_t):
		sint->s64 = (int64_t)bt_buf_simple_pull_be64(buf);
		break;
	case BIT(BT_SDP_INT128 & BT_SDP_SIZE_DESC_MASK):
		memcpy(sint->s128, bt_buf_simple_pull_mem(buf, size), size);
		break;
	default:
		LOG_WRN("Invalid size %u", size);
		return -EINVAL;
...
static int sdp_attr_get_uuid(uint8_t size, struct bt_buf_simple *buf, union sdp_attr_uuid *u)
{
	switch (size) {
	case sizeof(uint16_t):
	...
	case BT_UUID_SIZE_128:
		...
		sys_memcpy_swap(u->u128.val, bt_buf_simple_pull_mem(buf, size), size);
		break;
	default:
		LOG_WRN("Invalid size %u for sequence", size);
		return -EINVAL;
...
static int sdp_attr_val_parse(uint8_t type, uint32_t len, struct bt_buf_simple *buf,
			      ...
			      struct bt_sdp_attr_value *v)
{
	...
	case BT_SDP_BOOL:
		...
		v->value = bt_buf_simple_pull_u8(buf) > 0 ? true : false;
		pair->value = v;
		break;
	case BT_SDP_TEXT_STR_UNSPEC:
		v->type = BT_SDP_ATTR_VALUE_TYPE_TEXT;
		v->text.len = len;
		v->text.text = bt_buf_simple_pull_mem(buf, len);
		pair->value = v;
		break;
	case BT_SDP_URL_STR_UNSPEC:
		v->type = BT_SDP_ATTR_VALUE_TYPE_URL;
		v->url.len = len;
		v->url.url = bt_buf_simple_pull_mem(buf, len);
		pair->value = v;
		break;
...
static int sdp_attr_parse(struct bt_buf_simple *buf,
			  ...
			  void *user_data, uint8_t nest_level)
{
	...
	if (nest_level == SDP_DATA_ELEM_NEST_LEVEL_MAX) {
		...
		bt_buf_simple_pull_mem(buf, len);
		return 0;
	}
	...
	bt_buf_simple_init_with_data(&vbuf, bt_buf_simple_pull_mem(buf, len), len);

	if (sdp_attr_is_seq(type)) {
		type = bt_buf_simple_pull_u8(&vbuf);
			    ...
			    void *user_data)
{
	...
	bt_buf_simple_init_with_data(&buf, (uint8_t *)attr->val.data, attr->val.data_size);
...
static int bt_sdp_parse_addl_proto(struct bt_buf_simple *buf, struct bt_sdp_attribute *attr)
{
	uint8_t type;
	uint32_t len;
	int err;
...
static int bt_sdp_attr_addl_proto_pre_parse(struct bt_buf_simple *buf)
{
	uint8_t type;
	uint32_t len;
	int err;
				 ...
				 void *user_data)
{
	...
	bt_buf_simple_init_with_data(&sbuf, (uint8_t *)attr->val.data, attr->val.data_size);
