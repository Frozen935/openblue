...
struct tbs_inst {
	...
	struct bt_work_delayable reporting_interval_work;
...
BT_BUF_SIMPLE_DEFINE_STATIC(read_buf, READ_BUF_SIZE);
...
static void bt_buf_put_call_states_by_inst(const struct tbs_inst *inst, struct bt_buf_simple *buf)
{
	const struct bt_tbs_call *call;
	...
	for (size_t i = 0; i < call_count; i++) {
		...
		bt_buf_simple_add_u8(buf, call->index);
		bt_buf_simple_add_u8(buf, call->state);
		bt_buf_simple_add_u8(buf, call->flags);
	}
}

static void bt_buf_put_call_states(const struct tbs_inst *inst, struct bt_buf_simple *buf)
{
	bt_buf_simple_reset(buf);

	bt_buf_put_call_states_by_inst(inst, buf);
	...
	if (inst_is_gtbs(inst)) {
		for (size_t i = 0; i < ARRAY_SIZE(svc_insts); i++) {
			bt_buf_put_call_states_by_inst(&svc_insts[i], buf);
		}
	}
}

static void bt_buf_put_current_calls_by_inst(const struct tbs_inst *inst,
					      struct bt_buf_simple *buf)
{
	...
	for (size_t i = 0; i < call_count; i++) {
		...
		bt_buf_simple_add_u8(buf, (uint8_t)item_len);
		bt_buf_simple_add_u8(buf, call->index);
		bt_buf_simple_add_u8(buf, call->state);
		bt_buf_simple_add_u8(buf, call->flags);
		bt_buf_simple_add_mem(buf, call->remote_uri, uri_length);
	}
}

static void bt_buf_put_current_calls(const struct tbs_inst *inst, struct bt_buf_simple *buf)
{
	bt_buf_simple_reset(buf);

	bt_buf_put_current_calls_by_inst(inst, buf);
	...
	if (inst_is_gtbs(inst)) {
		for (size_t i = 0; i < ARRAY_SIZE(svc_insts); i++) {
			bt_buf_put_current_calls_by_inst(&svc_insts[i], buf);
		}
	}
}
...
static void bt_buf_put_uri_scheme_list(const struct tbs_inst *inst, struct bt_buf_simple *buf)
{
	bt_buf_simple_reset(buf);

	bt_buf_simple_add_mem(buf, inst->uri_scheme_list, strlen(inst->uri_scheme_list));
	...
	for (size_t i = 0; i < ARRAY_SIZE(svc_insts); i++) {
		...
		while (uri_to_search != NULL) {
			if (!is_tbs_uri_unique((char *)buf->data, uri_to_search)) {
				...
				bt_buf_simple_add_mem(buf, ",", 1);
				bt_buf_simple_add_mem(buf, uri_to_search,
...
static void notify_handler_cb(struct bt_conn *conn, void *data)
{
	...
	if (flags->bearer_uri_schemes_supported_list_changed) {
		...
		bt_buf_put_uri_scheme_list(inst, &read_buf);
	...
	if (flags->bearer_list_current_calls_changed) {
		...
		bt_buf_put_current_calls(inst, &read_buf);
	...
	if (flags->call_state_changed) {
		...
		bt_buf_put_call_states(inst, &read_buf);
...
{
	struct tbs_inst *inst =
		CONTAINER_OF(bt_work_delayable_from_work(work), struct tbs_inst, notify_work);
					      ...
					      uint8_t flags)
{
	...
	bt_buf_simple_init_with_data(&bt_buf, (void *)buf, len);
	signal_strength_interval = bt_buf_simple_pull_u8(&bt_buf);
...
static void signal_interval_timeout(struct bt_work *work)
{
	...
	if (inst->signal_strength_interval) {
		bt_work_reschedule(&inst->reporting_interval_work,
				  OS_SECONDS(inst->signal_strength_interval));
	}
...
int bt_tbs_unregister_bearer(uint8_t bearer_index)
{
	...
	restart_reporting_interval =
		bt_work_cancel_delayable_sync(&inst->reporting_interval_work, &sync);
	...
	if (err != 0) {
		...
		if (restart_reporting_interval && inst->signal_strength_interval != 0U) {
			 ...
			 * dream anyways
			 */
			bt_work_schedule(&inst->reporting_interval_work,
					OS_SECONDS(inst->signal_strength_interval));
		}
...
int bt_tbs_set_signal_strength(uint8_t bearer_index, uint8_t new_signal_strength)
{
	...
	timer_status = bt_work_delayable_remaining_get(&inst->reporting_interval_work);
	if (timer_status == 0) {
		bt_work_reschedule(&inst->reporting_interval_work, OS_TIMEOUT_NO_WAIT);
	}
