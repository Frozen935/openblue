...
{
	struct bt_buf_simple *in = data;
	struct bt_buf_simple *out = BT_BUF_SIMPLE(17);
	int err, i;
	uint32_t sseq;
	uint16_t ssrc;
...
void bt_mesh_sol_recv(struct bt_buf_simple *buf, uint8_t uuid_list_len)
{
#if CONFIG_BT_MESH_OD_PRIV_PROXY_SRV
	uint8_t type;
	...
	while (uuid_list_len >= 2) {
		uuid = bt_buf_simple_pull_le16(buf);
	...
	while (buf->len >= 22) {
		reported_len = bt_buf_simple_pull_u8(buf);
		svc_data_type = bt_buf_simple_pull_u8(buf);
		uuid = bt_buf_simple_pull_le16(buf);
		...
		bt_buf_simple_pull_mem(buf, reported_len - 3);
	}
...
#if CONFIG_BT_MESH_PROXY_SOLICITATION
static int sol_pdu_create(struct bt_mesh_subnet *sub, struct bt_buf_simple *pdu)
{
	int err;

	bt_buf_simple_add_u8(pdu, sub->keys[SUBNET_KEY_TX_IDX(sub)].msg.nid);
	/* CTL = 1, TTL = 0 */
	bt_buf_simple_add_u8(pdu, 0x80);
	bt_buf_simple_add_le24(pdu, sys_cpu_to_be24(sseq_out));
	bt_buf_simple_add_le16(pdu, sys_cpu_to_be16(bt_mesh_primary_addr()));
	/* DST = 0x0000 */
	bt_buf_simple_add_le16(pdu, 0x0000);
	...
	bt_buf_simple_push_u8(pdu, 0);
	bt_buf_simple_push_le16(pdu, BT_UUID_MESH_PROXY_SOLICITATION_VAL);
