...
{
	enum bt_mesh_brg_cfg_state status = (enum bt_mesh_brg_cfg_state)bt_buf_simple_pull_u8(buf);
			...
			struct bt_buf_simple *buf)
{
	...
	table_status.status = bt_buf_simple_pull_u8(buf);
	table_status.entry.directions = bt_buf_simple_pull_u8(buf);
	key_idx_unpack_pair(buf, &table_status.entry.net_idx1, &table_status.entry.net_idx2);
	table_status.entry.addr1 = bt_buf_simple_pull_le16(buf);
	table_status.entry.addr2 = bt_buf_simple_pull_le16(buf);
			...
			struct bt_buf_simple *buf)
{
	...
	net_idx_filter = bt_buf_simple_pull_le16(buf);
	...
	subnets_list.start_idx = bt_buf_simple_pull_u8(buf);
	...
	if (bt_mesh_msg_ack_ctx_match(&cli->ack_ctx, OP_BRIDGED_SUBNETS_LIST, ctx->addr,
		...
		if (rsp->list) {
			...
			to_copy = MIN(bt_buf_simple_tailroom(rsp->list), buf->len);
			bt_buf_simple_add_mem(rsp->list, buf->data, to_copy);
		}
		      ...
		      struct bt_buf_simple *buf)
{
	...
	table_list.status = bt_buf_simple_pull_u8(buf);
	key_idx_unpack_pair(buf, &table_list.net_idx1, &table_list.net_idx2);
	table_list.start_idx = bt_buf_simple_pull_le16(buf);
	...
	if (bt_mesh_msg_ack_ctx_match(&cli->ack_ctx, OP_BRIDGING_TABLE_LIST, ctx->addr,
		...
		if (rsp->list) {
			...
			to_copy = MIN(bt_buf_simple_tailroom(rsp->list), (buf->len / 5) * 5);
			bt_buf_simple_add_mem(rsp->list, buf->data, to_copy);
		}
...
static int table_size_status(const struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
			     struct bt_buf_simple *buf)
{
	uint16_t size = bt_buf_simple_pull_le16(buf);
	uint16_t *rsp;
			    ...
			    enum bt_mesh_brg_cfg_state *status)
{
	...
	bt_buf_simple_add_u8(&msg, (uint8_t)val);
				  ...
				  struct bt_mesh_brg_cfg_table_status *rsp)
{
	...
	bt_buf_simple_add_u8(&msg, entry->directions);
	key_idx_pack_pair(&msg, entry->net_idx1, entry->net_idx2);
	bt_buf_simple_add_le16(&msg, entry->addr1);
	bt_buf_simple_add_le16(&msg, entry->addr2);
				     ...
				     struct bt_mesh_brg_cfg_table_status *rsp)
{
	...
	bt_buf_simple_add_le16(&msg, addr1);
	bt_buf_simple_add_le16(&msg, addr2);
				    ...
				    uint8_t start_idx, struct bt_mesh_brg_cfg_subnets_list *rsp)
{
	...
	bt_buf_simple_add_le16(&msg, (filter_net_idx.filter | filter_net_idx.net_idx << 4));
	bt_buf_simple_add_u8(&msg, start_idx);
				  ...
				  struct bt_mesh_brg_cfg_table_list *rsp)
{
	...
	key_idx_pack_pair(&msg, net_idx1, net_idx2);
	bt_buf_simple_add_le16(&msg, start_idx);
