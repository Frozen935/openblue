...
#define REPORT_TIMER_TIMEOUT OS_SECONDS(CONFIG_BT_MESH_BLOB_REPORT_TIMEOUT)
...
static void reset_timer(struct bt_mesh_blob_srv *srv)
{
	...
	bt_work_reschedule(&srv->rx_timeout, OS_SECONDS(timeout_secs));
}

static void buf_chunk_index_add(struct bt_buf_simple *buf, uint16_t chunk)
{
	/* utf-8 encoded: */
	if (chunk < 0x80) {
		bt_buf_simple_add_u8(buf, chunk);
	} else if (chunk < 0x8000) {
		bt_buf_simple_add_u8(buf, 0xc0 | chunk >> 6);
		bt_buf_simple_add_u8(buf, 0x80 | (chunk & BIT_MASK(6)));
	} else {
		bt_buf_simple_add_u8(buf, 0xe0 | chunk >> 12);
		bt_buf_simple_add_u8(buf, 0x80 | ((chunk >> 6) & BIT_MASK(6)));
		bt_buf_simple_add_u8(buf, 0x80 | (chunk & BIT_MASK(6)));
	}
}
...
static void report_sent(int err, void *cb_data)
{
	...
	if (bt_work_delayable_is_pending(&srv->rx_timeout)) {
		bt_work_reschedule(&srv->pull.report, REPORT_TIMER_TIMEOUT);
	}
}
...
static void block_report(struct bt_mesh_blob_srv *srv)
{
	...
	LOG_DBG("rx BLOB Timeout Timer: %i", bt_work_delayable_is_pending(&srv->rx_timeout));
...
static void cancel(struct bt_mesh_blob_srv *srv)
{
	...
	bt_work_cancel_delayable(&srv->rx_timeout);
	bt_work_cancel_delayable(&srv->pull.report);
	io_close(srv);
	erase_state(srv);
...
static void timeout(struct bt_work *work)
{
...
static void report_timeout(struct bt_work *work)
{
	struct bt_mesh_blob_srv *srv =
			    ...
			    enum bt_mesh_blob_status status)
{
	...
	bt_buf_simple_add_u8(&buf, ((status & BIT_MASK(4)) |
				     (srv->state.xfer.mode << 6)));
	bt_buf_simple_add_u8(&buf, srv->phase);

	if (srv->phase == BT_MESH_BLOB_XFER_PHASE_INACTIVE) {
		goto send;
	}

	bt_buf_simple_add_le64(&buf, srv->state.xfer.id);

	if (srv->phase == BT_MESH_BLOB_XFER_PHASE_WAITING_FOR_START) {
		goto send;
	}

	bt_buf_simple_add_le32(&buf, srv->state.xfer.size);
	bt_buf_simple_add_u8(&buf, srv->state.xfer.block_size_log);
	bt_buf_simple_add_le16(&buf, srv->state.mtu_size);
	bt_buf_simple_add_mem(&buf, srv->state.blocks,
			     ...
			     enum bt_mesh_blob_status status)
{
	...
	bt_buf_simple_add_u8(&buf, (status & BIT_MASK(4)) | (format << 6));
	bt_buf_simple_add_le16(&buf, srv->block.number);
	bt_buf_simple_add_le16(&buf, srv->state.xfer.chunk_size);

	if (format == BT_MESH_BLOB_CHUNKS_MISSING_SOME) {
		bt_buf_simple_add_mem(&buf, srv->block.missing,
			     ...
			     struct bt_buf_simple *buf)
{
	...
	mode = (bt_buf_simple_pull_u8(buf) >> 6);
	id = bt_buf_simple_pull_le64(buf);
	size = bt_buf_simple_pull_le32(buf);
	block_size_log = bt_buf_simple_pull_u8(buf);
	mtu_size = bt_buf_simple_pull_le16(buf);
			      ...
			      struct bt_buf_simple *buf)
{
	...
	block_number = bt_buf_simple_pull_le16(buf);
			...
			struct bt_buf_simple *buf)
{
	...
	chunk.size = buf->len;
	chunk.data = bt_buf_simple_pull_mem(buf, chunk.size);
			   ...
			   struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(&rsp, BLOB_BLOCK_SIZE_LOG_MIN);
	bt_buf_simple_add_u8(&rsp, BLOB_BLOCK_SIZE_LOG_MAX);
	bt_buf_simple_add_le16(&rsp, CONFIG_BT_MESH_BLOB_CHUNK_COUNT_MAX);
	...
	if (bt_mesh_lpn_established() && bt_mesh.lpn.queue_size > 0) {
		...
		bt_buf_simple_add_le16(&rsp, chunk_size);
		if (bt_mesh.lpn.queue_size <= 2) {
			LOG_WRN("FndQ too small %u", bt_mesh.lpn.queue_size);
		}
	} else {
		bt_buf_simple_add_le16(&rsp, BLOB_RX_CHUNK_SIZE);
	}
#else
	bt_buf_simple_add_le16(&rsp, BLOB_RX_CHUNK_SIZE);
#endif

	bt_buf_simple_add_le32(&rsp, CONFIG_BT_MESH_BLOB_SIZE_MAX);
	bt_buf_simple_add_le16(&rsp, MTU_SIZE_MAX);
	bt_buf_simple_add_u8(&rsp, BT_MESH_BLOB_XFER_MODE_ALL);
...
static int blob_srv_init(const struct bt_mesh_model *mod)
{
	...
	srv->block.number = 0xffff;
	srv->state.xfer.chunk_size = 0xffff;
	bt_work_init_delayable(&srv->rx_timeout, timeout);
	bt_work_init_delayable(&srv->pull.report, report_timeout);
