...
static void iso_rx_buf_destroy(struct bt_buf *buf)
{
	bt_buf_destroy(buf);
...
BT_BUF_POOL_FIXED_DEFINE(iso_rx_pool, CONFIG_BT_ISO_RX_BUF_COUNT,
...
#define iso_info(buf) (&iso_info_data[bt_buf_id(buf)])
...
static void bt_iso_sent_cb(struct bt_conn *iso, void *user_data, int err)
{
	...
	__ASSERT_MSG(chan != NULL, "NULL chan for iso %p", iso);
...
void hci_iso(struct bt_buf *buf)
{
...
static struct bt_buf *iso_data_pull(struct bt_conn *conn, size_t amount, size_t *length);

/* Returns true if the ISO layer has data to send on this conn */
static bool iso_has_data(struct bt_conn *conn);

static void iso_get_and_clear_cb(struct bt_conn *conn, struct bt_buf *buf, bt_conn_tx_cb_t *cb,
				 void **ud)
{
...
static void bt_iso_chan_disconnected(struct bt_iso_chan *chan, uint8_t reason)
{
	...
	__ASSERT_MSG(chan->iso != NULL, "NULL conn for iso chan %p", chan);

	/* release buffers from tx_queue */
	while ((buf = bt_fifo_get(&chan->iso->iso.txq, OS_TIMEOUT_NO_WAIT))) {
	...
	if (IS_ENABLED(CONFIG_BT_ISO_UNICAST) &&
		...
		if (conn_type == BT_ISO_CHAN_TYPE_PERIPHERAL) {
			...
			/* Update CIG state */
			cig = get_cig(chan);
			__ASSERT_MSG(cig != NULL, "CIG was NULL");

			is_chan_connected = false;
			BT_SLIST_FOR_EACH_CONTAINER(&cig->cis_channels, cis_chan, node) {
...
#if defined(CONFIG_BT_ISO_RX)
struct bt_buf *bt_iso_get_rx(os_timeout_t timeout)
{
	struct bt_buf *buf = bt_buf_alloc(&iso_rx_pool, timeout);

	if (buf) {
		bt_buf_add_u8(buf, BT_HCI_H4_ISO);
	}
...
void bt_iso_recv(struct bt_conn *iso, struct bt_buf *buf, uint8_t flags)
{
	...
	case BT_ISO_SINGLE:
		...
		if (ts) {
			...
			ts_hdr = bt_buf_pull_mem(buf, sizeof(*ts_hdr));
	...
	case BT_ISO_CONT:
		...
		if (buf->len > bt_buf_tailroom(iso->rx)) {
		...
		bt_buf_add_mem(iso->rx, buf->data, buf->len);
		iso->rx_len -= buf->len;
		bt_buf_unref(buf);
		return;
	...
	case BT_ISO_END:
		...
		(void)bt_buf_add_mem(iso->rx, buf->data, buf->len);
		iso->rx_len -= buf->len;
		bt_buf_unref(buf);
...
static struct bt_buf *iso_data_pull(struct bt_conn *conn, size_t amount, size_t *length)
{
	 ...
	 * fragments.
	 */
	struct bt_buf *q_frag = bt_fifo_peek_head(&conn->iso.txq);
	...
	struct bt_buf *frag = bt_buf_ref(q_frag);
	bool last_frag = amount >= frag->len;

	if (last_frag) {
		q_frag = bt_fifo_get(&conn->iso.txq, OS_TIMEOUT_NO_WAIT);
...
int conn_iso_send(struct bt_conn *conn, struct bt_buf *buf, enum bt_iso_timestamp has_ts)
{
	if (buf->user_data_size < CONFIG_BT_CONN_TX_USER_DATA_SIZE) {
		LOG_ERR("not enough room in user_data %d < %d pool %p", buf->user_data_size,
			CONFIG_BT_CONN_TX_USER_DATA_SIZE, buf->pool);
		return -EINVAL;
	}

	bt_fifo_put(&conn->iso.txq, buf);
...
static int validate_send(const struct bt_iso_chan *chan, const struct bt_buf *buf,
			 uint8_t hdr_size)
{
	...
	BT_ISO_DATA_DBG("chan %p len %zu", chan, bt_buf_frags_len(buf));
	...
	if (bt_buf_headroom(buf) != BT_BUF_ISO_SIZE(0)) {
		LOG_DBG("Buffer headroom (%d) != BT_BUF_ISO_SIZE(0) (%d) bytes",
			bt_buf_headroom(buf), BT_BUF_ISO_SIZE(0));
...
int bt_iso_chan_send(struct bt_iso_chan *chan, struct bt_buf *buf, uint16_t seq_num)
{
	...
	hdr->slen = sys_cpu_to_le16(
		bt_iso_pkt_len_pack(bt_buf_frags_len(buf) - sizeof(*hdr), BT_ISO_DATA_VALID));
...
int bt_iso_chan_send_ts(struct bt_iso_chan *chan, struct bt_buf *buf, uint16_t seq_num,
			uint32_t ts)
{
	...
	BT_ISO_DATA_DBG("chan %p len %zu ts %u", chan, bt_buf_frags_len(buf), ts);
...
void hci_le_cis_established(struct bt_buf *buf)
{
...
void hci_le_cis_established_v2(struct bt_buf *buf)
{
...
void hci_le_cis_req(struct bt_buf *buf)
{
...
static struct bt_buf *hci_le_set_cig_params(const struct bt_iso_cig *cig,
					     const struct bt_iso_cig_param *param)
{
	...
	for (i = 0; i < param->num_cis; i++) {
		...
		cis_param = bt_buf_add(buf, sizeof(*cis_param));
...
#if defined(CONFIG_BT_ISO_TEST_PARAMS)
static struct bt_buf *hci_le_set_cig_test_params(const struct bt_iso_cig *cig,
						  const struct bt_iso_cig_param *param)
{
...
static struct bt_iso_cig *get_cig(const struct bt_iso_chan *iso_chan)
{
	...
	__ASSERT_MSG(iso_chan->iso->iso.info.unicast.cig_id < ARRAY_SIZE(cigs), "Invalid cig_id %u",
...
static struct bt_iso_cig *get_free_cig(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(cigs); i++) {
		if (cigs[i].state == BT_ISO_CIG_STATE_IDLE) {
			...
			cigs[i].id = i;
			bt_slist_init(&cigs[i].cis_channels);
			return &cigs[i];
		}
	}
...
static int cig_init_cis(struct bt_iso_cig *cig, const struct bt_iso_cig_param *param)
{
	for (uint8_t i = 0; i < param->num_cis; i++) {
		...
		if (cis->iso == NULL) {
			...
			bt_slist_append(&cig->cis_channels, &cis->node);
...
static void cleanup_cig(struct bt_iso_cig *cig)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&cig->cis_channels, cis, tmp, node) {
		if (cis->iso != NULL) {
			bt_conn_unref(cis->iso);
			cis->iso = NULL;
		}

		bt_slist_remove(&cig->cis_channels, NULL, &cis->node);
	}
...
int bt_iso_cig_create(const struct bt_iso_cig_param *param, struct bt_iso_cig **out_cig)
{
	...
	*out_cig = NULL;
	...
	i = 0;
	BT_SLIST_FOR_EACH_CONTAINER(&cig->cis_channels, cis, node) {
...
#if defined(CONFIG_BT_ISO_BROADCAST)
static bt_slist_t iso_big_cbs = BT_SLIST_STATIC_INIT(&iso_big_cbs);
...
static void cleanup_big(struct bt_iso_big *big)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&big->bis_channels, bis, tmp, node) {
		if (bis->iso != NULL) {
			bt_conn_unref(bis->iso);
			bis->iso = NULL;
		}

		bt_slist_remove(&big->bis_channels, NULL, &bis->node);
	}
...
static void big_disconnect(struct bt_iso_big *big, uint8_t reason)
{
	...
	if (!bt_slist_is_empty(&iso_big_cbs)) {
		...
		BT_SLIST_FOR_EACH_CONTAINER(&iso_big_cbs, listener, _node) {
			...
			bool broadcaster)
{
	...
	bt_slist_append(&big->bis_channels, &bis->node);
...
int bt_iso_big_register_cb(struct bt_iso_big_cb *cb)
{
	...
	if (bt_slist_find(&iso_big_cbs, &cb->_node, NULL)) {
		LOG_DBG("cb %p is already registered", cb);

		return -EEXIST;
	}

	bt_slist_append(&iso_big_cbs, &cb->_node);
			     ...
			     struct bt_iso_big_create_param *param)
{
	...
	bis = BT_SLIST_PEEK_HEAD_CONTAINER(&big->bis_channels, bis, node);
	__ASSERT_MSG(bis != NULL, "bis was NULL");
...
static bool is_advanced_big_param(const struct bt_iso_big_create_param *param)
{
	...
	for (uint8_t i = 0U; i < param->num_bis; i++) {
		...
		__ASSERT_MSG(qos->tx != NULL, "TX cannot be NULL for broadcaster");
...
void hci_le_big_complete(struct bt_buf *buf)
{
...
void hci_le_big_terminate(struct bt_buf *buf)
{
...
void hci_le_big_sync_established(struct bt_buf *buf)
{
...
void hci_le_big_sync_lost(struct bt_buf *buf)
{
				  ...
				  const struct bt_iso_big_sync_param *param)
{
	...
	req = bt_buf_add(buf, sizeof(*req) + big->num_bis);
	req->big_handle = big->handle;
