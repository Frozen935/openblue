...
static void hci_rx_buf_destroy(struct bt_buf *buf)
{
	bt_buf_destroy(buf);
...
BT_BUF_POOL_FIXED_DEFINE(hci_rx_pool, BT_BUF_RX_COUNT, BT_BUF_RX_SIZE, 0, hci_rx_buf_destroy);
BT_BUF_POOL_FIXED_DEFINE(hci_cmd_pool, BT_BUF_CMD_TX_COUNT,
			  BT_BUF_CMD_SIZE(CONFIG_BT_BUF_CMD_TX_SIZE), 0, NULL);
BT_BUF_POOL_FIXED_DEFINE(hci_acl_pool, CONFIG_BT_BUF_ACL_TX_COUNT,
			  BT_BUF_ACL_SIZE(CONFIG_BT_BUF_ACL_TX_SIZE), 0, NULL);
#if defined(CONFIG_BT_ISO)
BT_BUF_POOL_FIXED_DEFINE(hci_iso_pool, CONFIG_BT_ISO_TX_BUF_COUNT,
			      ...
			      const void *data, size_t size)
{
	...
	if (data && size) {
		if (bt_buf_tailroom(buf) < size) {
			bt_buf_unref(buf);
			return NULL;
		}

		bt_buf_add_mem(buf, data, size);
	}
