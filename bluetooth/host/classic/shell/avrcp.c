...
BT_BUF_POOL_DEFINE(avrcp_big_tx_pool, CONFIG_BT_MAX_CONN,
			       ...
			       struct bt_buf *buf)
{
	...
	rsp = bt_buf_pull_mem(buf, sizeof(*rsp));
	switch (rsp->cap_id) {
	case BT_AVRCP_CAP_COMPANY_ID:
		for (i = 0; i < rsp->cap_cnt; i++) {
			...
			bt_buf_pull_mem(buf, BT_AVRCP_COMPANY_ID_SIZE);
	...
	case BT_AVRCP_CAP_EVENTS_SUPPORTED:
		...
		bt_buf_pull_mem(buf, rsp->cap_cnt);
		for (i = 0; i < rsp->cap_cnt; i++) {
					...
					struct bt_buf *buf)
{
	...
	while (buf->len > 0) {
		...
		attr = bt_buf_pull_mem(buf, sizeof(struct bt_avrcp_media_attr));
		...
		bt_buf_pull_mem(buf, attr->attr_len);
		switch (attr->attr_id) {
...
static void avrcp_get_element_attrs_req(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
	...
	cmd = bt_buf_pull_mem(buf, sizeof(*cmd));
	...
	bt_buf_pull_mem(buf, expected_len);
					 ...
					 struct bt_buf *buf)
{
	...
	while (buf->len > 0) {
		...
		folder_name = bt_buf_pull_mem(buf, sizeof(struct bt_avrcp_folder_name));
		...
		bt_buf_pull_mem(buf, folder_name->folder_name_len);
...
static void avrcp_get_folder_items_rsp(struct bt_avrcp_ct *ct, uint8_t tid, struct bt_buf *buf)
{
	...
	if (hdr->item_type == BT_AVRCP_ITEM_TYPE_MEDIA_PLAYER) {
		...
		item = bt_buf_pull_mem(buf, sizeof(struct bt_avrcp_media_player_item));
		...
		name = bt_buf_pull_mem(buf, sys_be16_to_cpu(item->name_len));
...
static void avrcp_get_item_attrs_rsp(struct bt_avrcp_ct *ct, uint8_t tid, struct bt_buf *buf)
{
...
static void avrcp_search_rsp(struct bt_avrcp_ct *ct, uint8_t tid, struct bt_buf *buf)
{
...
static void avrcp_list_player_app_setting_attrs_rsp(struct bt_avrcp_ct *ct, uint8_t tid,
						    uint8_t status, struct bt_buf *buf)
{
	...
	while (buf->len > 0) {
		uint8_t attr = bt_buf_pull_u8(buf);
						   ...
						   uint8_t status, struct bt_buf *buf)
{
	...
	rsp = bt_buf_pull_mem(buf, sizeof(*rsp));
	while (buf->len > 0) {
		uint8_t val = bt_buf_pull_u8(buf);
						      ...
						      uint8_t status, struct bt_buf *buf)
{
	...
	while (buf->len > 0) {
		...
		attr_vals.attr_id = bt_buf_pull_u8(buf);
		attr_vals.value_id = bt_buf_pull_u8(buf);
						       ...
						       uint8_t status, struct bt_buf *buf)
{
	...
	while (buf->len > 0) {
		...
		attr_text = bt_buf_pull_mem(buf, sizeof(struct bt_avrcp_app_setting_attr_text));
		...
		bt_buf_pull_mem(buf, attr_text->text_len);
						      ...
						      uint8_t status, struct bt_buf *buf)
{
	...
	while (buf->len > 0) {
		...
		val_text = bt_buf_pull_mem(buf, sizeof(struct bt_avrcp_app_setting_val_text));
		...
		uint8_t *text = bt_buf_pull_mem(buf, val_text->text_len);
...
static void avrcp_get_folder_items_req(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
	...
	cmd = bt_buf_pull_mem(buf, sizeof(struct bt_avrcp_get_folder_items_cmd));
	...
	while (buf->len > 0) {
		...
		memcpy(&be_attr_id, bt_buf_pull_mem(buf, sizeof(uint32_t)), sizeof(uint32_t));
...
static void avrcp_change_path_req(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
...
static void avrcp_get_item_attrs_req(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
	...
	cmd = bt_buf_pull_mem(buf, sizeof(struct bt_avrcp_get_item_attrs_cmd));
	...
	while (buf->len > 0) {
		...
		memcpy(&attr_id, bt_buf_pull_mem(buf, sizeof(uint32_t)), sizeof(uint32_t));
...
static void avrcp_search_req(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
	...
	uint8_t *search_str = bt_buf_pull_mem(buf, search_str_len);
...
static void avrcp_passthrough_req(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
	...
	if (cmd->data_len > 0U) {
		...
		opvu = bt_buf_pull_mem(buf, sizeof(*opvu));
	}
						      ...
						      uint8_t tid, struct bt_buf *buf)
{
	...
	while (buf->len > 0) {
		uint8_t attr_ids = bt_buf_pull_u8(buf);
						 ...
						 struct bt_buf *buf)
{
	...
	for (uint8_t i = 0; i < cmd->num_attrs; i++) {
		cmd->attr_vals[i].attr_id = bt_buf_pull_u8(buf);
		cmd->attr_vals[i].value_id = bt_buf_pull_u8(buf);
						       ...
						       uint8_t tid, struct bt_buf *buf)
{
	...
	for (uint8_t i = 0; i < cmd->num_attrs; i++) {
		bt_shell_print(" attr_id[%u]=0x%02x", i, bt_buf_pull_u8(buf));
	}
						      ...
						      uint8_t tid, struct bt_buf *buf)
{
	...
	for (uint8_t i = 0; i < cmd->num_values; i++) {
		bt_shell_print(" val_id[%u]=0x%02x", i, bt_buf_pull_u8(buf));
	}
}
						  ...
						  struct bt_buf *buf)
{
	...
	for (uint8_t i = 0; i < cmd->num_charsets; i++) {
		bt_shell_print("  charset[%u]: 0x%04x", i, bt_buf_pull_be16(buf));
	}
}
...
static void avrcp_play_item_req(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
...
static void avrcp_add_to_now_playing_req(struct bt_avrcp_tg *tg, uint8_t tid, struct bt_buf *buf)
{
...
static int cmd_send_passthrough_rsp(const struct bt_shell *sh, int32_t argc, char *argv[])
{
	...
	if (err < 0) {
		...
		bt_shell_print("Passthrough opid=0x%02x (%s), state=%s sent successfully", opid,
...
static int cmd_send_get_caps_rsp(const struct bt_shell *sh, int32_t argc, char *argv[])
{
	struct bt_avrcp_get_caps_rsp *rsp;
	struct bt_buf *buf;
	int err;
	...
	if (buf == NULL) {
		bt_shell_error("Failed to allocate buffer for AVRCP get caps rsp");
		return -ENOMEM;
	}

	if (bt_buf_tailroom(buf) < sizeof(*rsp)) {
		bt_shell_error("Not enough tailroom in buffer for get caps rsp");
		goto failed;
	}
	rsp = bt_buf_add(buf, sizeof(*rsp));
	...
	case BT_AVRCP_CAP_COMPANY_ID:
		...
		rsp->cap_cnt = 1;
		if (bt_buf_tailroom(buf) < BT_AVRCP_COMPANY_ID_SIZE) {
			bt_shell_error("Not enough tailroom for company ID capability rsp");
			goto failed;
		}
		bt_buf_add(buf, BT_AVRCP_COMPANY_ID_SIZE);
		sys_put_be24(BT_AVRCP_COMPANY_ID_BLUETOOTH_SIG, rsp->cap);
		bt_shell_print("Sending company ID capability rsp: 0x%06x",
			    BT_AVRCP_COMPANY_ID_BLUETOOTH_SIG);
		break;

	case BT_AVRCP_CAP_EVENTS_SUPPORTED:
		rsp->cap_cnt = ARRAY_SIZE(supported_avrcp_events);
		if (bt_buf_tailroom(buf) < rsp->cap_cnt) {
			bt_shell_error("Not enough tailroom for events supported capability rsp");
			goto failed;
		}
		bt_buf_add_mem(buf, supported_avrcp_events, rsp->cap_cnt);
		bt_shell_print("Sending events supported capability rsp with %u events",
			    rsp->cap_cnt);
		break;

	default:
		bt_shell_error("Unknown capability ID: 0x%02x", tg_cap_id);
		return -EINVAL;
	}

	err = bt_avrcp_tg_get_caps(default_tg, tg_tid, status, buf);
	if (err < 0) {
		bt_shell_error("Failed to send get capabilities response: %d", err);
	} else {
		bt_shell_print("Get capabilities response sent successfully");
	}
			   ...
			   uint8_t len)
{
	...
	err = bt_avrcp_ct_passthrough(default_ct, get_next_tid(), opid, BT_AVRCP_BUTTON_PRESSED,
				      payload, len);
	if (err < 0) {
		bt_shell_error("Failed to send passthrough PRESSED command: opid=0x%02x, err=%d",
			    opid, err);
		return err;
	}
	bt_shell_print("Passthrough PRESSED command sent successfully: opid=0x%02x", opid);

	err = bt_avrcp_ct_passthrough(default_ct, get_next_tid(), opid, BT_AVRCP_BUTTON_RELEASED,
				      payload, len);
	if (err < 0) {
		bt_shell_error("Failed to send passthrough RELEASED command: opid=0x%02x, err=%d",
			    opid, err);
		return err;
	}
	bt_shell_print("Passthrough RELEASED command sent successfully: opid=0x%02x", opid);
...
static int cmd_get_caps(const struct bt_shell *sh, int32_t argc, char *argv[])
{
	const char *cap_id;
	int err;
	...
	if (!strcmp(cap_id, "company")) {
		...
		if (err < 0) {
			bt_shell_error("Failed to send get_caps command: cap_id=company, err=%d",
				    err);
			return err;
		}
		bt_shell_print("Get capabilities command sent successfully: cap_id=company");
	} else if (!strcmp(cap_id, "events")) {
		err = bt_avrcp_ct_get_caps(default_ct, get_next_tid(),
					   BT_AVRCP_CAP_EVENTS_SUPPORTED);
		if (err < 0) {
			bt_shell_error("Failed to send get_caps command: cap_id=events, err=%d",
				    err);
			return err;
		}
		bt_shell_print("Get capabilities command sent successfully: cap_id=events");
	} else {
		bt_shell_error("Invalid capability ID: %s (expected: company or events)", cap_id);
		return -EINVAL;
	}

	return 0;
}
static int cmd_ct_register_notification(const struct bt_shell *sh, int argc, char *argv[])
{
	uint8_t event_id;
	...
	if (default_ct == NULL) {
		bt_shell_error("AVRCP CT is not connected");
		return -ENOEXEC;
	}
	...
	if (err < 0) {
		bt_shell_error("Failed to send register_notification: %d", err);
	} else {
		bt_shell_print("Sent register notification event_id=0x%02x", event_id);
	}
	return err;
}

static int cmd_tg_send_notification_rsp(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_avrcp_event_data data;
	bool interim = false;
	...
	if (strcmp(argv[2], "changed") == 0) {
		...
		bt_shell_error("Invalid type: %s (expected: changed|interim)", argv[2]);
	...
	case BT_AVRCP_EVT_TRACK_CHANGED:
		if (argc < 4) {
			...
			if (*endptr != '\0') {
				bt_shell_error("Invalid identifier: %s", argv[3]);
			}
			sys_put_be64(identifier, data.identifier);
		}
		break;
	...
	case BT_AVRCP_EVT_NOW_PLAYING_CONTENT_CHANGED:
		break;
	default:
		bt_shell_error("Unknown event_id: 0x%02x", event_id);
		status = BT_AVRCP_STATUS_INVALID_PARAMETER;
		goto done;
	}
	...
	if (err < 0) {
		bt_shell_error("Failed to send notification rsp: %d", err);
	} else {
		bt_shell_print("Sent notification rsp event_id=0x%02x type=%s",
			    event_id, (interim) ? "interim" : "changed");
	}
	return err;
}

static int cmd_ct_list_app_attrs(const struct bt_shell *sh, int argc, char *argv[])
{
	int err;
	...
	if (err < 0) {
		bt_shell_error("list player app setting attrs failed: %d", err);
	} else {
		bt_shell_print("Sent list player app setting attrs");
	}

	return err;
}

static int cmd_ct_list_app_vals(const struct bt_shell *sh, int argc, char *argv[])
{
	uint8_t attr;
	int err;
	...
	if (err < 0) {
		bt_shell_error("Failed to send list player app setting vals: %d", err);
		return -ENOEXEC;
	}

	bt_shell_print("Sent list player app setting vals attr=0x%02x", attr);
	return 0;
}

static int cmd_ct_get_app_curr(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_avrcp_get_curr_player_app_setting_val_cmd *cmd;
	struct bt_buf *buf;
	size_t expected_len;
	...
	if (buf == NULL) {
		bt_shell_error("Failed to allocate vendor dependent command buffer");
		return -ENOMEM;
	}

	if (bt_buf_tailroom(buf) < expected_len) {
		bt_shell_error("Not enough tailroom in buffer");
		goto failed;
	}
	...
	for (i = 1; i < argc; i++) {
		bt_buf_add_u8(buf, (uint8_t)strtoul(argv[i], NULL, 0));
	}

	err = bt_avrcp_ct_get_curr_player_app_setting_val(default_ct, get_next_tid(), buf);
	if (err < 0) {
		bt_shell_error("Failed to send get_curr_player_app_setting_val: %d", err);
		goto failed;
	}

	bt_shell_print("Sent get_curr_player_app_setting_val num=%u", cmd->num_attrs);
	return 0;
...
static int cmd_ct_set_app_val(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_avrcp_set_player_app_setting_val_cmd *cmd;
	struct bt_buf *buf;
	size_t expected_len;
	...
	if ((argc < 3) || (((argc - 1) % 2) != 0)) {
		bt_shell_error("usage: set_app_val <attr1> <val1> [<attr2> <val2> ...]");
		return -ENOEXEC;
	}
	...
	cmd = bt_buf_add(buf, expected_len);
	cmd->num_attrs = pairs;
	...
	if (err < 0) {
		bt_shell_error("Failed to send set_player_app_setting_val: %d", err);
		goto failed;
	}

	bt_shell_print("Sent SetPlayerApplicationSettingValue num_attrs=%u", cmd->num_attrs);
	return 0;
...
static int cmd_ct_get_app_attr_text(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_avrcp_get_player_app_setting_attr_text_cmd *cmd;
	struct bt_buf *buf;
	int err;
	...
	if (err < 0) {
		bt_shell_error("get_player_app_setting_attr_text failed: %d", err);
		bt_buf_unref(buf);
		return err;
	}

	bt_shell_print("Sent get_player_app_setting_attr_text num_attrs=%u", cmd->num_attrs);
	return 0;
}

static int cmd_ct_get_app_val_text(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_avrcp_get_player_app_setting_val_text_cmd *cmd;
	struct bt_buf *buf;
	int err;
	...
	if (err < 0) {
		bt_shell_error("GetPlayerApplicationSettingValueText failed: %d", err);
		bt_buf_unref(buf);
	}

	bt_shell_print("Sent GetPlayerApplicationSettingValueText attr=0x%02x num=%u",
		    cmd->attr_id, cmd->num_values);

	return err;
}

static int cmd_ct_inform_displayable_char(const struct bt_shell *sh, size_t argc, char **argv)
{
	struct bt_buf *buf;
	uint8_t num_chars;
	uint16_t charset_id;
	...
	if (num_chars == 0 || num_chars > 7) {
		bt_shell_error("Number of charsets must be between 1 and 7");
		return -EINVAL;
	}

	buf = bt_avrcp_create_vendor_pdu(&avrcp_tx_pool);
	if (buf == NULL) {
		bt_shell_error("Failed to allocate buffer");
		return -ENOMEM;
	}

	/* Add number of character sets */
	bt_buf_add_u8(buf, num_chars);

	/* Add character set IDs */
	for (int i = 1; i < argc; i++) {
		charset_id = (uint16_t)strtoul(argv[i], NULL, 0);
		bt_buf_add_be16(buf, charset_id);
	}

	err = bt_avrcp_ct_inform_displayable_char_set(default_ct, get_next_tid(), buf);
	if (err < 0) {
		bt_shell_error("Failed to send InformDisplayableCharacterSet: %d", err);
		bt_buf_unref(buf);
	} else {
		bt_shell_print("AVRCP InformDisplayableCharacterSet sent with %u charsets",
			    num_chars);
	}
...
static int cmd_ct_inform_batt(const struct bt_shell *sh, size_t argc, char **argv)
{
	uint8_t battery_status;
	int err;
	...
	if (err < 0) {
		bt_shell_error("fail to InformBatteryStatusOfCT");
	} else {
		bt_shell_print("AVRCP InformBatteryStatusOfCT");
	}

	return err;
}

static int cmd_get_element_attrs(const struct bt_shell *sh, int32_t argc, char *argv[])
{
	struct bt_avrcp_get_element_attrs_cmd *cmd;
	struct bt_buf *buf;
	...
	if (bt_buf_tailroom(buf) < sizeof(*cmd) + (7 * sizeof(uint32_t))) {
	...
	if (argc > 1) {
		...
		if (*endptr != '\0') {
			bt_shell_error("Invalid identifier: %s", argv[1]);
			goto failed;
		}
	...
	if (argc > 2 && identifier != 0) {
		for (i = 2; i < argc && i < 9; i++) { /* Max 7 attributes + cmd + identifier */
			...
			if (*endptr != '\0' || val > 0xFFFFFFFFUL) {
				bt_shell_error("Invalid attribute ID: %s", argv[i]);
				goto failed;
			}
			bt_buf_add_be32(buf, (uint32_t)val);
			cmd->num_attrs++;
		}
	}

	bt_shell_print("Requesting element attributes: identifier=0x%016llx, num_attrs=%u",
		    identifier, cmd->num_attrs);

	err = bt_avrcp_ct_get_element_attrs(default_ct, get_next_tid(), buf);
	if (err < 0) {
		bt_shell_error("Failed to send get element attrs command: %d", err);
		goto failed;
	} else {
		bt_shell_print("AVRCP CT get element attrs command sent");
		return 0;
	}
failed:
	bt_buf_unref(buf);
...
static int cmd_send_get_element_attrs_rsp(const struct bt_shell *sh, int32_t argc, char *argv[])
{
	...
	if (bt_buf_tailroom(buf) < sizeof(*rsp)) {
		bt_shell_error("Not enough tailroom in buffer for get element attrs rsp");
		goto failed;
	}
	...
	if (use_large_attrs) {
		...
		if (bt_buf_tailroom(buf) < total_size) {
			bt_shell_error("Not enough tailroom in buffer for large attrs");
			goto failed;
		}

		ARRAY_FOR_EACH(large_media_attrs, i) {
			attr = bt_buf_add(buf, sizeof(struct bt_avrcp_media_attr));
			...
			bt_buf_add(buf, large_media_attrs[i].attr_len);
		...
		bt_shell_print("Sending large Attributes response (%u attrs) for fragment test",
			    rsp->num_attrs);
	} else {
		...
		ARRAY_FOR_EACH(test_media_attrs, i) {
			attr = bt_buf_add(buf, sizeof(*attr));
			...
			bt_buf_add_mem(buf, test_media_attrs[i].attr_val,
					test_media_attrs[i].attr_len);
		}
		bt_shell_print("Sending standard GetElementAttributes response (%u attrs)",
			    rsp->num_attrs);
	}

	err = bt_avrcp_tg_get_element_attrs(default_tg, tg_tid, BT_AVRCP_STATUS_SUCCESS, buf);
	if (err < 0) {
		bt_shell_error("Failed to send GetElementAttributes response: %d", err);
		goto failed;
	} else {
		bt_shell_print("GetElementAttributes response sent successfully");
		return 0;
	}
...
static int cmd_ct_set_absolute_volume(const struct bt_shell *sh, int argc, char *argv[])
{
	uint8_t absolute_volume;
	int err;
	...
	if (err < 0) {
		bt_shell_error("Failed to set absolute volume: %d", err);
	} else {
		bt_shell_print("set absolute volume"
			    " absolute_volume=0x%02x", absolute_volume);
	}
	return err;
}

static int cmd_ct_get_play_status(const struct bt_shell *sh, int argc, char *argv[])
{
	int err;
	...
	if (err < 0) {
		bt_shell_error("Fail to get_play_status");
	} else {
		bt_shell_print("AVRCP GetPlayStatus");
	}

	return err;
}

static int cmd_ct_set_addressed_player(const struct bt_shell *sh, int argc, char *argv[])
{
	uint16_t player_id;
	int err;
	...
	if (err < 0) {
		bt_shell_error("fail to set addressed player");
	} else {
		bt_shell_print("AVRCP send set addressed player req");
	}

	return 0;
}

static int cmd_ct_play_item(const struct bt_shell *sh, size_t argc, char **argv)
{
	struct bt_avrcp_play_item_cmd *cmd;
	struct bt_buf *buf;
	char *endptr = NULL;
	...
	if (cmd->scope > BT_AVRCP_SCOPE_NOW_PLAYING) {
		bt_shell_error("scope out of range ");
		goto failed;
	}
	...
	if ((endptr == argv[2]) || (*endptr != '\0')) {
		bt_shell_error("invalid uid hex: %s", argv[2]);
		goto failed;
	}
	...
	bt_shell_print("Sent PlayItem scope=0x%02x uid=0x%016llx uid_counter=0x%04x",
...
static int cmd_ct_add_to_now_playing(const struct bt_shell *sh, size_t argc, char **argv)
{
	struct bt_avrcp_add_to_now_playing_cmd *cmd;
	struct bt_buf *buf;
	char *endptr = NULL;
	...
	bt_shell_print("Sent AddToNowPlaying scope=0x%02x uid=0x%016llx  uid_counter=0x%04x",
		    cmd->scope, uid, cmd->uid_counter);
	return 0;
...
static int cmd_tg_send_absolute_volume_rsp(const struct bt_shell *sh, int32_t argc, char *argv[])
{
	uint8_t absolute_volume;
	int err;
	...
	err = bt_avrcp_tg_absolute_volume(default_tg, tg_tid, BT_AVRCP_STATUS_SUCCESS,
					  absolute_volume);
	if (err < 0) {
		bt_shell_error("Failed to send set absolute volume response: %d", err);
	} else {
		bt_shell_print("Set absolute volume response sent successfully");
	}

	return err;
}

static int cmd_tg_send_list_player_app_setting_attrs_rsp(const struct bt_shell *sh, int argc,
							 char *argv[])
{
	struct bt_avrcp_list_player_app_setting_attrs_rsp *rsp;
	struct bt_buf *buf;
	...
	buf = bt_avrcp_create_vendor_pdu(&avrcp_tx_pool);
	if (buf == NULL) {
		bt_shell_error("Failed to allocate buffer for AVRCP response");
		return -ENOMEM;
	}
	...
	rsp = bt_buf_add(buf, expected_len);
	...
	err = bt_avrcp_tg_list_player_app_setting_attrs(default_tg, tg_tid, BT_AVRCP_STATUS_SUCCESS,
							buf);
	if (err < 0) {
		bt_shell_error("Failed to send ListPlayerAppSettingAttributes rsp: %d", err);
		goto failed;
	}

	bt_shell_print("ListPlayerApplicationSettingAttributes rsp sent (num=%u)", num);
	return 0;
...
static int cmd_tg_send_list_player_app_setting_vals_rsp(const struct bt_shell *sh, int argc,
							char *argv[])
{
	struct bt_avrcp_list_player_app_setting_vals_rsp *rsp;
	struct bt_buf *buf;
	...
	err = bt_avrcp_tg_list_player_app_setting_vals(default_tg, tg_tid, BT_AVRCP_STATUS_SUCCESS,
						       buf);
	if (err < 0) {
		bt_shell_error("Failed to send list player app setting vals rsp: %d", err);
		goto failed;
	}

	bt_shell_print("List player app setting vals rsp sent (num=%u)", num);
	return 0;
...
static int cmd_tg_send_get_curr_player_app_setting_val_rsp(const struct bt_shell *sh, int argc,
							   char *argv[])
{
	struct bt_buf *buf;
	struct bt_avrcp_get_curr_player_app_setting_val_rsp *rsp;
	...
	if (err < 0) {
		bt_shell_error("Failed to send get curr player app setting val rsp: %d", err);
		goto failed;
	}

	bt_shell_print("Send get curr player app setting val rsp (num=%u)", rsp->num_attrs);
	return 0;
...
static int  cmd_tg_send_set_player_app_setting_val_rsp(const struct bt_shell *sh, int argc,
						       char *argv[])
{
	int err;
	uint8_t status = BT_AVRCP_STATUS_OPERATION_COMPLETED;
	...
	if (err < 0) {
		bt_shell_error("Failed to send set set_player_app_setting_val rsp: %d", err);
		return -ENOEXEC;
	}

	bt_shell_print("set_player_app_setting_val rsp sent ");
	return 0;
}

static int cmd_tg_send_get_player_app_setting_attr_text_rsp(const struct bt_shell *sh, int argc,
							    char *argv[])
{
	struct bt_avrcp_get_player_app_setting_attr_text_rsp *rsp;
	struct bt_buf *buf;
	...
	if (bt_buf_tailroom(buf) < sizeof(*rsp) + sizeof(struct bt_avrcp_app_setting_attr_text)) {
		bt_shell_error("Not enough tailroom in buffer");
		goto failed;
	}
	rsp = bt_buf_add(buf, sizeof(*rsp) + sizeof(struct bt_avrcp_app_setting_attr_text));
	...
	bt_buf_add_mem(buf, text_str, strlen(text_str));

	err = bt_avrcp_tg_get_player_app_setting_attr_text(default_tg, tg_tid, status, buf);
	if (err < 0) {
		bt_shell_error("Failed to send get player app setting attr text rsp: %d", err);
		return -ENOEXEC;
	}

	bt_shell_print("Get player app setting attr text rsp sent");
	return 0;
...
static int cmd_tg_send_get_player_app_setting_val_text_rsp(const struct bt_shell *sh, int argc,
							   char *argv[])
{
	struct bt_avrcp_get_player_app_setting_val_text_rsp *rsp;
	struct bt_buf *buf;
	...
	if (bt_buf_tailroom(buf) < sizeof(*rsp) + sizeof(struct bt_avrcp_app_setting_val_text)) {
		bt_shell_error("Not enough tailroom in buffer");
		goto failed;
	}
	rsp = bt_buf_add(buf, sizeof(*rsp) + sizeof(struct bt_avrcp_app_setting_val_text));
	...
	bt_buf_add_mem(buf, value_str, strlen(value_str));

	err = bt_avrcp_tg_get_player_app_setting_val_text(default_tg, tg_tid, status, buf);
	if (err < 0) {
		bt_shell_error("Failed to send get player app setting val text rsp: %d", err);
		return -ENOEXEC;
	}

	bt_shell_print("Get player app setting val text rsp sent");
	return 0;
...
static int cmd_tg_send_inform_displayable_char_rsp(const struct bt_shell *sh, int argc, char *argv[])
{
	int err;
	uint8_t status = BT_AVRCP_STATUS_OPERATION_COMPLETED;
	...
	if (err < 0) {
		bt_shell_error("Failed to send inform displayable char rsp: err=%d", err);
		return err;
	}

	bt_shell_print("Sent inform displayable char rsp (status=0x%02x)", status);
	return 0;
}

static int cmd_tg_send_inform_batt_status_of_ct_rsp(const struct bt_shell *sh, int argc, char *argv[])
{
	int err;
	uint8_t status = BT_AVRCP_STATUS_OPERATION_COMPLETED;
	...
	if (err < 0) {
		bt_shell_error("send inform batt status rsp send failed: err=%d", err);
		return err;
	}

	bt_shell_print("send inform batt status rsp sent (status=0x%02x)", status);
	return 0;

}

static int cmd_tg_send_get_play_status_rsp(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_avrcp_get_play_status_rsp *rsp;
	struct bt_buf *buf;
	int err;
	...
	if (err < 0) {
		bt_shell_error("Failed to send GetPlayStatus rsp: %d", err);
		goto failed;
	}

	bt_shell_print("GetPlayStatus rsp sent");
	return 0;
...
static int cmd_tg_send_set_addressed_player_rsp(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err;
	uint8_t status = BT_AVRCP_STATUS_OPERATION_COMPLETED;
	...
	if (err < 0) {
		bt_shell_error("SetAddressedPlayer rsp send failed: err=%d", err);
		return err;
	}

	bt_shell_print("SetAddressedPlayer rsp sent (status=0x%02x)", status);
	return 0;
}

static int cmd_tg_send_play_item_rsp(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err;
	uint8_t status = BT_AVRCP_STATUS_OPERATION_COMPLETED;
	...
	if (err < 0) {
		bt_shell_error("Play item rsp send failed: err=%d", err);
		return err;
	}

	bt_shell_print("Play item rsp sent (status=0x%02x)", status);
	return 0;
}

static int cmd_tg_send_add_to_now_playing_rsp(const struct bt_shell *sh, size_t argc, char **argv)
{
	int err;
	uint8_t status = BT_AVRCP_STATUS_OPERATION_COMPLETED;
	...
	if (err < 0) {
		bt_shell_error("Add to now playing rsp send failed: err=%d", err);
		return err;
	}

	bt_shell_print("Add to now playing rsp sent (status=0x%02x)", status);
	return 0;
}
...
static int cmd_send_set_browsed_player_rsp(const struct bt_shell *sh, int32_t argc, char *argv[])
{
	...
	if (status > BT_AVRCP_STATUS_ADDRESSED_PLAYER_CHANGED) {
		bt_shell_error("Invalid status value: 0x%02x (max: 0x%02x)",
	...
	if (status != BT_AVRCP_STATUS_OPERATION_COMPLETED) {
		if (bt_buf_tailroom(buf) < sizeof(uint8_t)) {
			bt_shell_error("Not enough tailroom in buffer for status");
			goto failed;
		}
		bt_buf_add_u8(buf, status);
		goto send_response;
	}
	...
	folder_name = bt_buf_add(buf, sizeof(*folder_name));
	folder_name->folder_name_len = sys_cpu_to_be16(folder_name_len);
	if (rsp->charset_id == sys_cpu_to_be16(BT_AVRCP_CHARSET_UTF8)) {
		bt_buf_add_mem(buf, folder_name_str, folder_name_len);
	} else {
		bt_buf_add_mem(buf, folder_name_hex, folder_name_len);
	}
send_response:
	err = bt_avrcp_tg_set_browsed_player(default_tg, tg_tid, buf);
	if (err == 0) {
		bt_shell_print("Send set browsed player response, status = 0x%02x", status);
	} else {
...
static int cmd_ct_get_folder_items(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_buf *buf;
	struct bt_avrcp_get_folder_items_cmd *cmd;
	int err;
	...
	if (bt_buf_tailroom(buf) < sizeof(*cmd)) {
		bt_shell_error("Not enough tailroom in buffer");
		goto failed;
	}
	cmd = bt_buf_add(buf, sizeof(*cmd));
	...
	if (err < 0) {
		bt_shell_error("Failed to send get_folder_items: %d", err);
		goto failed;
	}

	bt_shell_print("Sent GetFolderItems command");
	return 0;
failed:
	bt_buf_unref(buf);
	return -ENOEXEC;
...
static int cmd_ct_change_path(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_buf *buf;
	struct bt_avrcp_change_path_cmd *cmd;
	int err;
	...
	if (err < 0) {
		bt_shell_error("Failed to send change_path: %d", err);
		goto failed;
	}

	bt_shell_print("Sent ChangePath command");
	return 0;
failed:
	bt_buf_unref(buf);
	return -ENOEXEC;
...
static int cmd_ct_get_item_attrs(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_buf *buf;
	...
	if (err < 0) {
		bt_shell_error("Failed to send get_item_attrs: %d", err);
		goto failed;
	}

	bt_shell_print("Sent GetItemAttributes command");
	return 0;
failed:
	bt_buf_unref(buf);
	return -ENOEXEC;
...
static int cmd_ct_get_total_number_of_items(const struct bt_shell *sh, int argc, char *argv[])
{
	uint8_t scope = BT_AVRCP_SCOPE_VFS;
	int err;
	...
	if (err < 0) {
		bt_shell_error("Failed to send get_total_number_of_items: %d", err);
		return err;
	}

	bt_shell_print("Sent GetTotalNumberOfItems command");
	return 0;
}

static int cmd_ct_search(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_buf *buf;
	struct bt_avrcp_search_cmd *cmd;
	...
	bt_buf_add_mem(buf, search_str, strlen(search_str));

	err = bt_avrcp_ct_search(default_ct, get_next_tid(), buf);
	if (err < 0) {
		bt_shell_error("Failed to send search: %d", err);
		goto failed;
	}

	bt_shell_print("Sent Search command");
	return 0;
failed:
	bt_buf_unref(buf);
	return -ENOEXEC;
...
static int cmd_tg_send_get_folder_items_rsp(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_buf *buf;
	struct bt_avrcp_get_folder_items_rsp *rsp;
	struct bt_avrcp_media_player_item *item;
	...
	if (bt_buf_tailroom(buf) < sizeof(*rsp)) {
		bt_shell_error("Not enough tailroom in buffer for rsp");
		goto failed;
	}
	rsp = bt_buf_add(buf, sizeof(*rsp));
	rsp->status = status;
	...
	if (bt_buf_tailroom(buf) < sizeof(struct bt_avrcp_media_player_item) + name_len) {
		bt_shell_error("Not enough tailroom in buffer for media_player_item");
		goto failed;
	}
	item = bt_buf_add(buf, sizeof(struct bt_avrcp_media_player_item) + name_len);
...
send_response:
	...
	if (err < 0) {
		bt_shell_error("Failed to send get_folder_items_rsp: %d", err);
		goto failed;
	}

	bt_shell_print("TG: Sent GetFolderItems response");
	return 0;
failed:
	bt_buf_unref(buf);
	return -ENOEXEC;
...
static int cmd_tg_send_change_path_rsp(const struct bt_shell *sh, int argc, char *argv[])
{
	int err;
	uint8_t status = BT_AVRCP_STATUS_OPERATION_COMPLETED;
	...
	if (err < 0) {
		bt_shell_error("Failed to send change_path_rsp: %d", err);
		return err;
	}

	bt_shell_print("TG: Sent ChangePath response");
	return 0;
}

static int cmd_tg_send_get_item_attrs_rsp(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_buf *buf;
	struct bt_avrcp_get_item_attrs_rsp *rsp;
	struct bt_avrcp_media_attr *attr;
	...
	if (bt_buf_tailroom(buf) < (sizeof(*attr) + strlen(title))) {
		bt_shell_error("Not enough tailroom in buffer for title");
		goto failed;
	}
	attr = bt_buf_add(buf, sizeof(*attr) + strlen(title));
...
send_response:
	...
	if (err < 0) {
		bt_shell_error("Failed to send get_item_attrs_rsp: %d", err);
		goto failed;
	}

	bt_shell_print("TG: Sent GetItemAttributes response");
	return 0;
failed:
	bt_buf_unref(buf);
	return -ENOEXEC;
...
static int cmd_tg_send_get_total_number_of_items_rsp(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_buf *buf;
	struct bt_avrcp_get_total_number_of_items_rsp *rsp;
	int err;
...
send_response:
	...
	if (err < 0) {
		bt_shell_error("Failed to send get_total_number_of_items_rsp: %d", err);
		goto failed;
	}

	bt_shell_print("TG: Sent GetTotalNumberOfItems response");
	return 0;
failed:
	bt_buf_unref(buf);
	return -ENOEXEC;
...
static int cmd_tg_send_search_rsp(const struct bt_shell *sh, int argc, char *argv[])
{
	struct bt_buf *buf;
	struct bt_avrcp_search_rsp *rsp;
	int err;
...
send_response:
	...
	if (err < 0) {
		bt_shell_error("Failed to send search_rsp: %d", err);
		goto failed;
	}

	bt_shell_print("TG: Sent Search response");
	return 0;
failed:
	bt_buf_unref(buf);
	return -ENOEXEC;
...
static int cmd_tg_send_browsing_general_reject_rsp(const struct bt_shell *sh, int argc, char *argv[])
{
	...
	if (err < 0) {
		bt_shell_error("Failed to send browsing general reject rsp: %d", err);
		return err;
	}

	bt_shell_print("Sent browsing general reject response with reason=0x%02x", reason);
	return 0;
}
...
BT_SHELL_SUBCMD_SET_CREATE(
	...
	BT_SHELL_CMD_ARG(get_caps, NULL, "get capabilities <cap_id: company or events>", cmd_get_caps,
		      2, 0),
	...
	BT_SHELL_CMD_ARG(register_notification, NULL, "register notify <event_id> [playback_interval]",
	...
	BT_SHELL_CMD_ARG(get_folder_items, NULL, HELP_NONE, cmd_ct_get_folder_items, 1, 0),
	BT_SHELL_CMD_ARG(change_path, NULL, HELP_NONE, cmd_ct_change_path, 1, 0),
	BT_SHELL_CMD_ARG(get_item_attrs, NULL, "get item attrs [scope]",
		      cmd_ct_get_item_attrs, 1, 1),
	BT_SHELL_CMD_ARG(get_total_number_of_items, NULL, "get total number of items [scope]",
		      cmd_ct_get_total_number_of_items, 1, 1),
	BT_SHELL_CMD_ARG(search, NULL, "search [search_string]", cmd_ct_search, 1, 1),
	BT_SHELL_CMD_ARG(list_app_attrs, NULL, HELP_NONE, cmd_ct_list_app_attrs, 1, 0),
	BT_SHELL_CMD_ARG(list_app_vals,  NULL, "List App vals <attr_id>", cmd_ct_list_app_vals, 2, 0),
	BT_SHELL_CMD_ARG(get_app_curr,   NULL, "Get curr player app setting val [attr1] [attr2] ...",
		      cmd_ct_get_app_curr, 1, 8),
	BT_SHELL_CMD_ARG(set_app_val, NULL, "Set app setting Val <attr1> <val1> [<attr2> <val2>]  ...",
		      cmd_ct_set_app_val, 3, 14),
	BT_SHELL_CMD_ARG(get_app_attr_text, NULL, "Get app setting attrs text <attr1> [attr2] ...",
		      cmd_ct_get_app_attr_text, 2, 7),
	BT_SHELL_CMD_ARG(get_app_val_text,  NULL, "Get setting vals Text <attr_id> <val1> [val2] ...",
		      cmd_ct_get_app_val_text, 3, 6),
	BT_SHELL_CMD_ARG(inform_displayable_char, NULL,
		      "Inform Displayable Character Set <charset_id1> [charset_id2] ...",
		      cmd_ct_inform_displayable_char, 2, 7),
	BT_SHELL_CMD_ARG(inform_batt, NULL, "Inform Battery Status Of CT <Battery status>",
		      cmd_ct_inform_batt, 2, 0),
	BT_SHELL_CMD_ARG(set_absolute_volume, NULL, "set absolute volume <volume>",
		      cmd_ct_set_absolute_volume, 2, 0),
	BT_SHELL_CMD_ARG(get_element_attrs, NULL, "get element attrs [identifier] [attr1] [attr2] ...",
		      cmd_get_element_attrs, 1, 9),
	BT_SHELL_CMD_ARG(get_play_status, NULL, HELP_NONE, cmd_ct_get_play_status, 1, 0),
	BT_SHELL_CMD_ARG(set_addressed_player, NULL, "set addressed player <player_id>",
		      cmd_ct_set_addressed_player, 2, 0),
	BT_SHELL_CMD_ARG(play_item, NULL, "PlayItem <scope> <uid_hex> <uid_counter>",
		      cmd_ct_play_item, 4, 0),
	BT_SHELL_CMD_ARG(add_to_now_playing, NULL, "AddToNowPlaying <scope> <uid_hex> <uid_counter>",
...
BT_SHELL_SUBCMD_SET_CREATE(
	...
	BT_SHELL_CMD_ARG(send_get_caps_rsp, NULL, "send get capabilities response [status]",
		      cmd_send_get_caps_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_notification_rsp, NULL, "send notify rsp <event_id> <type> [value...]",
	...
	BT_SHELL_CMD_ARG(send_get_folder_items_rsp, NULL, "send get folder items [status]",
		      cmd_tg_send_get_folder_items_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_change_path_rsp, NULL, "send change path [status]",
		      cmd_tg_send_change_path_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_get_item_attrs_rsp, NULL, "send get item attrs [status]",
		      cmd_tg_send_get_item_attrs_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_get_total_number_of_items_rsp, NULL,
		      "send get total number of items [status]",
		      cmd_tg_send_get_total_number_of_items_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_search_rsp, NULL, "search [status]", cmd_tg_send_search_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_browsing_general_reject, NULL, "send browsing general reject [reason]",
	...
	BT_SHELL_CMD_ARG(send_passthrough_rsp, NULL, HELP_PASSTHROUGH_RSP, cmd_send_passthrough_rsp,
		      4, 0),
	BT_SHELL_CMD_ARG(send_list_player_app_setting_attrs_rsp, NULL,
		      "send attrs rsp <num> [attr_id...]",
		       cmd_tg_send_list_player_app_setting_attrs_rsp, 2, 8),
	BT_SHELL_CMD_ARG(send_list_player_app_setting_vals_rsp, NULL,
		      "send vals rsp <num> [val_id...]",
		      cmd_tg_send_list_player_app_setting_vals_rsp, 2, 16),
	BT_SHELL_CMD_ARG(send_get_curr_player_app_setting_val_rsp, NULL,
		      "send current vals rsp <num_pairs> [attr val]...",
		      cmd_tg_send_get_curr_player_app_setting_val_rsp, 2, 16),
	BT_SHELL_CMD_ARG(send_set_player_app_setting_val_rsp, NULL, "set app setting val rsp [status]",
		      cmd_tg_send_set_player_app_setting_val_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_get_player_app_setting_attr_text_rsp, NULL,
		      "send get player app setting attr text rsp [status]",
		      cmd_tg_send_get_player_app_setting_attr_text_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_get_player_app_setting_val_text_rsp, NULL,
		      "send get player app setting val text rsp [status]",
		      cmd_tg_send_get_player_app_setting_val_text_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_inform_displayable_char_rsp, NULL, "send displayable char rsp [status]",
		      cmd_tg_send_inform_displayable_char_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_inform_batt_status_of_ct_rsp, NULL, "send inform batt rsp [status]",
		      cmd_tg_send_inform_batt_status_of_ct_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_get_element_attrs_rsp, NULL, "send get element attrs response<large: 1>",
		      cmd_send_get_element_attrs_rsp, 2, 0),
	BT_SHELL_CMD_ARG(send_absolute_volume_rsp, NULL, "send absolute volume rsp <volume>",
		      cmd_tg_send_absolute_volume_rsp, 2, 0),
	BT_SHELL_CMD_ARG(send_get_play_status_rsp, NULL, "send get play status [status]",
		      cmd_tg_send_get_play_status_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_set_addressed_player_rsp, NULL, "send set addressed player rsp [status]",
		      cmd_tg_send_set_addressed_player_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_play_item_rsp, NULL, "send play item rsp [status]",
		      cmd_tg_send_play_item_rsp, 1, 1),
	BT_SHELL_CMD_ARG(send_add_to_now_playing_rsp, NULL, "send add to now playing rsp [status]",
