...
#define TRACK_POS_WORK_DELAY    OS_MSEC(TRACK_POS_WORK_DELAY_MS)
...
static int setup_icon_object(void)
{
	...
	bt_buf_simple_reset(obj.content);
	...
	for (index = 0, k = 0;
	     ...
	     index++, k++) {
		bt_buf_simple_add_u8(obj.content, k);
	}
...
static uint32_t setup_segments_object(struct mpl_track *track)
{
	...
	if (seg) {
		...
		while (seg) {
			...
			bt_buf_simple_add_u8(obj.content, seg->name_len);
			bt_buf_simple_add_mem(obj.content, seg->name,
					       seg->name_len);
			bt_buf_simple_add_le32(obj.content, seg->pos);
...
static uint32_t setup_parent_group_object(struct mpl_group *group)
{
	...
	if (group) {
		...
		while (group && (next_size <= obj.content->size)) {
			bt_buf_simple_add_u8(obj.content, type);
			bt_buf_simple_add_le48(obj.content, group->id);
			group = group->next;
...
static uint32_t setup_group_object(struct mpl_group *group)
{
	...
	if (track) {
		...
		while (track && (next_size <= obj.content->size)) {
			bt_buf_simple_add_u8(obj.content, type);
			bt_buf_simple_add_le48(obj.content, track->id);
			track = track->next;
			next_size += record_size;
		}
		if (next_size > obj.content->size) {
...
static void mpl_set_state(uint8_t state)
{
	...
	case MEDIA_PROXY_STATE_PAUSED:
		(void)bt_work_cancel_delayable(&media_player.pos_work);
		break;
...
static void parse_search(const struct mpl_search *search)
{
	...
	if (search->len > SEARCH_LEN_MAX) {
		...
		/* Copy so that we can parse it using the bt_buf_simple when search is const */
		memcpy(search_ltv, search->search, search->len);

		bt_buf_simple_init_with_data(&buf, search_ltv, search->len);
...
static void pos_work_cb(struct bt_work *work)
{
...
int media_proxy_pl_init(void)
{
	...
	bt_buf_simple_init(obj.content, 0);
	...
	bt_work_init_delayable(&media_player.pos_work, pos_work_cb);
