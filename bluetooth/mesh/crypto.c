...
int bt_mesh_net_encrypt(const struct bt_mesh_key *key, struct bt_buf_simple *buf,
			...
			struct bt_buf_simple *buf)
{
	...
	if (!err) {
		bt_buf_simple_add(buf, APP_MIC_LEN(ctx->aszmic));
...
int bt_mesh_app_decrypt(const struct bt_mesh_key *key, const struct bt_mesh_app_crypto_ctx *ctx,
			struct bt_buf_simple *buf, struct bt_buf_simple *out)
{
	uint8_t nonce[13];
	int err;
...
bool bt_mesh_fcs_check(struct bt_buf_simple *buf, uint8_t received_fcs)
{
