...
{
	if (bt_slist_find(&cb_slist, &cb->node, NULL)) {
		return -EEXIST;
	}

	bt_slist_append(&cb_slist, &cb->node);
...
{
	bt_slist_find_and_remove(&cb_slist, &cb->node);
}
			   ...
			   const void *payload, size_t payload_len)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&cb_slist, cb, node) {
		if (cb->net_recv) {
