...
#ifndef __INCLUDE_BLUETOOTH_BUF_H__
#define __INCLUDE_BLUETOOTH_BUF_H__
 ...
 *                 special values OS_TIMEOUT_NO_WAIT and OS_TIMEOUT_FOREVER.
 *  @return A new buffer.
 */
 ...
 *  @return A new buffer.
 */
struct bt_buf *bt_buf_get_tx(enum bt_buf_type type, os_timeout_t timeout,
 ...
 *                      the special values OS_TIMEOUT_NO_WAIT and OS_TIMEOUT_FOREVER.
 *  @return A new buffer.
 */
struct bt_buf *bt_buf_get_evt(uint8_t evt, bool discardable, os_timeout_t timeout);
 ...
 */
static inline void __deprecated bt_buf_set_type(struct bt_buf *buf, enum bt_buf_type type)
{
	__ASSERT_NO_MSG(bt_buf_headroom(buf) >= 1);
	bt_buf_push_u8(buf, bt_buf_type_to_h4(type));
}
 ...
 */
static inline enum bt_buf_type __deprecated bt_buf_get_type(struct bt_buf *buf)
{
	 ...
	 */
	return bt_buf_type_from_h4(bt_buf_pull_u8(buf), BT_BUF_OUT);
}
...
#endif /* __INCLUDE_BLUETOOTH_BUF_H__ */
