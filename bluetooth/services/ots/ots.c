...
static void oacp_indicate_work_handler(struct bt_work *work)
{
...
static void olcp_indicate_work_handler(struct bt_work *work)
{
	struct bt_gatt_ots_indicate *ind = CONTAINER_OF(work, struct bt_gatt_ots_indicate, work);
		     ...
		     struct bt_ots_init_param *ots_init)
{
	...
	__ASSERT_MSG(ots_init->cb->obj_created || !BT_OTS_OACP_GET_FEAT_CREATE(ots_init->features.oacp),
		 "Callback for object creation is not set and object creation is enabled");
	__ASSERT_MSG(ots_init->cb->obj_deleted || !BT_OTS_OACP_GET_FEAT_DELETE(ots_init->features.oacp),
		 "Callback for object deletion is not set and object deletion is enabled");
#if defined(CONFIG_BT_OTS_OACP_CHECKSUM_SUPPORT)
	__ASSERT_MSG(ots_init->cb->obj_cal_checksum ||
...
#endif
	__ASSERT_MSG(ots_init->cb->obj_read || !BT_OTS_OACP_GET_FEAT_READ(ots_init->features.oacp),
		 "Callback for object reading is not set and object read is enabled");
	__ASSERT_MSG(ots_init->cb->obj_write || !BT_OTS_OACP_GET_FEAT_WRITE(ots_init->features.oacp),
	...
	__ASSERT_MSG(!BT_OTS_OACP_GET_FEAT_CREATE(ots_init->features.oacp) ||
	...
	bt_work_init(&ots->oacp_ind.work, oacp_indicate_work_handler);
	bt_work_init(&ots->olcp_ind.work, olcp_indicate_work_handler);
...
static void ots_conn_disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	for (instance = BT_GATT_OTS_INSTANCE_LIST_START, index = 0;
		...
		if (instance->cur_obj != NULL) {
			__ASSERT_MSG(instance->cur_obj->state.type == BT_GATT_OTS_OBJECT_IDLE_STATE,
