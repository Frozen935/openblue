				 ...
				 enum bt_mesh_dfd_status status)
{
	...
	bt_buf_simple_add_le16(&buf, srv->target_cnt);
				...
				struct bt_buf_simple *buf)
{
	...
	while (buf->len >= 3 && status == BT_MESH_DFD_SUCCESS) {
		...
		img_idx = bt_buf_simple_pull_u8(buf);
				...
				struct bt_buf_simple *buf)
{
	...
	first = bt_buf_simple_pull_le16(buf);
	cnt = bt_buf_simple_pull_le16(buf);
	if (cnt == 0) {
		return -EINVAL;
	}

	/* Create a buffer that can fit the full target list, maxing out at TX_SDU_MAX: */
	BT_BUF_SIMPLE_DEFINE(
	...
	bt_buf_simple_add_le16(&rsp, srv->target_cnt);
	bt_buf_simple_add_le16(&rsp, first);

	cnt = MIN(cnt, srv->target_cnt - first);
	progress = bt_mesh_dfu_cli_progress(&srv->dfu) / 2;

	for (i = 0; i < cnt && bt_buf_simple_tailroom(&rsp) >= 5 + BT_MESH_MIC_SHORT; i++) {
		const struct bt_mesh_dfu_target *t = &srv->targets[i + first];

		bt_buf_simple_add_le32(
		...
		bt_buf_simple_add_u8(&rsp, t->img_idx);
	}
				   ...
				   struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_le16(&rsp, CONFIG_BT_MESH_DFD_SRV_TARGETS_MAX);
	bt_buf_simple_add_le16(&rsp, CONFIG_BT_MESH_DFU_SLOT_CNT);
	bt_buf_simple_add_le32(&rsp, CONFIG_BT_MESH_DFD_SRV_SLOT_MAX_SIZE);
	bt_buf_simple_add_le32(&rsp, CONFIG_BT_MESH_DFD_SRV_SLOT_SPACE);

	/* Remaining size */
	(void)bt_mesh_dfu_slot_foreach(slot_space_cb, &size);
	size = MIN(size, CONFIG_BT_MESH_DFD_SRV_SLOT_SPACE);

	bt_buf_simple_add_le32(&rsp, CONFIG_BT_MESH_DFD_SRV_SLOT_SPACE - size);

#ifdef CONFIG_BT_MESH_DFD_SRV_OOB_UPLOAD
	struct bt_mesh_dfd_srv *srv = mod->rt->user_data;

	if (srv->oob_schemes.count > 0) {
		bt_buf_simple_add_u8(&rsp, 1);
		bt_buf_simple_add_mem(&rsp, srv->oob_schemes.schemes,
				       srv->oob_schemes.count);
	} else
#endif
	{
		bt_buf_simple_add_u8(&rsp, 0);
	}
		       ...
		       enum bt_mesh_dfd_status status)
{
	...
	bt_buf_simple_add_u8(&rsp, srv->phase);
	...
	bt_buf_simple_add_le16(&rsp, srv->inputs.group);
	bt_buf_simple_add_le16(&rsp, srv->inputs.app_idx);
	bt_buf_simple_add_u8(&rsp, srv->inputs.ttl);
	bt_buf_simple_add_le16(&rsp, srv->inputs.timeout_base);
	bt_buf_simple_add_u8(&rsp, ((srv->dfu.xfer.blob.mode & BIT_MASK(2)) |
				     ((srv->apply & BIT_MASK(1)) << 2)));
	bt_buf_simple_add_le16(&rsp, srv->slot_idx);
			...
			struct bt_buf_simple *buf)
{
	...
	params.app_idx = bt_buf_simple_pull_le16(buf);
	params.ttl = bt_buf_simple_pull_u8(buf);
	params.timeout_base = bt_buf_simple_pull_le16(buf);
	byte = bt_buf_simple_pull_u8(buf);
	params.xfer_mode = byte & BIT_MASK(2);
	params.apply = (byte >> 2U) & BIT_MASK(1);
	params.slot_idx = bt_buf_simple_pull_le16(buf);
	...
	params.group = bt_buf_simple_pull_le16(buf);
					    ...
					    uint8_t progress)
{
	...
	bt_buf_simple_add_u8(&rsp, status);
	bt_buf_simple_add_u8(&rsp, srv->upload.phase);
	...
	if (srv->upload.is_oob) {
		bt_buf_simple_add_u8(&rsp, progress | BIT(7));
		bt_buf_simple_add_mem(&rsp, srv->upload.oob.current_fwid,
				       srv->upload.oob.current_fwid_len);
	} else
#endif
	{
		bt_buf_simple_add_u8(&rsp, progress);
		bt_buf_simple_add_mem(&rsp, srv->upload.slot->fwid,
			       ...
			       struct bt_buf_simple *buf)
{
	...
	ttl = bt_buf_simple_pull_u8(buf);
	timeout_base = bt_buf_simple_pull_le16(buf);
	blob_id = bt_buf_simple_pull_le64(buf);
	size = bt_buf_simple_pull_le32(buf);
	meta_len = bt_buf_simple_pull_u8(buf);
	if (buf->len < meta_len) {
		return -EINVAL;
	}

	meta = bt_buf_simple_pull_mem(buf, meta_len);
	fwid_len = buf->len;
	fwid = bt_buf_simple_pull_mem(buf, fwid_len);
				   ...
				   struct bt_buf_simple *buf)
{
	...
	uri_len = bt_buf_simple_pull_u8(buf);

	if (uri_len > buf->len) {
		return -EINVAL;
	}

	uri = bt_buf_simple_pull_mem(buf, uri_len);
	fwid_len = buf->len;
	fwid = bt_buf_simple_pull_mem(buf, fwid_len);
			  ...
			  const uint8_t *fwid, size_t fwid_len)
{
	...
	bt_buf_simple_add_u8(&rsp, status);
	bt_buf_simple_add_le16(&rsp, bt_mesh_dfu_slot_count());

	bt_buf_simple_add_le16(&rsp, idx);
	if (fwid) {
		bt_buf_simple_add_mem(&rsp, fwid, fwid_len);
	}
...
static void dfd_srv_reset(const struct bt_mesh_model *mod)
{
	...
	bt_slist_init(&srv->inputs.targets);
	srv->target_cnt = 0;
					      ...
					      struct bt_mesh_dfd_start_params *params)
{
	...
	for (i = 0; i < srv->target_cnt; i++) {
		...
		bt_slist_append(&srv->inputs.targets, &srv->targets[i].blob.n);
	}
