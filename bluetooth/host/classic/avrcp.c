...
static void process_get_cap_rsp(struct bt_avrcp *avrcp, uint8_t tid, struct bt_buf *buf)
{
					       ...
					       struct bt_buf *buf)
{
	...
	company_id = bt_buf_pull_be24(buf);
...
error_rsp:
	rsp_buf = bt_avrcp_create_pdu(NULL);
	__ASSERT_MSG(rsp_buf != NULL, "Failed to allocate response buffer");
	...
	bt_buf_add_u8(rsp_buf, BT_AVRCP_STATUS_INTERNAL_ERROR);
			       ...
			       uint8_t tid)
{
	...
	brow = bt_buf_pull_mem(buf, sizeof(struct bt_avrcp_avc_brow_pdu));
...
int bt_avrcp_ct_get_cap(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t cap_id)
{
	...
	bt_buf_add_be24(buf, BT_AVRCP_COMPANY_ID_BLUETOOTH_SIG);
...
int bt_avrcp_ct_set_browsed_player(struct bt_avrcp_ct *ct, uint8_t tid, uint16_t player_id)
{
	...
	if (bt_buf_tailroom(buf) < sizeof(*pdu) + sizeof(player_id)) {
					    ...
					    struct bt_buf *buf)
{
	...
	if (bt_buf_headroom(buf) < sizeof(struct bt_avrcp_avc_brow_pdu)) {
		LOG_ERR("Not enough headroom in buffer for bt_avrcp_avc_brow_pdu");
		return -ENOMEM;
	}
	hdr = bt_buf_push(buf, sizeof(struct bt_avrcp_avc_brow_pdu));
