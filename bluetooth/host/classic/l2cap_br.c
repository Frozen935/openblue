...
static void l2cap_br_chan_destroy(struct bt_l2cap_chan *chan)
{
	 ...
	 */
	struct bt_work_q *rtx_work_queue = br_chan->rtx_work.queue;

	if (rtx_work_queue == NULL || os_thread_self() != bt_work_queue_thread_get(rtx_work_queue)) {
...
#if defined(CONFIG_BT_L2CAP_RET_FC)
static uint16_t get_pdu_len(struct bt_l2cap_br_chan *br_chan, struct bt_buf *buf)
{
...
static bool l2cap_br_send_i_frame(struct bt_l2cap_br_chan *br_chan, struct bt_buf *sdu)
{
	...
	if (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAMES)) {
		return true;
	}
...
static struct bt_l2cap_br_window *l2cap_br_get_ret_win(struct bt_l2cap_br_chan *br_chan)
{
	...
	bt_atomic_clear_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAMES);
	return NULL;
}
						 ...
						 size_t *length)
{
	...
	if (br_chan->_pdu_remaining == 0) {
		...
		if (bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAME)) {
			tx_win = (void *)bt_slist_peek_head(&br_chan->_pdu_outstanding);
			if (tx_win != NULL) {
				first = false;
				goto send_i_frame;
...
static void l2cap_br_retransmit_i_frames(struct bt_l2cap_br_chan *br_chan)
{
	...
	if (bt_atomic_test_and_set_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAMES)) {
...
static void bt_l2cap_br_update_r(struct bt_l2cap_br_chan *br_chan, uint8_t r)
{
	...
	if (!r) {
		if (!bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_REMOTE_BUSY)) {
		...
		if (bt_atomic_test_and_set_bit(br_chan->flags, L2CAP_FLAG_REMOTE_BUSY)) {
			/* R bit without any change */
			return;
		}
...
static void l2cap_br_stop_retransmit_i_frames(struct bt_l2cap_br_chan *br_chan)
{
	...
	if (!bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAMES)) {
...
static void bt_l2cap_br_ret_fc_s_recv(struct bt_l2cap_br_chan *br_chan, struct bt_buf *buf)
{
	...
	case BT_L2CAP_CONTROL_S_RR:
		...
		remote_busy = bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_REMOTE_BUSY);
	...
	case BT_L2CAP_CONTROL_S_RNR:
		...
		if ((bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ) &&
		     !bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY)) || (p != 0)) {
...
valid_frame:
	...
	if (br_chan->chan.ops->alloc_buf) {
		...
		if (!bt_atomic_test_and_set_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY)) {
...
static void bt_l2cap_br_ret_fc_recv(struct bt_l2cap_br_chan *br_chan, struct bt_buf *buf)
{
	...
	if (sar == BT_L2CAP_CONTROL_SAR_START) {
		...
		sdu_length = bt_buf_pull_le16(buf);
...
int bt_l2cap_br_chan_recv_complete(struct bt_l2cap_chan *chan)
{
	...
	if (!bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY)) {
	...
	/* ERET Mode */
	if (bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY_CHANGED)) {
		return 0;
	}
