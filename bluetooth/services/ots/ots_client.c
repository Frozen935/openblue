...
BT_BUF_SIMPLE_DEFINE_STATIC(otc_tx_buf, OTS_CLIENT_MAX_WRITE_SIZE);
...
static ssize_t rx_done(struct bt_gatt_ots_l2cap *l2cap_ctx,
		       struct bt_conn *conn, struct bt_buf *buf)
{
...
static void date_time_decode(struct bt_buf_simple *buf,
			     struct bt_ots_date_time *p_date_time)
{
	p_date_time->year = bt_buf_simple_pull_le16(buf);
	p_date_time->month = bt_buf_simple_pull_u8(buf);
	p_date_time->day = bt_buf_simple_pull_u8(buf);
	p_date_time->hours = bt_buf_simple_pull_u8(buf);
	p_date_time->minutes = bt_buf_simple_pull_u8(buf);
	p_date_time->seconds = bt_buf_simple_pull_u8(buf);
}
			     ...
			     const void *data, uint16_t length)
{
	...
	bt_buf_simple_init_with_data(&bt_buf, (void *)data, length);

	op_code = bt_buf_simple_pull_u8(&bt_buf);

	LOG_DBG("OLCP indication");

	if (op_code == BT_GATT_OTS_OLCP_PROC_RESP) {
		if (bt_buf.len < (sizeof(uint8_t) + sizeof(uint8_t))) {
		...
		case BT_GATT_OTS_OLCP_PROC_REQ_NUM_OBJS:
			...
			if (bt_buf.len == sizeof(uint32_t)) {
				uint32_t obj_cnt =
					bt_buf_simple_pull_le32(&bt_buf);
			     ...
			     const void *data, uint16_t length)
{
	...
	if (op_code == BT_GATT_OTS_OACP_PROC_RESP) {
		if (bt_buf.len >= (sizeof(req_opcode) + sizeof(result_code))) {
			req_opcode = bt_buf_simple_pull_u8(&bt_buf);
			result_code = bt_buf_simple_pull_u8(&bt_buf);
		} else {
			LOG_ERR("Invalid indication data len %u", bt_buf.len);
			return;
		}

		if (req_opcode == BT_GATT_OTS_OACP_PROC_CHECKSUM_CALC) {
			if (bt_buf.len == sizeof(checksum)) {
				checksum = bt_buf_simple_pull_le32(&bt_buf);
		      ...
		      uint8_t *params, uint8_t param_len)
{
	...
	bt_buf_simple_reset(&otc_tx_buf);

	bt_buf_simple_add_u8(&otc_tx_buf, opcode);

	if (param_len && params) {
		bt_buf_simple_add_mem(&otc_tx_buf, params, param_len);
	}
			      ...
			      const void *data, uint16_t length)
{
	...
	if (err) {
		...
		if (length == BT_OTS_OBJ_ID_SIZE) {
			uint64_t obj_id = bt_buf_simple_pull_le48(&bt_buf);
				      ...
				      const void *data, uint16_t length)
{
	...
	if (err) {
		...
		cur_object->props = bt_buf_simple_pull_le32(&bt_buf);
		     ...
		     struct bt_otc_internal_instance_t *inst)
{
	...
	/* OP Code */
	bt_buf_simple_add_u8(&otc_tx_buf, BT_GATT_OTS_OACP_PROC_READ);

	/* Offset */
	bt_buf_simple_add_le32(&otc_tx_buf, offset);

	/* Len */
	length = inst->otc_inst->cur_object.size.cur - offset;
	bt_buf_simple_add_le32(&otc_tx_buf, length);
		      ...
		      enum bt_ots_oacp_write_op_mode mode)
{
	...
	/* OP Code */
	bt_buf_simple_add_u8(&otc_tx_buf, BT_GATT_OTS_OACP_PROC_WRITE);

	/* Offset */
	bt_buf_simple_add_le32(&otc_tx_buf, offset);

	/* Len */
	bt_buf_simple_add_le32(&otc_tx_buf, len);

	/* Mode, truncate or not */
	bt_buf_simple_add_u8(&otc_tx_buf, mode);
			 ...
			 uint32_t offset, uint32_t len)
{
	...
	/* OP Code */
	bt_buf_simple_add_u8(&otc_tx_buf, BT_GATT_OTS_OACP_PROC_CHECKSUM_CALC);
...
static int decode_record(struct bt_buf_simple *buf,
			 struct dirlisting_record_t *rec)
{
	uint16_t start_len = buf->len;

	rec->len = bt_buf_simple_pull_le16(buf);
	...
	rec->metadata.id = bt_buf_simple_pull_le48(buf);
	...
	rec->name_len = bt_buf_simple_pull_u8(buf);
	...
	if (rec->name_len > 0) {
		...
		name = bt_buf_simple_pull_mem(buf, rec->name_len);
	...
	rec->flags = bt_buf_simple_pull_u8(buf);
	...
	if (BT_OTS_DIR_LIST_GET_FLAG_TYPE_128(rec->flags)) {
		...
		uuid = bt_buf_simple_pull_mem(buf, BT_UUID_SIZE_128);
	...
	if (BT_OTS_DIR_LIST_GET_FLAG_CUR_SIZE(rec->flags)) {
		...
		rec->metadata.size.cur = bt_buf_simple_pull_le32(buf);
	}
	...
	if (BT_OTS_DIR_LIST_GET_FLAG_ALLOC_SIZE(rec->flags)) {
		...
		rec->metadata.size.alloc = bt_buf_simple_pull_le32(buf);
	}
	...
	if (BT_OTS_DIR_LIST_GET_FLAG_PROPERTIES(rec->flags)) {
		...
		rec->metadata.props = bt_buf_simple_pull_le32(buf);
	}
				    ...
				    bt_ots_client_dirlisting_cb cb)
{
	...
	while (bt_buf.len) {
		...
		if (bt_buf.len < sizeof(uint16_t)) {
			LOG_WRN("incorrect DirListing record, len %u too short", bt_buf.len);
			return -EINVAL;
		}

		LOG_DBG("Decoding record %u", count);
		ret = decode_record(&bt_buf, &record);
