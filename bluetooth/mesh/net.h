...
#define BT_MESH_IVU_TIMEOUT        OS_HOURS(BT_MESH_IVU_HOURS)
...
struct bt_mesh_friend {
	...
	struct {
		...
		struct bt_work_delayable timer;   /* Repeat timer */
	} clear;
};
...
struct bt_mesh_net {
	...
	struct bt_work local_work;
	bt_slist_t local_queue;
	...
	struct bt_work_delayable ivu_timer;
