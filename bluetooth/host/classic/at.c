...
static void pull_chars(struct bt_buf_simple *buf, pull_char_t cb, void *data)
{
	while (buf->len > 0) {
		if (!cb(buf->data, data)) {
			break;
		}
		bt_buf_simple_pull_u8(buf);
	}
}
...
int at_check_byte(struct bt_buf *buf, char check_byte)
{
...
static int at_parse_result(struct at_client *at, struct bt_buf *buf,
...
static void init_rsp_buffer(struct at_client *at, struct bt_buf *buf)
{
...
static int get_cmd_value(struct at_client *at, struct bt_buf *buf,
			 char stop_byte, enum at_cmd_state cmd_state)
{
	...
	while (buf->len > 0) {
		...
		bt_buf_simple_add(&at->rsp_buf, sizeof(uint8_t));
		bt_buf_pull_u8(buf);
	}
...
static int get_response_string(struct at_client *at, struct bt_buf *buf, char *stop_string,
			       enum at_state state)
{
	if (at->rsp_buf.len == 0) {
		init_rsp_buffer(at, buf);
...
{
	bt_buf_simple_init_with_data(&at->rsp_buf, NULL, 0);
}

static int at_state_start(struct at_client *at, struct bt_buf *buf)
{
	int err;
...
static int at_state_start_cr(struct at_client *at, struct bt_buf *buf)
{
	int err;
...
static int at_state_start_lf(struct at_client *at, struct bt_buf *buf)
{
	reset_buffer(at);
...
static int at_state_get_cmd_string(struct at_client *at, struct bt_buf *buf)
{
...
static int at_state_process_cmd(struct at_client *at, struct bt_buf *buf)
{
	if (is_cmer(at)) {
...
static int at_state_get_result_string(struct at_client *at, struct bt_buf *buf)
{
...
static int at_state_process_result(struct at_client *at, struct bt_buf *buf)
{
	enum at_cme cme_err;
	enum at_result result;
...
static int at_state_process_ag_nw_err(struct at_client *at, struct bt_buf *buf)
{
...
static int at_state_unsolicited_cmd(struct at_client *at, struct bt_buf *buf)
{
	if (at->unsolicited) {
...
int at_parse_input(struct at_client *at, struct bt_buf *buf)
{
	int ret;
...
static int at_cmd_start(struct at_client *at, struct bt_buf *buf,
...
static int at_cmd_get_value(struct at_client *at, struct bt_buf *buf,
			    const char *prefix, parse_val_t func,
			    enum at_cmd_type type)
{
...
static int at_cmd_process_value(struct at_client *at, struct bt_buf *buf,
				const char *prefix, parse_val_t func,
				enum at_cmd_type type)
{
	int ret;
...
static int at_cmd_state_end_lf(struct at_client *at, struct bt_buf *buf,
			       const char *prefix, parse_val_t func,
			       enum at_cmd_type type)
{
	int err;
...
int at_parse_cmd_input(struct at_client *at, struct bt_buf *buf,
		       const char *prefix, parse_val_t func,
		       enum at_cmd_type type)
{
	int ret;
