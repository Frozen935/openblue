...
static void bip_rfcomm_connected(struct bt_conn *conn, struct bt_goep *goep)
{
	...
	bt_atomic_set(&bip->_transport_state, BT_BIP_TRANSPORT_STATE_CONNECTED);
...
static void bip_rfcomm_disconnected(struct bt_goep *goep)
{
	...
	bt_atomic_set(&bip->_transport_state, BT_BIP_TRANSPORT_STATE_DISCONNECTED);

	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&bip->_clients, client, cnext, _node) {
		...
		bt_atomic_set(&client->_state, BT_BIP_STATE_DISCONNECTED);
	}

	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&bip->_servers, server, snext, _node) {
		...
		bt_atomic_set(&server->_state, BT_BIP_STATE_DISCONNECTED);
	}

	bt_slist_init(&bip->_clients);
			     ...
			     struct bt_goep **goep)
{
	...
	*goep = &bip->goep;
	...
	bt_atomic_set(&bip->_transport_state, BT_BIP_TRANSPORT_STATE_CONNECTING);
...
int bt_bip_rfcomm_connect(struct bt_conn *conn, struct bt_bip *bip, uint8_t channel)
{
	...
	if (bt_atomic_get(&bip->_transport_state) != BT_BIP_TRANSPORT_STATE_DISCONNECTED) {
		return -EINPROGRESS;
	}
...
int bt_bip_rfcomm_disconnect(struct bt_bip *bip)
{
	...
	if (bt_atomic_get(&bip->_transport_state) != BT_BIP_TRANSPORT_STATE_CONNECTED) {
		return -EINPROGRESS;
	}

	bt_atomic_set(&bip->_transport_state, BT_BIP_TRANSPORT_STATE_DISCONNECTING);
			       ...
			       uint16_t mopl, struct bt_buf *buf)
{
	...
	if (rsp_code == BT_OBEX_RSP_CODE_SUCCESS) {
		bt_atomic_set(&c->_state, BT_BIP_STATE_CONNECTED);
		bt_obex_get_header_conn_id(buf, &c->_conn_id);
	} else {
		bt_atomic_set(&c->_state, BT_BIP_STATE_DISCONNECTED);
		bt_slist_find_and_remove(&c->_bip->_clients, &c->_node);
	}
...
static void bip_client_put(struct bt_obex_client *client, uint8_t rsp_code, struct bt_buf *buf)
{
	struct bt_bip_client *c = CONTAINER_OF(client, struct bt_bip_client, _client);
...
static void bip_client_get(struct bt_obex_client *client, uint8_t rsp_code, struct bt_buf *buf)
{
	struct bt_bip_client *c = CONTAINER_OF(client, struct bt_bip_client, _client);
...
static void bip_client_abort(struct bt_obex_client *client, uint8_t rsp_code, struct bt_buf *buf)
{
	struct bt_bip_client *c = CONTAINER_OF(client, struct bt_bip_client, _client);
	int err;
			       ...
			       struct bt_buf *buf)
{
	...
	if (!is_bip_primary_connect(s->_type) && s->_primary_client != NULL &&
	    bt_atomic_get(&s->_primary_client->_state) != BT_BIP_STATE_CONNECTED) {
		LOG_ERR("Primary conn needs to be connected firstly");
		bt_bip_connect_rsp(s, BT_OBEX_RSP_CODE_NOT_ALLOW, NULL);
		return;
	}

	bt_atomic_set(&s->_state, BT_BIP_STATE_CONNECTING);
...
static void bip_server_disconnect(struct bt_obex_server *server, struct bt_buf *buf)
{
	struct bt_bip_server *s = CONTAINER_OF(server, struct bt_bip_server, _server);
	...
	bt_atomic_set(&s->_state, BT_BIP_STATE_DISCONNECTING);
...
static bool has_required_hdrs(struct bt_buf *buf, const struct bip_required_hdr *hdr)
{
...
static void bip_server_put(struct bt_obex_server *server, bool final, struct bt_buf *buf)
{
	struct bt_bip_server *s = CONTAINER_OF(server, struct bt_bip_server, _server);
	int err;
...
static void bip_server_get(struct bt_obex_server *server, bool final, struct bt_buf *buf)
{
	struct bt_bip_server *s = CONTAINER_OF(server, struct bt_bip_server, _server);
	int err;
	enum bt_obex_rsp_code rsp_code;
...
static void bip_server_abort(struct bt_obex_server *server, struct bt_buf *buf)
{
	struct bt_bip_server *s = CONTAINER_OF(server, struct bt_bip_server, _server);
	int err;
				  ...
				  struct bt_bip_server_cb *cb, struct bt_bip_client *primary_client)
{
	...
	if (bt_atomic_get(&server->_state) != BT_BIP_STATE_DISCONNECTED) {
		LOG_ERR("Invalid state %u", (uint8_t)bt_atomic_get(&server->_state));
		return -EINVAL;
	}
	...
	if (is_bip_primary_connect(type)) {
		...
		if (!bt_slist_find(&primary_bip->_clients, &primary_client->_node, NULL)) {
			LOG_ERR("Primary client %p is not found", primary_client);
			return -EINVAL;
		}
	}

	if (bt_slist_find(&bip->_servers, &server->_node, NULL)) {
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&bip->_servers, s, next, _node) {
	...
	bt_slist_append(&bip->_servers, &server->_node);
	return 0;
}
...
int bt_bip_server_unregister(struct bt_bip_server *server)
{
	...
	if (!bt_slist_find(&server->_bip->_servers, &server->_node, NULL)) {
	...
	bt_slist_find_and_remove(&server->_bip->_servers, &server->_node);
	return 0;
}

static int bip_check_conn_id(uint32_t id, struct bt_buf *buf)
{
	uint32_t conn_id;
	int err;
...
static int bip_check_who(const struct bt_uuid *uuid, struct bt_buf *buf)
{
	uint16_t len;
	const uint8_t *who;
...
static int bt_bip_client_connect(struct bt_bip *bip, struct bt_bip_client *client,
				 ...
				 struct bt_buf *buf, struct bt_bip_server *primary_server)
{
	...
	if (bt_atomic_get(&client->_state) != BT_BIP_STATE_DISCONNECTED) {
		LOG_ERR("Invalid state %u", (uint8_t)bt_atomic_get(&client->_state));
		return -EINVAL;
	}
	...
	if (is_bip_primary_connect(type)) {
		...
		if (bt_atomic_get(&primary_server->_state) != BT_BIP_STATE_CONNECTED) {
			LOG_ERR("Invalid primary server state %u",
				(uint8_t)bt_atomic_get(&primary_server->_state));
			return -EINVAL;
		}

		primary_bip = primary_server->_bip;
		if (!bt_slist_find(&primary_bip->_servers, &primary_server->_node, NULL)) {
			LOG_ERR("Primary server %p is not found", primary_server);
			return -EINVAL;
		}
	}

	if (bt_slist_find(&bip->_clients, &client->_node, NULL)) {
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&bip->_clients, c, next, _node) {
	...
	bt_atomic_set(&client->_state, BT_BIP_STATE_CONNECTING);
	bt_slist_append(&bip->_clients, &client->_node);
	return 0;
...
int bt_bip_connect_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	uint8_t val[BT_UUID_SIZE_128];
	int err;
	bool allocated = false;
	...
	if (bt_atomic_get(&server->_state) != BT_BIP_STATE_CONNECTING) {
		LOG_ERR("Invalid state %u", (uint8_t)bt_atomic_get(&server->_state));
		return -EINVAL;
	}
	...
	if (rsp_code == BT_OBEX_RSP_CODE_SUCCESS && !is_bip_primary_connect(server->_type) &&
	    ...
	    bt_atomic_get(&server->_primary_client->_state) != BT_BIP_STATE_CONNECTED) {
		LOG_ERR("Primary conn needs to be connected firstly");
		return -EINVAL;
	}
	...
	bt_atomic_set(&server->_state, BT_BIP_STATE_CONNECTED);
	return 0;
...
int bt_bip_disconnect(struct bt_bip_client *client, struct bt_buf *buf)
{
	int err;
	bool allocated = false;
	...
	if (bt_atomic_get(&client->_state) != BT_BIP_STATE_CONNECTED) {
		LOG_ERR("Invalid state %u", (uint8_t)bt_atomic_get(&client->_state));
		return -EINVAL;
	}

	if (is_bip_primary_connect(client->_type) && client->_secondary_server != NULL &&
	    bt_atomic_get(&client->_secondary_server->_state) == BT_BIP_STATE_CONNECTED) {
	...
	bt_atomic_set(&client->_state, BT_BIP_STATE_DISCONNECTING);
	return 0;
failed:
	if (allocated) {
		bt_buf_unref(buf);
...
int bt_bip_disconnect_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	int err;
	...
	if (bt_atomic_get(&server->_state) != BT_BIP_STATE_DISCONNECTING) {
		LOG_ERR("Invalid state %u", (uint8_t)bt_atomic_get(&server->_state));
		return -EINVAL;
	}
	...
	if (rsp_code == BT_OBEX_RSP_CODE_SUCCESS && is_bip_primary_connect(server->_type) &&
	    ...
	    bt_atomic_get(&server->_secondary_client->_state) == BT_BIP_STATE_CONNECTED) {
		LOG_ERR("Secondary conn needs to be disconnected firstly");
		return -EINVAL;
	}
...
int bt_bip_abort(struct bt_bip_client *client, struct bt_buf *buf)
{
	int err;
	bool allocated = false;
...
int bt_bip_abort_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	int err;
	...
	if (bt_atomic_get(&server->_state) != BT_BIP_STATE_CONNECTED) {
		LOG_ERR("Invalid state %u", (uint8_t)bt_atomic_get(&server->_state));
		return -EINVAL;
	}
...
static int bip_client_get_req_cb(struct bt_bip_client *client, const char *type,
				 struct bt_buf *buf, bool is_get, bt_bip_client_cb_t *cb,
				 const char **req_type)
{
...
static int bip_get(struct bt_bip_client *client, const char *type, bool final, struct bt_buf *buf)
{
	return bip_get_or_put(client, true, type, final, buf);
}

static int bip_put(struct bt_bip_client *client, const char *type, bool final, struct bt_buf *buf)
{
	return bip_get_or_put(client, false, type, final, buf);
}

int bt_bip_get_capabilities(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
...
int bt_bip_get_capabilities_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	return bip_get_or_put_rsp(server, true, BT_BIP_HDR_TYPE_GET_CAPS, rsp_code, buf);
}

int bt_bip_get_image_list(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
	return bip_get(client, BT_BIP_HDR_TYPE_GET_IMAGE_LIST, final, buf);
}

int bt_bip_get_image_list_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	return bip_get_or_put_rsp(server, true, BT_BIP_HDR_TYPE_GET_IMAGE_LIST, rsp_code, buf);
}

int bt_bip_get_image_properties(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
...
int bt_bip_get_image(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
	return bip_get(client, BT_BIP_HDR_TYPE_GET_IMAGE, final, buf);
}

int bt_bip_get_image_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	return bip_get_or_put_rsp(server, true, BT_BIP_HDR_TYPE_GET_IMAGE, rsp_code, buf);
}

int bt_bip_get_linked_thumbnail(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
...
int bt_bip_get_linked_attachment(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
...
int bt_bip_get_partial_image(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
...
int bt_bip_get_monitoring_image(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
...
int bt_bip_get_status(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
	return bip_get(client, BT_BIP_HDR_TYPE_GET_STATUS, final, buf);
}

int bt_bip_get_status_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	return bip_get_or_put_rsp(server, true, BT_BIP_HDR_TYPE_GET_STATUS, rsp_code, buf);
}

int bt_bip_put_image(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
	return bip_put(client, BT_BIP_HDR_TYPE_PUT_IMAGE, final, buf);
}

int bt_bip_put_image_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	return bip_get_or_put_rsp(server, false, BT_BIP_HDR_TYPE_PUT_IMAGE, rsp_code, buf);
}

int bt_bip_put_linked_thumbnail(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
...
int bt_bip_put_linked_attachment(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
...
int bt_bip_remote_display(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
	return bip_put(client, BT_BIP_HDR_TYPE_REMOTE_DISPLAY, final, buf);
}

int bt_bip_remote_display_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	return bip_get_or_put_rsp(server, false, BT_BIP_HDR_TYPE_REMOTE_DISPLAY, rsp_code, buf);
}

int bt_bip_delete_image(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
	return bip_put(client, BT_BIP_HDR_TYPE_DELETE_IMAGE, final, buf);
}

int bt_bip_delete_image_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	return bip_get_or_put_rsp(server, false, BT_BIP_HDR_TYPE_DELETE_IMAGE, rsp_code, buf);
}

int bt_bip_start_print(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
	return bip_put(client, BT_BIP_HDR_TYPE_START_PRINT, final, buf);
}

int bt_bip_start_print_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	return bip_get_or_put_rsp(server, false, BT_BIP_HDR_TYPE_START_PRINT, rsp_code, buf);
}

int bt_bip_start_archive(struct bt_bip_client *client, bool final, struct bt_buf *buf)
{
	return bip_put(client, BT_BIP_HDR_TYPE_START_ARCHIVE, final, buf);
}

int bt_bip_start_archive_rsp(struct bt_bip_server *server, uint8_t rsp_code, struct bt_buf *buf)
{
	return bip_get_or_put_rsp(server, false, BT_BIP_HDR_TYPE_START_ARCHIVE, rsp_code, buf);
}

int bt_bip_add_header_image_desc(struct bt_buf *buf, uint16_t len, const uint8_t *desc)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_BIP_HEADER_ID_IMG_DESC);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, desc, len);
	return 0;
}

int bt_bip_add_header_image_handle(struct bt_buf *buf, uint16_t len, const uint8_t *handle)
{
	size_t total;
	...
	bt_buf_add_u8(buf, BT_BIP_HEADER_ID_IMG_HANDLE);
	bt_buf_add_be16(buf, (uint16_t)total);
	bt_buf_add_mem(buf, handle, len);
	return 0;
}
