...
static os_thread_t adv_thread_data;
...
static void adv_thread(void *p1)
{
	...
	while (!is_mesh_suspended()) {
		if (IS_ENABLED(CONFIG_BT_MESH_GATT_SERVER)) {
			adv = bt_mesh_adv_get(OS_TIMEOUT_NO_WAIT);
			...
			adv = bt_mesh_adv_get(OS_TIMEOUT_FOREVER);
		}
	...
	while ((adv = bt_mesh_adv_get(OS_TIMEOUT_NO_WAIT))) {
...
{
	os_thread_create(&adv_thread_data, adv_thread,
			NULL, "BT Mesh adv", OS_PRIORITY(CONFIG_BT_MESH_ADV_PRIO), CONFIG_BT_MESH_ADV_STACK_SIZE);
}
...
int bt_mesh_adv_enable(void)
{
	...
	os_thread_start(&adv_thread_data);
	return 0;
}
...
int bt_mesh_adv_disable(void)
{
	...
	err = os_thread_join(&adv_thread_data, OS_TIMEOUT_FOREVER);
...
int bt_mesh_wq_submit(struct bt_work *work)
{
	return bt_work_submit(work);
}
