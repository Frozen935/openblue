 ...
 */
#ifndef __INCLUDE_BLUETOOTH_L2CAP_H__
#define __INCLUDE_BLUETOOTH_L2CAP_H__
...
struct bt_l2cap_le_chan {
	...
	struct bt_work			rx_work;
	struct bt_fifo			rx_queue;
...
#define BT_L2CAP_FIXED_CHANNEL_DEFINE(_name)                                                       \
	static const struct bt_l2cap_fixed_chan UTIL_CONCAT(bt_l2cap_fixed_chan, _name)
...
struct bt_l2cap_br_chan {
	...
	/** @internal Holds the sending buffer. */
	struct bt_buf                  *_pdu_buf;

	/** @internal TX windows for outstanding frame */
	bt_slist_t                     _pdu_outstanding;

	/** @internal PDU restore state */
	struct bt_buf_simple_state     _pdu_state;

	/** @internal Free TX windows */
	struct bt_fifo                   _free_tx_win;
	...
	struct bt_work_delayable         ret_work;
	/** @internal Monitor Timer */
	struct bt_work_delayable         monitor_work;
#endif /* CONFIG_BT_L2CAP_RET_FC */
};
...
struct bt_l2cap_chan_ops {
	 ...
	 */
	struct bt_buf *(*alloc_seg)(struct bt_l2cap_chan *chan);
	 ...
	 *  @return Allocated buffer.
	 */
	struct bt_buf *(*alloc_buf)(struct bt_l2cap_chan *chan);
	 ...
	 *  calls @ref bt_buf_ref or @ref bt_buf_unref.)
	 *
	 ...
	 *                       used by the packet.
	 */
	int (*recv)(struct bt_l2cap_chan *chan, struct bt_buf *buf);
	...
	void (*seg_recv)(struct bt_l2cap_chan *chan, size_t sdu_len,
			 off_t seg_offset, struct bt_buf_simple *seg);
#endif /* CONFIG_BT_L2CAP_SEG_RECV */
};
...
#endif /* __INCLUDE_BLUETOOTH_L2CAP_H__ */
