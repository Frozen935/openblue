...
static bt_slist_t tbs_client_cbs = BT_SLIST_STATIC_INIT(&tbs_client_cbs);
...
static uint8_t bt_buf_pull_call_state(struct bt_buf_simple *buf,
				       struct bt_tbs_client_call_state *call_state)
{
	...
	call_state->index = bt_buf_simple_pull_u8(buf);
	call_state->state = bt_buf_simple_pull_u8(buf);
	call_state->flags = bt_buf_simple_pull_u8(buf);

	return 0;
}

#if defined(CONFIG_BT_TBS_CLIENT_BEARER_LIST_CURRENT_CALLS)
static uint8_t bt_buf_pull_call(struct bt_buf_simple *buf,
				 struct bt_tbs_client_call *call)
{
	...
	item_len = bt_buf_simple_pull_u8(buf);
	...
	err = bt_buf_pull_call_state(buf, &call->call_info);
	if (err != 0) {
		return err;
	}
				  ...
				  uint8_t call_count, const struct bt_tbs_client_call *calls)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&tbs_client_cbs, listener, next, _node) {
				      ...
				      const void *data, uint16_t length)
{
	...
	while (buf.len) {
		...
		err = bt_buf_pull_call_state(&buf, call_state);
		if (err != 0) {
			LOG_DBG("Invalid current call notification: %d", err);
			return;
		}
...
static void tbs_client_gatt_read_complete(struct bt_tbs_instance *inst)
{
	...
	bt_atomic_clear_bit(inst->flags, BT_TBS_CLIENT_FLAG_BUSY);
}
				...
				bt_gatt_read_func_t func)
{
	...
	if (bt_atomic_test_and_set_bit(inst->flags, BT_TBS_CLIENT_FLAG_BUSY)) {
	...
	initialize_bt_buf_read_buffer(inst);
...
#if defined(CONFIG_BT_TBS_CLIENT_GTBS)
	bt_atomic_clear_bit(srv_inst->gtbs_inst.flags, BT_TBS_CLIENT_FLAG_BUSY);
#endif /* CONFIG_BT_TBS_CLIENT_GTBS */
#if defined(CONFIG_BT_TBS_CLIENT_TBS)
	for (size_t i = 0U; i < ARRAY_SIZE(srv_inst->tbs_insts); i++) {
		bt_atomic_clear_bit(srv_inst->tbs_insts[i].flags, BT_TBS_CLIENT_FLAG_BUSY);
	}
#endif /* CONFIG_BT_TBS_CLIENT_TBS */
...
static bool can_add_string_to_bt_buf(const struct bt_buf_simple *buf, size_t len)
{
				   ...
				   uint16_t offset, uint16_t length, bool truncatable)
{
	...
	if (data != NULL) {
		...
		if (!can_add_string_to_bt_buf(&inst->bt_buf, length)) {
			...
			if (truncatable) {
				...
				length = bt_buf_simple_tailroom(&inst->bt_buf) -
					 sizeof(char) /* NULL terminator size */;
				bt_buf_simple_add_mem(&inst->bt_buf, data, length);

				/* Ensure that the data is correctly truncated */
				utf8_trunc(inst->bt_buf.data);
			} else {
					    ...
					    const void *data, uint16_t length)
{
	...
	if (ret != BT_GATT_ITER_CONTINUE) {
		if (ret == BT_GATT_ITER_STOP) {
			/* At this point the inst->bt_buf.data contains a NULL terminator string */
			provider_name_changed(conn, 0, inst_index, (char *)inst->bt_buf.data);
		} else {
				   ...
				   const void *data, uint16_t length)
{
	...
	if (ret != BT_GATT_ITER_CONTINUE) {
		if (ret == BT_GATT_ITER_STOP) {
			/* At this point the inst->bt_buf.data contains a NULL terminator string */
			bearer_uci_changed(conn, 0, inst_index, (char *)inst->bt_buf.data);
		} else {
				    ...
				    const void *data, uint16_t length)
{
	...
	if (ret != BT_GATT_ITER_CONTINUE) {
		if (ret == BT_GATT_ITER_STOP) {
			/* At this point the inst->bt_buf.data contains a NULL terminator string */
			uri_list_changed(conn, 0, inst_index, (char *)inst->bt_buf.data);
		} else {
				      ...
				      const void *data, uint16_t length)
{
	...
	if ((tbs_err == 0) && (data != NULL) &&
	    (bt_buf_simple_tailroom(&inst->bt_buf) < length)) {
	...
	if (inst->bt_buf.len == 0) {
		current_calls_changed(conn, 0, inst_index, 0, NULL);

		return BT_GATT_ITER_STOP;
	}

	bearer_list_current_calls(conn, inst, &inst->bt_buf);
				...
				const void *data, uint16_t length)
{
	...
	if (ret != BT_GATT_ITER_CONTINUE) {
		if (ret == BT_GATT_ITER_STOP) {
			/* At this point the inst->bt_buf.data contains a NULL terminator string */
			call_uri_changed(conn, 0, inst_index, (char *)inst->bt_buf.data);
		} else {
				  ...
				  const void *data, uint16_t length)
{
	...
	while (inst->bt_buf.len != 0) {
		...
		tbs_err = bt_buf_pull_call_state(&inst->bt_buf, call_state);
		if (tbs_err != 0) {
			LOG_DBG("Invalid current call notification: %d", err);
			break;
		}
				  ...
				  const void *data, uint16_t length)
{
	...
	if (ret != BT_GATT_ITER_CONTINUE) {
		if (ret == BT_GATT_ITER_STOP) {
			/* At this point the inst->bt_buf.data contains a NULL terminator string */
			remote_uri_changed(conn, 0, inst_index, (char *)inst->bt_buf.data);
		} else {
					 ...
					 const void *data, uint16_t length)
{
	...
	if (ret != BT_GATT_ITER_CONTINUE) {
		if (ret == BT_GATT_ITER_STOP) {
			/* At this point the inst->bt_buf.data contains a NULL terminator string */
			friendly_name_changed(conn, 0, inst_index, (char *)inst->bt_buf.data);
		} else {
...
static bool check_and_set_all_busy(struct bt_tbs_server_inst *srv_inst)
{
...
#if defined(CONFIG_BT_TBS_CLIENT_GTBS)
	if (bt_atomic_test_and_set_bit(srv_inst->gtbs_inst.flags, BT_TBS_CLIENT_FLAG_BUSY)) {
	...
	for (num_free = 0U; num_free < ARRAY_SIZE(srv_inst->tbs_insts); num_free++) {
		...
		if (bt_atomic_test_and_set_bit(tbs_inst->flags, BT_TBS_CLIENT_FLAG_BUSY)) {
...
int bt_tbs_client_register_cb(struct bt_tbs_client_cb *cb)
{
	...
	if (bt_slist_find(&tbs_client_cbs, &cb->_node, NULL)) {
		return -EEXIST;
	}

	bt_slist_append(&tbs_client_cbs, &cb->_node);
