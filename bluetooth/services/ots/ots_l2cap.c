...
static int ots_l2cap_send(struct bt_gatt_ots_l2cap *l2cap_ctx)
{
	...
	buf = bt_buf_alloc(&ot_chan_tx_pool, OS_TIMEOUT_FOREVER);
	bt_buf_reserve(buf, BT_L2CAP_SDU_CHAN_SEND_RESERVE);
	bt_buf_add_mem(buf, &l2cap_ctx->tx.data[l2cap_ctx->tx.len_sent], len);
...
static struct bt_buf *l2cap_alloc_buf(struct bt_l2cap_chan *chan)
{
	...
	return bt_buf_alloc(&ot_chan_rx_pool, OS_TIMEOUT_FOREVER);
}
#endif
...
static struct bt_gatt_ots_l2cap *find_free_l2cap_ctx(void)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&channels, l2cap_ctx, node) {
...
{
	bt_slist_append(&channels, &l2cap_ctx->node);
...
{
	bt_slist_find_and_remove(&channels, &l2cap_ctx->node);
