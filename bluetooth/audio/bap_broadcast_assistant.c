...
struct bap_broadcast_assistant_instance {
	...
	struct bt_work_delayable bap_read_work;
...
static bt_slist_t broadcast_assistant_cbs = BT_SLIST_STATIC_INIT(&broadcast_assistant_cbs);
...
BT_BUF_SIMPLE_DEFINE_STATIC(att_buf, ATT_BUF_SIZE);
			    ...
			    struct bt_bap_scan_delegator_recv_state *recv_state)
{
	...
	recv_state->src_id = bt_buf_simple_pull_u8(&buf);
	recv_state->addr.type = bt_buf_simple_pull_u8(&buf);
	addr = bt_buf_simple_pull_mem(&buf, sizeof(*addr));
	bt_addr_copy(&recv_state->addr.a, addr);
	recv_state->adv_sid = bt_buf_simple_pull_u8(&buf);
	recv_state->broadcast_id = bt_buf_simple_pull_le24(&buf);
	recv_state->pa_sync_state = bt_buf_simple_pull_u8(&buf);
	recv_state->encrypt_state = bt_buf_simple_pull_u8(&buf);
	if (recv_state->encrypt_state == BT_BAP_BIG_ENC_STATE_BAD_CODE) {
		...
		broadcast_code = bt_buf_simple_pull_mem(&buf, BT_ISO_BROADCAST_CODE_SIZE);
		(void)memcpy(recv_state->bad_code, broadcast_code,
			     sizeof(recv_state->bad_code));
	}

	recv_state->num_subgroups = bt_buf_simple_pull_u8(&buf);
	...
	for (int i = 0; i < recv_state->num_subgroups; i++) {
		...
		subgroup->bis_sync = bt_buf_simple_pull_le32(&buf);
		...
		metadata = bt_buf_simple_pull_mem(&buf,
						   subgroup->metadata_len);
						     ...
						     const void *data, uint16_t length)
{
	...
	if (data != NULL) {
		if (bt_buf_simple_tailroom(&att_buf) < length) {
...
static void long_bap_read(struct bt_conn *conn, uint16_t handle)
{
	...
	if (bt_atomic_test_and_set_bit(inst->flags, BAP_BA_FLAG_BUSY)) {
		...
		err = bt_work_reschedule(&inst->bap_read_work,
...
static void delayed_bap_read_handler(struct bt_work *work)
{
	struct bap_broadcast_assistant_instance *inst =
		CONTAINER_OF((struct bt_work_delayable *)work,
			      ...
			      const void *data, uint16_t length)
{
	...
	if (length != 0) {
		...
		(void)bt_work_cancel_delayable(&inst->bap_read_work);
...
{
	struct bt_bap_broadcast_assistant_cb *listener, *next;
	uint8_t opcode = bt_buf_simple_pull_u8(&att_buf);
	struct bap_broadcast_assistant_instance *inst = inst_by_conn(conn);
...
static int bt_bap_broadcast_assistant_common_cp(struct bt_conn *conn,
				    const struct bt_buf_simple *buf)
{
	int err;
	struct bap_broadcast_assistant_instance *inst;
...
int bt_bap_broadcast_assistant_register_cb(struct bt_bap_broadcast_assistant_cb *cb)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&broadcast_assistant_cbs, tmp, _node) {
		if (tmp == cb) {
	...
	bt_slist_append(&broadcast_assistant_cbs, &cb->_node);
...
int bt_bap_broadcast_assistant_unregister_cb(struct bt_bap_broadcast_assistant_cb *cb)
{
	...
	if (!bt_slist_find_and_remove(&broadcast_assistant_cbs, &cb->_node)) {
		return -EALREADY;
	}
...
int bt_bap_broadcast_assistant_scan_start(struct bt_conn *conn, bool start_scan)
{
	...
	bt_buf_simple_reset(&att_buf);
	cp = bt_buf_simple_add(&att_buf, sizeof(*cp));
				       ...
				       const struct bt_bap_broadcast_assistant_add_src_param *param)
{
	...
	for (int i = 0; i < param->num_subgroups; i++) {
		...
		subgroup = bt_buf_simple_add(&att_buf, subgroup_size);
