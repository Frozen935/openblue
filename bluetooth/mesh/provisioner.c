...
static void send_invite(void)
{
	...
	bt_buf_simple_add_u8(&inv, provisionee.attention_duration);
...
static void send_start(void)
{
	...
	bt_buf_simple_add_u8(&start, bt_mesh_prov_link.oob_method);

	bt_buf_simple_add_u8(&start, bt_mesh_prov_link.oob_action);

	bt_buf_simple_add_u8(&start, bt_mesh_prov_link.oob_size);
...
static void send_confirm(void)
{
	...
	bt_buf_simple_add_mem(&cfm, bt_mesh_prov_link.conf, auth_size);
...
static void send_pub_key(void)
{
	...
	bt_buf_simple_add_mem(&buf, key, PUB_KEY_SIZE);
...
static void prov_dh_key_gen_handler(struct bt_work *work)
{
	prov_dh_key_gen();
}

static BT_WORK_DEFINE(dh_gen_work, prov_dh_key_gen_handler);
...
static void send_prov_data(void)
{
	...
	bt_buf_simple_add_mem(&pdu, net_key, sizeof(net_key));
	bt_buf_simple_add_be16(&pdu, provisionee.node->net_idx);
	bt_buf_simple_add_u8(&pdu, bt_mesh_cdb_subnet_flags(sub));
	bt_buf_simple_add_be32(&pdu, bt_mesh_cdb.iv_index);
	bt_buf_simple_add_be16(&pdu, bt_mesh_prov_link.addr);
	bt_buf_simple_add(&pdu, 8); /* For MIC */
...
static void send_random(void)
{
	...
	bt_buf_simple_add_mem(&rnd, bt_mesh_prov_link.rand, rand_size);
