...
static BT_FIFO_DEFINE(bt_mesh_adv_queue);
static BT_FIFO_DEFINE(bt_mesh_relay_queue);
static BT_FIFO_DEFINE(bt_mesh_friend_queue);

BT_MEM_POOL_DEFINE_STATIC(local_adv_pool, sizeof(struct bt_mesh_adv),
			 CONFIG_BT_MESH_ADV_BUF_COUNT, __alignof__(struct bt_mesh_adv));

#if defined(CONFIG_BT_MESH_RELAY_BUF_COUNT)
BT_MEM_POOL_DEFINE_STATIC(relay_adv_pool, sizeof(struct bt_mesh_adv),
			 CONFIG_BT_MESH_RELAY_BUF_COUNT, __alignof__(struct bt_mesh_adv));
#endif

#if defined(CONFIG_BT_MESH_ADV_EXT_FRIEND_SEPARATE)
BT_MEM_POOL_DEFINE_STATIC(friend_adv_pool, sizeof(struct bt_mesh_adv),
...
static struct bt_mesh_adv *adv_create_from_pool(struct bt_mem_pool *buf_pool,
						enum bt_mesh_adv_type type,
						enum bt_mesh_adv_tag tag,
						uint8_t xmit, os_timeout_t timeout)
{
	...
	err = bt_mem_pool_alloc(buf_pool, (void **)&adv, timeout);
	if (err) {
		return NULL;
	}
	...
	bt_buf_simple_init_with_data(&adv->b, adv->__bufs, BT_MESH_ADV_DATA_SIZE);
	bt_buf_simple_reset(&adv->b);
...
static struct bt_mesh_adv *process_events(struct bt_poll_event *ev, int count)
{
	for (; count; ev++, count--) {
		...
		case BT_POLL_STATE_CANCELLED:
			break;
		default:
			LOG_WRN("Unexpected bt_poll event state %u", ev->state);
			break;
		}
	}
...
struct bt_mesh_adv *bt_mesh_adv_get(os_timeout_t timeout)
{
	int err;
	...
	err = bt_poll(events, ARRAY_SIZE(events), timeout);
	if (err) {
		return NULL;
	}
...
struct bt_mesh_adv *bt_mesh_adv_get_by_tag(enum bt_mesh_adv_tag_bit tags, os_timeout_t timeout)
{
	if (IS_ENABLED(CONFIG_BT_MESH_ADV_EXT_FRIEND_SEPARATE) &&
	    tags & BT_MESH_ADV_TAG_BIT_FRIEND) {
		return bt_fifo_get(&bt_mesh_friend_queue, timeout);
	}

	if ((IS_ENABLED(CONFIG_BT_MESH_RELAY) || IS_ENABLED(CONFIG_BT_MESH_BRG_CFG_SRV)) &&
	    !(tags & BT_MESH_ADV_TAG_BIT_LOCAL)) {
		return bt_fifo_get(&bt_mesh_relay_queue, timeout);
	}
...
void bt_mesh_adv_get_cancel(void)
{
	...
	bt_fifo_cancel_wait(&bt_mesh_adv_queue);

	if ((IS_ENABLED(CONFIG_BT_MESH_RELAY) || IS_ENABLED(CONFIG_BT_MESH_BRG_CFG_SRV))) {
		bt_fifo_cancel_wait(&bt_mesh_relay_queue);
	}

	if (IS_ENABLED(CONFIG_BT_MESH_ADV_EXT_FRIEND_SEPARATE)) {
		bt_fifo_cancel_wait(&bt_mesh_friend_queue);
	}
}
		      ...
		      void *cb_data)
{
	...
	if (IS_ENABLED(CONFIG_BT_MESH_ADV_EXT_FRIEND_SEPARATE) &&
		...
		bt_fifo_put(&bt_mesh_friend_queue, bt_mesh_adv_ref(adv));
	...
	if (((IS_ENABLED(CONFIG_BT_MESH_RELAY) || IS_ENABLED(CONFIG_BT_MESH_BRG_CFG_SRV)) &&
		...
		bt_fifo_put(&bt_mesh_relay_queue, bt_mesh_adv_ref(adv));
		bt_mesh_adv_relay_ready();
		return;
	}

	bt_fifo_put(&bt_mesh_adv_queue, bt_mesh_adv_ref(adv));
...
static void bt_mesh_scan_cb(const bt_addr_le_t *addr, int8_t rssi,
			    uint8_t adv_type, struct bt_buf_simple *buf)
{
