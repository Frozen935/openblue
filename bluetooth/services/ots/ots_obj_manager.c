...
{
	bt_dnode_t *obj_dnode;
	struct bt_gatt_ots_pool_item *first_item;

	if (bt_dlist_is_empty(&obj_manager->list)) {
		return -ENOENT;
	}

	obj_dnode = bt_dlist_peek_head_not_empty(&obj_manager->list);
	...
	struct bt_gatt_ots_object **obj)
{
	...
	obj_dnode = bt_dlist_peek_tail(&obj_manager->list);
	...
	struct bt_gatt_ots_object **prev_obj)
{
	...
	prev_obj_dnode = bt_dlist_peek_prev_no_check(&obj_manager->list,
						      &cur_item->dnode);
	if (!prev_obj_dnode) {
		return -ENFILE;
	}
	...
	struct bt_gatt_ots_object **next_obj)
{
	...
	cur_item = CONTAINER_OF(cur_obj, struct bt_gatt_ots_pool_item, val);
	next_obj_dnode = bt_dlist_peek_next_no_check(&obj_manager->list,
						      &cur_item->dnode);
	if (!next_obj_dnode) {
		return -ENFILE;
	}
	...
	struct bt_gatt_ots_object **object)
{
	for (uint64_t i = 0; i < ARRAY_SIZE(obj_manager->pool); i++) {
		...
		if (!cur_obj->is_allocated) {
			...
			bt_dlist_append(&obj_manager->list, &cur_obj->dnode);
...
void *bt_gatt_ots_obj_manager_assign(void)
{
	...
	bt_dlist_init(&cur_manager->list);
