...
static struct {
	...
	struct {
		...
		struct bt_work_delayable timeout;
		/* Extended scanning */
		bt_addr_le_t addr;
	...
	.scan = {
		.adv_data = BT_BUF_SIMPLE(CONFIG_BT_MESH_RPR_SRV_AD_DATA_MAX)
	}
};
			     ...
			     enum bt_mesh_rpr_status status)
{
	...
	bt_buf_simple_add_u8(&rsp, status);
	bt_buf_simple_add_u8(&rsp, srv.scan.state);
	bt_buf_simple_add_u8(&rsp, srv.scan.max_devs);
	bt_buf_simple_add_u8(&rsp, timeout);
			     ...
			     enum bt_mesh_rpr_status status)
{
	...
	bt_buf_simple_add_u8(&buf, status);
	bt_buf_simple_add_u8(&buf, srv.link.state);
...
static void scan_report_schedule(void)
{
	...
	bt_work_reschedule(&srv.scan.report, OS_MSEC(delay));
}
...
static void scan_report_send(void)
{
	...
	for (i = 0; i < srv.scan.max_devs; ++i) {
		...
		bt_buf_simple_add_u8(&buf, dev->rssi);
		bt_buf_simple_add_mem(&buf, dev->uuid, 16);
		bt_buf_simple_add_le16(&buf, dev->oob);
...
static void scan_ext_report_send(void)
{
	...
	bt_buf_simple_add_u8(&buf, BT_MESH_RPR_SUCCESS);
	bt_buf_simple_add_mem(&buf, srv.scan.dev->uuid, 16);

	if (srv.scan.dev->flags & BT_MESH_RPR_UNPROV_FOUND) {
		bt_buf_simple_add_le16(&buf, srv.scan.dev->oob);
	} else {
		LOG_DBG("not found");
		goto send;
	}
	...
	if (srv.scan.dev->flags & BT_MESH_RPR_UNPROV_EXT_ADV_RXD) {
		bt_buf_simple_add_mem(&buf, srv.scan.adv_data->data,
...
static void scan_ext_stop(uint32_t remaining_time)
{
	...
	if ((remaining_time + srv.scan.additional_time) &&
		...
		bt_work_reschedule(
			&srv.scan.timeout,
			OS_MSEC(remaining_time + srv.scan.additional_time));
...
static void scan_timeout(struct bt_work *work)
{
...
static void outbound_pdu_report_send(void)
{
	...
	bt_buf_simple_add_u8(&buf, srv.link.tx_pdu);
...
static int inbound_pdu_send(struct bt_buf_simple *buf,
			    const struct bt_mesh_send_cb *cb)
{
	...
	bt_buf_simple_add_u8(&msg, srv.link.rx_pdu);
	bt_buf_simple_add_mem(&msg, buf->data, buf->len);
...
static void link_report_send_and_clear(struct bt_work *work)
{
	link_report_send();
	cli_link_clear();
}
			   ...
			   enum prov_bearer_link_status reason)
{
	...
	srv.link.state = BT_MESH_RPR_LINK_IDLE;
	bt_work_submit(&srv.link.report);
}
				...
				struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(&rsp, CONFIG_BT_MESH_RPR_SRV_SCANNED_ITEMS_MAX);
	bt_buf_simple_add_u8(&rsp, true);
			     ...
			     struct bt_buf_simple *buf)
{
	...
	max_devs = bt_buf_simple_pull_u8(buf);
	timeout = bt_buf_simple_pull_u8(buf);
	if (!timeout) {
		return -EINVAL;
	}
				      ...
				      struct bt_buf_simple *buf)
{
	...
	ad_count = bt_buf_simple_pull_u8(buf);
	if (buf->len < ad_count || ad_count == 0 || ad_count > 0x10) {
		/* Prohibited */
		return -EINVAL;
	}

	ad = bt_buf_simple_pull_mem(buf, ad_count);
	...
	if (!buf->len) {
		...
		bt_buf_simple_add_u8(&rsp, BT_MESH_RPR_SUCCESS);
		bt_buf_simple_add_mem(&rsp, prov->uuid, 16);
		bt_buf_simple_add_le16(&rsp, prov->oob_info);
		...
		if (prov->uri && get_ad_type(ad, ad_count, BT_DATA_URI)) {
			...
			if (uri_len < CONFIG_BT_MESH_RPR_SRV_AD_DATA_MAX - 2) {
				bt_buf_simple_add_u8(&rsp, uri_len + 1);
				bt_buf_simple_add_u8(&rsp, BT_DATA_URI);
				bt_buf_simple_add_mem(&rsp, prov->uri,
						       uri_len);
...
rsp:
	bt_mesh_model_msg_init(&rsp, RPR_OP_EXTENDED_SCAN_REPORT);
	bt_buf_simple_add_u8(&rsp, status);
	bt_buf_simple_add_mem(&rsp, uuid, 16);
	bt_mesh_model_send(mod, ctx, &rsp, NULL, NULL);
			     ...
			     struct bt_buf_simple *buf)
{
	...
	reason = bt_buf_simple_pull_u8(buf);
	if (reason != PROV_BEARER_LINK_STATUS_SUCCESS &&
			   ...
			   struct bt_buf_simple *buf)
{
	...
	pdu_num = bt_buf_simple_pull_u8(buf);
...
static bool pull_ad_data(struct bt_buf_simple *buf, struct bt_data *ad)
{
	uint8_t len;
	...
	ad->type = bt_buf_simple_pull_u8(buf);
	ad->data_len = len - sizeof(ad->type);
	ad->data = bt_buf_simple_pull_mem(buf, ad->data_len);
	return true;
}
...
{
	struct bt_mesh_rpr_unprov *dev = NULL;
	struct bt_buf_simple_state initial;
	struct bt_data ad;
	...
	bt_buf_simple_save(buf, &initial);
	...
	bt_buf_simple_restore(buf, &initial);
	...
	while (srv.scan.ad_count && pull_ad_data(buf, &ad)) {
		...
		if (ad.data_len + 2 >
		    bt_buf_simple_tailroom(srv.scan.adv_data)) {
			LOG_WRN("Can't fit AD 0x%02x in scan report", ad.type);
			continue;
		}

		bt_buf_simple_add_u8(srv.scan.adv_data, ad.data_len + 1);
		bt_buf_simple_add_u8(srv.scan.adv_data, ad.type);
		bt_buf_simple_add_mem(srv.scan.adv_data, ad.data, ad.data_len);
...
static int rpr_srv_init(const struct bt_mesh_model *mod)
{
	...
	bt_work_init_delayable(&srv.scan.timeout, scan_timeout);
	bt_work_init_delayable(&srv.scan.report, scan_report_timeout);
	bt_work_init(&srv.link.report, link_report_send_and_clear);
...
static int node_refresh_buf_send(struct bt_buf_simple *buf,
