...
static void (*msg_cb)(uint32_t opcode, struct bt_mesh_msg_ctx *ctx, struct bt_buf_simple *buf);
...
static uint8_t *data_buf_add_u8_offset(struct bt_buf_simple *buf,
				       uint8_t val, size_t *offset)
{
	...
	return bt_buf_simple_add_u8(buf, val);
}

static void data_buf_add_le16_offset(struct bt_buf_simple *buf,
				     uint16_t val, size_t *offset)
{
	if (*offset >= 2) {
		...
		*offset -= 1;
		bt_buf_simple_add_u8(buf, (val >> 8));
	} else {
		bt_buf_simple_add_le16(buf, val);
	}
}

static void data_buf_add_mem_offset(struct bt_buf_simple *buf, const uint8_t *data, size_t len,
				    size_t *offset)
{
	if (*offset >= len) {
		*offset -= len;
	...
	bt_buf_simple_add_mem(buf, data + *offset, len - *offset);
	*offset = 0;
}
			      ...
			      void *user_data)
{
	const struct bt_mesh_models_metadata_entry *entry;
	struct comp_foreach_model_arg *arg = user_data;
	struct bt_buf_simple *buf = arg->buf;
	...
	if (bt_buf_simple_tailroom(buf) < (model_size + BT_MESH_MIC_SHORT)) {
...
int bt_mesh_metadata_get_page_0(struct bt_buf_simple *buf, size_t offset)
{
	const struct bt_mesh_comp *comp;
	...
	for (i = 0; i < comp->elem_count; i++) {
		...
		if (bt_buf_simple_tailroom(buf) < (((offset == 0) ? 2 : (offset == 1) ? 1 : 0)
				+ BT_MESH_MIC_SHORT)) {
			LOG_DBG("Model metadata didn't fit in the buffer");
			return -E2BIG;
		}
...
static int comp_add_elem(struct bt_buf_simple *buf, const struct bt_mesh_elem *elem,
			 size_t *offset)
{
	struct comp_foreach_model_arg arg = {
		.buf = buf,
	...
	if (bt_buf_simple_tailroom(buf) < ((elem_size - *offset) + BT_MESH_MIC_SHORT)) {
...
int bt_mesh_comp_data_get_page_0(struct bt_buf_simple *buf, size_t offset)
{
	uint16_t feat = 0U;
	const struct bt_mesh_comp *comp;
	int i;
...
static void prep_model_item_header(const struct bt_mesh_model *mod, uint8_t *cor_id,
				   uint8_t *mod_cnt, struct bt_buf_simple *buf,
...
static void add_items_to_page(struct bt_buf_simple *buf, const struct bt_mesh_model *mod,
...
static int bt_mesh_comp_data_get_page_1(struct bt_buf_simple *buf, size_t offset)
{
	const struct bt_mesh_comp *comp;
	uint8_t cor_id = 0;
...
static int bt_mesh_comp_data_get_page_2(struct bt_buf_simple *buf, size_t offset)
{
	if (!dev_comp2) {
...
static void publish_sent(int err, void *user_data)
{
	...
	if (delay) {
		 ...
		 */
		bt_work_schedule(&mod->pub->timer, OS_MSEC(delay));
	}
}
...
static int publish_transmit(const struct bt_mesh_model *mod)
{
	...
	bt_buf_simple_add_mem(&sdu, pub->msg->data, pub->msg->len);
...
static int pub_delay_schedule(struct bt_mesh_model_pub *pub, int delay)
{
	...
	err = bt_work_reschedule(&pub->timer, OS_MSEC(random));
	if (err < 0) {
...
static void mod_publish(struct bt_work *work)
{
		     ...
		     bool vnd, bool primary, void *user_data)
{
	...
	if (mod->pub) {
		mod->pub->mod = mod;
		bt_work_init_delayable(&mod->pub->timer, mod_publish);
	}
...
int bt_mesh_access_send(struct bt_mesh_msg_ctx *ctx, struct bt_buf_simple *buf, uint16_t src_addr,
...
static int get_opcode(struct bt_buf_simple *buf, uint32_t *opcode)
{
	...
	case 0x01:
		...
		*opcode = bt_buf_simple_pull_u8(buf);
		return 0;
	case 0x02:
		if (buf->len < 2) {
		...
		*opcode = bt_buf_simple_pull_be16(buf);
		return 0;
	case 0x03:
		if (buf->len < 3) {
		...
		*opcode = bt_buf_simple_pull_u8(buf) << 16;
		/* Using LE for the CID since the model layer is defined as
		 * little-endian in the mesh spec and using BT_MESH_MODEL_OP_3
		 * will declare the opcode in this way.
		 */
		*opcode |= bt_buf_simple_pull_le16(buf);
		return 0;
	}
...
static int element_model_recv(struct bt_mesh_msg_ctx *ctx, struct bt_buf_simple *buf,
...
int bt_mesh_model_recv(struct bt_mesh_msg_ctx *ctx, struct bt_buf_simple *buf)
{
...
int bt_mesh_access_recv(struct bt_mesh_msg_ctx *ctx, struct bt_buf_simple *buf)
{
	int err;
...
int bt_mesh_model_publish(const struct bt_mesh_model *model)
{
	...
	bt_work_reschedule(&pub->timer, OS_TIMEOUT_NO_WAIT);
			    ...
			    const char *key, char *path, size_t path_len)
{
	...
	if (vnd) {
		snprintf(path, path_len, "bt/mesh/v/%x/%s", mod_key, key);
	} else {
		snprintf(path, path_len, "bt/mesh/s/%x/%s", mod_key, key);
	}
}
...
int bt_mesh_comp_data_get_page(struct bt_buf_simple *buf, size_t page, size_t offset)
{
	if (page == 0 || page == 128) {
...
{
	BT_BUF_SIMPLE_DEFINE(buf, CONFIG_BT_MESH_COMP_PST_BUF_SIZE);
	int err;
			...
			void *cb_arg, void *param)
{
	...
	if (len > bt_buf_simple_tailroom(buf)) {
		return -ENOBUFS;
	}

	len = read_cb(cb_arg, bt_buf_simple_tail(buf), len);
	if (len > 0) {
		bt_buf_simple_add(buf, len);
	}

	return -EALREADY;
}

int bt_mesh_comp_read(struct bt_buf_simple *buf, uint8_t page)
{
...
{
	BT_BUF_SIMPLE_DEFINE(buf, CONFIG_BT_MESH_MODELS_METADATA_PAGE_LEN);
	size_t total_size;
	int err;
	...
	bt_buf_simple_add_le16(&buf, total_size);
...
int bt_mesh_models_metadata_read(struct bt_buf_simple *buf, size_t offset)
{
	BT_BUF_SIMPLE_DEFINE(stored_buf, CONFIG_BT_MESH_MODELS_METADATA_PAGE_LEN);
	size_t original_len = buf->len;
	int err;
	...
	bt_buf_simple_add_mem(buf, &stored_buf.data, MIN(bt_buf_simple_tailroom(buf), 2));

	if (offset >= stored_buf.len) {
		return 0;
	}

	bt_buf_simple_add_mem(buf, &stored_buf.data[offset],
			       MIN(bt_buf_simple_tailroom(buf), stored_buf.len - offset));
		       ...
		       bool vnd, bool primary, void *user_data)
{
	if (mod->pub && mod->pub->update &&
		...
		if (ms > 0) {
			...
			bt_work_schedule(&mod->pub->timer, OS_MSEC(ms + random));
		}
	}
...
uint8_t bt_mesh_comp_parse_page(struct bt_buf_simple *buf)
{
