				  ...
				  struct bt_buf_simple *msg)
{
	...
	test_id = bt_buf_simple_add(msg, 1);
	bt_buf_simple_add_le16(msg, company_id);

	if (srv->cb && srv->cb->fault_get_reg) {
		uint8_t fault_count = bt_buf_simple_tailroom(msg) - 4;
		int err;

		err = srv->cb->fault_get_reg(mod, company_id, test_id,
					     bt_buf_simple_tail(msg),
					     &fault_count);
		if (err) {
			LOG_ERR("Failed to get faults (err %d)", err);
			*test_id = HEALTH_TEST_STANDARD;
		} else {
			bt_buf_simple_add(msg, fault_count);
		}
	} else {
				 ...
				 struct bt_buf_simple *msg)
{
	...
	test_id = bt_buf_simple_add(msg, 1);
	company_ptr = bt_buf_simple_add(msg, sizeof(company_id));
	comp = bt_mesh_comp_get();

	if (srv->cb && srv->cb->fault_get_cur) {
		fault_count = bt_buf_simple_tailroom(msg);
		int err;
			    ...
			    struct bt_buf_simple *buf)
{
	...
	company_id = bt_buf_simple_pull_le16(buf);
				    ...
				    struct bt_buf_simple *buf)
{
	...
	test_id = bt_buf_simple_pull_u8(buf);
	company_id = bt_buf_simple_pull_le16(buf);
			       ...
			       struct bt_buf_simple *buf)
{
	...
	time = bt_buf_simple_pull_u8(buf);
				     ...
				     struct bt_mesh_msg_ctx *ctx)
{
	...
	bt_buf_simple_add_u8(&msg, model->pub->period_div);
				    ...
				    struct bt_buf_simple *buf)
{
	...
	period = bt_buf_simple_pull_u8(buf);
	if (period > 15) {
...
static void attention_off(struct bt_work *work)
{
...
static int health_srv_init(const struct bt_mesh_model *model)
{
	...
	bt_work_init_delayable(&srv->attn_timer, attention_off);
...
void bt_mesh_attention(const struct bt_mesh_model *model, uint8_t time)
{
	...
	bt_work_reschedule(&srv->attn_timer, OS_SECONDS(time));
}
