...
static void bridge_status_send(const struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx)
{
	...
	bt_buf_simple_add_u8(&msg, bt_mesh_brg_cfg_enable_get() ? 1 : 0);
...
static int subnet_bridge_set(const struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
			     struct bt_buf_simple *buf)
{
	uint8_t enable = bt_buf_simple_pull_u8(buf);
			      ...
			      struct bt_buf_simple *buf)
{
	...
	entry.directions = bt_buf_simple_pull_u8(buf);
	key_idx_unpack_pair(buf, &entry.net_idx1, &entry.net_idx2);
	entry.addr1 = bt_buf_simple_pull_le16(buf);
	entry.addr2 = bt_buf_simple_pull_le16(buf);
			       ...
			       struct bt_buf_simple *buf)
{
	...
	int16_t net_idx_filter = bt_buf_simple_pull_le16(buf);
	...
	uint8_t start_id = bt_buf_simple_pull_u8(buf);

	bt_buf_simple_add_le16(&msg, net_idx_filter);
	bt_buf_simple_add_u8(&msg, start_id);
	...
	for (int i = 0; i < rows; i++) {
		...
		if (bt_buf_simple_tailroom(&msg) < 3 + BT_MESH_MIC_SHORT) {
			break;
		}
		...
		if (is_first_instance &&
		    (filter_net_idx.filter == 0 ||
		     ...
		     (filter_net_idx.filter == 3 &&
		      (net_idx1 == filter_net_idx.net_idx || net_idx2 == filter_net_idx.net_idx)))) {
			if (start_id > 0) {
				start_id--;
			} else {
			      ...
			      struct bt_buf_simple *buf)
{
	...
	uint16_t start_id = bt_buf_simple_pull_le16(buf);
	...
	bt_buf_simple_add_u8(&msg, status);
	key_idx_pack_pair(&msg, net_idx1, net_idx2);
	bt_buf_simple_add_le16(&msg, start_id);
	...
	for (int i = 0; i < rows; i++) {
		if (brg_tbl[i].net_idx1 == net_idx1 && brg_tbl[i].net_idx2 == net_idx2) {
			if (cnt >= start_id) {
				if (bt_buf_simple_tailroom(&msg) < 5 + BT_MESH_MIC_SHORT) {
					LOG_WRN("Bridging Table List message too large");
					break;
				}

				bt_buf_simple_add_le16(&msg, brg_tbl[i].addr1);
				bt_buf_simple_add_le16(&msg, brg_tbl[i].addr2);
				bt_buf_simple_add_u8(&msg, brg_tbl[i].direction);
			}
			cnt++;
		}
	}
				   ...
				   struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_le16(&msg, CONFIG_BT_MESH_BRG_TABLE_ITEMS_MAX);
