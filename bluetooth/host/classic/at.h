 ...
 */
typedef int (*at_resp_cb_t)(struct at_client *at, struct bt_buf *buf);
...
typedef int (*handle_parse_input_t)(struct at_client *at, struct bt_buf *buf);
typedef int (*handle_cmd_input_t)(struct at_client *at, struct bt_buf *buf,
				  const char *prefix, parse_val_t func,
				  enum at_cmd_type type);

struct at_client {
	struct bt_buf_simple rsp_buf;
	uint8_t state;
	uint8_t cmd_state;
	at_resp_cb_t resp;
...
/* This parsing will only works for non-fragmented bt_buf */
int at_parse_input(struct at_client *at, struct bt_buf *buf);
/* This command parsing will only works for non-fragmented bt_buf */
