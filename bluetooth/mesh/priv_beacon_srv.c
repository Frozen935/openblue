			      ...
			      struct bt_mesh_msg_ctx *ctx)
{
	...
	bt_buf_simple_add_u8(&buf, bt_mesh_priv_beacon_get());
	bt_buf_simple_add_u8(&buf, bt_mesh_priv_beacon_update_interval_get());
			     ...
			     struct bt_buf_simple *buf)
{
	...
	beacon = bt_buf_simple_pull_u8(buf);
	...
	if (buf->len == 1U) {
		bt_mesh_priv_beacon_update_interval_set(bt_buf_simple_pull_u8(buf));
	}
				  ...
				  struct bt_mesh_msg_ctx *ctx)
{
	...
	bt_buf_simple_add_u8(&buf, bt_mesh_priv_gatt_proxy_get());
				 ...
				 struct bt_buf_simple *buf)
{
	...
	gatt_proxy = bt_buf_simple_pull_u8(buf);
			       ...
			       uint16_t net_idx, uint8_t node_id)
{
	...
	bt_buf_simple_add_le16(&buf, net_idx);
	bt_buf_simple_add_u8(&buf, node_id);
