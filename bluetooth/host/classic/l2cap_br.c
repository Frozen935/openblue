...
#define BR_CHAN_RET(_w) CONTAINER_OF(bt_work_delayable_from_work(_w), \
				     struct bt_l2cap_br_chan, ret_work)

#define BR_CHAN_MONITOR(_w) CONTAINER_OF(bt_work_delayable_from_work(_w), \
...
/* Pool for outgoing BR/EDR signaling packets, min MTU is 48 */
BT_BUF_POOL_FIXED_DEFINE(br_tx_pool, CONFIG_BT_L2CAP_TX_BUF_COUNT,
...
static bt_slist_t bt_l2cap_br_echo_cbs = BT_SLIST_STATIC_INIT(&bt_l2cap_br_echo_cbs);
						...
						uint16_t cid)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&conn->channels, chan, node) {
...
static void l2cap_br_chan_destroy(struct bt_l2cap_chan *chan)
{
	 ...
	 */
	struct bt_work_q *rtx_worbt_queue = br_chan->rtx_work.queue;

	if (rtx_worbt_queue == NULL || !os_thread_is_current(&rtx_worbt_queue->thread)) {
		bt_work_cancel_delayable_sync(&br_chan->rtx_work, &br_chan->rtx_sync);
	} else {
		bt_work_cancel_delayable(&br_chan->rtx_work);
	}
	...
	bt_atomic_clear(BR_CHAN(chan)->flags);
}

static void l2cap_br_rtx_timeout(struct bt_work *work)
{
	...
	if (chan->rx.cid == BT_L2CAP_CID_BR_SIG) {
		...
		bt_atomic_clear_bit(chan->flags, L2CAP_FLAG_SIG_INFO_PENDING);
		chan->ident = 0;
		return;
	}
...
{
	return !bt_slist_is_empty(&br_chan->_pdu_tx_queue);
}

static void raise_data_ready(struct bt_l2cap_br_chan *br_chan)
{
	if (!bt_atomic_set(&br_chan->_pdu_ready_lock, 1)) {
		bt_slist_append(&br_chan->chan.conn->l2cap_data_ready,
				 &br_chan->_pdu_ready);
...
static void lower_data_ready(struct bt_l2cap_br_chan *br_chan)
{
	...
	__maybe_unused bt_snode_t *s = bt_slist_get(&conn->l2cap_data_ready);

	__ASSERT_NO_MSG(s == &br_chan->_pdu_ready);

	__maybe_unused bt_atomic_t old = bt_atomic_set(&br_chan->_pdu_ready_lock, 0);
...
static void cancel_data_ready(struct bt_l2cap_br_chan *br_chan)
{
	...
	bt_slist_find_and_remove(&conn->l2cap_data_ready,
				  &br_chan->_pdu_ready);

	bt_atomic_set(&br_chan->_pdu_ready_lock, 0);
}
...
static uint16_t bt_l2cap_br_get_outstanding_count(struct bt_l2cap_br_chan *br_chan)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&br_chan->_pdu_outstanding, tx_win, next, node) {
		count++;
	}
...
static struct bt_l2cap_br_window *l2cap_br_find_window(struct bt_l2cap_br_chan *br_chan)
{
	...
	win = bt_fifo_get(&br_chan->_free_tx_win, OS_TIMEOUT_NO_WAIT);
	if (win) {
				 ...
				 struct bt_l2cap_br_window *tx_win)
{
	if (tx_win) {
		...
		bt_fifo_put(&br_chan->_free_tx_win, tx_win);
	}
}
...
static void l2cap_br_sdu_is_done(struct bt_l2cap_br_chan *br_chan, struct bt_buf *sdu, int err)
{
	bt_conn_tx_cb_t cb;
	...
	if (!bt_slist_find_and_remove(&br_chan->_pdu_tx_queue, &sdu->node)) {
					     ...
					     bool rej)
{
	...
	while (tx_win && (tx_win->tx_seq != req_seq)) {
		...
		tx_win = (void *)bt_slist_get(&br_chan->_pdu_outstanding);
	...
	if (!bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R)) {
...
static int l2cap_br_send_s_frame(struct bt_l2cap_br_chan *br_chan, uint8_t s, os_timeout_t timeout)
{
	struct bt_buf *buf;
	int err;

	if (bt_atomic_test_and_set_bit(br_chan->flags, L2CAP_FLAG_SEND_S_FRAME)) {
...
static void l2cap_br_ret_timeout(struct bt_work *work)
{
	...
	if (bt_slist_peek_head(&br_chan->_pdu_outstanding) == NULL) {
		return;
	}
	...
	case BT_L2CAP_BR_LINK_MODE_RET:
		...
		bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_PDU_RETRANS);
		/* Append channel to list if it still has data */
		if (chan_has_data(br_chan)) {
			LOG_DBG("chan %p ready", br_chan);
			raise_data_ready(br_chan);
	...
	case BT_L2CAP_BR_LINK_MODE_ERET:
		if (!bt_atomic_test_and_set_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P)) {
			int err;
			...
			bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P_CHANGED);
			err = l2cap_br_send_s_frame(br_chan, BT_L2CAP_CONTROL_S_RR, OS_TIMEOUT_NO_WAIT);
			if (err) {
...
static void l2cap_br_monitor_timeout(struct bt_work *work)
{
	...
	if (br_chan->tx.mode == BT_L2CAP_BR_LINK_MODE_ERET) {
		if (!bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P_CHANGED) &&
		    bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P)) {
			/* State: WAIT_F
		...
		err = l2cap_br_send_s_frame(br_chan, BT_L2CAP_CONTROL_S_RR, OS_TIMEOUT_NO_WAIT);
		if (!err) {
			bt_atomic_clear_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R);
		} else {
			bt_l2cap_chan_disconnect(&br_chan->chan);
		}
	}
			      ...
			      bt_l2cap_chan_destroy_t destroy)
{
	...
	bt_slist_init(&ch->_pdu_tx_queue);
	 ...
	 */
	bt_work_init_delayable(&ch->rtx_work, l2cap_br_rtx_timeout);
#if defined(CONFIG_BT_L2CAP_RET_FC)
	bt_work_init_delayable(&ch->ret_work, l2cap_br_ret_timeout);
	bt_work_init_delayable(&ch->monitor_work, l2cap_br_monitor_timeout);
#endif /* CONFIG_BT_L2CAP_RET_FC */
...
int bt_l2cap_br_send_cb(struct bt_conn *conn, uint16_t cid, struct bt_buf *buf,
			bt_conn_tx_cb_t cb, void *user_data)
{
	...
	if (buf->user_data_size < sizeof(struct closure)) {
		LOG_WRN("not enough room in user_data %d < %d pool %p",
			buf->user_data_size,
	...
	bt_slist_append(&br_chan->_pdu_tx_queue, &buf->node);
	raise_data_ready(br_chan);
...
#if defined(CONFIG_BT_L2CAP_RET_FC)
static uint16_t get_pdu_len(struct bt_l2cap_br_chan *br_chan, struct bt_buf *buf, bool start_seg)
{
...
static int bt_l2cap_br_pack_s_frame_header(struct bt_l2cap_br_chan *br_chan, struct bt_buf *buf,
					   uint8_t s)
{
	struct bt_l2cap_hdr *hdr;
	...
	if (s == BT_L2CAP_CONTROL_S_RR) {
		if (bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ_CHANGED) &&
		    bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ)) {
			s = BT_L2CAP_CONTROL_S_REJ;
		}
	}

	if (s == BT_L2CAP_CONTROL_S_RR) {
		if (bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_P)) {
			f_bit = 1;
		} else if (bt_atomic_test_and_clear_bit(br_chan->flags,
						     L2CAP_FLAG_SEND_FRAME_P_CHANGED) &&
			   bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P)) {
			p_bit = 1;
		} else if (bt_atomic_test_and_clear_bit(br_chan->flags,
						     L2CAP_FLAG_LOCAL_BUSY_CHANGED) &&
			   bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY)) {
			s = BT_L2CAP_CONTROL_S_RNR;
		}

		if (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY)) {
			s = BT_L2CAP_CONTROL_S_RNR;
		}
	}
	...
	if (bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R_CHANGED) &&
	    bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R)) {
		r_bit = 1;
	}
	...
	if ((br_chan->tx.mode == BT_L2CAP_BR_LINK_MODE_ERET) ||
		...
		if (br_chan->tx.extended_control) {
			...
			bt_buf_add_le32(buf, ext_control);
		} else {
			...
			bt_buf_add_le16(buf, enh_control);
		}
	} else {
		...
		bt_buf_add_le16(buf, std_control);
	}

	return 0;
}

static void bt_l2cap_br_pack_i_frame_header(struct bt_l2cap_br_chan *br_chan, struct bt_buf *buf,
...
static struct bt_buf *l2cap_br_get_next_sdu(struct bt_l2cap_br_chan *br_chan)
{
	struct bt_buf *sdu, *next;

	if (br_chan->_pdu_buf) {
		return br_chan->_pdu_buf;
	}

	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&br_chan->_pdu_tx_queue, sdu, next, node) {
		if (sdu->len) {
			return sdu;
		}
...
static bool l2cap_br_send_i_frame(struct bt_l2cap_br_chan *br_chan, struct bt_buf *sdu)
{
	if (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R)) {
		return false;
	}

	if (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_RET_REQ_I_FRAME)) {
		if (l2cap_br_find_srej(br_chan)) {
			return true;
		}
	}

	if (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAME) &&
	    (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_PDU_RETRANS) ||
	     bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_REQ_SEQ_UPDATED))) {
		return true;
	}

	if (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P)) {
		return false;
	}

	if (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_REMOTE_BUSY)) {
		return false;
	}

	if (!bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_NEW_I_FRAME)) {
		return false;
	}
	...
	if (bt_fifo_peek_head(&br_chan->_free_tx_win)) {
		return true;
	}
...
static struct bt_buf *l2cap_br_ret_fc_data_pull(struct bt_conn *conn, size_t amount,
						 size_t *length)
{
	const bt_snode_t *pdu_ready = bt_slist_peek_head(&conn->l2cap_data_ready);
	 ...
	 * fragments.
	 */
	struct bt_buf *pdu = l2cap_br_get_next_sdu(br_chan);
	struct bt_buf *send_buf = NULL;
	...
	if (br_chan->_pdu_remaining == 0) {
		...
		if ((bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R) &&
		     bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R_CHANGED)) ||
		    (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ) &&
		     bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ_CHANGED)) ||
		    (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P) &&
		     bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P_CHANGED)) ||
		    (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY) &&
		     bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY_CHANGED)) ||
		    bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_S_FRAME)) {
			alloc = true;
		}

		if (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_P) &&
		...
		if (alloc) {
			...
			send_buf = bt_l2cap_create_pdu_timeout(&br_tx_pool, 0, OS_TIMEOUT_NO_WAIT);
			if (send_buf == NULL) {
		...
		if (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAME) &&
		    (bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_PDU_RETRANS) ||
		     bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_REQ_SEQ_UPDATED))) {
			tx_win = (void *)bt_slist_peek_head(&br_chan->_pdu_outstanding);
			if (tx_win) {
				first = false;
				goto send_i_frame;
...
send_i_frame:
		...
		if (first) {
			...
			bt_buf_simple_save(&pdu->b, &tx_win->sdu_state);
		...
		bt_buf_add_mem(send_buf, tx_win->data, pdu_len);
		...
		if (first) {
			...
			bt_buf_pull(pdu, pdu_len);

			if (br_chan->rx.mode != BT_L2CAP_BR_LINK_MODE_STREAM) {
				bt_slist_append(&br_chan->_pdu_outstanding, &tx_win->node);
			} else {
				l2cap_br_free_window(br_chan, tx_win);
...
struct bt_buf *l2cap_br_data_pull(struct bt_conn *conn, size_t amount, size_t *length)
{
	const bt_snode_t *pdu_ready = bt_slist_peek_head(&conn->l2cap_data_ready);
	...
	if (br_chan->tx.mode != BT_L2CAP_BR_LINK_MODE_BASIC) {
		struct bt_buf *buf;
		const bt_snode_t *next_pdu_ready;
		 ...
		 */
		next_pdu_ready = bt_slist_peek_head(&conn->l2cap_data_ready);
	...
	struct bt_buf *pdu = bt_buf_ref(q_pdu);
	...
	if (last_frag) {
		...
		found = bt_slist_find_and_remove(&br_chan->_pdu_tx_queue, &q_pdu->node);
...
static void l2cap_br_get_info(struct bt_l2cap_br *l2cap, uint16_t info_type)
{
	...
	if (bt_atomic_test_bit(l2cap->chan.flags, L2CAP_FLAG_SIG_INFO_PENDING)) {
		return;
	}
	...
	bt_atomic_set_bit(l2cap->chan.flags, L2CAP_FLAG_SIG_INFO_PENDING);
...
{
	if (bt_atomic_test_and_set_bit(chan->flags, L2CAP_FLAG_FIXED_CONNECTED)) {
		return;
	}
			     ...
			     struct bt_buf *buf)
{
	...
	if (bt_atomic_test_bit(l2cap->chan.flags, L2CAP_FLAG_SIG_INFO_DONE)) {
		return 0;
	}

	if (bt_atomic_test_and_clear_bit(l2cap->chan.flags,
				      L2CAP_FLAG_SIG_INFO_PENDING)) {
		/*
		 * Release RTX timer since got the response & there's pending
		 * command request.
		 */
		bt_work_cancel_delayable(&l2cap->chan.rtx_work);
	}
	...
	case BT_L2CAP_INFO_FEAT_MASK:
		...
		l2cap->info_feat_mask = bt_buf_pull_le32(buf);
	...
	case BT_L2CAP_INFO_FIXED_CHAN:
		 ...
		 * of peer device.
		 */
		l2cap->info_fixed_chan = bt_buf_pull_u8(buf);
	...
	case BT_L2CAP_INFO_CONNLESS_MTU:
		{
			...
			if (connless_chan != NULL) {
				BR_CHAN(connless_chan)->tx.mtu = bt_buf_pull_le16(buf);
				break;
			}
		}
		__fallthrough;
	...
	bt_atomic_set_bit(l2cap->chan.flags, L2CAP_FLAG_SIG_INFO_DONE);
	l2cap->chan.ident = 0U;
	return err;
}
			     ...
			     struct bt_buf *buf)
{
	...
	hdr_info = bt_buf_add(rsp_buf, sizeof(*hdr_info));
	...
	case BT_L2CAP_INFO_FEAT_MASK:
		...
		bt_buf_add_le32(rsp_buf, L2CAP_EXTENDED_FEAT_MASK);
	...
	case BT_L2CAP_INFO_FIXED_CHAN:
		...
		(void)memset(bt_buf_add(rsp_buf, 8), 0, 8);
	...
	case BT_L2CAP_INFO_CONNLESS_MTU:
		{
			...
			if (connless_chan != NULL) {
				...
				rsp->result = sys_cpu_to_le16(BT_L2CAP_INFO_SUCCESS);
				bt_buf_add_le16(rsp_buf, BR_CHAN(connless_chan)->rx.mtu);
...
void bt_l2cap_br_disconnected(struct bt_conn *conn)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&conn->channels, chan, next, node) {
		bt_l2cap_br_chan_del(chan);
	}
}
...
static struct bt_l2cap_server *l2cap_br_server_lookup_psm(uint16_t psm)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&br_servers, server, node) {
		if (server->psm == psm) {
			return server;
		}
	}
...
static void l2cap_br_conf_add_mtu(struct bt_buf *buf, const uint16_t mtu)
{
	if (bt_buf_tailroom(buf) < (sizeof(struct bt_l2cap_conf_opt) + sizeof(mtu))) {
		LOG_WRN("No tail room for opt mtu");
		return;
	}
	bt_buf_add_u8(buf, BT_L2CAP_CONF_OPT_MTU);
	bt_buf_add_u8(buf, sizeof(mtu));
	bt_buf_add_le16(buf, mtu);
}

#if defined(CONFIG_BT_L2CAP_RET_FC)
static void l2cap_br_conf_add_ret_fc(struct bt_buf *buf,
				     const struct bt_l2cap_conf_opt_ret_fc *ret_fc)
{
	if (bt_buf_tailroom(buf) < (sizeof(struct bt_l2cap_conf_opt) + sizeof(*ret_fc))) {
		LOG_WRN("No tail room for opt flash_timeout");
		return;
	}
	bt_buf_add_u8(buf, BT_L2CAP_CONF_OPT_RET_FC);
	bt_buf_add_u8(buf, sizeof(*ret_fc));
	bt_buf_add_mem(buf, ret_fc, sizeof(*ret_fc));
}

static void l2cap_br_conf_add_fcs(struct bt_buf *buf, const struct bt_l2cap_conf_opt_fcs *fcs)
{
	if (bt_buf_tailroom(buf) < (sizeof(struct bt_l2cap_conf_opt) + sizeof(*fcs))) {
		LOG_WRN("No tail room for opt flash_timeout");
		return;
	}
	bt_buf_add_u8(buf, BT_L2CAP_CONF_OPT_FCS);
	bt_buf_add_u8(buf, sizeof(*fcs));
	bt_buf_add_mem(buf, fcs, sizeof(*fcs));
}

static void
l2cap_br_conf_add_ext_win_size(struct bt_buf *buf,
			       const struct bt_l2cap_conf_opt_ext_win_size *ext_win_size)
{
	if (bt_buf_tailroom(buf) < (sizeof(struct bt_l2cap_conf_opt) + sizeof(*ext_win_size))) {
		LOG_WRN("No tail room for opt flash_timeout");
		return;
	}
	bt_buf_add_u8(buf, BT_L2CAP_CONF_OPT_EXT_WIN_SIZE);
	bt_buf_add_u8(buf, sizeof(*ext_win_size));
	bt_buf_add_mem(buf, ext_win_size, sizeof(*ext_win_size));
}
#endif /* CONFIG_BT_L2CAP_RET_FC */

static void l2cap_br_conf_add_opt(struct bt_buf *buf, const struct bt_l2cap_conf_opt *opt)
{
	if (bt_buf_tailroom(buf) < (sizeof(*opt) + opt->len)) {
		LOG_WRN("No tail room for opt flash_timeout");
		return;
	}

	bt_buf_add_u8(buf, opt->type & BT_L2CAP_CONF_MASK);
	bt_buf_add_u8(buf, opt->len);
	bt_buf_add_mem(buf, opt->data, opt->len);
}
...
static int l2cap_br_check_chan_config(struct bt_conn *conn, struct bt_l2cap_br_chan *br_chan)
{
	...
	bt_slist_init(&br_chan->_pdu_outstanding);
	bt_fifo_init(&br_chan->_free_tx_win);

	for (int i = 0; i < ARRAY_SIZE(br_chan->tx_win); i++) {
		bt_fifo_put(&br_chan->_free_tx_win, &br_chan->tx_win[i]);
	}
	...
	bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_NEW_I_FRAME);
	bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAME);
...
static void l2cap_br_conf(struct bt_l2cap_chan *chan, bool init)
{
	...
	conf = bt_buf_add(buf, sizeof(*conf));
...
l2cap_br_conn_security(struct bt_l2cap_chan *chan, const uint16_t psm)
{
	...
	if (check == 0) {
		 ...
		 */
		bt_atomic_set_bit(chan->conn->flags, BT_CONN_BR_GENERAL_BONDING);
...
static void l2cap_br_conn_rsp_sent_cb(struct bt_conn *conn, void *user_data, int err)
{
	...
	if (bt_atomic_test_and_clear_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_DISCONNECT_ACL)) {
...
static int l2cap_br_conn_req_reply(struct bt_l2cap_chan *chan, uint16_t result)
{
	...
	if (!bt_atomic_test_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_CONN_ACCEPTOR)) {
		return -ESRCH;
	}
...
void bt_l2cap_br_chan_del(struct bt_l2cap_chan *chan)
{
	...
	while (chan_has_data(br_chan)) {
		const bt_snode_t *tx_buf = bt_slist_get(&br_chan->_pdu_tx_queue);

		struct bt_buf *buf = CONTAINER_OF(tx_buf, struct bt_buf, node);
			      ...
			      struct bt_buf *buf)
{
	...
	bt_atomic_set_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_CONN_ACCEPTOR);
	...
	default:
		...
		bt_atomic_set_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_DISCONNECT_ACL);
		break;
	}
...
static uint16_t l2cap_br_conf_rsp_opt_mtu(struct bt_l2cap_chan *chan, struct bt_buf *buf,
					  size_t len)
{
...
static uint16_t l2cap_br_conf_rsp_opt_flush_timeout(struct bt_l2cap_chan *chan, struct bt_buf *buf,
						    size_t len)
{
...
static uint16_t l2cap_br_conf_rsp_opt_qos(struct bt_l2cap_chan *chan, struct bt_buf *buf,
					  size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
...
#if defined(CONFIG_BT_L2CAP_RET_FC)
static uint16_t l2cap_br_conf_rsp_opt_ret_fc(struct bt_l2cap_chan *chan, struct bt_buf *buf,
					     size_t len)
{
...
static uint16_t l2cap_br_conf_rsp_opt_fcs(struct bt_l2cap_chan *chan, struct bt_buf *buf,
					  size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
...
static uint16_t l2cap_br_conf_rsp_opt_ext_flow_spec(struct bt_l2cap_chan *chan, struct bt_buf *buf,
						    size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
...
static uint16_t l2cap_br_conf_rsp_opt_ext_win_size(struct bt_l2cap_chan *chan, struct bt_buf *buf,
						   size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
				       ...
				       struct bt_buf *buf)
{
	...
	while (opt_len >= sizeof(*opt)) {
		opt = bt_buf_pull_mem(buf, sizeof(*opt));
...
static uint16_t l2cap_br_conf_rsp_unaccept_opt_mtu(struct bt_l2cap_chan *chan, struct bt_buf *buf,
						   size_t len)
{
	uint16_t mtu, result = BT_L2CAP_CONF_SUCCESS;
	struct bt_l2cap_conf_opt_mtu *opt_mtu;
...
static uint16_t l2cap_br_conf_rsp_unaccept_opt_qos(struct bt_l2cap_chan *chan, struct bt_buf *buf,
						   size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
	struct bt_l2cap_conf_opt_qos *opt_qos;
...
static uint16_t l2cap_br_conf_rsp_unaccept_opt_fcs(struct bt_l2cap_chan *chan, struct bt_buf *buf,
						   size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
	struct bt_l2cap_conf_opt_fcs *opt_fcs;
			      ...
			      struct bt_buf *buf)
{
	...
	case BT_L2CAP_CONF_SUCCESS:
		bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_CONN_LCONF_DONE);
		...
		if (br_chan->state == BT_L2CAP_CONFIG &&
		    bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_CONN_RCONF_DONE)) {
...
int bt_l2cap_br_server_register(struct bt_l2cap_server *server)
{
	...
	if (bt_slist_find(&br_servers, &server->node, NULL)) {
		return -EEXIST;
	}
	...
	bt_slist_append(&br_servers, &server->node);
...
int bt_l2cap_br_server_unregister(struct bt_l2cap_server *server)
{
	...
	if (!bt_slist_find_and_remove(&br_servers, &server->node)) {
		return -ENOENT;
	}
				 ...
				 uint8_t data_len)
{
	 ...
	 */
	if (data) {
		bt_buf_add_mem(buf, data, data_len);
	}

	l2cap_send(conn, BT_L2CAP_CID_BR_SIG, buf);
}

static uint16_t l2cap_br_conf_opt_mtu(struct bt_l2cap_chan *chan, struct bt_buf *buf, size_t len)
{
	uint16_t mtu, result = BT_L2CAP_CONF_SUCCESS;
	struct bt_l2cap_conf_opt_mtu *opt_mtu;
...
static uint16_t l2cap_br_conf_opt_flush_timeout(struct bt_l2cap_chan *chan, struct bt_buf *buf,
						size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
	struct bt_l2cap_conf_opt_flush_timeout *opt_to;
...
static uint16_t l2cap_br_conf_opt_qos(struct bt_l2cap_chan *chan, struct bt_buf *buf, size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
	struct bt_l2cap_conf_opt_qos *opt_qos;
	struct bt_l2cap_chan *chan_sig;
...
#if defined(CONFIG_BT_L2CAP_RET_FC)
static uint16_t l2cap_br_conf_opt_ret_fc(struct bt_l2cap_chan *chan, struct bt_buf *buf,
					 size_t len)
{
	uint16_t result = BT_L2CAP_CONF_UNACCEPT;
	struct bt_l2cap_conf_opt_ret_fc *opt_ret_fc;
...
static uint16_t l2cap_br_conf_opt_fcs(struct bt_l2cap_chan *chan, struct bt_buf *buf, size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
	struct bt_l2cap_conf_opt_fcs *opt_fcs;
	struct bt_l2cap_br_chan *br_chan;
...
static uint16_t l2cap_br_conf_opt_ext_flow_spec(struct bt_l2cap_chan *chan, struct bt_buf *buf,
						size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
	struct bt_l2cap_conf_opt_ext_flow_spec *ext_flow_spec;
...
static uint16_t l2cap_br_conf_opt_ext_win_size(struct bt_l2cap_chan *chan, struct bt_buf *buf,
					       size_t len)
{
	uint16_t result = BT_L2CAP_CONF_SUCCESS;
	struct bt_l2cap_conf_opt_ext_win_size *opt_ext_win_size;
			      ...
			      struct bt_buf *buf)
{
	...
	req = bt_buf_pull_mem(buf, sizeof(*req));
	...
	hdr = bt_buf_add(rsp_buf, sizeof(*hdr));
...
send_rsp:
	...
	bt_atomic_set_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_CONN_RCONF_DONE);

	if (bt_atomic_test_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_CONN_LCONF_DONE) &&
...
static struct bt_l2cap_br_chan *l2cap_br_remove_tx_cid(struct bt_conn *conn, uint16_t cid)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&conn->channels, chan, node) {
		if (BR_CHAN(chan)->tx.cid == cid) {
			bt_slist_remove(&conn->channels, prev, &chan->node);
			return BR_CHAN(chan);
		}
...
static void l2cap_br_disconn_req(struct bt_l2cap_br *l2cap, uint8_t ident, struct bt_buf *buf)
{
	struct bt_conn *conn = l2cap->chan.chan.conn;
	struct bt_l2cap_br_chan *chan;
...
static void l2cap_br_disconnected(struct bt_l2cap_chan *chan)
{
	...
	if (bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_SIG_INFO_PENDING)) {
		/* Cancel RTX work on signal channel.
		 * Disconnected callback is always called from system workqueue
		 * so this should always succeed.
		 */
		(void)bt_work_cancel_delayable(&br_chan->rtx_work);
		br_chan->ident = 0;
	}
}
...
static void l2cap_br_disconn_rsp(struct bt_l2cap_br *l2cap, uint8_t ident, struct bt_buf *buf)
{
	struct bt_conn *conn = l2cap->chan.chan.conn;
	struct bt_l2cap_br_chan *chan;
...
int bt_l2cap_br_chan_connect(struct bt_conn *conn, struct bt_l2cap_chan *chan, uint16_t psm)
{
	...
	br_chan->psm = psm;
	bt_l2cap_br_chan_set_state(chan, BT_L2CAP_CONNECTING);
	bt_atomic_set_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_CONN_PENDING);
...
static void l2cap_br_conn_rsp(struct bt_l2cap_br *l2cap, uint8_t ident, struct bt_buf *buf)
{
	struct bt_conn *conn = l2cap->chan.chan.conn;
	struct bt_l2cap_chan *chan;
	...
	case BT_L2CAP_BR_SUCCESS:
		...
		l2cap_br_conf(chan, true);
		bt_l2cap_br_chan_set_state(chan, BT_L2CAP_CONFIG);
		bt_atomic_clear_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_CONN_PENDING);
		break;
...
int bt_l2cap_br_chan_send_cb(struct bt_l2cap_chan *chan, struct bt_buf *buf, bt_conn_tx_cb_t cb,
			     void *user_data)
{
	struct bt_l2cap_br_chan *br_chan;
	...
	LOG_DBG("chan %p buf %p len %zu", chan, buf, bt_buf_frags_len(buf));
...
int bt_l2cap_br_chan_send(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
...
static void l2cap_br_reject_rsp(struct bt_l2cap_br *l2cap, uint8_t ident, struct bt_buf *buf)
{
	struct bt_conn *conn = l2cap->chan.chan.conn;
	struct bt_l2cap_br_chan *chan;
...
static void l2cap_br_echo_req(struct bt_l2cap_br *l2cap, uint8_t ident, struct bt_buf *buf)
{
	struct bt_conn *conn = l2cap->chan.chan.conn;
	struct bt_l2cap_br_echo_cb *callback;

	BT_SLIST_FOR_EACH_CONTAINER(&bt_l2cap_br_echo_cbs, callback, _node) {
		if (callback->req) {
...
static void l2cap_br_echo_rsp(struct bt_l2cap_br *l2cap, uint8_t ident, struct bt_buf *buf)
{
	struct bt_conn *conn = l2cap->chan.chan.conn;
	struct bt_l2cap_br_echo_cb *callback;
				...
				struct bt_buf *buf)
{
	...
	(void)bt_buf_pull_mem(buf, len);
}

static int l2cap_br_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
...
static void l2cap_br_conn_pend(struct bt_l2cap_chan *chan, uint8_t status)
{
	...
	if (status) {
		...
		if (bt_atomic_test_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_CONN_PENDING)) {
			l2cap_br_chan_cleanup(chan);
		}
	...
	if (l2cap_br_conn_req_reply(chan, BT_L2CAP_BR_SUCCESS) == 0) {
		 ...
		 */
		l2cap_br_conf(chan, true);
	} else if (bt_atomic_test_and_clear_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_CONN_PENDING)) {
		buf = bt_l2cap_create_pdu(&br_sig_pool, 0);
...
static void bt_l2cap_br_update_r(struct bt_l2cap_br_chan *br_chan, uint8_t r)
{
	...
	if (!r) {
		if (!bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R)) {
			/* R bit without any change */
			return;
		}

		/* R bit is changed from 1 -> 0 */
		bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R_CHANGED);
		if (bt_l2cap_br_get_outstanding_count(br_chan)) {
		...
		if (bt_atomic_test_and_set_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_R)) {
			/* R bit without any change */
			return;
		}
...
static int bt_l2cap_br_update_f(struct bt_l2cap_br_chan *br_chan, uint8_t f)
{
	...
	if (f) {
		if (!bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P_CHANGED)) {
			if (!bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P)) {
...
static void bt_l2cap_br_ret_fc_s_recv(struct bt_l2cap_br_chan *br_chan, struct bt_buf *buf)
{
	uint16_t control;
	uint32_t ext_control;
	uint16_t req_seq;
	...
	if ((br_chan->rx.mode == BT_L2CAP_BR_LINK_MODE_ERET) ||
		...
		if (br_chan->rx.extended_control) {
			ext_control = bt_buf_pull_le32(buf);
	...
	case BT_L2CAP_CONTROL_S_RR:
		if (p) {
			bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_RECV_FRAME_P);
		} else {
			if (!bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ)) {
				bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_NEW_I_FRAME);
			} else if (f) {
				bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_NEW_I_FRAME);
			}
		...
		remote_busy = bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_REMOTE_BUSY);
		bt_atomic_clear_bit(br_chan->flags, L2CAP_FLAG_REMOTE_BUSY);

		if (f && !bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_REJ_ACTIONED)) {
			/* Retransmit I-frames */
			bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAME);
			bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_REQ_SEQ_UPDATED);
		}
	...
	case BT_L2CAP_CONTROL_S_REJ:
		...
		if (!f && bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P)) {
			bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_REJ_ACTIONED);
		}

		if (!f && bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ)) {
			bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAME);
			bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_REQ_SEQ_UPDATED);
		}
	...
	case BT_L2CAP_CONTROL_S_RNR:
		...
		bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_REMOTE_BUSY);
		bt_l2cap_br_update_req_seq(br_chan, req_seq, false);
		...
		if (!bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ)) {
			l2cap_br_start_timer(br_chan, BT_L2CAP_BR_TIMER_MONITOR, true);
			bt_atomic_clear_bit(br_chan->flags, L2CAP_FLAG_RET_I_FRAME);
		}

		if (p || bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ)) {
	...
	case BT_L2CAP_CONTROL_S_SREJ:
		...
		if (f) {
			if (bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_SREJ_ACTIONED) &&
			    (br_chan->srej_save_req_seq == req_seq)) {
				/* Clear flag L2CAP_FLAG_SREJ_ACTIONED */
			} else {
				/* Retransmit-Requested-I-frame */
				bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_RET_REQ_I_FRAME);
			}
		} else {
			bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_RET_REQ_I_FRAME);
			if (bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P)) {
				bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_SREJ_ACTIONED);
				br_chan->srej_save_req_seq = req_seq;
			}
		}
...
static void bt_l2cap_br_recv_seg_direct(struct bt_l2cap_br_chan *br_chan, struct bt_buf *seg,
					uint8_t sar)
{
	...
	case BT_L2CAP_CONTROL_SAR_START:
		if (sar == BT_L2CAP_CONTROL_SAR_START) {
			br_chan->_sdu_len = bt_buf_pull_le16(seg);
		} else {
...
static struct bt_buf *l2cap_br_alloc_frag(os_timeout_t timeout, void *user_data)
{
	...
	LOG_DBG("frag %p tailroom %zu", frag, bt_buf_tailroom(frag));

	return frag;
}

static int bt_l2cap_br_recv_seg(struct bt_l2cap_br_chan *br_chan, struct bt_buf *seg, uint8_t sar)
{
	uint16_t len;
	struct bt_buf *buf;
	...
	len = bt_buf_append_bytes(br_chan->_sdu, seg->len, seg->data, OS_TIMEOUT_NO_WAIT,
...
static void bt_l2cap_br_rej_exception(struct bt_l2cap_br_chan *br_chan)
{
	...
	if (bt_atomic_test_and_set_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ)) {
		return;
	}

	LOG_DBG("Set REJ Exception received flag");
	bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ_CHANGED);
	err = l2cap_br_send_s_frame(br_chan, BT_L2CAP_CONTROL_S_RR, OS_TIMEOUT_NO_WAIT);
	if (err) {
		LOG_ERR("Fail to send frame %d on %p", err, br_chan);
		bt_l2cap_chan_disconnect(&br_chan->chan);
...
static void bt_l2cap_br_ret_fc_i_recv(struct bt_l2cap_br_chan *br_chan, struct bt_buf *buf)
{
	uint16_t control;
	uint32_t ext_control;
	uint16_t req_seq;
	...
	if (br_chan->req_seq == tx_seq) {
		...
		bt_atomic_clear_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_REJ);
	}
...
valid_frame:
	...
	if (br_chan->chan.ops->alloc_buf) {
		...
		if ((br_chan->rx.mode == BT_L2CAP_BR_LINK_MODE_ERET) &&
		    (!bt_atomic_test_and_set_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY))) {
			bt_atomic_set_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY_CHANGED);
			err = l2cap_br_send_s_frame(br_chan, BT_L2CAP_CONTROL_S_RR, OS_TIMEOUT_NO_WAIT);
			if (err) {
				LOG_ERR("Fail to send frame %d on %p", err, br_chan);
				bt_l2cap_chan_disconnect(&br_chan->chan);
...
static bool bt_l2cap_br_check_valid_fcs(struct bt_l2cap_br_chan *br_chan, struct bt_buf *buf)
{
	if (br_chan->rx.mode != BT_L2CAP_BR_LINK_MODE_BASIC) {
...
static void bt_l2cap_br_ret_fc_recv(struct bt_l2cap_br_chan *br_chan, struct bt_buf *buf)
{
	struct bt_l2cap_hdr *hdr;
	uint16_t control;
	uint32_t ext_control;
	...
	bt_buf_pull_mem(buf, sizeof(*hdr));
...
void bt_l2cap_br_recv(struct bt_conn *conn, struct bt_buf *buf)
{
	struct bt_l2cap_hdr *hdr;
	struct bt_l2cap_chan *chan;
	uint16_t cid;
...
int bt_l2cap_br_chan_recv_complete(struct bt_l2cap_chan *chan)
{
	...
	if ((br_chan->rx.mode == BT_L2CAP_BR_LINK_MODE_ERET) &&
	    bt_atomic_test_and_clear_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY)) {

		if (!bt_atomic_test_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY_CHANGED) &&
		    !bt_atomic_test_and_set_bit(br_chan->flags, L2CAP_FLAG_SEND_FRAME_P)) {
			/* State: XMIT
			 * Local Busy Clears
			 * Action: send RR(P=1)
			 */
			...
			bt_atomic_clear_bit(br_chan->flags, L2CAP_FLAG_LOCAL_BUSY_CHANGED);
		}
	}
...
static int l2cap_br_accept(struct bt_conn *conn, struct bt_l2cap_chan **chan)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_l2cap_br_pool); i++) {
		...
		*chan = &l2cap->chan.chan;
		bt_atomic_set(l2cap->chan.flags, 0);
		return 0;
	}
...
int bt_l2cap_br_echo_cb_register(struct bt_l2cap_br_echo_cb *cb)
{
	...
	if (bt_slist_find(&bt_l2cap_br_echo_cbs, &cb->_node, NULL)) {
		return -EEXIST;
	}

	bt_slist_append(&bt_l2cap_br_echo_cbs, &cb->_node);
...
int bt_l2cap_br_echo_cb_unregister(struct bt_l2cap_br_echo_cb *cb)
{
	...
	if (!bt_slist_find_and_remove(&bt_l2cap_br_echo_cbs, &cb->_node)) {
		return -ENOENT;
	}
...
int bt_l2cap_br_echo_req(struct bt_conn *conn, struct bt_buf *buf)
{
	struct bt_l2cap_chan *chan;
	struct bt_l2cap_sig_hdr *hdr;
	int err;
	...
	if (bt_buf_headroom(buf) < BT_L2CAP_BR_ECHO_REQ_RESERVE) {
		/* Call `bt_buf_reserve(buf, BT_L2CAP_BR_ECHO_REQ_RESERVE)`
...
int bt_l2cap_br_echo_rsp(struct bt_conn *conn, uint8_t identifier, struct bt_buf *buf)
{
	struct bt_l2cap_chan *chan;
	struct bt_l2cap_sig_hdr *hdr;
	...
	if (bt_buf_headroom(buf) < BT_L2CAP_BR_ECHO_RSP_RESERVE) {
		/* Call `bt_buf_reserve(buf, BT_L2CAP_BR_ECHO_RSP_RESERVE)`
...
static bt_slist_t br_connless_cbs = BT_SLIST_STATIC_INIT(br_connless_cbs);
...
int bt_l2cap_br_connless_register(struct bt_l2cap_br_connless_cb *cb)
{
	...
	if (bt_slist_find(&br_connless_cbs, &cb->_node, NULL)) {
		return -EEXIST;
	}

	bt_slist_append(&br_connless_cbs, &cb->_node);
	return 0;
}
...
int bt_l2cap_br_connless_unregister(struct bt_l2cap_br_connless_cb *cb)
{
	...
	if (!bt_slist_find_and_remove(&br_connless_cbs, &cb->_node)) {
		return -ENOENT;
	}
...
int bt_l2cap_br_connless_send(struct bt_conn *conn, uint16_t psm, struct bt_buf *buf)
{
	struct bt_l2cap_chan *chan;
	...
	if (bt_buf_headroom(buf) < BT_L2CAP_CONNLESS_RESERVE) {
		/* Call `bt_buf_reserve(buf, BT_L2CAP_CONNLESS_RESERVE)`
...
static int l2cap_br_conless_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
	struct bt_conn *conn;
	uint16_t psm;
	...
	conn = chan->conn;
	psm = bt_buf_pull_le16(buf);

	/* Iterate through registered connless callbacks to find matching PSM */
	BT_SLIST_FOR_EACH_CONTAINER(&br_connless_cbs, cb, _node) {
