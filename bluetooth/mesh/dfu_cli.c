...
#define TARGETS_FOR_EACH(cli, target)                                          \
	BT_SLIST_FOR_EACH_CONTAINER(                                          \
		(bt_slist_t *)&((cli)->blob.inputs)->targets, target, blob.n)
...
static int req_wait(struct bt_mesh_dfu_cli *cli, os_timeout_t timeout)
{
	int err;

	err = os_sem_take(&cli->req.sem, timeout);
...
static int tx(const struct bt_mesh_model *mod, struct bt_mesh_msg_ctx *ctx,
	      struct bt_buf_simple *buf, const struct bt_mesh_send_cb *cb,
		    ...
		    const struct bt_mesh_send_cb *cb)
{
	...
	bt_buf_simple_add_u8(&buf, idx);
	bt_buf_simple_add_u8(&buf, max_count);
...
static void send_update_start(struct bt_mesh_blob_cli *b, uint16_t dst)
{
	...
	if (b->tx.ctx.force_unicast) {
		...
		target = BT_SLIST_PEEK_HEAD_CONTAINER(
						(bt_slist_t *)&((cli)->blob.inputs)->targets,
						target, blob.n);
	}
	...
	bt_buf_simple_add_u8(&buf, cli->blob.inputs->ttl);
	bt_buf_simple_add_le16(&buf, cli->blob.inputs->timeout_base);
	bt_buf_simple_add_le64(&buf, cli->xfer.blob.id);
	bt_buf_simple_add_u8(&buf, target->img_idx);
	bt_buf_simple_add_mem(&buf, cli->xfer.slot->metadata,
			 ...
			 struct bt_buf_simple *buf)
{
	...
	if (cli->req.type == REQ_STATUS && cli->req.addr == ctx->addr) {
		if (cli->req.params) {
			...
			rsp->phase = phase;
			if (buf->len == 13) {
				rsp->ttl = bt_buf_simple_pull_u8(buf);
				rsp->effect = bt_buf_simple_pull_u8(buf) & BIT_MASK(5);
				rsp->timeout_base = bt_buf_simple_pull_le16(buf);
				rsp->blob_id = bt_buf_simple_pull_le64(buf);
				rsp->img_idx = bt_buf_simple_pull_u8(buf);
			} else if (buf->len) {
				return -EINVAL;
			}
	...
	if (buf->len == 13) {
		bt_buf_simple_pull_u8(buf); /* ttl */
		target->effect = bt_buf_simple_pull_u8(buf) & BIT_MASK(5);
		bt_buf_simple_pull_le16(buf); /* timeout */

		if (bt_buf_simple_pull_le64(buf) != cli->xfer.blob.id) {
		...
		target->img_idx = bt_buf_simple_pull_u8(buf);
			      ...
			      struct bt_buf_simple *buf)
{
	...
	img_cnt = bt_buf_simple_pull_u8(buf);
	if (img_cnt < cli->req.img_cnt) {
		cli->req.img_cnt = img_cnt;
	}

	idx = bt_buf_simple_pull_u8(buf);
	if (idx >= img_cnt) {
	...
	while (buf->len && cli->req.img_cb && idx < cli->req.img_cnt) {
		...
		img.fwid_len = bt_buf_simple_pull_u8(buf);
		if (buf->len < img.fwid_len + 1) {
			LOG_WRN("Invalid format: fwid");
			return -EINVAL;
		}

		img.fwid = bt_buf_simple_pull_mem(buf, img.fwid_len);
		...
		if (uri_len) {
			...
			memcpy(uri_buf, bt_buf_simple_pull_mem(buf, uri_len),
			       uri_buf_len);
			   ...
			   struct bt_mesh_msg_ctx *ctx)
{
	if (ctx) {
		...
		return req_wait(cli, OS_MSEC(dfu_cli_timeout));
	}
			     ...
			     uint8_t max_count)
{
	...
	err = req_wait(cli, OS_MSEC(dfu_cli_timeout));
				   ...
				   struct bt_mesh_dfu_metadata_status *rsp)
{
	...
	bt_buf_simple_add_u8(&buf, img_idx);

	if (slot->metadata_len) {
		bt_buf_simple_add_mem(&buf, slot->metadata,
				       slot->metadata_len);
	}
