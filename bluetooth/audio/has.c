...
#if defined(CONFIG_BT_HAS_PRESET_SUPPORT) || defined(CONFIG_BT_HAS_FEATURES_NOTIFIABLE)
	(void)bt_work_cancel_delayable(&client->notify_work);
...
static struct has_client *client_alloc(struct bt_conn *conn)
{
	...
	__ASSERT_MSG(client, "failed to get client for conn %p", (void *)conn);
...
#if defined(CONFIG_BT_HAS_PRESET_SUPPORT) || defined(CONFIG_BT_HAS_FEATURES_NOTIFIABLE)
	bt_work_init_delayable(&client->notify_work, notify_work_handler);
#endif /* CONFIG_BT_HAS_PRESET_SUPPORT || CONFIG_BT_HAS_FEATURES_NOTIFIABLE */
...
static void notify_work_reschedule(struct has_client *client, os_timeout_t delay)
{
	int err;

	__ASSERT_MSG(client->conn, "Not connected");

	if (bt_work_delayable_remaining_get(&client->notify_work) > 0) {
		return;
	}

	err = bt_work_reschedule(&client->notify_work, delay);
	if (err < 0) {
...
static bt_slist_t preset_list = BT_SLIST_STATIC_INIT(&preset_list);
static bt_slist_t preset_free_list = BT_SLIST_STATIC_INIT(&preset_free_list);
			   ...
			   void *user_data)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&preset_list, preset, tmp, node) {
...
static void preset_insert(struct has_preset *preset)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&preset_list, tmp, node) {
		if (tmp->index > preset->index) {
			if (prev) {
				bt_slist_insert(&preset_list, &prev->node, &preset->node);
			} else {
				bt_slist_prepend(&preset_list, &preset->node);
			}
			return;
		}
	...
	bt_slist_append(&preset_list, &preset->node);
}
				       ...
				       const char *name, const struct bt_has_preset_ops *ops)
{
	...
	node = bt_slist_get(&preset_free_list);
	if (node == NULL) {
		return NULL;
	}
...
static void preset_free(struct has_preset *preset)
{
	...
	removed = bt_slist_find_and_remove(&preset_list, &preset->node);
	if (removed) {
		bt_slist_append(&preset_free_list, &preset->node);
	}
}
...
static struct has_preset *preset_get_head(void)
{
	...
	return BT_SLIST_PEEK_HEAD_CONTAINER(&preset_list, next, node);
}
...
static struct has_preset *preset_get_tail(void)
{
	...
	return BT_SLIST_PEEK_TAIL_CONTAINER(&preset_list, prev, node);
}
...
static struct has_preset *preset_get_prev(const struct has_preset *preset)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&preset_list, prev, node) {
		if (BT_SLIST_PEEK_NEXT_CONTAINER(prev, node) == preset) {
			return prev;
		}
	}
...
static struct has_preset *preset_lookup_index(uint8_t index)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&preset_list, preset, node) {
...
static struct has_preset *preset_get_next(struct has_preset *preset)
{
	...
	next = BT_SLIST_PEEK_NEXT_CONTAINER(preset, node);
	if (next == NULL) {
...
static int control_point_send(struct has_client *client, struct bt_buf_simple *buf)
{
...
static int control_point_send_all(struct bt_buf_simple *buf)
{
	int result = 0;
				     ...
				     bool is_last)
{
	...
	BT_BUF_SIMPLE_DEFINE(buf, sizeof(*hdr) + sizeof(*rsp) + BT_HAS_PRESET_NAME_MAX);
	...
	hdr = bt_buf_simple_add(&buf, sizeof(*hdr));
	hdr->opcode = BT_HAS_OP_READ_PRESET_RSP;
	rsp = bt_buf_simple_add(&buf, sizeof(*rsp));
	rsp->is_last = is_last ? 0x01 : 0x00;
	rsp->index = preset->index;
	rsp->properties = preset->properties;
	bt_buf_simple_add_mem(&buf, preset->name, strlen(preset->name));

	return control_point_send(client, &buf);
}

static void preset_changed_prepare(struct bt_buf_simple *buf, uint8_t change_id, uint8_t is_last)
{
	struct bt_has_cp_hdr *hdr;
	...
	preset_changed = bt_buf_simple_add(buf, sizeof(*preset_changed));
				    ...
				    uint8_t properties, const char *name, uint8_t is_last)
{
	...
	BT_BUF_SIMPLE_DEFINE(buf, sizeof(struct bt_has_cp_hdr) +
	...
	generic_update = bt_buf_simple_add(&buf, sizeof(*generic_update));
	generic_update->prev_index = prev_index;
	generic_update->index = index;
	generic_update->properties = properties;
	bt_buf_simple_add_mem(&buf, name, strlen(name));
...
static int bt_has_cp_preset_record_deleted(struct has_client *client, uint8_t index)
{
	...
	bt_buf_simple_add_u8(&buf, index);
...
static uint8_t handle_read_preset_req(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static int set_preset_name(uint8_t index, const char *name, size_t len)
{
	...
	IF_ENABLED(CONFIG_BT_HAS_PRESET_NAME_DYNAMIC, (
		__ASSERT_MSG(len < ARRAY_SIZE(preset->name), "No space for name");
...
static uint8_t handle_write_preset_name(struct bt_conn *conn, struct bt_buf_simple *buf)
{
...
static uint8_t handle_set_active_preset(struct bt_buf_simple *buf, bool sync)
{
	...
	pdu = bt_buf_simple_pull_mem(buf, sizeof(*pdu));
...
static uint8_t handle_control_point_op(struct bt_conn *conn, struct bt_buf_simple *buf)
{
	const struct bt_has_cp_hdr *hdr;

	hdr = bt_buf_simple_pull_mem(buf, sizeof(*hdr));
...
static int set_preset_availability(uint8_t index, bool available)
{
	...
	bt_buf_simple_add_u8(&buf, preset->index);
