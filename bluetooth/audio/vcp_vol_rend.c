				   ...
				   os_timeout_t delay)
{
	...
	if (err < 0) {
		...
		LOG_DBG("%s notification scheduled in %dms", vol_rend_notify_str(notify),
			bt_work_delayable_remaining_get(&inst->notify_work));
	}
}
		   ...
		   const struct bt_uuid *uuid, const void *data, uint16_t len)
{
	...
	if (err == -ENOMEM) {
		notify_work_reschedule(inst, notify, OS_USEC(BT_AUDIO_NOTIFY_RETRY_DELAY_US));
...
{
	notify_work_reschedule(inst, notify, OS_TIMEOUT_NO_WAIT);
}
