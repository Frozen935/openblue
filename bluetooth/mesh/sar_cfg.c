...
{
	bt_buf_simple_add_u8(buf, (tx->seg_int_step & 0xf) |
					   (tx->unicast_retrans_count << 4));
	bt_buf_simple_add_u8(buf, (tx->unicast_retrans_without_prog_count &
				    0xf) | (tx->unicast_retrans_int_step << 4));
	bt_buf_simple_add_u8(buf, (tx->unicast_retrans_int_inc & 0xf) |
					   (tx->multicast_retrans_count << 4));
	bt_buf_simple_add_u8(buf, tx->multicast_retrans_int & 0xf);
}

void bt_mesh_sar_rx_encode(struct bt_buf_simple *buf,
			   const struct bt_mesh_sar_rx *rx)
{
	bt_buf_simple_add_u8(buf, (rx->seg_thresh & 0x1f) |
					   (rx->ack_delay_inc << 5));
	bt_buf_simple_add_u8(buf, (rx->discard_timeout & 0xf) |
					   ((rx->rx_seg_int_step & 0xf) << 4));
	bt_buf_simple_add_u8(buf, (rx->ack_retrans_count & 0x3));
}

void bt_mesh_sar_tx_decode(struct bt_buf_simple *buf,
			   struct bt_mesh_sar_tx *tx)
{
	uint8_t val;

	val = bt_buf_simple_pull_u8(buf);
...
void bt_mesh_sar_rx_decode(struct bt_buf_simple *buf,
