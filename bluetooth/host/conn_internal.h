...
struct bt_conn {
	...
	bt_slist_t		tx_pending;

	/* Completed TX for which we need to call the callback */
	bt_slist_t		tx_complete;
	 ...
	 * - Connection cleanup.
	 */
	struct bt_work_delayable	deferred_work;
	 ...
	 *
	 * Details about the returned bt_buf when it is not NULL:
	 *   - If the bt_buf->len <= *length, then the bt_buf has been removed
	 *     from the tx_queue of the connection and the caller is now the
	 *     owner of the only reference to the bt_buf. The caller now has to
	 *     invoke get_and_clear_cb to get the callback and user-data for the
	 *     bt_buf and is responsible for invoking the callback eventually
	 ...
	 *   - Otherwise, the bt_buf is still on the tx_queue of the connection,
	 *     and the callback has incremented the reference count to account
	 *     for it having a reference still.
	 *   - The caller must consume *length bytes from the bt_buf before
	 *     calling this function again.
	 */
	struct bt_buf * (*tx_data_pull)(struct bt_conn *conn,
					 size_t amount,
					 size_t *length);

	/* Get (and clears for ACL conns) callback and user-data for `buf`. */
	void (*get_and_clear_cb)(struct bt_conn *conn, struct bt_buf *buf,
 ...
 *
 * We pack the struct to make sure it fits in the bt_buf user_data field.
 */
struct closure {
	void *cb;
	void *data;
 ...
 * ignored.
 */
int bt_conn_send_cb(struct bt_conn *conn, struct bt_buf *buf,
 ...
 */
int bt_conn_send_iso_cb(struct bt_conn *conn, struct bt_buf *buf,
...
#define bt_conn_create_pdu(_pool, _reserve) \
	bt_conn_create_pdu_timeout_debug(_pool, _reserve, OS_TIMEOUT_FOREVER, \
					 __func__, __LINE__)
#else
...
#define bt_conn_create_pdu(_pool, _reserve) \
	bt_conn_create_pdu_timeout(_pool, _reserve, OS_TIMEOUT_FOREVER)
#endif

/* Prepare a PDU to be sent over a connection */
#if defined(CONFIG_BT_BUF_LOG)
struct bt_buf *bt_conn_create_frag_timeout_debug(size_t reserve,
						  os_timeout_t timeout,
						  const char *func, int line);
...
#define bt_conn_create_frag(_reserve) \
	bt_conn_create_frag_timeout_debug(_reserve, OS_TIMEOUT_FOREVER, \
					  __func__, __LINE__)
#else
struct bt_buf *bt_conn_create_frag_timeout(size_t reserve,
					    os_timeout_t timeout);

#define bt_conn_create_frag(_reserve) \
	bt_conn_create_frag_timeout(_reserve, OS_TIMEOUT_FOREVER)
#endif
