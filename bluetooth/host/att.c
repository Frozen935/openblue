...
BT_BUF_POOL_DEFINE(prep_pool, CONFIG_BT_ATT_PREPARE_COUNT, BT_ATT_BUF_SIZE,
...
struct bt_att_chan {
	...
	struct bt_work_delayable	timeout_work;
	bt_snode_t		node;
};
...
struct bt_att {
	...
	struct bt_fifo		tx_queue;
#if CONFIG_BT_ATT_PREPARE_COUNT > 0
	bt_slist_t		prep_queue;
#endif
	/* Contains bt_att_chan instance(s) */
	bt_slist_t		chans;
#if defined(CONFIG_BT_EATT)
	struct {
		struct bt_work_delayable connection_work;
 ...
 */
static os_tid_t att_handle_rsp_thread;
...
static struct bt_att_tx_meta_data *att_get_tx_meta_data(const struct bt_buf *buf)
{
	 ...
	 */
	return &tx_meta_data_storage[bt_buf_id((struct bt_buf *)buf)];
}
...
static struct bt_buf *att_create_rsp_pdu(struct bt_att_chan *chan, uint8_t op);
 ...
 */
static int chan_send(struct bt_att_chan *chan, struct bt_buf *buf)
{
...
static struct bt_buf *get_first_buf_matching_chan(struct bt_fifo *fifo, struct bt_att_chan *chan)
{
	if (IS_ENABLED(CONFIG_BT_EATT)) {
		...
		bt_fifo_init(&skipped);

		while ((buf = bt_fifo_get(fifo, OS_TIMEOUT_NO_WAIT))) {
			meta = att_get_tx_meta_data(buf);
			if (!ret &&
			    att_chan_matches_chan_opt(chan, meta->chan_opt)) {
				ret = buf;
			} else {
				bt_fifo_put(&skipped, buf);
			}
		}

		__ASSERT_NO_MSG(bt_fifo_is_empty(fifo));

		while ((buf = bt_fifo_get(&skipped, OS_TIMEOUT_NO_WAIT))) {
			bt_fifo_put(fifo, buf);
		}

		__ASSERT_NO_MSG(bt_fifo_is_empty(&skipped));

		return ret;
	} else {
		return bt_fifo_get(fifo, OS_TIMEOUT_NO_WAIT);
	}
}

static struct bt_att_req *get_first_req_matching_chan(bt_slist_t *reqs, struct bt_att_chan *chan)
{
	if (IS_ENABLED(CONFIG_BT_EATT)) {
		bt_snode_t *curr, *prev = NULL;
		struct bt_att_tx_meta_data *meta = NULL;

		BT_SLIST_FOR_EACH_NODE(reqs, curr) {
		...
		if (curr) {
			bt_slist_remove(reqs, prev, curr);
	...
	bt_snode_t *node = bt_slist_get(reqs);
...
static int process_queue(struct bt_att_chan *chan, struct bt_fifo *queue)
{
	struct bt_buf *buf;
	int err;
	...
	if (buf) {
		...
		if (err) {
			...
			bt_queue_prepend(&queue->_queue, buf);
			return err;
		}
...
static int bt_att_chan_req_send(struct bt_att_chan *chan, struct bt_att_req *req)
{
	...
	if (bt_att_mtu(chan) < bt_buf_frags_len(req->buf)) {
		return -EMSGSIZE;
	}

	LOG_DBG("chan %p req %p len %zu", chan, req, bt_buf_frags_len(req->buf));
...
static void bt_att_sent(struct bt_l2cap_chan *ch)
{
	 ...
	 * request queue.
	 */
	if (!chan->req && !bt_slist_is_empty(&att->reqs)) {
		bt_snode_t *node = bt_slist_get(&att->reqs);
		...
		bt_slist_prepend(&att->reqs, node);
	}
...
static struct bt_buf *bt_att_chan_create_pdu(struct bt_att_chan *chan, uint8_t op, size_t len)
{
	struct bt_att_hdr *hdr;
	struct bt_buf *buf;
	struct bt_att_tx_meta_data *data;
	...
	if (IS_ENABLED(CONFIG_BT_EATT)) {
		bt_buf_reserve(buf, BT_L2CAP_SDU_BUF_SIZE(0));
	}
...
static void att_send_process(struct bt_att *att)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&att->chans, chan, tmp, node) {
		if (err == -ENOENT && prev &&
...
static void bt_att_chan_send_rsp(struct bt_att_chan *chan, struct bt_buf *buf)
{
	int err;
...
static uint8_t att_mtu_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
...
static void att_req_send_process(struct bt_att *att)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&att->chans, chan, tmp, node) {
		...
		/* Prepend back to the list as it could not be sent */
		bt_slist_prepend(&att->reqs, &req->node);
	}
}
			      ...
			      int err)
{
	...
	bt_work_cancel_delayable(&chan->timeout_work);
...
#if defined(CONFIG_BT_GATT_CLIENT)
static uint8_t att_mtu_rsp(struct bt_att_chan *chan, struct bt_buf *buf)
{
	struct bt_att_exchange_mtu_rsp *rsp;
	uint16_t mtu;
...
static uint8_t att_find_info_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
			    ...
			    void *user_data)
{
	...
	if (bt_att_mtu(chan) - bt_buf_frags_len(data->buf) <
	    sizeof(*data->group)) {
		return BT_GATT_ITER_STOP;
	}
	...
	/* Fast forward to next item position */
	data->group = bt_buf_add(frag, sizeof(*data->group));
...
static uint8_t att_find_type_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
...
typedef bool (*attr_read_cb)(struct bt_buf *buf, ssize_t read,
			     void *user_data);

static bool attr_read_type_cb(struct bt_buf *frag, ssize_t read,
			      void *user_data)
{
...
static ssize_t att_chan_read(struct bt_att_chan *chan,
			     ...
			     struct bt_buf *buf, uint16_t offset,
			     attr_read_cb cb, void *user_data)
{
	...
	if (bt_att_mtu(chan) <= bt_buf_frags_len(buf)) {
		return 0;
	}

	frag = bt_buf_frag_last(buf);
	...
	do {
		...
		bt_buf_add(frag, read);
		total += read;
		offset += read;
	} while (bt_att_mtu(chan) > bt_buf_frags_len(buf) && read == len);
			    ...
			    void *user_data)
{
	...
	/* Fast forward to next item position */
	data->item = bt_buf_add(bt_buf_frag_last(data->buf),
				 sizeof(*data->item));
	...
	return bt_att_mtu(chan) - bt_buf_frags_len(data->buf) >
...
static uint8_t att_read_type_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
...
static uint8_t att_read_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
...
static uint8_t att_read_blob_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
...
static uint8_t att_read_mult_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
	struct read_data data;
	uint16_t handle;
...
static uint8_t att_read_mult_vl_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
	struct read_data data;
	uint16_t handle;
...
static bool attr_read_group_cb(struct bt_buf *frag, ssize_t read,
			       void *user_data)
{
...
static uint8_t att_read_group_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
...
static uint8_t att_write_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
	uint16_t handle;
...
append:
	...
	data->buf = bt_buf_alloc(&prep_pool, OS_TIMEOUT_NO_WAIT);
	if (!data->buf) {
		data->err = BT_ATT_ERR_PREPARE_QUEUE_FULL;
		return BT_GATT_ITER_STOP;
	}

	attr_data = bt_buf_user_data(data->buf);
	attr_data->handle = handle;
	attr_data->offset = data->offset;

	bt_buf_add_mem(data->buf, data->value, data->len);
			       ...
			       uint16_t offset, const void *value, uint8_t len)
{
	...
	bt_buf_slist_put(&chan->att->prep_queue, data.buf);
...
static uint8_t att_prepare_write_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
...
{
	struct bt_buf *entry, *next;
	bt_snode_t *prev;

	prev = NULL;
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(list, entry, next, node) {
		struct bt_attr_data *tmp_data = bt_buf_user_data(entry);
		...
		if (tmp_data->handle == handle) {
			...
			bt_buf_simple_add_mem(buf, entry->data, entry->len);
			bt_slist_remove(list, prev, &entry->node);
			bt_buf_unref(entry);
		} else {
			prev = &entry->node;
		}
...
static uint8_t att_exec_write_rsp(struct bt_att_chan *chan, uint8_t flags)
{
	 ...
	 */
	while (!bt_slist_is_empty(&chan->att->prep_queue)) {
		struct bt_attr_data *data;
		uint16_t handle;

		BT_BUF_SIMPLE_DEFINE_STATIC(reassembled_data,
					     MIN(BT_ATT_MAX_ATTRIBUTE_LEN,
						 CONFIG_BT_ATT_PREPARE_COUNT * BT_ATT_BUF_SIZE));

		buf = bt_buf_slist_get(&chan->att->prep_queue);
		data = bt_buf_user_data(buf);
		handle = data->handle;

		LOG_DBG("buf %p handle 0x%04x offset %u", buf, handle, data->offset);

		bt_buf_simple_reset(&reassembled_data);
		bt_buf_simple_add_mem(&reassembled_data, buf->data, buf->len);
...
static uint8_t att_exec_write_req(struct bt_att_chan *chan, struct bt_buf *buf)
{
...
static uint8_t att_write_cmd(struct bt_att_chan *chan, struct bt_buf *buf)
{
	uint16_t handle;
...
static uint8_t att_signed_write_cmd(struct bt_att_chan *chan, struct bt_buf *buf)
{
	struct bt_conn *conn = chan->chan.chan.conn;
...
static uint8_t att_error_rsp(struct bt_att_chan *chan, struct bt_buf *buf)
{
	struct bt_att_error_rsp *rsp;
	uint8_t err;
...
static uint8_t att_notify(struct bt_att_chan *chan, struct bt_buf *buf)
{
	uint16_t handle;
...
static uint8_t att_indicate(struct bt_att_chan *chan, struct bt_buf *buf)
{
	uint16_t handle;
...
static uint8_t att_notify_mult(struct bt_att_chan *chan, struct bt_buf *buf)
{
	LOG_DBG("chan %p", chan);
...
static uint8_t att_confirm(struct bt_att_chan *chan, struct bt_buf *buf)
{
	LOG_DBG("");
...
static const struct att_handler {
	...
	uint8_t       expect_len;
	att_type_t type;
	uint8_t       (*func)(struct bt_att_chan *chan, struct bt_buf *buf);
} handlers[] = {
	{ BT_ATT_OP_MTU_REQ,
...
static int bt_att_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
...
static struct bt_buf *att_create_rsp_pdu(struct bt_att_chan *chan, uint8_t op)
{
	...
	buf = bt_buf_alloc(&att_pool, BT_ATT_TIMEOUT);
	if (!buf) {
...
static void att_reset(struct bt_att *att)
{
	...
	while ((buf = bt_buf_slist_get(&att->prep_queue))) {
		bt_buf_unref(buf);
	}
#endif /* CONFIG_BT_ATT_PREPARE_COUNT > 0 */
	...
	(void)bt_work_cancel_delayable_sync(&att->eatt.connection_work, &sync);
#endif /* CONFIG_BT_EATT */

	while ((buf = bt_fifo_get(&att->tx_queue, OS_TIMEOUT_NO_WAIT))) {
		bt_buf_unref(buf);
	}

	/* Notify pending requests */
	while (!bt_slist_is_empty(&att->reqs)) {
		struct bt_att_req *req;
		bt_snode_t *node;

		node = bt_slist_get_not_empty(&att->reqs);
	 ...
	 */
	att->conn = NULL;
	bt_mem_pool_free(&att_slab, (void *)att);
}
...
static void att_chan_detach(struct bt_att_chan *chan)
{
	...
	bt_slist_find_and_remove(&chan->att->chans, &chan->node);

	/* Release pending buffers */
	while ((buf = bt_fifo_get(&chan->tx_queue, OS_TIMEOUT_NO_WAIT))) {
		bt_buf_unref(buf);
	}
...
static void att_chan_attach(struct bt_att *att, struct bt_att_chan *chan)
{
	...
	if (bt_slist_is_empty(&att->chans)) {
		...
		bt_fifo_init(&att->tx_queue);
#if CONFIG_BT_ATT_PREPARE_COUNT > 0
		bt_slist_init(&att->prep_queue);
#endif
	}

	bt_slist_prepend(&att->chans, &chan->node);
}
...
static void bt_att_connected(struct bt_l2cap_chan *chan)
{
	...
	bt_work_init_delayable(&att_chan->timeout_work, att_timeout);
...
static void bt_att_status(struct bt_l2cap_chan *ch, atomic_t *status)
{
	...
	/* Pull next request from the list */
	node = bt_slist_get(&chan->att->reqs);
	if (!node) {
		return;
	}
	...
	/* Prepend back to the list as it could not be sent */
	bt_slist_prepend(&chan->att->reqs, node);
}
...
static void bt_att_released(struct bt_l2cap_chan *ch)
{
	...
	bt_mem_pool_free(&chan_slab, (void *)chan);
}
...
static struct bt_att_chan *att_chan_new(struct bt_att *att, atomic_val_t flags)
{
	...
	if (bt_mem_pool_alloc(&chan_slab, (void **)&chan, OS_TIMEOUT_NO_WAIT)) {
...
static void att_enhanced_connection_work_handler(struct bt_work *work)
{
	const struct bt_work_delayable *dwork = bt_work_delayable_from_work(work);
...
static int bt_att_accept(struct bt_conn *conn, struct bt_l2cap_chan **ch)
{
	...
	if (bt_mem_pool_alloc(&att_slab, (void **)&att, OS_TIMEOUT_NO_WAIT)) {
		LOG_ERR("No available ATT context for conn %p", conn);
		return -ENOMEM;
	}

	att_handle_rsp_thread = os_thread_self();

	(void)memset(att, 0, sizeof(*att));
	att->conn = conn;
	bt_slist_init(&att->reqs);
	bt_slist_init(&att->chans);

#if defined(CONFIG_BT_EATT)
	bt_work_init_delayable(&att->eatt.connection_work,
...
#if defined(CONFIG_BT_EATT)
static os_timeout_t credit_based_connection_delay(struct bt_conn *conn)
{
	/*
...
int bt_eatt_reconfigure(struct bt_conn *conn, uint16_t mtu)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&att->chans, att_chan, node) {
...
struct bt_att_req *bt_att_req_alloc(os_timeout_t timeout)
{
	struct bt_att_req *req = NULL;
	os_tid_t current_thread = os_thread_self();
	...
	if (bt_mem_pool_alloc(&req_slab, (void **)&req, timeout)) {
...
void bt_att_req_free(struct bt_att_req *req)
{
	...
	if (req->buf) {
		bt_buf_unref(req->buf);
		req->buf = NULL;
	}

	bt_mem_pool_free(&req_slab, (void *)req);
}

int bt_att_send(struct bt_conn *conn, struct bt_buf *buf)
{
	struct bt_att *att;
...
int bt_att_req_send(struct bt_conn *conn, struct bt_att_req *req)
{
	...
	bt_slist_append(&att->reqs, &req->node);
...
void bt_att_req_cancel(struct bt_conn *conn, struct bt_att_req *req)
{
	...
	bt_slist_find_and_remove(&att->reqs, &req->node);
...
struct bt_att_req *bt_att_find_req_by_user_data(struct bt_conn *conn, const void *user_data)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&att->reqs, req, node) {
...
void bt_att_increment_tx_meta_data_attr_count(struct bt_buf *buf, uint16_t attr_count)
{
	struct bt_att_tx_meta_data *data = att_get_tx_meta_data(buf);
...
bool bt_att_tx_meta_data_match(const struct bt_buf *buf, bt_gatt_complete_func_t func,
