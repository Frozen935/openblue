				     ...
				     struct bt_buf_simple *bt_buf)
{
	...
	start = bt_buf_simple_add(bt_buf, sizeof(len));

	/* ID */
	bt_buf_simple_add_le48(bt_buf, obj->id);
	...
	/* Name */
	bt_buf_simple_add_mem(bt_buf, obj->metadata.name, obj_name_len);

	/* Flags */
	bt_buf_simple_add_u8(bt_buf, flags);

	/* encode Object type */
	if (obj->metadata.type.uuid.type == BT_UUID_TYPE_16) {
		bt_buf_simple_add_le16(bt_buf, obj->metadata.type.uuid_16.val);
	} else {
		bt_buf_simple_add_mem(bt_buf, obj->metadata.type.uuid_128.val,
				       BT_UUID_SIZE_128);
	}

	/* encode Object Current size */
	bt_buf_simple_add_le32(bt_buf, obj->metadata.size.cur);

	/* Object properties */
	bt_buf_simple_add_le32(bt_buf, obj->metadata.props);
				    ...
				    void **data, size_t len, off_t offset)
{
	...
	bt_buf_simple_init_with_data(&dir_list->bt_buf, dir_list->_content,
				      sizeof(dir_list->_content));
	bt_buf_simple_reset(&dir_list->bt_buf);
	...
	last_rec_len = 0;
	rec_len = dir_list_object_record_size(obj);
	while (bt_buf_simple_tailroom(&dir_list->bt_buf) >= rec_len) {

		dir_list_object_encode(obj, &dir_list->bt_buf);

		dir_list->anchor_object = obj;
		dir_list->anchor_offset += last_rec_len;

		if (dir_list->bt_buf.len - (offset - rec_offset) >= len) {
	...
	*data = dir_list->bt_buf.data + (offset - rec_offset);

	return MIN(len, dir_list->bt_buf.len - (offset - rec_offset));
}
