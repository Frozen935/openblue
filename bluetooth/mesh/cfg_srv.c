...
static BT_WORK_DEFINE(node_reset_pending, node_reset_pending_handler);

static int dev_comp_data_get(const struct bt_mesh_model *model,
			     struct bt_mesh_msg_ctx *ctx,
			     struct bt_buf_simple *buf)
{
	BT_BUF_SIMPLE_DEFINE(sdu, BT_MESH_TX_SDU_MAX);
	uint8_t page;
	int err;
...
static const struct bt_mesh_model *get_model(const struct bt_mesh_elem *elem,
				       struct bt_buf_simple *buf, bool *vnd)
{
	if (buf->len < 4) {
		uint16_t id;

		id = bt_buf_simple_pull_le16(buf);
		...
		*vnd = false;
		...
		company = bt_buf_simple_pull_le16(buf);
		id = bt_buf_simple_pull_le16(buf);
			    ...
			    uint8_t ttl, uint8_t period, uint8_t retransmit, bool store)
{
	...
	if (pub_addr == BT_MESH_ADDR_UNASSIGNED) {
		...
		if (model->pub->update) {
			 ...
			 */
			(void)bt_work_cancel_delayable(&model->pub->timer);
		}
	...
	if (model->pub->update) {
		...
		if (period_ms > 0) {
			bt_work_reschedule(&model->pub->timer,
					  OS_MSEC(period_ms));
		} else {
...
static void key_idx_pack_list(struct bt_buf_simple *buf, uint16_t *arr, size_t cnt)
{
	uint16_t *idx = NULL;
	...
	if (idx) {
		bt_buf_simple_add_le16(buf, *idx);
	}
			       ...
			       uint16_t app_idx, uint16_t net_idx)
{
	...
	bt_buf_simple_add_u8(&msg, status);
		       ...
		       struct bt_buf_simple *buf)
{
	...
	get_idx = bt_buf_simple_pull_le16(buf);
	if (get_idx > 0xfff) {
	...
	bt_buf_simple_add_u8(&msg, status);
	bt_buf_simple_add_le16(&msg, get_idx);
		      ...
		      struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(&msg, bt_mesh_beacon_enabled());
		      ...
		      struct bt_buf_simple *buf)
{
	...
	bt_mesh_model_msg_init(&msg, OP_BEACON_STATUS);
	bt_buf_simple_add_u8(&msg, buf->data[0]);
			   ...
			   struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(&msg, bt_mesh_default_ttl_get());
				  ...
				  struct bt_mesh_msg_ctx *ctx)
{
	...
	bt_buf_simple_add_u8(&msg, bt_mesh_gatt_proxy_get());
			    ...
			    struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(&msg, bt_mesh_net_transmit_get());
		     ...
		     struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(&msg, bt_mesh_relay_get());
	bt_buf_simple_add_u8(&msg, bt_mesh_relay_retransmit_get());
			       ...
			       uint8_t *mod_id)
{
	...
	if (status != STATUS_SUCCESS) {
		(void)memset(bt_buf_simple_add(&msg, 7), 0, 7);
	} else {
		uint16_t idx_cred;

		bt_buf_simple_add_le16(&msg, pub_addr);

		idx_cred = mod->pub->key | (uint16_t)mod->pub->cred << 12;
		bt_buf_simple_add_le16(&msg, idx_cred);
		bt_buf_simple_add_u8(&msg, mod->pub->ttl);
		bt_buf_simple_add_u8(&msg, mod->pub->period);
		bt_buf_simple_add_u8(&msg, mod->pub->retransmit);
	}

	if (vnd) {
		memcpy(bt_buf_simple_add(&msg, 4), mod_id, 4);
	} else {
		memcpy(bt_buf_simple_add(&msg, 2), mod_id, 2);
	}
		       ...
		       struct bt_buf_simple *buf)
{
	...
	pub_addr = bt_buf_simple_pull_le16(buf);
	pub_app_idx = bt_buf_simple_pull_le16(buf);
	cred_flag = ((pub_app_idx >> 12) & BIT_MASK(1));
	pub_app_idx &= BIT_MASK(12);

	pub_ttl = bt_buf_simple_pull_u8(buf);
	if (pub_ttl > BT_MESH_TTL_MAX && pub_ttl != BT_MESH_TTL_DEFAULT) {
		LOG_ERR("Invalid TTL value 0x%02x", pub_ttl);
		return -EINVAL;
	}

	pub_period = bt_buf_simple_pull_u8(buf);
	retransmit = bt_buf_simple_pull_u8(buf);
	mod_id = buf->data;
			  ...
			  struct bt_buf_simple *buf)
{
	...
	uuid = bt_buf_simple_pull_mem(buf, 16);
	pub_app_idx = bt_buf_simple_pull_le16(buf);
	cred_flag = ((pub_app_idx >> 12) & BIT_MASK(1));
	pub_app_idx &= BIT_MASK(12);
	pub_ttl = bt_buf_simple_pull_u8(buf);
...
static enum bt_mesh_walk mod_sub_list_visitor(const struct bt_mesh_model *mod, void *ctx)
{
	...
	for (i = 0; i < mod->groups_cnt; i++) {
		...
		if (bt_buf_simple_tailroom(visit->msg) <
		    2 + BT_MESH_MIC_SHORT) {
			LOG_WRN("No room for all groups");
			return BT_MESH_WALK_STOP;
		}

		bt_buf_simple_add_le16(visit->msg, mod->groups[i]);
		count++;
	}
...
static int mod_sub_get(const struct bt_mesh_model *model,
		       ...
		       struct bt_buf_simple *buf)
{
	BT_BUF_SIMPLE_DEFINE(msg, BT_MESH_TX_SDU_MAX);
	struct mod_sub_list_ctx visit_ctx;
	const struct bt_mesh_model *mod;
	const struct bt_mesh_elem *elem;
	uint16_t addr, id;

	addr = bt_buf_simple_pull_le16(buf);
	...
	if (!elem) {
		bt_buf_simple_add_u8(&msg, STATUS_INVALID_ADDRESS);
		bt_buf_simple_add_le16(&msg, addr);
		bt_buf_simple_add_le16(&msg, id);
		goto send_list;
	}

	mod = bt_mesh_model_find(elem, id);
	if (!mod) {
		bt_buf_simple_add_u8(&msg, STATUS_INVALID_MODEL);
		bt_buf_simple_add_le16(&msg, addr);
		bt_buf_simple_add_le16(&msg, id);
		goto send_list;
	}

	bt_buf_simple_add_u8(&msg, STATUS_SUCCESS);
			   ...
			   struct bt_buf_simple *buf)
{
	...
	if (!elem) {
		bt_buf_simple_add_u8(&msg, STATUS_INVALID_ADDRESS);
		bt_buf_simple_add_le16(&msg, addr);
		bt_buf_simple_add_le16(&msg, company);
		bt_buf_simple_add_le16(&msg, id);
		goto send_list;
	}
			       ...
			       uint8_t status)
{
	...
	bt_buf_simple_add_u8(&msg, status);
	bt_buf_simple_add_le16(&msg, idx);
		       ...
		       struct bt_buf_simple *buf)
{
	...
	idx = bt_buf_simple_pull_le16(buf);
	if (idx > 0xfff) {
		       ...
		       struct bt_buf_simple *buf)
{
	...
	del_idx = bt_buf_simple_pull_le16(buf);
	if (del_idx > 0xfff) {
			       ...
			       uint16_t net_idx, uint8_t node_id)
{
	...
	bt_buf_simple_add_u8(&msg, status);
	bt_buf_simple_add_le16(&msg, net_idx);
	bt_buf_simple_add_u8(&msg, node_id);
			     ...
			     struct bt_buf_simple *buf)
{
	...
	node_id = bt_buf_simple_pull_u8(buf);
...
static void create_mod_app_status(struct bt_buf_simple *msg,
				  ...
				  uint8_t status, uint8_t *mod_id)
{
	...
	bt_buf_simple_add_u8(msg, status);
	bt_buf_simple_add_le16(msg, elem_addr);
	bt_buf_simple_add_le16(msg, app_idx);
			...
			struct bt_buf_simple *buf)
{
	...
	key_app_idx = bt_buf_simple_pull_le16(buf);
	mod_id = buf->data;
...
send_list:
	...
	if (vnd) {
		bt_buf_simple_add_mem(&msg, mod_id, 4);
	} else {
		bt_buf_simple_add_mem(&msg, mod_id, 2);
	}
...
static void reset_send_start(uint16_t duration, int err, void *cb_data)
{
	if (err) {
		...
		bt_work_submit(&node_reset_pending);
	}
}
			      ...
			      struct bt_mesh_msg_ctx *ctx)
{
	...
	bt_buf_simple_add_u8(&msg, bt_mesh_friend_get());
			   ...
			   struct bt_buf_simple *buf)
{
	...
	lpn_addr = bt_buf_simple_pull_le16(buf);
	...
	bt_buf_simple_add_le16(&msg, lpn_addr);
...
send_rsp:
	bt_buf_simple_add_le24(&msg, timeout_steps);
			   ...
			   uint8_t phase, uint8_t status)
{
	...
	bt_buf_simple_add_u8(&msg, status);
	bt_buf_simple_add_le16(&msg, idx);
	bt_buf_simple_add_u8(&msg, phase);
			      ...
			      const struct bt_mesh_hb_pub *pub)
{
	...
	if (pub->dst == BT_MESH_ADDR_UNASSIGNED) {
		...
		bt_buf_simple_add_u8(&msg, hb_pub_count_log(pub->count));
		bt_buf_simple_add_u8(&msg, bt_mesh_hb_log(pub->period));
		bt_buf_simple_add_u8(&msg, pub->ttl);
		bt_buf_simple_add_le16(&msg, pub->feat);
		bt_buf_simple_add_le16(&msg, pub->net_idx & 0xfff);
	}
			      ...
			      const struct bt_mesh_hb_sub *sub)
{
	...
	bt_buf_simple_add_u8(&msg, bt_mesh_hb_log(sub->remaining));
	bt_buf_simple_add_u8(&msg, hb_sub_count_log(sub->count));
	bt_buf_simple_add_u8(&msg, sub->min_hops);
	bt_buf_simple_add_u8(&msg, sub->max_hops);
			     ...
			     struct bt_buf_simple *buf)
{
	...
	sub_src = bt_buf_simple_pull_le16(buf);
	sub_dst = bt_buf_simple_pull_le16(buf);
	period_log = bt_buf_simple_pull_u8(buf);
