...
#define BT_MESH_SHELL_MDL_INSTANCE_CMDS(cmd_set_name, mod_id, mod_ptr) \
	...
	\
	static int cmd_##cmd_set_name##_set(const struct bt_shell *sh, size_t argc, \
					    char *argv[]) \
	{ \
		int err = 0; \
		uint8_t elem_idx = bt_shell_strtoul(argv[1], 0, &err); \
	\
		if (err) { \
			bt_shell_warn("Unable to parse input string arg"); \
			return err; \
		} \
	\
