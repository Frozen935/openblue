...
static bool bt_obex_has_header(struct bt_buf *buf, uint8_t id)
{
	struct bt_obex_has_header data;
	int err;
				  ...
				  struct bt_buf *buf)
{
	...
	if (!bt_atomic_cas(&server->_opcode, 0, req_code)) {
		...
		if (opcode != req_code) {
			bt_atomic_cas(&server->_opcode, opcode, req_code);
		}
	}
...
int bt_obex_put(struct bt_obex_client *client, bool final, struct bt_buf *buf)
{
	...
	if (!bt_atomic_cas(&client->_opcode, 0, req_code)) {
		...
		if (opcode != req_code) {
			bt_atomic_cas(&client->_opcode, opcode, req_code);
		}
	}
...
int bt_obex_add_header_action_id(struct bt_buf *buf, uint32_t action_id)
{
	size_t total;
...
int bt_obex_get_header_action_id(struct bt_buf *buf, uint32_t *action_id)
{
	struct bt_obex_find_header_data data;
	int err;
