...
/* RFCOMM Server list */
static bt_slist_t goep_rfcomm_server = BT_SLIST_STATIC_INIT(&goep_rfcomm_server);

static void goep_rfcomm_recv(struct bt_rfcomm_dlc *dlc, struct bt_buf *buf)
{
...
static int goep_rfcomm_send(struct bt_obex *obex, struct bt_buf *buf)
{
	...
	if (bt_buf_tailroom(buf) < BT_RFCOMM_FCS_SIZE) {
...
static struct bt_buf *goep_rfcomm_alloc_buf(struct bt_obex *obex, struct bt_buf_pool *pool)
{
	struct bt_goep *goep = CONTAINER_OF(obex, struct bt_goep, obex);
			      ...
			      struct bt_rfcomm_dlc **dlc)
{
	...
	if (!bt_slist_find(&goep_rfcomm_server, &rfcomm_server->node, NULL)) {
...
int bt_goep_transport_rfcomm_server_register(struct bt_goep_transport_rfcomm_server *server)
{
	...
	if (bt_slist_find(&goep_rfcomm_server, &server->node, NULL)) {
	...
	bt_slist_append(&goep_rfcomm_server, &server->node);
...
/* L2CAP Server list */
static bt_slist_t goep_l2cap_server = BT_SLIST_STATIC_INIT(&goep_l2cap_server);

BT_BUF_POOL_DEFINE(goep_rx_pool, BT_BUF_ACL_RX_COUNT, BT_BUF_ACL_SIZE(CONFIG_BT_BUF_ACL_RX_SIZE),
		    CONFIG_BT_CONN_TX_USER_DATA_SIZE, NULL);

static int goep_l2cap_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
...
static struct bt_buf *l2cap_alloc_buf(struct bt_l2cap_chan *chan)
{
	...
	buf = bt_buf_alloc(&goep_rx_pool, OS_TIMEOUT_FOREVER);
	if (buf == NULL) {
...
static int goep_l2cap_send(struct bt_obex *obex, struct bt_buf *buf)
{
	struct bt_goep *goep = CONTAINER_OF(obex, struct bt_goep, obex);
...
static struct bt_buf *goep_l2cap_alloc_buf(struct bt_obex *obex, struct bt_buf_pool *pool)
{
	struct bt_goep *goep = CONTAINER_OF(obex, struct bt_goep, obex);
			     ...
			     struct bt_l2cap_chan **chan)
{
	...
	if (!bt_slist_find(&goep_l2cap_server, &l2cap_server->node, NULL)) {
...
int bt_goep_transport_l2cap_server_register(struct bt_goep_transport_l2cap_server *server)
{
	...
	if (bt_slist_find(&goep_l2cap_server, &server->node, NULL)) {
	...
	bt_slist_append(&goep_l2cap_server, &server->node);
...
struct bt_buf *bt_goep_create_pdu(struct bt_goep *goep, struct bt_buf_pool *pool)
{
	struct bt_buf *buf;
	size_t len;
	...
	if (buf) {
		len = bt_buf_headroom(buf);
		bt_buf_reserve(buf, len + BT_OBEX_SEND_BUF_RESERVE);
	}
	return buf;
}
