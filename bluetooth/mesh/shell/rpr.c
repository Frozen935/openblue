			    ...
			    struct bt_buf_simple *adv_data)
{
	...
	while (adv_data && adv_data->len > 2) {
		...
		len = bt_buf_simple_pull_u8(adv_data);
		if (len == 0) {
		...
		type = bt_buf_simple_pull_u8(adv_data);
		if ((--len) > 0) {
			uint8_t dlen;

			/* Pull all length, but print only what fits into `data` array. */
			dlen = MIN(len, sizeof(data) - 1);
			memcpy(data, bt_buf_simple_pull_mem(adv_data, len), dlen);
			len = dlen;
		}
		data[len] = '\0';
...
static int cmd_scan(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	timeout = bt_shell_strtoul(argv[1], 0, &err);
	if (err) {
	...
	if (err) {
		bt_shell_print("Scan start failed: %d", err);
		return err;
	}

	if (rsp.status == BT_MESH_RPR_SUCCESS) {
		bt_shell_print("Scan started.");
	} else {
		bt_shell_print("Scan start response: %d", rsp.status);
	}

	return 0;
}

static int cmd_scan_ext(const struct bt_shell *sh, size_t argc, char *argv[])
{
	const struct bt_mesh_rpr_node srv = {
		.addr = bt_mesh_shell_target_ctx.dst,
		.net_idx = bt_mesh_shell_target_ctx.net_idx,
	...
	for (i = 0; i < argc - 3; i++) {
		ad_types[i] = bt_shell_strtoul(argv[3 + i], 0, &err);
	}
	...
	bt_shell_print("Extended scan started.");

	return 0;
}

static int cmd_scan_srv(const struct bt_shell *sh, size_t argc, char *argv[])
{
	const struct bt_mesh_rpr_node srv = {
		.addr = bt_mesh_shell_target_ctx.dst,
		.net_idx = bt_mesh_shell_target_ctx.net_idx,
	...
	for (i = 0; i < argc - 1; i++) {
		ad_types[i] = bt_shell_strtoul(argv[1 + i], 0, &err);
	}
...
static int cmd_scan_caps(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (err) {
		bt_shell_print("Scan capabilities get failed: %d", err);
		return err;
	}

	bt_shell_print("Remote Provisioning scan capabilities of 0x%04x:",
		    bt_mesh_shell_target_ctx.dst);
	bt_shell_print("\tMax devices:     %u", caps.max_devs);
	bt_shell_print("\tActive scanning: %s",
		    caps.active_scan ? "true" : "false");
	return 0;
}

static int cmd_scan_get(const struct bt_shell *sh, size_t argc, char *argv[])
{
	struct bt_mesh_rpr_scan_status rsp;
	const struct bt_mesh_rpr_node srv = {
		.addr = bt_mesh_shell_target_ctx.dst,
	...
	if (err) {
		bt_shell_print("Scan get failed: %d", err);
		return err;
	}

	bt_shell_print("Remote Provisioning scan on 0x%04x:", bt_mesh_shell_target_ctx.dst);
	bt_shell_print("\tStatus:         %u", rsp.status);
	bt_shell_print("\tScan type:      %u", rsp.scan);
	bt_shell_print("\tMax devices:    %u", rsp.max_devs);
	bt_shell_print("\tRemaining time: %u", rsp.timeout);
	return 0;
}

static int cmd_scan_stop(const struct bt_shell *sh, size_t argc, char *argv[])
{
	struct bt_mesh_rpr_scan_status rsp;
	const struct bt_mesh_rpr_node srv = {
		.addr = bt_mesh_shell_target_ctx.dst,
	...
	if (err || rsp.status) {
		bt_shell_print("Scan stop failed: %d %u", err, rsp.status);
		return err;
	}

	bt_shell_print("Remote Provisioning scan on 0x%04x stopped.",
		    bt_mesh_shell_target_ctx.dst);
	return 0;
}

static int cmd_link_get(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (err) {
		bt_shell_print("Link get failed: %d %u", err, rsp.status);
		return err;
	}

	bt_shell_print("Remote Provisioning Link on 0x%04x:", bt_mesh_shell_target_ctx.dst);
	bt_shell_print("\tStatus: %u", rsp.status);
	bt_shell_print("\tState:  %u", rsp.state);
	return 0;
}

static int cmd_link_close(const struct bt_shell *sh, size_t argc, char *argv[])
{
	struct bt_mesh_rpr_link rsp;
	const struct bt_mesh_rpr_node srv = {
		.addr = bt_mesh_shell_target_ctx.dst,
	...
	if (err) {
		bt_shell_print("Link close failed: %d %u", err, rsp.status);
		return err;
	}
...
static int cmd_provision_remote(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (err) {
		bt_shell_print("Prov remote start failed: %d", err);
	}

	return err;
}

static int cmd_reprovision_remote(const struct bt_shell *sh, size_t argc, char *argv[])
{
	struct bt_mesh_rpr_node srv = {
		.addr = bt_mesh_shell_target_ctx.dst,
		.net_idx = bt_mesh_shell_target_ctx.net_idx,
	...
	if (!BT_MESH_ADDR_IS_UNICAST(addr)) {
		bt_shell_print("Must be a valid unicast address");
		return -EINVAL;
	}

	composition_changed = (argc > 2 && bt_shell_strtobool(argv[2], 0, &err));
	if (err) {
		bt_shell_warn("Unable to parse input string argument");
		return err;
	}
	...
	if (err) {
		bt_shell_print("Reprovisioning failed: %d", err);
	}
...
BT_SHELL_SUBCMD_SET_CREATE(
	...
	BT_SHELL_CMD_ARG(scan-srv, NULL, "[<ADType> ... ]", cmd_scan_srv, 1,
		      CONFIG_BT_MESH_RPR_AD_TYPES_MAX),
	BT_SHELL_CMD_ARG(scan-caps, NULL, NULL, cmd_scan_caps, 1, 0),
	BT_SHELL_CMD_ARG(scan-get, NULL, NULL, cmd_scan_get, 1, 0),
	BT_SHELL_CMD_ARG(scan-stop, NULL, NULL, cmd_scan_stop, 1, 0),
	BT_SHELL_CMD_ARG(link-get, NULL, NULL, cmd_link_get, 1, 0),
	BT_SHELL_CMD_ARG(link-close, NULL, NULL, cmd_link_close, 1, 0),
	BT_SHELL_CMD_ARG(provision-remote, NULL, "<UUID(1-16 hex)> <NetKeyIdx> <Addr>",
		      cmd_provision_remote, 4, 0),
	BT_SHELL_CMD_ARG(reprovision-remote, NULL, "<Addr> [<CompChanged(false, true)>]",
