				       ...
				       struct bt_buf_simple *buf)
{
	...
	memcpy(dev.uuid, bt_buf_simple_pull_mem(buf, 16), 16);
	if (buf->len >= 2) {
		dev.oob = bt_buf_simple_pull_le16(buf);
		found_dev = true;
			      ...
			      struct bt_buf_simple *buf)
{
	...
	link.status = bt_buf_simple_pull_u8(buf);
	link.state = bt_buf_simple_pull_u8(buf);
	if (buf->len == 1) {
	...
	bt_work_reschedule(&cli->link.timeout, OS_SECONDS(cli->link.time));
				   ...
				   struct bt_buf_simple *buf)
{
	...
	caps->max_devs = bt_buf_simple_pull_u8(buf);
	caps->active_scan = bt_buf_simple_pull_u8(buf);
			      ...
			      struct bt_buf_simple *buf)
{
	...
	dev.rssi = bt_buf_simple_pull_u8(buf);
	memcpy(dev.uuid, bt_buf_simple_pull_mem(buf, 16), 16);
	dev.oob = bt_buf_simple_pull_le16(buf);
	if (buf->len == 4) {
		memcpy(&dev.hash, bt_buf_simple_pull_mem(buf, 4), 4);
			      ...
			      struct bt_buf_simple *buf)
{
	...
	status->status = bt_buf_simple_pull_u8(buf);
	status->scan = bt_buf_simple_pull_u8(buf);
	status->max_devs = bt_buf_simple_pull_u8(buf);
	status->timeout = bt_buf_simple_pull_u8(buf);
...
static void link_timeout(struct bt_work *work)
{
	struct bt_mesh_rpr_cli *cli = CONTAINER_OF(bt_work_delayable_from_work(work),
...
static int rpr_cli_init(const struct bt_mesh_model *mod)
{
	...
	bt_work_init_delayable(&cli->link.timeout, link_timeout);
...
static int tx_wait(struct bt_mesh_rpr_cli *cli,
		   ...
		   struct bt_buf_simple *buf, uint32_t rsp, void *rsp_ctx)
{
	...
	err = bt_mesh_msg_ack_ctx_wait(ack_ctx, OS_MSEC(tx_timeout));
...
static void link_reset(struct bt_mesh_rpr_cli *cli)
{
	bt_work_cancel_delayable(&cli->link.timeout);
	cli->link.srv.addr = BT_MESH_ADDR_UNASSIGNED;
	cli->link.state = BT_MESH_RPR_LINK_IDLE;
	bt_mesh_msg_ack_ctx_clear(&cli->prov_ack_ctx);
}
			   ...
			   struct bt_mesh_rpr_scan_status *status)
{
	...
	bt_buf_simple_add_u8(&buf, max_devs);
	bt_buf_simple_add_u8(&buf, timeout);

	if (uuid) {
		bt_buf_simple_add_mem(&buf, uuid, 16);
	}
			       ...
			       const uint8_t *ad_types, size_t ad_count)
{
	...
	bt_buf_simple_add_u8(&buf, ad_count);
	bt_buf_simple_add_mem(&buf, ad_types, ad_count);
	if (uuid) {
		bt_buf_simple_add_mem(&buf, uuid, 16);
		bt_buf_simple_add_u8(&buf, timeout);
	}
			   ...
			   struct bt_mesh_rpr_link *rsp)
{
	...
	bt_buf_simple_add_u8(&buf, PROV_BEARER_LINK_STATUS_FAIL);
			  ...
			  uint8_t timeout)
{
	...
	if (cli->link.time != LINK_TIMEOUT_SECONDS_DEFAULT) {
		bt_buf_simple_add_u8(&buf, cli->link.time);
	}
...
static int send(struct bt_mesh_rpr_cli *cli, struct bt_buf_simple *buf,
		void *cb_data)
{
	...
	if (bt_buf_simple_headroom(buf) < 3) {
	...
	bt_buf_simple_push_u8(buf, cli->link.tx_pdu);
	/* Assumes opcode is 2 bytes. Build assert at top of file ensures this.
	 */
	bt_buf_simple_push_be16(buf, RPR_OP_PDU_SEND);
...
static int pb_send(struct bt_buf_simple *buf, prov_bearer_send_complete_t cb,
		   void *cb_data)
{
	bearer.tx.cb = cb;
