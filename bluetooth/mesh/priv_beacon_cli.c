				...
				struct bt_buf_simple *buf)
{
	...
	rand_int = bt_buf_simple_pull_u8(buf);
				    ...
				    struct bt_buf_simple *buf)
{
	...
	proxy = bt_buf_simple_pull_u8(buf);
				 ...
				 struct bt_buf_simple *buf)
{
	...
	net_idx = bt_buf_simple_pull_le16(buf);
				...
				struct bt_mesh_priv_beacon *rsp)
{
	...
	bt_buf_simple_add_u8(&buf, val->enabled);
	if (val->rand_interval) {
		bt_buf_simple_add_u8(&buf, val->rand_interval);
	}
					...
					struct bt_mesh_priv_node_id *rsp)
{
	...
	bt_buf_simple_add_le16(&buf, val->net_idx);
	bt_buf_simple_add_u8(&buf, val->state);
					...
					struct bt_mesh_priv_node_id *val)
{
	...
	bt_buf_simple_add_le16(&buf, key_net_idx);
