...
static int
prepare_cl_cte_rx_enable_cmd_params(struct bt_buf **buf, struct bt_le_per_adv_sync *sync,
...
static int prepare_conn_cte_rx_enable_cmd_params(struct bt_buf **buf, struct bt_conn *conn,
				    ...
				    const struct bt_df_adv_cte_tx_param *params)
{
	...
	if (params->num_ant_ids) {
		uint8_t *dest_ant_ids = bt_buf_add(buf, params->num_ant_ids);
				    ...
				    bool enable)
{
	...
	*buf = bt_hci_cmd_alloc(OS_TIMEOUT_FOREVER);
	...
	if (enable) {
		...
		dest_ant_ids = bt_buf_add(*buf, cp->switch_pattern_len);
...
int hci_df_prepare_connectionless_iq_report(struct bt_buf *buf,
...
int hci_df_vs_prepare_connectionless_iq_report(struct bt_buf *buf,
					       struct bt_df_per_adv_sync_iq_samples_report *report,
					       struct bt_le_per_adv_sync **per_adv_sync_to_report)
{
...
static void prepare_conn_cte_tx_params_cmd(struct bt_buf *buf, const struct bt_conn *conn,
					   const struct bt_df_conn_cte_tx_param *params)
{
	...
	if (params->cte_types & (BT_DF_CTE_TYPE_AOD_1US | BT_DF_CTE_TYPE_AOD_2US)) {
		...
		ant_ids = bt_buf_add(buf, cp->switch_pattern_len);
...
int hci_df_prepare_connection_iq_report(struct bt_buf *buf,
...
int hci_df_vs_prepare_connection_iq_report(struct bt_buf *buf,
					   struct bt_df_conn_iq_samples_report *report,
					   struct bt_conn **conn_to_report)
{
...
static void prepare_conn_cte_req_enable_cmd_params(struct bt_buf *buf, const struct bt_conn *conn,
...
int hci_df_prepare_conn_cte_req_failed(struct bt_buf *buf,
				       struct bt_df_conn_iq_samples_report *report,
				       struct bt_conn **conn_to_report)
{
...
static void prepare_conn_cte_rsp_enable_cmd_params(struct bt_buf *buf, const struct bt_conn *conn,
						   bool enable)
{
