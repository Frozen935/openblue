...
struct bt_bap_unicast_client_ep {
	...
	struct bt_bap_ep ep;
	struct bt_work_delayable ase_read_work;
...
static bt_slist_t unicast_client_cbs = BT_SLIST_STATIC_INIT(&unicast_client_cbs);
...
static int unicast_client_ep_set_codec_cfg(struct bt_bap_ep *ep, uint8_t id, uint16_t cid,
					   ...
					   struct bt_audio_codec_cfg *codec_cfg);
static int unicast_client_ep_start(struct bt_bap_ep *ep, struct bt_buf_simple *buf);
...
static void delayed_ase_read_handler(struct bt_work *work);
static void unicast_client_ep_set_status(struct bt_bap_ep *ep, struct bt_buf_simple *buf);
...
static int unicast_client_send_start(struct bt_bap_ep *ep)
{
	...
	req = bt_buf_simple_add(buf, sizeof(*req));
	req->num_ases = 1U;
...
{
	bt_buf_simple_init_with_data(&client->bt_buf, &client->att_buf, sizeof(client->att_buf));
	bt_buf_simple_reset(&client->bt_buf);
}

#if defined(CONFIG_BT_AUDIO_RX)
static void unicast_client_ep_iso_recv(struct bt_iso_chan *chan,
				       const struct bt_iso_recv_info *info, struct bt_buf *buf)
{
	...
	if (IS_ENABLED(CONFIG_BT_BAP_DEBUG_STREAM_DATA)) {
		LOG_DBG("stream %p ep %p len %zu", stream, ep, bt_buf_frags_len(buf));
	}
...
static void unicast_client_ep_init(struct bt_bap_ep *ep, uint16_t handle, uint8_t dir)
{
	...
	bt_work_init_delayable(&client_ep->ase_read_work, delayed_ase_read_handler);
}
...
static void unicast_client_ep_set_local_idle_state(struct bt_bap_ep *ep)
{
	...
	bt_buf_simple_init_with_data(&buf, &status, sizeof(status));
					   ...
					   enum bt_audio_location loc)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&unicast_client_cbs, listener, next, _node) {
...
static void unicast_client_discover_complete(struct bt_conn *conn, int err)
{
	...
	bt_atomic_clear_bit(client->flags, UNICAST_CLIENT_FLAG_BUSY);
					 ...
					 const struct bt_ascs_ase_status_qos *qos)
{
	...
	if (ep->dir == BT_AUDIO_DIR_SOURCE) {
		...
		__ASSERT_MSG(false, "Invalid ep->dir: %u", ep->dir);
		return;
	}
...
static void check_and_reset_group_pd(struct bt_bap_unicast_group *group, enum bt_audio_dir dir)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&group->streams, stream, _node) {
	...
	if (dir_in_idle_or_config_state) {
		if (dir == BT_AUDIO_DIR_SINK) {
			...
			__ASSERT_MSG(false, "Invalid dir %d", dir);
		}
	}
}

static void unicast_client_ep_config_state(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	struct bt_bap_unicast_client_ep *client_ep =
		CONTAINER_OF(ep, struct bt_bap_unicast_client_ep, ep);
	...
	cfg = bt_buf_simple_pull_mem(buf, sizeof(*cfg));
	...
	cc = bt_buf_simple_pull_mem(buf, cfg->cc_len);
...
static void unicast_client_ep_qos_state(struct bt_bap_ep *ep, struct bt_buf_simple *buf,
					uint8_t old_state)
{
	...
	qos = bt_buf_simple_pull_mem(buf, sizeof(*qos));
...
static void unicast_client_ep_enabling_state(struct bt_bap_ep *ep, struct bt_buf_simple *buf,
					     bool state_changed)
{
	...
	enable = bt_buf_simple_pull_mem(buf, sizeof(*enable));
	...
	metadata = bt_buf_simple_pull_mem(buf, enable->metadata_len);
...
static void unicast_client_ep_streaming_state(struct bt_bap_ep *ep, struct bt_buf_simple *buf,
					      bool state_changed)
{
	...
	stream_status = bt_buf_simple_pull_mem(buf, sizeof(*stream_status));
...
static void unicast_client_ep_disabling_state(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	...
	disable = bt_buf_simple_pull_mem(buf, sizeof(*disable));
...
static void unicast_client_ep_releasing_state(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	struct bt_bap_stream *stream;
...
static void unicast_client_ep_set_status(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	...
	status = bt_buf_simple_pull_mem(buf, sizeof(*status));
					...
					struct bt_audio_codec_cap *codec_cap)
{
	...
	if (data_len > 0U) {
		...
		bt_buf_simple_init_with_data(&buf, data, data_len);
	...
	if (meta_len > 0U) {
		...
		bt_buf_simple_init_with_data(&buf, meta, meta_len);
					...
					uint16_t length)
{
	...
	bt_buf_simple_init_with_data(&buf, (void *)data, length);
	...
	rsp = bt_buf_simple_pull_mem(&buf, sizeof(*rsp));
	...
	for (uint8_t i = 0U; i < rsp->num_ase; i++) {
		...
		ase_rsp = bt_buf_simple_pull_mem(&buf, sizeof(*ase_rsp));
						...
						uint16_t length)
{
	...
	struct bt_buf_simple buf_clone;
	...
	if (data != NULL) {
		if (bt_buf_simple_tailroom(buf) < length) {
			LOG_DBG("Buffer full, invalid server response of size %u",
				length + client->bt_buf.len);
			reset_att_buf(client);
			bt_atomic_clear_bit(client->flags, UNICAST_CLIENT_FLAG_BUSY);
		...
		/* store data*/
		bt_buf_simple_add_mem(buf, data, length);
	 ...
	 * layers
	 */
	bt_buf_simple_clone(buf, &buf_clone);
	reset_att_buf(client);
	bt_atomic_clear_bit(client->flags, UNICAST_CLIENT_FLAG_BUSY);
...
static void long_ase_read(struct bt_bap_unicast_client_ep *client_ep)
{
	...
	struct unicast_client *client = &uni_cli_insts[bt_conn_index(conn)];
	struct bt_buf_simple *long_read_buf = &client->bt_buf;
	int err;

	LOG_DBG("conn %p ep %p 0x%04X", conn, &client_ep->ep, client_ep->handle);

	if (bt_atomic_test_and_set_bit(client->flags, UNICAST_CLIENT_FLAG_BUSY)) {
		...
		err = bt_work_reschedule(&client_ep->ase_read_work,
					OS_USEC(BT_CONN_INTERVAL_TO_US(conn_info.le.interval)));
		if (err < 0) {
					...
					uint16_t length)
{
	...
	(void)bt_work_cancel_delayable(&client_ep->ase_read_work);
	...
	if (length == max_ntf_size) {
		...
		if (!bt_atomic_test_bit(client->flags, UNICAST_CLIENT_FLAG_BUSY)) {
			struct bt_buf_simple *long_read_buf = &client->bt_buf;

			/* store data*/
			bt_buf_simple_add_mem(long_read_buf, data, length);
		}
...
static int unicast_client_ep_subscribe(struct bt_conn *conn, struct bt_bap_ep *ep)
{
	...
	bt_atomic_set_bit(client_ep->subscribe.flags, BT_GATT_SUBSCRIBE_FLAG_VOLATILE);
...
static void unicast_client_ep_set_cp(struct bt_conn *conn, uint16_t handle)
{
	...
	if (!client->cp_subscribe.value_handle) {
		...
		bt_atomic_set_bit(client->cp_subscribe.flags, BT_GATT_SUBSCRIBE_FLAG_VOLATILE);
...
struct bt_buf_simple *bt_bap_unicast_client_ep_create_pdu(struct bt_conn *conn, uint8_t op)
{
	struct unicast_client *client = &uni_cli_insts[bt_conn_index(conn)];
	...
	hdr = bt_buf_simple_add(&client->bt_buf, sizeof(*hdr));
	hdr->op = op;

	return &client->bt_buf;
}

static int unicast_client_ep_config(struct bt_bap_ep *ep, struct bt_buf_simple *buf,
				    const struct bt_audio_codec_cfg *codec_cfg)
{
	...
	bt_buf_simple_add_mem(buf, codec_cfg->data, codec_cfg->data_len);

	return 0;
}

int bt_bap_unicast_client_ep_qos(struct bt_bap_ep *ep, struct bt_buf_simple *buf,
...
static int unicast_client_ep_enable(struct bt_bap_ep *ep, struct bt_buf_simple *buf,
				    const uint8_t meta[], size_t meta_len)
{
	...
	req->len = meta_len;
	bt_buf_simple_add_mem(buf, meta, meta_len);

	return 0;
}

static int unicast_client_ep_metadata(struct bt_bap_ep *ep, struct bt_buf_simple *buf,
				      const uint8_t meta[], size_t meta_len)
{
	struct bt_ascs_metadata *req;
...
static int unicast_client_ep_start(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(buf, ep->id);

	return 0;
}

static int unicast_client_ep_disable(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	LOG_DBG("ep %p buf %p", ep, buf);
...
static int unicast_client_ep_stop(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	LOG_DBG("ep %p buf %p", ep, buf);
...
static int unicast_client_ep_release(struct bt_bap_ep *ep, struct bt_buf_simple *buf)
{
	LOG_DBG("ep %p buf %p", ep, buf);
				     ...
				     struct bt_bap_iso *iso, enum bt_audio_dir dir)
{
	...
	bt_slist_append(&group->streams, &stream->_node);
}
				     ...
				     struct bt_bap_stream *stream, enum bt_audio_dir dir)
{
	...
	if (bt_slist_find_and_remove(&group->streams, &stream->_node)) {
...
static void unicast_group_free(struct bt_bap_unicast_group *group)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&group->streams, stream, next, _node) {
		...
		bt_slist_remove(&group->streams, NULL, &stream->_node);
	}
				  ...
				  const struct bt_bap_unicast_group_param *param)
{
	 ...
	 */
	idx = 0U;
	BT_SLIST_FOR_EACH_CONTAINER(&unicast_group->streams, tmp_stream, _node) {
...
int bt_bap_unicast_group_delete(struct bt_bap_unicast_group *unicast_group)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&unicast_group->streams, stream, _node) {
					...
					void *user_data)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&unicast_group->streams, stream, next, _node) {
				 ...
				 const struct bt_audio_codec_cfg *codec_cfg)
{
	...
	op = bt_buf_simple_add(buf, sizeof(*op));
	op->num_ases = 0x01;
...
int bt_bap_unicast_client_qos(struct bt_conn *conn, struct bt_bap_unicast_group *group)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&group->streams, stream, _node) {
		...
		case BT_AUDIO_DIR_SOURCE:
			...
			break;
		default:
			__ASSERT_MSG(false, "invalid endpoint dir: %u", ep->dir);
			return -EINVAL;
		}
						       ...
						       const void *data, uint16_t length)
{
	...
	bt_buf_simple_init_with_data(&buf, (void *)data, length);
	context.snk = bt_buf_simple_pull_le16(&buf);
	context.src = bt_buf_simple_pull_le16(&buf);
						      ...
						      const void *data, uint16_t length)
{
	...
	bt_buf_simple_init_with_data(&buf, (void *)data, length);
	location = bt_buf_simple_pull_le32(&buf);
						     ...
						     const void *data, uint16_t length)
{
	...
	bt_buf_simple_init_with_data(&buf, (void *)data, length);
	context = bt_buf_simple_pull_mem(&buf, sizeof(*context));
					...
					uint16_t length)
{
	...
	for (i = 0U; i < rsp->num_pac; i++) {
		...
		pac_codec = bt_buf_simple_pull_mem(buf, sizeof(*pac_codec));
		...
		cc = bt_buf_simple_pull_mem(buf, sizeof(*cc));
		if (buf->len < cc->len) {
			LOG_ERR("Malformed PAC: remaining len %u expected %zu", buf->len, cc->len);
			break;
		}

		cc_ltv = bt_buf_simple_pull_mem(buf, cc->len);
		...
		meta = bt_buf_simple_pull_mem(buf, sizeof(*meta));
		if (buf->len < meta->len) {
			LOG_ERR("Malformed PAC: remaining len %u expected %u", buf->len, meta->len);
			break;
		}

		meta_ltv = bt_buf_simple_pull_mem(buf, meta->len);
...
int bt_bap_unicast_client_register_cb(struct bt_bap_unicast_client_cb *cb)
{
	...
	if (bt_slist_find(&unicast_client_cbs, &cb->_node, NULL)) {
		return -EEXIST;
	}

	bt_slist_append(&unicast_client_cbs, &cb->_node);
