...
#define PROV_BUF(name, len)                                                                        \
	BT_BUF_SIMPLE_DEFINE(name, PROV_BEARER_BUF_HEADROOM + PDU_OP_LEN + len +                  \
...
static inline int bt_mesh_prov_send(struct bt_buf_simple *buf,
				    prov_bearer_send_complete_t cb)
{
	return bt_mesh_prov_link.bearer->send(buf, cb, NULL);
}

static inline void bt_mesh_prov_buf_init(struct bt_buf_simple *buf, uint8_t type)
{
	bt_buf_simple_reserve(buf, PROV_BEARER_BUF_HEADROOM);
...
static inline os_timeout_t bt_mesh_prov_protocol_timeout_get(void)
{
...
#endif /* __BLUETOOTH_MESH_PROV_H__ */
