			 ...
			 struct bt_buf_simple *buf)
{
	...
	while (buf->len > 0) {
		...
		addr = bt_buf_simple_pull_le16(cli.srcs);
				 ...
				 uint16_t elem_addr)
{
	...
	bt_buf_simple_init(cli.srcs, 0);
	bt_mesh_model_msg_init(cli.ctx.sdu, OP_OPCODES_AGGREGATOR_SEQUENCE);
	bt_buf_simple_add_le16(cli.ctx.sdu, elem_addr);
...
int bt_mesh_op_agg_cli_seq_send(void)
{
	...
	return bt_mesh_msg_ack_ctx_wait(&cli.ack_ctx, OS_MSEC(msg_timeout));
}
...
{
	return bt_buf_simple_tailroom(cli.ctx.sdu);
}
...
int bt_mesh_op_agg_cli_send(const struct bt_mesh_model *model, struct bt_buf_simple *msg)
{
	...
	if (bt_buf_simple_tailroom(&srcs) < 2) {
		return -ENOMEM;
	}

	bt_buf_simple_add_le16(&srcs, src);
