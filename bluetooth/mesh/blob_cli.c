...
#define TARGETS_FOR_EACH(cli, target)                                          \
	BT_SLIST_FOR_EACH_CONTAINER((bt_slist_t *)&(cli)->inputs->targets,   \
				     target, n)
...
static void start_retry_timer(struct bt_mesh_blob_cli *cli)
{
	...
	if (SENDING_CHUNKS_IN_PULL_MODE(cli)) {
		 ...
		 */
		next_timeout_ms -= (int64_t)os_time_get_ms();
	...
	(void)bt_work_reschedule(&cli->tx.retry, next_timeout);
}

static void cli_state_reset(struct bt_mesh_blob_cli *cli)
{
	bt_work_cancel_delayable(&cli->tx.retry);
	cli->xfer = NULL;
...
static int chunk_idx_decode(struct bt_buf_simple *buf)
{
	uint16_t data;
	uint8_t byte;
	...
	byte = bt_buf_simple_pull_u8(buf);
	...
	if ((byte & 0xf0) == 0xe0) { /* 0x800 - 0xffff */
		...
		data = (byte & 0x0f) << 12;
		data |= (bt_buf_simple_pull_u8(buf) & 0x3f) << 6;
		data |= (bt_buf_simple_pull_u8(buf) & 0x3f);
...
{
	if (*current) {
		*current = BT_SLIST_PEEK_NEXT_CONTAINER(*current, n);
	} else {
		*current = BT_SLIST_PEEK_HEAD_CONTAINER(
			(bt_slist_t *)&cli->inputs->targets, *current, n);
	}
	...
	while (*current) {
		...
		if (SENDING_CHUNKS_IN_PULL_MODE(cli) &&
		    ((int64_t)os_time_get_ms() < (*current)->pull->block_report_timestamp ||
...
static void broadcast_complete(struct bt_mesh_blob_cli *cli)
{
	...
	if (cli->tx.cancelled) {
		...
		__ASSERT_MSG(cli->tx.ctx.next, "No next callback");
		cli->tx.ctx.next(cli);
	}
}

static void tx_complete(struct bt_work *work)
{
...
static void retry_timeout(struct bt_work *work)
{
	 ...
	 */
	if (SENDING_CHUNKS_IN_PULL_MODE(cli)) {
		if ((int64_t)os_time_get_ms() >= cli->tx.cli_timestamp) {
	...
	__ASSERT_MSG(!cli->tx.sending, "still sending");
	__ASSERT_MSG(cli->tx.ctx.is_inited, "ctx is not initialized");
...
{
	bt_work_schedule(&cli->tx.complete, OS_MSEC(cli->tx.ctx.post_send_delay_ms));
}
...
static void xfer_start_tx(struct bt_mesh_blob_cli *cli, uint16_t dst)
{
	...
	bt_buf_simple_add_u8(&buf, cli->xfer->mode << 6);
	bt_buf_simple_add_le64(&buf, cli->xfer->id);
	bt_buf_simple_add_le32(&buf, cli->xfer->size);
	bt_buf_simple_add_u8(&buf, cli->xfer->block_size_log);
	bt_buf_simple_add_le16(&buf, BT_MESH_TX_SDU_MAX);
...
static void block_start_tx(struct bt_mesh_blob_cli *cli, uint16_t dst)
{
	...
	bt_buf_simple_add_le16(&buf, cli->block.number);
	bt_buf_simple_add_le16(&buf, cli->xfer->chunk_size);
...
static void chunk_tx_complete(struct bt_mesh_blob_cli *cli, uint16_t dst)
{
	 ...
	 */
	struct bt_mesh_blob_target *target;
	int64_t timestamp = (int64_t)os_time_get_ms() + BLOCK_REPORT_TIME_MSEC;
...
static void block_report_wait(struct bt_mesh_blob_cli *cli)
{
	...
	if (!cli->tx.cli_timestamp) {
		cli->tx.cli_timestamp = (int64_t)os_time_get_ms() + CLIENT_TIMEOUT_MSEC(cli);
	}
			      ...
			      struct bt_buf_simple *buf)
{
	...
	status_and_mode = bt_buf_simple_pull_u8(buf);
	info.status = status_and_mode & BIT_MASK(4);
	info.mode = status_and_mode >> 6;
	info.phase = bt_buf_simple_pull_u8(buf);

	if (buf->len) {
		info.id = bt_buf_simple_pull_le64(buf);
	}

	if (buf->len >= 7) {
		info.size = bt_buf_simple_pull_le32(buf);
		info.block_size_log = bt_buf_simple_pull_u8(buf);
		info.mtu_size = bt_buf_simple_pull_le16(buf);
		info.missing_blocks = bt_buf_simple_pull(buf, buf->len);
	}
			       ...
			       struct bt_buf_simple *buf)
{
	...
	status_and_format = bt_buf_simple_pull_u8(buf);
	status.status = status_and_format & BIT_MASK(4);
	status.missing = status_and_format >> 6;
	status.block.number = bt_buf_simple_pull_le16(buf);
	chunk_size = bt_buf_simple_pull_le16(buf);
	...
	case BT_MESH_BLOB_CHUNKS_MISSING_SOME:
		...
		len = buf->len;
		memcpy(status.block.missing, bt_buf_simple_pull_mem(buf, len),
		       len);
			      ...
			      struct bt_buf_simple *buf)
{
	...
	caps.min_block_size_log = bt_buf_simple_pull_u8(buf);
	caps.max_block_size_log = bt_buf_simple_pull_u8(buf);
	caps.max_chunks = bt_buf_simple_pull_le16(buf);
	caps.max_chunk_size = bt_buf_simple_pull_le16(buf);
	caps.max_size = bt_buf_simple_pull_le32(buf);
	caps.mtu_size = bt_buf_simple_pull_le16(buf);
	caps.modes = bt_buf_simple_pull_u8(buf);
...
static int blob_cli_init(const struct bt_mesh_model *mod)
{
	...
	cli->tx.cli_timestamp = 0ll;
	bt_work_init_delayable(&cli->tx.retry, retry_timeout);
	bt_work_init_delayable(&cli->tx.complete, tx_complete);
...
int bt_mesh_blob_cli_suspend(struct bt_mesh_blob_cli *cli)
{
	...
	cli->state = BT_MESH_BLOB_CLI_STATE_SUSPENDED;
	(void)bt_work_cancel_delayable(&cli->tx.retry);
	cli->tx.ctx.is_inited = 0;
	cli->tx.sending = 0;
	cli->tx.cli_timestamp = 0ll;
	return 0;
