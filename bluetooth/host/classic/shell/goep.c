...
BT_BUF_POOL_FIXED_DEFINE(tx_pool, CONFIG_BT_MAX_CONN, BT_RFCOMM_BUF_SIZE(GOEP_MOPL),
...
static int goep_parse_headers(struct bt_buf *buf)
{
	int err;
...
static void goep_server_disconnect(struct bt_obex_server *server, struct bt_buf *buf)
{
	bt_shell_print("OBEX server %p disconn req", server);
	goep_parse_headers(buf);
}

static void goep_server_put(struct bt_obex_server *server, bool final, struct bt_buf *buf)
{
...
static void goep_server_get(struct bt_obex_server *server, bool final, struct bt_buf *buf)
{
...
static void goep_server_abort(struct bt_obex_server *server, struct bt_buf *buf)
{
	bt_shell_print("OBEX server %p abort req", server);
	goep_parse_headers(buf);
}

static void goep_server_setpath(struct bt_obex_server *server, uint8_t flags, struct bt_buf *buf)
{
...
static void goep_server_action(struct bt_obex_server *server, bool final, struct bt_buf *buf)
{
...
static void goep_client_put(struct bt_obex_client *client, uint8_t rsp_code, struct bt_buf *buf)
{
...
static void goep_client_get(struct bt_obex_client *client, uint8_t rsp_code, struct bt_buf *buf)
{
...
static int cmd_register_rfcomm(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	bt_shell_print("RFCOMM server (channel %02x) is registered", rfcomm_server.rfcomm.channel);
	return 0;
}
...
static int cmd_connect_rfcomm(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (!g_app) {
		bt_shell_error("Cannot allocate goep instance");
		return -ENOMEM;
	}
	...
	if (err) {
		...
		bt_shell_print("GOEP RFCOMM connection pending");
	}
...
static int cmd_disconnect_rfcomm(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (!goep_app.conn) {
		bt_shell_error("No goep transport connection");
		return -ENOEXEC;
	}
	...
	if (err) {
		...
		bt_shell_print("GOEP RFCOMM disconnection pending");
	}
	return err;
}
...
static int cmd_register_l2cap(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	bt_shell_print("L2CAP server (psm %04x) is registered", l2cap_server.l2cap.psm);
	return 0;
}
...
static int cmd_connect_l2cap(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (err) {
		...
		bt_shell_print("GOEP L2CAP connection pending");
	}
...
static int cmd_disconnect_l2cap(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (err) {
		...
		bt_shell_print("GOEP L2CAP disconnection pending");
	}
	return err;
}
...
static int cmd_goep_client_conn(const struct bt_shell *sh, size_t argc, char *argv[])
{
	uint16_t mopl;
	int err;
...
static int cmd_goep_client_disconn(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err;
...
static int cmd_goep_client_put(const struct bt_shell *sh, size_t argc, char *argv[])
{
	bool final;
	int err = 0;
	...
	if (err) {
		bt_shell_error("Fail to send put req %d", err);
	} else {
		goep_app.tx_buf = NULL;
	}
	return err;
...
static int cmd_goep_client_get(const struct bt_shell *sh, size_t argc, char *argv[])
{
	bool final;
	int err = 0;
	...
	if (err) {
		bt_shell_error("Fail to send get req %d", err);
	} else {
		goep_app.tx_buf = NULL;
	}
	return err;
...
static int cmd_goep_client_abort(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err;
...
static int cmd_goep_client_setpath(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err;
	uint8_t flags = BIT(1);
	...
	if (err) {
		bt_shell_error("Fail to send setpath req %d", err);
	} else {
		goep_app.tx_buf = NULL;
	}
	return err;
...
static int cmd_goep_client_action(const struct bt_shell *sh, size_t argc, char *argv[])
{
	bool final;
	int err = 0;
	...
	if (err) {
		bt_shell_error("Fail to send action req %d", err);
	} else {
		goep_app.tx_buf = NULL;
	}
	return err;
...
static int cmd_goep_server_reg(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err;
	...
	if (err != 0) {
		bt_shell_error("Fail to register obex server %d", err);
	}

	return err;
}

static int cmd_goep_server_unreg(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err;
...
static int cmd_goep_server_conn(const struct bt_shell *sh, size_t argc, char *argv[])
{
	uint8_t rsp_code;
	uint16_t mopl;
	const char *rsp;
...
static int cmd_goep_server_disconn(const struct bt_shell *sh, size_t argc, char *argv[])
{
	uint8_t rsp_code;
	const char *rsp;
	int err;
...
static int cmd_goep_server_put(const struct bt_shell *sh, size_t argc, char *argv[])
{
	uint8_t rsp_code;
	const char *rsp;
	int err;
	...
	if (err) {
		bt_shell_error("Fail to send put rsp %d", err);
	} else {
		goep_app.tx_buf = NULL;
	}
	return err;
...
static int cmd_goep_server_get(const struct bt_shell *sh, size_t argc, char *argv[])
{
	uint8_t rsp_code;
	const char *rsp;
	int err;
	...
	if (err) {
		bt_shell_error("Fail to send get rsp %d", err);
	} else {
		goep_app.tx_buf = NULL;
	}
	return err;
...
static int cmd_goep_server_abort(const struct bt_shell *sh, size_t argc, char *argv[])
{
	uint8_t rsp_code;
	const char *rsp;
	int err;
...
static int cmd_goep_server_setpath(const struct bt_shell *sh, size_t argc, char *argv[])
{
	uint8_t rsp_code;
	const char *rsp;
	int err;
	...
	if (err) {
		bt_shell_error("Fail to send setpath rsp %d", err);
	} else {
		goep_app.tx_buf = NULL;
	}
	return err;
...
static int cmd_goep_server_action(const struct bt_shell *sh, size_t argc, char *argv[])
{
	uint8_t rsp_code;
	const char *rsp;
	int err;
	...
	if (err) {
		bt_shell_error("Fail to send action rsp %d", err);
	} else {
		goep_app.tx_buf = NULL;
	}
	return err;
...
BT_SHELL_SUBCMD_SET_CREATE(obex_add_header_cmds,
	...
	BT_SHELL_CMD_ARG(conn_id, NULL, "<an identifier used for OBEX connection multiplexing>",
	...
	BT_SHELL_CMD_ARG(wan_uuid, NULL, "<uniquely identifies the network client (OBEX server)>",
		      cmd_add_header_wan_uuid, 2, 0),
	BT_SHELL_CMD_ARG(obj_class, NULL, "<OBEX Object class of object>", cmd_add_header_obj_class, 2,
		      0),
...
BT_SHELL_SUBCMD_SET_CREATE(obex_client_cmds,
	...
	BT_SHELL_CMD_ARG(get, NULL, "<final: true, false>", cmd_goep_client_get, 2, 0),
	BT_SHELL_CMD_ARG(abort, NULL, HELP_NONE, cmd_goep_client_abort, 1, 0),
	BT_SHELL_CMD_ARG(setpath, NULL, "[parent] [create]", cmd_goep_client_setpath, 1, 2),
	BT_SHELL_CMD_ARG(action, NULL, "<final: true, false>", cmd_goep_client_action, 2, 0),
	BT_SHELL_SUBCMD_SET_END
);
...
BT_SHELL_SUBCMD_SET_CREATE(obex_server_cmds,
	...
	BT_SHELL_CMD_ARG(put, NULL, "<rsp: continue, success, error> [rsp_code]", cmd_goep_server_put,
		      2, 1),
	BT_SHELL_CMD_ARG(get, NULL, "<rsp: continue, success, error> [rsp_code]", cmd_goep_server_get,
		      2, 1),
	BT_SHELL_CMD_ARG(abort, NULL, "<rsp: continue, success, error> [rsp_code]",
		      cmd_goep_server_abort, 2, 1),
	BT_SHELL_CMD_ARG(setpath, NULL, "<rsp: continue, success, error> [rsp_code]",
		      cmd_goep_server_setpath, 2, 1),
	BT_SHELL_CMD_ARG(action, NULL, "<rsp: continue, success, error> [rsp_code]",
...
{
	if (goep_app.tx_buf) {
		bt_shell_error("Buf %p is using", goep_app.tx_buf);
		return -EBUSY;
	}
