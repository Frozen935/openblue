...
int bt_conn_auth_pincode_entry(struct bt_conn *conn, const char *pin)
{
	...
	if (!bt_atomic_test_and_clear_bit(conn->flags, BT_CONN_USER)) {
		return -EPERM;
	}

	if (len == 16) {
		bt_atomic_set_bit(conn->flags, BT_CONN_BR_LEGACY_SECURE);
	}
...
static void pin_code_req(struct bt_conn *conn)
{
	if (bt_auth && bt_auth->pincode_entry) {
		...
		bt_atomic_set_bit(conn->flags, BT_CONN_USER);
		bt_atomic_set_bit(conn->flags, BT_CONN_BR_PAIRING);
...
static void ssp_pairing_complete(struct bt_conn *conn, uint8_t status)
{
	...
	bt_atomic_set_bit_to(conn->flags, BT_CONN_BR_PAIRED, !status);

	LOG_DBG("Pairing completed status %d", status);

	if (!status) {
		bool bond = !bt_atomic_test_bit(conn->flags, BT_CONN_BR_NOBOND);
...
static int conn_auth(struct bt_conn *conn)
{
	...
	auth = bt_buf_add(buf, sizeof(*auth));
	auth->handle = sys_cpu_to_le16(conn->handle);

	bt_atomic_set_bit(conn->flags, BT_CONN_BR_PAIRING_INITIATOR);
...
{
	if (bt_atomic_test_bit(conn->flags, BT_CONN_BR_PAIRING)) {
		return -EBUSY;
	}
...
void bt_hci_link_key_notify(struct bt_buf *buf)
{
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS) &&
	    !bt_atomic_test_bit(conn->flags, BT_CONN_BR_NOBOND)) {
...
void io_capa_neg_reply(const bt_addr_t *bdaddr, const uint8_t reason)
{
	...
	resp_buf = bt_hci_cmd_alloc(OS_TIMEOUT_FOREVER);
	if (!resp_buf) {
		LOG_ERR("Out of command buffers");
		return;
	}

	cp = bt_buf_add(resp_buf, sizeof(*cp));
	bt_addr_copy(&cp->bdaddr, bdaddr);
	cp->reason = reason;
...
void bt_hci_io_capa_resp(struct bt_buf *buf)
{
	...
	if (bt_atomic_test_bit(conn->flags, BT_CONN_BR_PAIRING_INITIATOR) &&
	    (evt->authentication > BT_HCI_NO_BONDING_MITM) &&
	    !bt_atomic_test_bit(conn->flags, BT_CONN_BR_BONDABLE)) {
		/*
...
void bt_hci_io_capa_req(struct bt_buf *buf)
{
	...
	if (bt_atomic_test_bit(conn->flags, BT_CONN_BR_PAIRING_INITIATOR)) {
		if (get_io_capa() != BT_IO_NO_INPUT_OUTPUT) {
			if (bt_atomic_test_bit(conn->flags, BT_CONN_BR_GENERAL_BONDING)) {
		 ...
		 */
		if (!bt_atomic_test_bit(conn->flags, BT_CONN_BR_BONDABLE) &&
...
void bt_hci_ssp_complete(struct bt_buf *buf)
{
	...
	if (ssp_get_auth(conn) < BT_HCI_DEDICATED_BONDING) {
		bt_atomic_set_bit(conn->flags, BT_CONN_BR_NOBOND);
	}
...
static void link_encr(const uint16_t handle)
{
	...
	encr = bt_buf_add(buf, sizeof(*encr));
