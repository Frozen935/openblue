				...
				struct bt_buf_simple *buf)
{
	...
	divisor = bt_buf_simple_pull_u8(buf);
				   ...
				   struct bt_buf_simple *buf)
{
	...
	attention = bt_buf_simple_pull_u8(buf);
				     ...
				     uint8_t attention, uint8_t *updated_attention)
{
	...
	bt_buf_simple_add_u8(&msg, attention);
				  ...
				  uint8_t divisor, uint8_t *updated_divisor)
{
	...
	bt_buf_simple_add_u8(&msg, divisor);
				  ...
				  size_t *fault_count)
{
	...
	bt_buf_simple_add_u8(&msg, test_id);
...
static int health_cli_init(const struct bt_mesh_model *model)
{
	...
	bt_buf_simple_init_with_data(&cli->pub_buf, cli->pub_data, sizeof(cli->pub_data));
...
static void health_cli_reset(const struct bt_mesh_model *model)
{
	...
	bt_buf_simple_reset(cli->pub.msg);
}
