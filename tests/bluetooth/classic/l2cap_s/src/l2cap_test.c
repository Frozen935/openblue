...
BT_BUF_POOL_FIXED_DEFINE(l2cap_s_data_tx_pool, 1, BT_L2CAP_SDU_BUF_SIZE(DATA_POOL_SIZE),
			 CONFIG_BT_CONN_TX_USER_DATA_SIZE, NULL);
BT_BUF_POOL_FIXED_DEFINE(l2cap_s_data_rx_pool, 1, DATA_POOL_SIZE, 8, NULL);
...
static struct bt_buf *l2cap_alloc_buf(struct bt_l2cap_chan *chan)
{
	...
	return bt_buf_alloc(&l2cap_s_data_rx_pool, OS_TIMEOUT_NO_WAIT);
}
...
static int cmd_l2cap_send(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (br_l2cap[id].active == true) {
		buf = bt_buf_alloc(&l2cap_s_data_tx_pool, OS_MSEC(2000));
		if (buf == NULL) {
		...
		bt_buf_add_mem(buf, argv[2], send_len);
		...
		bt_shell_print("channel %d is invalid", id);
		return -EINVAL;
	}
	return 0;
}
...
static int cmd_l2cap_register(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	bt_shell_print("L2CAP psm %u registered", l2cap_server->psm);

	return 0;
}

static int cmd_change_mtu(const struct bt_shell *sh, size_t argc, char *argv[])
{
	data_bredr_mtu = strtoul(argv[1], NULL, 16);
	return 0;
}

static int cmd_read_mtu(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (br_l2cap[id].active == true) {
		if (!strcmp(role, "local")) {
			bt_shell_print("local mtu = %d", br_l2cap[id].chan.rx.mtu);
		} else if (!strcmp(role, "peer")) {
			bt_shell_print("peer mtu = %d", br_l2cap[id].chan.tx.mtu);
		} else {
			bt_shell_error("role must be local or peer");
			return -EINVAL;
		}
	}
	return 0;
...
BT_SHELL_STATIC_SUBCMD_SET_CREATE(l2cap_br_cmds,
	BT_SHELL_CMD_ARG(register, NULL, HELP_REGISTER, cmd_l2cap_register, 1, 2),
	BT_SHELL_CMD_ARG(connect, NULL, HELP_CONNECT, cmd_connect, 1, 4),
	BT_SHELL_CMD_ARG(disconnect, NULL, "<id>", cmd_l2cap_disconnect, 2, 0),
	BT_SHELL_CMD_ARG(send, NULL, "<id> <length of data> <data> ", cmd_l2cap_send, 4, 0),
	BT_SHELL_CMD_ARG(change_mtu, NULL, "<mtu>", cmd_change_mtu, 2, 0),
	BT_SHELL_CMD_ARG(read_mtu, NULL, "<id> <local/peer>", cmd_read_mtu, 3, 0),
