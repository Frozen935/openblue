...
#define BUF_TIMEOUT                 OS_TIMEOUT_NO_WAIT
...
static struct seg_tx {
	...
	void                  *cb_data;
	struct bt_work_delayable retransmit;    /* Retransmit timer */
...
static struct seg_rx {
	...
	uint32_t                    block;
	uint32_t                    last_ack;
	struct bt_work_delayable    ack;
	struct bt_work_delayable    discard;
} seg_rx[CONFIG_BT_MESH_RX_SEG_MSG_COUNT];

BT_MEM_POOL_DEFINE(segs, BT_MESH_APP_SEG_SDU_MAX, CONFIG_BT_MESH_SEG_BUFS, 4);

static int send_unseg(struct bt_mesh_net_tx *tx, struct bt_buf_simple *sdu,
		      ...
		      const uint8_t *ctl_op)
{
	...
	bt_buf_simple_reserve(&adv->b, BT_MESH_NET_HDR_LEN);

	if (ctl_op) {
		bt_buf_simple_add_u8(&adv->b, TRANS_CTL_HDR(*ctl_op, 0));
	} else if (BT_MESH_IS_DEV_KEY(tx->ctx->app_idx)) {
		bt_buf_simple_add_u8(&adv->b, UNSEG_HDR(0, 0));
	} else {
		bt_buf_simple_add_u8(&adv->b, UNSEG_HDR(1, tx->aid));
	}

	bt_buf_simple_add_mem(&adv->b, sdu->data, sdu->len);
...
{
	bt_mem_pool_free(&segs, (void *)tx->seg[seg_idx]);
	tx->seg[seg_idx] = NULL;
	tx->nack_count--;
}
...
static void seg_tx_unblock_check(struct seg_tx *tx)
{
	...
	if (blocked) {
		...
		bt_work_reschedule(&blocked->retransmit, OS_TIMEOUT_NO_WAIT);
	}
}
...
static void seg_tx_reset(struct seg_tx *tx)
{
	...
	(void)bt_work_cancel_delayable(&tx->retransmit);
...
static void schedule_transmit_continue(struct seg_tx *tx, uint32_t delta)
{
	 ...
	 */
	bt_work_reschedule(&tx->retransmit,
			  (tx->seg_o <= tx->seg_n) ?
					OS_MSEC(timeout) :
					OS_TIMEOUT_NO_WAIT);
}
			     ...
			     struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(buf, tx->hdr);
	bt_buf_simple_add_u8(buf, (tx->aszmic << 7) | seq_zero >> 6);
	bt_buf_simple_add_u8(buf, (((seq_zero & 0x3f) << 2) | (seg_o >> 3)));
	bt_buf_simple_add_u8(buf, ((seg_o & 0x07) << 5) | tx->seg_n);
	bt_buf_simple_add_mem(buf, tx->seg[seg_o], len);
}
...
static void seg_tx_send_unacked(struct seg_tx *tx)
{
	...
	while (tx->seg_o <= tx->seg_n) {
		...
		bt_buf_simple_reserve(&seg->b, BT_MESH_NET_HDR_LEN);
	...
	bt_work_reschedule(&tx->retransmit, OS_MSEC(timeout));
}

static void seg_retransmit(struct bt_work *work)
{
...
static int send_seg(struct bt_mesh_net_tx *net_tx, struct bt_buf_simple *sdu,
		    const struct bt_mesh_send_cb *cb, void *cb_data,
		    uint8_t *ctl_op)
{
	bool blocked = false;
	...
	for (seg_o = 0U; sdu->len; seg_o++) {
		...
		err = bt_mem_pool_alloc(&segs, &buf, BUF_TIMEOUT);
		if (err) {
		...
		memcpy(buf, bt_buf_simple_pull_mem(sdu, len), len);
...
int bt_mesh_trans_send(struct bt_mesh_net_tx *tx, struct bt_buf_simple *msg,
		       const struct bt_mesh_send_cb *cb, void *cb_data)
{
	...
	if (bt_buf_simple_tailroom(msg) < BT_MESH_MIC_SHORT) {
	...
	if (!tx->ctx->send_rel || bt_buf_simple_tailroom(msg) < 8) {
		tx->aszmic = 0U;
	} else {
		tx->aszmic = 1U;
	}
...
static void seg_rx_assemble(struct seg_rx *rx, struct bt_buf_simple *buf,
			    uint8_t aszmic)
{
	int i;
	...
	for (i = 0; i <= rx->seg_n; i++) {
		bt_buf_simple_add_mem(buf, rx->seg[i],
				       MIN(seg_len(rx->ctl),
...
static int sdu_recv(struct bt_mesh_net_rx *rx, uint8_t hdr, uint8_t aszmic,
		    struct bt_buf_simple *buf, struct bt_buf_simple *sdu,
		    struct seg_rx *seg)
{
...
static int trans_ack(struct bt_mesh_net_rx *rx, uint8_t hdr,
		     struct bt_buf_simple *buf, uint64_t *seq_auth)
{
	...
	seq_zero = bt_buf_simple_pull_be16(buf);
	obo = seq_zero >> 15;
	...
	ack = bt_buf_simple_pull_be32(buf);
	...
	*seq_auth = tx->seq_auth;
	...
	if (tx->nack_count) {
		 ...
		 */
		if (tx->seg_o == 0) {
			os_timeout_t timeout = OS_TIMEOUT_NO_WAIT;
			...
			if (delta_ms < BT_MESH_SAR_TX_SEG_INT_MS) {
				timeout = OS_MSEC(BT_MESH_SAR_TX_SEG_INT_MS - delta_ms);
			}

reschedule:
			bt_work_reschedule(&tx->retransmit, timeout);
		} else {
			tx->ack_received = 1U;
		}
	} else {
...
static int trans_unseg(struct bt_buf_simple *buf, struct bt_mesh_net_rx *rx,
		       uint64_t *seq_auth)
{
	BT_BUF_SIMPLE_DEFINE_STATIC(sdu, BT_MESH_SDU_UNSEG_MAX);
	uint8_t hdr;
...
static void seg_rx_reset(struct seg_rx *rx, bool full_reset)
{
	 ...
	 * it checks rx->in_use.
	 */
	(void)bt_work_cancel_delayable(&rx->ack);
	(void)bt_work_cancel_delayable(&rx->discard);
	...
	for (i = 0; i <= rx->seg_n; i++) {
		...
		bt_mem_pool_free(&segs, rx->seg[i]);
		rx->seg[i] = NULL;
	}
...
static void seg_discard(struct bt_work *work)
{
	struct bt_work_delayable *dwork = bt_work_delayable_from_work(work);
...
static void seg_ack(struct bt_work *work)
{
	struct bt_work_delayable *dwork = bt_work_delayable_from_work(work);
	...
	if (rx->seg_n > BT_MESH_SAR_RX_SEG_THRESHOLD) {
		--rx->attempts_left;
		bt_work_schedule(&rx->ack, OS_MSEC(BT_MESH_SAR_RX_SEG_INT_MS));
	}
}
				   ...
				   uint8_t seg_n)
{
	 ...
	 */
	if (bt_mem_pool_num_free_get(&segs) < 1) {
...
static int trans_seg(struct bt_buf_simple *buf, struct bt_mesh_net_rx *net_rx,
		     ...
		     uint8_t *seg_count)
{
	...
	seq_zero = (seq_zero >> 2) & TRANS_SEQ_ZERO_MASK;
	seg_n = bt_buf_simple_pull_u8(buf);
	seg_o |= seg_n >> 5;
	seg_n &= 0x1f;
...
found_rx:
	...
	bt_work_schedule(&rx->discard,
			OS_MSEC(BT_MESH_SAR_RX_DISCARD_TIMEOUT_MS));
	...
	if (!bt_mesh_lpn_established() && BT_MESH_ADDR_IS_UNICAST(rx->dst)) {
		...
		bt_work_reschedule(&rx->ack, OS_MSEC(ACK_DELAY(rx->seg_n)));
	}

	/* Allocated segment here */
	err = bt_mem_pool_alloc(&segs, &rx->seg[seg_o], OS_TIMEOUT_NO_WAIT);
	if (err) {
	...
	*pdu_type = BT_MESH_FRIEND_PDU_COMPLETE;
	...
	if (net_rx->ctl) {
		...
		BT_BUF_SIMPLE_DEFINE_STATIC(seg_buf, BT_MESH_RX_SDU_MAX);
		struct bt_buf_simple sdu;
		 ...
		 * decryption attempt.
		 */
		bt_buf_simple_init(&seg_buf, 0);
		bt_buf_simple_init_with_data(
...
int bt_mesh_trans_recv(struct bt_buf_simple *buf, struct bt_mesh_net_rx *rx)
{
	...
	bt_buf_simple_pull(buf, BT_MESH_NET_HDR_LEN);
...
void bt_mesh_trans_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(seg_tx); i++) {
		bt_work_init_delayable(&seg_tx[i].retransmit, seg_retransmit);
	}

	for (i = 0; i < ARRAY_SIZE(seg_rx); i++) {
		bt_work_init_delayable(&seg_rx[i].ack, seg_ack);
		bt_work_init_delayable(&seg_rx[i].discard, seg_discard);
	}
}
