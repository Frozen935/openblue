...
struct scanner_state {
	...
	os_mutex_t scan_update_mutex;
	os_mutex_t scan_explicit_params_mutex;
};
...
static bt_slist_t scan_cbs = BT_SLIST_STATIC_INIT(&scan_cbs);

static struct scanner_state scan_state;

#if defined(CONFIG_BT_EXT_ADV)
/* A buffer used to reassemble advertisement data from the controller. */
BT_BUF_SIMPLE_DEFINE(ext_scan_buf, CONFIG_BT_EXT_SCAN_BUF_SIZE);
...
{
	bt_buf_simple_reset(&ext_scan_buf);
...
static bt_slist_t pa_sync_cbs = BT_SLIST_STATIC_INIT(&pa_sync_cbs);
...
void bt_scan_reset(void)
{
	...
	os_mutex_init(&scan_state.scan_update_mutex);
	os_mutex_init(&scan_state.scan_explicit_params_mutex);
	bt_scan_softreset();
}
...
static int start_le_scan_ext(struct bt_le_scan_param *scan_param)
{
	...
	if (scan_param->timeout > 0) {
		bt_atomic_set_bit(bt_dev.flags, BT_DEV_SCAN_LIMITED);
		...
		if (IS_ENABLED(CONFIG_BT_PRIVACY) && !bt_id_rpa_is_new()) {
			bt_atomic_clear_bit(bt_dev.flags, BT_DEV_RPA_VALID);
		}
	}
	...
	if (phy_1m) {
		...
		bt_buf_add_mem(buf, phy_1m, sizeof(*phy_1m));
	}

	if (phy_coded) {
		set_param->phys |= BT_HCI_LE_EXT_SCAN_PHY_CODED;
		bt_buf_add_mem(buf, phy_coded, sizeof(*phy_coded));
	}
...
static int start_le_scan_legacy(struct bt_le_scan_param *param)
{
	...
	bt_buf_add_mem(buf, &set_param, sizeof(set_param));
...
{
	return bt_atomic_test_bit(bt_dev.flags, BT_DEV_SCANNING) &&
...
static void select_scan_params(struct bt_le_scan_param *scan_param)
{
	...
	if (bt_atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_EXPLICIT_SCAN)) {
	...
	else if (bt_atomic_test_bit(bt_dev.flags, BT_DEV_INITIATING)) {
	...
	/* 3. Priority: choose custom parameters */
		...
		*scan_param = (struct bt_le_scan_param){
		...
		if (bt_atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_PER_SYNC) ||
		    bt_atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_CONN)) {
...
{
	bool is_scanning = bt_atomic_test_bit(bt_dev.flags, BT_DEV_SCANNING);

	/* Check if there is reason to have the scanner running */
	if (bt_atomic_get(scan_state.scan_flags) != 0) {
		if (is_scanning) {
...
static int scan_update(void)
{
	...
	err = os_mutex_lock(&scan_state.scan_update_mutex, OS_TIMEOUT_NO_WAIT);
	...
	os_mutex_unlock(&scan_state.scan_update_mutex);
...
static int scan_check_if_state_allowed(enum bt_le_scan_user flag)
{
	...
	if (bt_atomic_test_bit(scan_state.scan_flags, flag)) {
		return -EALREADY;
	}

	if (flag == BT_LE_SCAN_USER_EXPLICIT_SCAN && !BT_LE_STATES_SCAN_INIT(bt_dev.le.states) &&
	    bt_atomic_test_bit(bt_dev.flags, BT_DEV_INITIATING)) {
		return -EPERM;
	}
...
int bt_le_scan_user_add(enum bt_le_scan_user flag)
{
	...
	bt_atomic_set_bit(scan_state.scan_flags, flag);

	err = scan_update();
	if (err) {
		bt_atomic_clear_bit(scan_state.scan_flags, flag);
	}
			       ...
			       uint8_t adv_props)
{
	...
	if (!BT_LE_STATES_SCAN_INIT(bt_dev.le.states) &&
	    bt_atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_EXPLICIT_SCAN)) {
		return;
	}
...
static void le_adv_recv(bt_addr_le_t *addr, struct bt_le_scan_recv_info *info,
			struct bt_buf_simple *buf, uint16_t len)
{
	struct bt_le_scan_cb *listener, *next;
	struct bt_buf_simple_state state;
	bt_addr_le_t id_addr;
	bool explicit_scan = bt_atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_EXPLICIT_SCAN);
	bool conn_scan = bt_atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_CONN);
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&scan_cbs, listener, next, node) {
		if (listener->recv) {
			bt_buf_simple_save(buf, &state);
...
#if defined(CONFIG_BT_EXT_ADV)
void bt_hci_le_scan_timeout(struct bt_buf *buf)
{
	struct bt_le_scan_cb *listener, *next;
	...
	bt_atomic_clear_bit(bt_dev.flags, BT_DEV_SCAN_LIMITED);
	bt_atomic_clear_bit(bt_dev.flags, BT_DEV_RPA_VALID);
...
void bt_hci_le_adv_ext_report(struct bt_buf *buf)
{
	uint8_t num_reports = bt_buf_pull_u8(buf);
	bool explicit_scan = bt_atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_EXPLICIT_SCAN);
	bool conn_scan = bt_atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_CONN);
	...
	while (num_reports--) {
		...
		bt_buf_simple_add_mem(&ext_scan_buf, buf->data, evt->length);
		if (more_to_come) {
...
{
	bt_atomic_clear(per_adv_sync->flags);
}
...
static struct bt_le_per_adv_sync *per_adv_sync_new(void)
{
	...
	for (int i = 0; i < ARRAY_SIZE(per_adv_sync_pool); i++) {
		if (!bt_atomic_test_bit(per_adv_sync_pool[i].flags, BT_PER_ADV_SYNC_CREATED)) {
	...
	bt_atomic_set_bit(per_adv_sync->flags, BT_PER_ADV_SYNC_CREATED);

#if CONFIG_BT_PER_ADV_SYNC_BUF_SIZE > 0
	bt_buf_simple_init_with_data(&per_adv_sync->reassembly, per_adv_sync->reassembly_data,
				      CONFIG_BT_PER_ADV_SYNC_BUF_SIZE);
	bt_buf_simple_reset(&per_adv_sync->reassembly);
...
{
	for (int i = 0; i < ARRAY_SIZE(per_adv_sync_pool); i++) {
		if (bt_atomic_test_bit(per_adv_sync_pool[i].flags, BT_PER_ADV_SYNC_SYNCING)) {
...
struct bt_le_per_adv_sync *bt_hci_per_adv_sync_lookup_handle(uint16_t handle)
{
	for (int i = 0; i < ARRAY_SIZE(per_adv_sync_pool); i++) {
		if (per_adv_sync_pool[i].handle == handle &&
		    bt_atomic_test_bit(per_adv_sync_pool[i].flags, BT_PER_ADV_SYNC_SYNCED)) {
			return &per_adv_sync_pool[i];
		}
	}
				   ...
				   const struct bt_le_per_adv_sync_recv_info *info)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&pa_sync_cbs, listener, tmp, node) {
		if (listener->recv) {
			bt_buf_simple_save(buf, &state);
...
static void bt_hci_le_per_adv_report_common(struct bt_buf *buf)
{
	...
	if (bt_atomic_test_bit(per_adv_sync->flags, BT_PER_ADV_SYNC_RECV_DISABLED)) {
	...
	if (!per_adv_sync->report_truncated) {
#if CONFIG_BT_PER_ADV_SYNC_BUF_SIZE > 0
		if (bt_buf_simple_tailroom(&per_adv_sync->reassembly) < evt->length) {
		...
		if (evt->data_status == BT_HCI_LE_ADV_EVT_TYPE_DATA_STATUS_COMPLETE) {
			if (per_adv_sync->reassembly.len == 0) {
				...
				bt_buf_simple_add_mem(&per_adv_sync->reassembly, buf->data,
						       evt->length);
			...
			bt_buf_simple_add_mem(&per_adv_sync->reassembly, buf->data, evt->length);
#if defined(CONFIG_BT_PER_ADV_SYNC_RSP)
...
void bt_hci_le_per_adv_report(struct bt_buf *buf)
{
...
static void bt_hci_le_per_adv_sync_established_common(struct bt_buf *buf)
{
#if defined(CONFIG_BT_PER_ADV_SYNC_RSP)
	...
	if (pending_per_adv_sync) {
		bt_atomic_clear_bit(pending_per_adv_sync->flags, BT_PER_ADV_SYNC_SYNCING);
	...
	if (!pending_per_adv_sync ||
	    (!bt_atomic_test_bit(pending_per_adv_sync->flags, BT_PER_ADV_SYNC_SYNCING_USE_LIST) &&
	...
	if (unexpected_evt || evt->status != BT_HCI_ERR_SUCCESS) {
		if (pending_per_adv_sync) {
			...
			if (bt_atomic_test_bit(pending_per_adv_sync->flags,
	...
	bt_atomic_set_bit(pending_per_adv_sync->flags, BT_PER_ADV_SYNC_SYNCED);
	...
	if (bt_atomic_test_bit(pending_per_adv_sync->flags, BT_PER_ADV_SYNC_SYNCING_USE_LIST)) {
	...
	sync_info.recv_enabled =
		!bt_atomic_test_bit(pending_per_adv_sync->flags, BT_PER_ADV_SYNC_RECV_DISABLED);
...
void bt_hci_le_per_adv_sync_established(struct bt_buf *buf)
{
	if (IS_ENABLED(CONFIG_BT_PER_ADV_SYNC_RSP)) {
		LOG_ERR("The controller shall raise the latest unmasked version of the event");
				...
				struct bt_le_per_adv_sync_subevent_params *params)
{
	...
	bt_buf_add_mem(buf, params->subevents, cp->num_subevents);
...
int bt_le_per_adv_set_response_data(struct bt_le_per_adv_sync *per_adv_sync,
				    ...
				    const struct bt_buf_simple *data)
{
	...
	bt_buf_add_mem(buf, data->data, cp->response_data_length);

	return bt_hci_cmd_send_sync(BT_HCI_OP_LE_SET_PER_ADV_RESPONSE_DATA, buf, NULL);
}
#endif /* CONFIG_BT_PER_ADV_SYNC_RSP */

void bt_hci_le_per_adv_sync_lost(struct bt_buf *buf)
{
...
static void bt_hci_le_past_received_common(struct bt_buf *buf)
{
#if defined(CONFIG_BT_PER_ADV_SYNC_RSP)
	...
	bt_atomic_set_bit(per_adv_sync->flags, BT_PER_ADV_SYNC_SYNCED);
...
void bt_hci_le_past_received(struct bt_buf *buf)
{
	if (IS_ENABLED(CONFIG_BT_PER_ADV_SYNC_RSP)) {
		LOG_ERR("The controller shall raise the latest unmasked version of the event");
...
#if defined(CONFIG_BT_PER_ADV_SYNC_RSP)
void bt_hci_le_past_received_v2(struct bt_buf *buf)
{
	bt_hci_le_past_received_common(buf);
}
#endif /* CONFIG_BT_PER_ADV_SYNC_RSP */
...
#if defined(CONFIG_BT_PER_ADV_SYNC_RSP)
void bt_hci_le_per_adv_sync_established_v2(struct bt_buf *buf)
{
	bt_hci_le_per_adv_sync_established_common(buf);
}

void bt_hci_le_per_adv_report_v2(struct bt_buf *buf)
{
	bt_hci_le_per_adv_report_common(buf);
}
#endif /* CONFIG_BT_PER_ADV_SYNC_RSP */
...
void bt_hci_le_biginfo_adv_report(struct bt_buf *buf)
{
...
static void bt_hci_le_df_connectionless_iq_report_common(uint8_t event, struct bt_buf *buf)
{
	int err;
...
void bt_hci_le_df_connectionless_iq_report(struct bt_buf *buf)
{
...
void bt_hci_le_vs_df_connectionless_iq_report(struct bt_buf *buf)
{
...
void bt_hci_le_adv_report(struct bt_buf *buf)
{
	uint8_t num_reports = bt_buf_pull_u8(buf);
	...
	while (num_reports--) {
		...
		bt_buf_pull(buf, evt->length + sizeof(adv_info.rssi));
	}
}
...
int bt_le_scan_start(const struct bt_le_scan_param *param, bt_le_scan_cb_t cb)
{
	 ...
	 */
	err = os_mutex_lock(&scan_state.scan_explicit_params_mutex, OS_TIMEOUT_NO_WAIT);
	...
	if (err) {
		os_mutex_unlock(&scan_state.scan_explicit_params_mutex);
		return err;
	}
...
int bt_le_scan_stop(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_EXT_ADV) &&
	    bt_atomic_test_and_clear_bit(bt_dev.flags, BT_DEV_SCAN_LIMITED)) {
		bt_atomic_clear_bit(bt_dev.flags, BT_DEV_RPA_VALID);
...
{
	if (bt_slist_find(&scan_cbs, &cb->node, NULL)) {
		return -EEXIST;
	}

	bt_slist_append(&scan_cbs, &cb->node);
...
{
	bt_slist_find_and_remove(&scan_cbs, &cb->node);
}
			      ...
			      struct bt_le_per_adv_sync **out_sync)
{
	...
	if (param->options & BT_LE_PER_ADV_SYNC_OPT_USE_PER_ADV_LIST) {
		bt_atomic_set_bit(per_adv_sync->flags, BT_PER_ADV_SYNC_SYNCING_USE_LIST);
	...
	if (param->options & BT_LE_PER_ADV_SYNC_OPT_REPORTING_INITIALLY_DISABLED) {
		...
		bt_atomic_set_bit(per_adv_sync->flags, BT_PER_ADV_SYNC_RECV_DISABLED);
	}
	...
	bt_atomic_set_bit(per_adv_sync->flags, BT_PER_ADV_SYNC_SYNCING);
...
static int bt_le_per_adv_sync_terminate(struct bt_le_per_adv_sync *per_adv_sync)
{
	...
	if (!bt_atomic_test_bit(per_adv_sync->flags, BT_PER_ADV_SYNC_SYNCED)) {
		return -EINVAL;
	}
...
{
	if (bt_slist_find(&pa_sync_cbs, &cb->node, NULL)) {
		return -EEXIST;
	}

	bt_slist_append(&pa_sync_cbs, &cb->node);
...
int bt_le_per_adv_sync_cb_unregister(struct bt_le_per_adv_sync_cb *cb)
{
	...
	if (!bt_slist_find_and_remove(&pa_sync_cbs, &cb->node)) {
		return -ENOENT;
	}
...
static int bt_le_set_per_adv_recv_enable(struct bt_le_per_adv_sync *per_adv_sync, bool enable)
{
	...
	if ((enable && !bt_atomic_test_bit(per_adv_sync->flags, BT_PER_ADV_SYNC_RECV_DISABLED)) ||
	    (!enable && bt_atomic_test_bit(per_adv_sync->flags, BT_PER_ADV_SYNC_RECV_DISABLED))) {
		return -EALREADY;
	}
	...
	info.recv_enabled = !bt_atomic_test_bit(per_adv_sync->flags, BT_PER_ADV_SYNC_RECV_DISABLED);
...
{
	return bt_atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_EXPLICIT_SCAN);
}
