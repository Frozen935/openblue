...
struct bt_mesh_net bt_mesh = {
	.local_queue = BT_SLIST_STATIC_INIT(&bt_mesh.local_queue),
...
BT_MEM_POOL_DEFINE(loopback_buf_pool,
...
static bool check_dup(struct bt_buf_simple *data)
{
	const uint8_t *tail = bt_buf_simple_tail(data);
	uint32_t val;
	int i;
...
static void bt_mesh_net_local(struct bt_work *work)
{
	struct bt_buf_simple sbuf;
	bt_snode_t *node;

	while ((node = bt_slist_get(&bt_mesh.local_queue))) {
		...
		bt_mem_pool_free(&loopback_buf_pool, (void *)buf);
	}
}
			     ...
			     struct bt_buf_simple *buf)
{
	...
	if (ctl && bt_buf_simple_tailroom(buf) < 8) {
		LOG_ERR("Insufficient MIC space for CTL PDU");
		return -EINVAL;
	} else if (bt_buf_simple_tailroom(buf) < 4) {
	...
	bt_buf_simple_push_be16(buf, tx->ctx->addr);
	bt_buf_simple_push_be16(buf, tx->src);
	bt_buf_simple_push_be24(buf, bt_mesh_next_seq());

	if (ctl) {
		bt_buf_simple_push_u8(buf, tx->ctx->send_ttl | 0x80);
	} else {
		bt_buf_simple_push_u8(buf, tx->ctx->send_ttl);
	}

	bt_buf_simple_push_u8(buf, (nid | (BT_MESH_NET_IVI_TX & 1) << 7));

	return 0;
}

static int net_encrypt(struct bt_buf_simple *buf,
...
int bt_mesh_net_encode(struct bt_mesh_net_tx *tx, struct bt_buf_simple *buf,
		    ...
		    size_t len)
{
	...
	err = bt_mem_pool_alloc(&loopback_buf_pool, (void **)&buf, OS_TIMEOUT_NO_WAIT);
	if (err) {
	...
	bt_slist_append(&bt_mesh.local_queue, &buf->node);

	bt_work_submit(&bt_mesh.local_work);
		     ...
		     const struct bt_mesh_send_cb *cb, void *cb_data)
{
	...
	LOG_DBG("src 0x%04x dst 0x%04x len %u headroom %zu tailroom %zu", tx->src, tx->ctx->addr,
		adv->b.len, bt_buf_simple_headroom(&adv->b), bt_buf_simple_tailroom(&adv->b));
...
void bt_mesh_net_loopback_clear(uint16_t net_idx)
{
	...
	while ((node = bt_slist_get(&bt_mesh.local_queue))) {
		...
		if (net_idx == BT_MESH_KEY_ANY || net_idx == buf->sub->net_idx) {
			...
			bt_slist_append(&new_list, &buf->node);
		}
	}
...
static bool net_decrypt(struct bt_mesh_net_rx *rx, struct bt_buf_simple *in,
			...
			const struct bt_mesh_net_cred *cred)
{
	...
	bt_buf_simple_add_mem(out, in->data, in->len);
...
static void bt_mesh_net_relay(struct bt_buf_simple *sbuf, struct bt_mesh_net_rx *rx, bool bridge)
{
	const struct bt_mesh_net_cred *cred;
	...
	bt_buf_simple_add_mem(&adv->b, sbuf->data, sbuf->len);
...
static void bt_mesh_sbr_check_cb(uint16_t new_net_idx, void *user_data)
{
	...
	if (new_net_idx < BT_MESH_BRG_CFG_NETIDX_NOMATCH) {
		...
		bt_buf_simple_restore(ctx->sbuf, ctx->state);
		bt_mesh_net_relay(ctx->sbuf, ctx->rx, true);
	}
}
#endif

void bt_mesh_net_header_parse(struct bt_buf_simple *buf,
			      struct bt_mesh_net_rx *rx)
{
...
int bt_mesh_net_decode(struct bt_buf_simple *in, enum bt_mesh_net_if net_if,
		       struct bt_mesh_net_rx *rx, struct bt_buf_simple *out)
{
...
void bt_mesh_net_recv(struct bt_buf_simple *data, int8_t rssi,
		      enum bt_mesh_net_if net_if)
{
	BT_BUF_SIMPLE_DEFINE(buf, BT_MESH_NET_MAX_PDU_LEN);
...
static void ivu_refresh(struct bt_work *work)
{
	if (!bt_mesh_is_provisioned()) {
		return;
	}
...
{
	bt_work_init_delayable(&bt_mesh.ivu_timer, ivu_refresh);
