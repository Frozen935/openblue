...
static struct bt_work_delayable beacon_timer;
...
static void beacon_start(uint16_t duration, int err, void *user_data)
{
	if (err) {
		...
		if (beacon_send_sub_curr) {
			bt_work_reschedule(&beacon_timer, OS_TIMEOUT_NO_WAIT);
		}
	}
}
				...
				struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(buf, BEACON_TYPE_SECURE);

	keys = &sub->keys[SUBNET_KEY_TX_IDX(sub)];

	bt_buf_simple_add_u8(buf, flags);

	/* Network ID */
	bt_buf_simple_add_mem(buf, keys->net_id, 8);

	/* IV Index */
	bt_buf_simple_add_be32(buf, bt_mesh.iv_index);

	bt_buf_simple_add_mem(buf, sub->secure_beacon.auth, 8);
				 ...
				 struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(buf, BEACON_TYPE_PRIVATE);
	bt_buf_simple_add_mem(buf, priv_random.val, 13);
	bt_buf_simple_add_mem(buf, sub->priv_beacon_ctx.data, 5);
	bt_buf_simple_add_mem(buf, sub->priv_beacon.auth, 8);
...
int bt_mesh_beacon_create(struct bt_mesh_subnet *sub, struct bt_buf_simple *buf, bool priv)
{
#if defined(CONFIG_BT_MESH_PRIV_BEACONS)
	if (priv) {
...
static int net_beacon_for_subnet_send(struct bt_mesh_subnet *sub)
{
	...
	struct {
		struct bt_mesh_beacon *beacon;
		bool enabled;
		int (*create_fn)(struct bt_mesh_subnet *sub, struct bt_buf_simple *buf);
	} beacons[] = {
		[0] = {
...
static int unprovisioned_beacon_send(void)
{
	...
	adv = bt_mesh_adv_create(BT_MESH_ADV_BEACON, BT_MESH_ADV_TAG_LOCAL,
				 UNPROV_XMIT, OS_TIMEOUT_NO_WAIT);
	if (!adv) {
		LOG_ERR("Unable to allocate beacon adv");
		return -ENOBUFS;
	}
	...
	bt_buf_simple_add_u8(&adv->b, BEACON_TYPE_UNPROVISIONED);
	bt_buf_simple_add_mem(&adv->b, prov->uuid, 16);
	...
	bt_buf_simple_add_be16(&adv->b, oob_info);
	bt_buf_simple_add_mem(&adv->b, uri_hash, 4);
	...
	if (prov->uri) {
		...
		len = strlen(prov->uri);
		if (bt_buf_simple_tailroom(&adv->b) < len) {
			LOG_WRN("Too long URI to fit advertising data");
		} else {
			bt_buf_simple_add_mem(&adv->b, prov->uri, len);
			bt_mesh_adv_send(adv, NULL, NULL);
		}
...
static void unprovisioned_beacon_recv(struct bt_buf_simple *buf)
{
	const struct bt_mesh_prov *prov;
	uint8_t *uuid;
	uint16_t oob_info;
	...
	oob_info = bt_buf_simple_pull_be16(buf);

	if (buf->len == 4) {
		uri_hash_val = bt_buf_simple_pull_be32(buf);
...
static void beacon_send(struct bt_work *work)
{
	LOG_DBG("");
	...
	if (bt_mesh_is_provisioned()) {
		...
		if (!beacons_send_next()) {
			bt_work_schedule(&beacon_timer, PROVISIONED_INTERVAL);
		}
	...
	if (IS_ENABLED(CONFIG_BT_MESH_PB_ADV)) {
		...
		bt_work_schedule(&beacon_timer, OS_SECONDS(CONFIG_BT_MESH_UNPROV_BEACON_INT));
	}
}
...
static void secure_beacon_recv(struct bt_buf_simple *buf)
{
	struct beacon_params params;
	...
	params.flags = bt_buf_simple_pull_u8(buf);
	params.net_id = bt_buf_simple_pull_mem(buf, 8);
	params.iv_index = bt_buf_simple_pull_be32(buf);
	params.auth = buf->data;

	net_beacon_resolve(&params, secure_beacon_authenticate);
}

static void private_beacon_recv(struct bt_buf_simple *buf)
{
	struct beacon_params params;
	...
	params.private = true;
	params.random = bt_buf_simple_pull_mem(buf, 13);
	params.data = bt_buf_simple_pull_mem(buf, 5);
	params.auth = buf->data;

	net_beacon_resolve(&params, priv_beacon_decrypt);
}

void bt_mesh_beacon_recv(struct bt_buf_simple *buf)
{
	uint8_t type;
...
{
	bt_work_init_delayable(&beacon_timer, beacon_send);
...
void bt_mesh_beacon_ivu_initiator(bool enable)
{
	 ...
	 */
	beacon_send_sub_curr = NULL;
	bt_work_schedule(&beacon_timer, OS_TIMEOUT_NO_WAIT);
}
...
void bt_mesh_beacon_disable(void)
{
	...
	beacon_send_sub_curr = NULL;
	(void)bt_work_cancel_delayable(&beacon_timer);
}
