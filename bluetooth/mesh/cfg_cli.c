			    ...
			    struct bt_buf_simple *buf)
{
	...
	page = bt_buf_simple_pull_u8(buf);
	...
	if (bt_mesh_msg_ack_ctx_match(&cli->ack_ctx, OP_DEV_COMP_DATA_STATUS, ctx->addr,
		...
		if (param->comp) {
			to_copy = MIN(bt_buf_simple_tailroom(param->comp), buf->len);
			bt_buf_simple_add_mem(param->comp, buf->data, to_copy);
		}
			   ...
			   uint32_t expect_status)
{
	...
	status = bt_buf_simple_pull_u8(buf);
		       ...
		       struct bt_buf_simple *buf)
{
	...
	status = bt_buf_simple_pull_u8(buf);
	net_idx = bt_buf_simple_pull_le16(buf) & 0xfff;
	phase = bt_buf_simple_pull_u8(buf);
			...
			struct bt_buf_simple *buf)
{
	...
	status = bt_buf_simple_pull_u8(buf);
	transmit = bt_buf_simple_pull_u8(buf);
...
int bt_mesh_key_idx_unpack_list(struct bt_buf_simple *buf, uint16_t *dst_arr,
				      size_t *dst_cnt)
{
	size_t i;
	...
	if (i < *dst_cnt && buf->len >= 2) {
		dst_arr[i++] = bt_buf_simple_pull_le16(buf) & 0xfff;
	}
			  ...
			  struct bt_buf_simple *buf)
{
	...
	status = bt_buf_simple_pull_u8(buf);
	elem_addr = bt_buf_simple_pull_le16(buf);
	mod_app_idx = bt_buf_simple_pull_le16(buf);

	if (buf->len >= 4U) {
		cid = bt_buf_simple_pull_le16(buf);
	} else {
		cid = CID_NVAL;
	}

	mod_id = bt_buf_simple_pull_le16(buf);
...
static int mod_sub_list_handle(struct bt_mesh_msg_ctx *ctx, struct bt_buf_simple *buf, uint16_t op,
			       bool vnd)
{
	int err = 0;
	...
	status = bt_buf_simple_pull_u8(buf);
	elem_addr = bt_buf_simple_pull_le16(buf);
	cid = vnd ? bt_buf_simple_pull_le16(buf) : CID_NVAL;
	...
	if (bt_mesh_msg_ack_ctx_match(&cli->ack_ctx, op, ctx->addr, (void **)&param)) {
		...
		if (param->member_cnt && param->members) {
			...
			for (i = 0; i < *param->member_cnt && buf->len; i++) {
				param->members[i] = bt_buf_simple_pull_le16(buf);
			}
...
static int mod_app_list_handle(struct bt_mesh_msg_ctx *ctx, struct bt_buf_simple *buf, uint16_t op,
			       bool vnd)
{
	int err = 0;
	struct bt_buf_simple_state state;
...
static int mod_app_list(const struct bt_mesh_model *model,
			struct bt_mesh_msg_ctx *ctx, struct bt_buf_simple *buf)
{
	LOG_DBG("net_idx 0x%04x app_idx 0x%04x src 0x%04x len %u: %s", ctx->net_idx, ctx->app_idx,
		ctx->addr, buf->len, bt_hex(buf->data, buf->len));
			  ...
			  struct bt_buf_simple *buf)
{
	...
	pub.addr = bt_buf_simple_pull_le16(buf);
	pub.app_idx = bt_buf_simple_pull_le16(buf);
	pub.cred_flag = (pub.app_idx & BIT(12));
	pub.app_idx &= BIT_MASK(12);
	pub.ttl = bt_buf_simple_pull_u8(buf);
	pub.period = bt_buf_simple_pull_u8(buf);
	pub.transmit = bt_buf_simple_pull_u8(buf);
			  ...
			  struct bt_buf_simple *buf)
{
	...
	status = bt_buf_simple_pull_u8(buf);
	elem_addr = bt_buf_simple_pull_le16(buf);
	sub_addr = bt_buf_simple_pull_le16(buf);
			 ...
			 struct bt_buf_simple *buf)
{
	...
	status = bt_buf_simple_pull_u8(buf);
	sub.src = bt_buf_simple_pull_le16(buf);
	sub.dst = bt_buf_simple_pull_le16(buf);
	sub.period = bt_buf_simple_pull_u8(buf);
	sub.count = bt_buf_simple_pull_u8(buf);
	sub.min = bt_buf_simple_pull_u8(buf);
	sub.max = bt_buf_simple_pull_u8(buf);
			 ...
			 struct bt_buf_simple *buf)
{
	...
	status = bt_buf_simple_pull_u8(buf);
	pub.dst = bt_buf_simple_pull_le16(buf);
	pub.count = bt_buf_simple_pull_u8(buf);
	pub.period = bt_buf_simple_pull_u8(buf);
	pub.ttl = bt_buf_simple_pull_u8(buf);
	pub.feat = bt_buf_simple_pull_le16(buf);
	pub.net_idx = bt_buf_simple_pull_le16(buf) & 0xfff;
				 ...
				 struct bt_buf_simple *buf)
{
	...
	status = bt_buf_simple_pull_u8(buf);
	net_idx = bt_buf_simple_pull_le16(buf) & 0xfff;
	identity = bt_buf_simple_pull_u8(buf);
			      ...
			      struct bt_buf_simple *buf)
{
	...
	unicast_addr = bt_buf_simple_pull_le16(buf);
	polltimeout = bt_buf_simple_pull_le24(buf);
			...
			uint8_t *val)
{
	...
	bt_mesh_model_msg_init(&msg, op);
	bt_buf_simple_add_u8(&msg, new_val);
			    ...
			    uint8_t *phase)
{
	...
	bt_buf_simple_add_le16(&msg, key_net_idx);
			    ...
			    uint8_t transition, uint8_t *status, uint8_t *phase)
{
	...
	bt_buf_simple_add_le16(&msg, key_net_idx);
	bt_buf_simple_add_u8(&msg, transition);
			      ...
			      uint8_t new_transmit, uint8_t *status, uint8_t *transmit)
{
	...
	bt_buf_simple_add_u8(&msg, new_relay);
	bt_buf_simple_add_u8(&msg, new_transmit);
				...
				const uint8_t net_key[16], uint8_t *status)
{
	...
	bt_buf_simple_add_le16(&msg, key_net_idx);
	bt_buf_simple_add_mem(&msg, net_key, 16);
				...
				uint16_t key_app_idx, const uint8_t app_key[16], uint8_t *status)
{
	...
	bt_buf_simple_add_mem(&msg, app_key, 16);
			...
			uint16_t mod_id, uint16_t cid, uint8_t *status)
{
	...
	bt_buf_simple_add_le16(&msg, elem_addr);
	bt_buf_simple_add_le16(&msg, mod_app_idx);

	if (cid != CID_NVAL) {
		bt_buf_simple_add_le16(&msg, cid);
	}

	bt_buf_simple_add_le16(&msg, mod_id);
		   ...
		   uint16_t sub_addr, uint16_t mod_id, uint16_t cid, uint8_t *status)
{
	...
	if (sub_addr != BT_MESH_ADDR_UNASSIGNED) {
		bt_buf_simple_add_le16(&msg, sub_addr);
	}
		      ...
		      uint8_t *status)
{
	...
	bt_mesh_model_msg_init(&msg, op);
	bt_buf_simple_add_le16(&msg, elem_addr);
	bt_buf_simple_add_mem(&msg, label, 16);
		       ...
		       uint16_t cid, struct bt_mesh_cfg_cli_mod_pub *pub, uint8_t *status)
{
	...
	bt_buf_simple_add_le16(&msg, elem_addr);
	bt_buf_simple_add_le16(&msg, pub->addr);
	bt_buf_simple_add_le16(&msg, (pub->app_idx | (pub->cred_flag << 12)));
	bt_buf_simple_add_u8(&msg, pub->ttl);
	bt_buf_simple_add_u8(&msg, pub->period);
	bt_buf_simple_add_u8(&msg, pub->transmit);
			  ...
			  uint16_t cid, struct bt_mesh_cfg_cli_mod_pub *pub, uint8_t *status)
{
	...
	bt_buf_simple_add_le16(&msg, elem_addr);
	bt_buf_simple_add_mem(&msg, pub->uuid, 16);
	bt_buf_simple_add_le16(&msg, (pub->app_idx | (pub->cred_flag << 12)));
	bt_buf_simple_add_u8(&msg, pub->ttl);
	bt_buf_simple_add_u8(&msg, pub->period);
	bt_buf_simple_add_u8(&msg, pub->transmit);
			       ...
			       uint8_t *status)
{
	...
	bt_buf_simple_add_le16(&msg, sub->src);
	bt_buf_simple_add_le16(&msg, sub->dst);
	bt_buf_simple_add_u8(&msg, sub->period);
			       ...
			       const struct bt_mesh_cfg_cli_hb_pub *pub, uint8_t *status)
{
	...
	bt_buf_simple_add_le16(&msg, pub->dst);
	bt_buf_simple_add_u8(&msg, pub->count);
	bt_buf_simple_add_u8(&msg, pub->period);
	bt_buf_simple_add_u8(&msg, pub->ttl);
	bt_buf_simple_add_le16(&msg, pub->feat);
	bt_buf_simple_add_le16(&msg, pub->net_idx);
				      ...
				      uint8_t new_identity, uint8_t *status, uint8_t *identity)
{
	...
	bt_buf_simple_add_le16(&msg, key_net_idx);
	bt_buf_simple_add_u8(&msg, new_identity);
				    ...
				    int32_t *polltimeout)
{
	...
	bt_buf_simple_add_le16(&msg, unicast_addr);
			...
			struct bt_buf_simple *buf)
{
	...
	page->cid = bt_buf_simple_pull_le16(buf);
	page->pid = bt_buf_simple_pull_le16(buf);
	page->vid = bt_buf_simple_pull_le16(buf);
	page->crpl = bt_buf_simple_pull_le16(buf);
	page->feat = bt_buf_simple_pull_le16(buf);
	page->_buf = buf;
						       ...
						       struct bt_mesh_comp_p0_elem *elem)
{
	...
	elem->loc = bt_buf_simple_pull_le16(page->_buf);
	elem->nsig = bt_buf_simple_pull_u8(page->_buf);
	elem->nvnd = bt_buf_simple_pull_u8(page->_buf);
	...
	elem->_buf = bt_buf_simple_pull_mem(page->_buf, modlist_size);
...
struct bt_mesh_comp_p1_elem *bt_mesh_comp_p1_elem_pull(struct bt_buf_simple *buf,
						       struct bt_mesh_comp_p1_elem *elem)
{
	...
	elem->nsig = bt_buf_simple_pull_u8(buf);
	elem->nvnd = bt_buf_simple_pull_u8(buf);
	...
	bt_buf_simple_init_with_data(elem->_buf,
				      bt_buf_simple_pull_mem(buf, elem_size),
				      elem_size);
	return elem;
}
	...
	struct bt_mesh_comp_p1_elem *elem, struct bt_mesh_comp_p1_model_item *item)
{
	...
	header = bt_buf_simple_pull_u8(elem->_buf);
	...
	if (item->cor_present) {
		...
		item->cor_id = bt_buf_simple_pull_u8(elem->_buf);
	}
	...
	bt_buf_simple_init_with_data(item->_buf,
				      bt_buf_simple_pull_mem(elem->_buf, item_size),
				      item_size);
	return item;
}
	...
	struct bt_mesh_comp_p1_model_item *item, struct bt_mesh_comp_p1_item_short *ext_item)
{
	...
	uint8_t item_data = bt_buf_simple_pull_u8(item->_buf);
	...
	struct bt_mesh_comp_p1_model_item *item, struct bt_mesh_comp_p1_item_long *ext_item)
{
	...
	ext_item->elem_offset = bt_buf_simple_pull_u8(item->_buf);
	ext_item->mod_item_idx = bt_buf_simple_pull_u8(item->_buf);
...
struct bt_mesh_comp_p2_record *bt_mesh_comp_p2_record_pull(struct bt_buf_simple *buf,
							   struct bt_mesh_comp_p2_record *record)
{
	...
	record->id = bt_buf_simple_pull_le16(buf);
	record->version.x = bt_buf_simple_pull_u8(buf);
	record->version.y = bt_buf_simple_pull_u8(buf);
	record->version.z = bt_buf_simple_pull_u8(buf);
	elem_offset_cnt = bt_buf_simple_pull_u8(buf);
	if (buf->len < elem_offset_cnt + 2) {
		LOG_WRN("Invalid composition data offset count");
		return NULL;
	}

	bt_buf_simple_init_with_data(record->elem_buf,
				      bt_buf_simple_pull_mem(buf, elem_offset_cnt),
				      elem_offset_cnt);
	data_len = bt_buf_simple_pull_le16(buf);
	if (buf->len < data_len) {
		LOG_WRN("Invalid composition data additional data length");
		return NULL;
	}

	bt_buf_simple_init_with_data(record->data_buf,
				      bt_buf_simple_pull_mem(buf, data_len), data_len);
	return record;
}
