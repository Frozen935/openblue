...
static uint32_t base_pull_pd(struct bt_buf_simple *bt_buf)
{
	return bt_buf_simple_pull_le24(bt_buf);
}

static uint8_t base_pull_bis_count(struct bt_buf_simple *bt_buf)
{
	return bt_buf_simple_pull_u8(bt_buf);
}

static void base_pull_codec_id(struct bt_buf_simple *bt_buf,
			       struct bt_bap_base_codec_id *codec_id)
{
	struct bt_bap_base_codec_id codec;

	codec.id = bt_buf_simple_pull_u8(bt_buf);    /* coding format */
	codec.cid = bt_buf_simple_pull_le16(bt_buf); /* company id */
	codec.vid = bt_buf_simple_pull_le16(bt_buf); /* VS codec id */
...
static uint8_t base_pull_ltv(struct bt_buf_simple *bt_buf, uint8_t **data)
{
	const uint8_t len = bt_buf_simple_pull_u8(bt_buf);

	if (data == NULL) {
		bt_buf_simple_pull_mem(bt_buf, len);
	} else {
		*data = bt_buf_simple_pull_mem(bt_buf, len);
	}

	return len;
}

static bool check_pull_ltv(struct bt_buf_simple *bt_buf)
{
	uint8_t ltv_len;

	if (bt_buf->len < sizeof(ltv_len)) {
		return false;
	}

	ltv_len = bt_buf_simple_pull_u8(bt_buf);
	if (bt_buf->len < ltv_len) {
		return false;
	}
	bt_buf_simple_pull_mem(bt_buf, ltv_len);
...
const struct bt_bap_base *bt_bap_base_get_base_from_ad(const struct bt_data *ad)
{
	...
	bt_buf_simple_init_with_data(&bt_buf, (void *)ad->data, ad->data_len);

	uuid = bt_buf_simple_pull_mem(&bt_buf, BT_UUID_SIZE_16);
	...
	base = (const struct bt_bap_base *)bt_buf.data;
...
int bt_bap_base_get_size(const struct bt_bap_base *base)
{
	...
	bt_buf_simple_init_with_data(&bt_buf, (void *)base, BT_BASE_MAX_SIZE);
	base_pull_pd(&bt_buf);
	size += BASE_PD_SIZE;
	subgroup_count = bt_buf_simple_pull_u8(&bt_buf);
...
int bt_bap_base_get_pres_delay(const struct bt_bap_base *base)
{
	...
	bt_buf_simple_init_with_data(&bt_buf, (void *)base, sizeof(pd));
	pd = base_pull_pd(&bt_buf);
				      ...
				      struct bt_bap_base_codec_id *codec_id)
{
	...
	bt_buf_simple_init_with_data(&bt_buf, (void *)subgroup, BASE_SUBGROUP_MAX_SIZE);
	base_pull_bis_count(&bt_buf);
	base_pull_codec_id(&bt_buf, codec_id);
...
int bt_bap_base_get_subgroup_codec_data(const struct bt_bap_base_subgroup *subgroup, uint8_t **data)
{
	...
	/* Codec config */
	return base_pull_ltv(&bt_buf, data);
}
...
int bt_bap_base_get_subgroup_codec_meta(const struct bt_bap_base_subgroup *subgroup, uint8_t **meta)
{
	...
	/* meta */
	return base_pull_ltv(&bt_buf, meta);
}
					    ...
					    struct bt_audio_codec_cfg *codec_cfg)
{
	...
	/* Codec config */
	ltv_len = base_pull_ltv(&bt_buf, &ltv_data);
...
int bt_bap_base_get_subgroup_bis_count(const struct bt_bap_base_subgroup *subgroup)
{
	...
	return base_pull_bis_count(&bt_buf);
}
				     ...
				     void *user_data)
{
	...
	for (uint8_t i = 0U; i < bis_count; i++) {
		...
		bis.index = bt_buf_simple_pull_u8(&bt_buf); /* index */

		/* Codec config */
		bis.data_len = base_pull_ltv(&bt_buf, &bis.data);
