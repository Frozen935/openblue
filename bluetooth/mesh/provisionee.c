...
static void prov_invite(const uint8_t *data)
{
	...
	bt_buf_simple_add_u8(&buf, bt_mesh_elem_count());
	...
	bt_buf_simple_add_be16(&buf, algorithm_bm);

	/* Public Key Type */
	bt_buf_simple_add_u8(&buf,
			      bt_mesh_prov->public_key_be == NULL ? PUB_KEY_NO_OOB : PUB_KEY_OOB);

	/* Static OOB Type */
	bt_buf_simple_add_u8(&buf, oob_type);

	/* Output OOB Size */
	bt_buf_simple_add_u8(&buf, bt_mesh_prov->output_size);

	/* Output OOB Action */
	bt_buf_simple_add_be16(&buf, bt_mesh_prov->output_actions);

	/* Input OOB Size */
	bt_buf_simple_add_u8(&buf, bt_mesh_prov->input_size);

	/* Input OOB Action */
	bt_buf_simple_add_be16(&buf, bt_mesh_prov->input_actions);
...
static void send_confirm(void)
{
	...
	if (bt_mesh_prov_conf(bt_mesh_prov_link.algorithm, bt_mesh_prov_link.conf_key,
			...
			bt_buf_simple_add(&cfm, auth_size))) {
...
static void send_random(void)
{
	...
	bt_buf_simple_add_mem(&rnd, bt_mesh_prov_link.rand, bt_mesh_prov_auth_size_get());
