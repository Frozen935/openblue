...
BT_BUF_POOL_FIXED_DEFINE(ag_pool, CONFIG_BT_HFP_AG_TX_BUF_COUNT,
...
{
	os_sem_take(&ag->lock, OS_TIMEOUT_FOREVER);
}
						   ...
						   uint8_t type)
{
	...
	for (size_t index = 0; index < ARRAY_SIZE(ag->calls); index++) {
		...
		if (!bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_IN_USING)) {
			continue;
		}
...
static void bt_ag_deferred_work(struct bt_work *work);
static void bt_ag_ringing_work(struct bt_work *work);
...
static struct bt_hfp_ag_call *get_new_call(struct bt_hfp_ag *ag, const char *number, uint8_t type)
{
	...
	for (size_t index = 0; index < ARRAY_SIZE(ag->calls); index++) {
		...
		if (!bt_atomic_test_and_set_bit(call->flags, BT_HFP_AG_CALL_IN_USING)) {
			...
			call->type = type;
			call->ag = ag;
			bt_work_init_delayable(&call->deferred_work, bt_ag_deferred_work);
			bt_work_init_delayable(&call->ringing_work, bt_ag_ringing_work);
			return call;
		}
	}
...
static struct bt_ag_tx *bt_ag_tx_alloc(void)
{
	 ...
	 */
	if (os_thread_self() == bt_work_queue_thread_get(bt_work_main_work_queue())) {
		return bt_fifo_get(&ag_tx_free, OS_TIMEOUT_NO_WAIT);
	}

	if (IS_ENABLED(CONFIG_BT_HFP_AG_LOG_LEVEL_DBG)) {
		struct bt_ag_tx *tx = bt_fifo_get(&ag_tx_free, OS_TIMEOUT_NO_WAIT);
	...
	return bt_fifo_get(&ag_tx_free, OS_TIMEOUT_FOREVER);
}
...
static int hfp_ag_next_step(struct bt_hfp_ag *ag, bt_hfp_ag_tx_cb_t cb, void *user_data)
{
	...
	hfp_ag_lock(ag);
	if (bt_atomic_test_bit(ag->flags, BT_HFP_AG_AT_PROCESS)) {
		bt_slist_append(&ag->tx_submit_pending, &tx->node);
	} else {
		bt_slist_append(&ag->tx_pending, &tx->node);
		/* Always active tx work */
		bt_work_reschedule(&ag->tx_work, OS_TIMEOUT_NO_WAIT);
	}
	hfp_ag_unlock(ag);
	...
	for (size_t index = 0; index < ARRAY_SIZE(ag->calls); index++) {
		...
		if (bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_IN_USING)) {
			...
			if ((call->call_state == BT_HFP_CALL_ACTIVE) &&
			    (!bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_INCOMING_HELD))) {
				count++;
			}
			hfp_ag_unlock(ag);
		}
...
{
	bt_atomic_clear_bit(call->flags, BT_HFP_AG_CALL_IN_USING);
	bt_work_cancel_delayable(&call->ringing_work);
	bt_work_cancel_delayable(&call->deferred_work);
	/*
...
static void bt_hfp_ag_set_call_state(struct bt_hfp_ag_call *call, bt_hfp_call_state_t call_state)
{
	...
	case BT_HFP_CALL_TERMINATE:
		bt_atomic_clear_bit(ag->flags, BT_HFP_AG_CREATING_SCO);
		free_call(call);
		break;
	...
	case BT_HFP_CALL_ALERTING:
		if (!bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_INCOMING_3WAY)) {
			bt_work_reschedule(&call->ringing_work, OS_TIMEOUT_NO_WAIT);
		} else {
			bt_work_cancel_delayable(&call->ringing_work);
		}
		bt_work_reschedule(&call->deferred_work,
...
static void hfp_ag_close_sco(struct bt_hfp_ag *ag)
{
	...
	sco = bt_atomic_ptr_set(&ag->sco_conn, NULL);
	if (sco != NULL) {
		bt_conn_unref(sco);
		sco = ag->sco_chan.sco;
	}
...
static void ag_reject_call(struct bt_hfp_ag_call *call)
{
	...
	if (!bt_atomic_test_bit(ag->flags, BT_HFP_AG_AUDIO_CONN)) {
...
static void bt_ag_notify_work(struct bt_work *work)
{
	...
	tx = (struct bt_ag_tx *)bt_fifo_get(&ag_tx_notify, OS_TIMEOUT_NO_WAIT);
...
{
	bt_slist_find_and_remove(&ag->tx_pending, &tx->node);
	tx->err = err;
	bt_ag_tx_notify(tx);
	/* Clear the tx ongoing flag */
	if (!bt_atomic_test_and_clear_bit(ag->flags, BT_HFP_AG_TX_ONGOING)) {
...
static void bt_ag_tx_work(struct bt_work *work)
{
	...
	node = bt_slist_peek_head(&ag->tx_pending);
	if (!node) {
	...
	if (!bt_atomic_test_and_set_bit(ag->flags, BT_HFP_AG_TX_ONGOING)) {
		int err;
...
static void skip_space(struct bt_buf *buf)
{
	size_t count = 0;
	...
	if (count > 0) {
		(void)bt_buf_pull(buf, count);
	}
}

static int get_number(struct bt_buf *buf, uint32_t *number)
{
	int err = -EINVAL;
...
static int get_char(struct bt_buf *buf, char *c)
{
	int err = -EINVAL;
...
static bool is_char(struct bt_buf *buf, uint8_t c)
{
	bool found = false;
...
static int bt_hfp_ag_brsf_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
...
static int bt_hfp_ag_bac_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	uint32_t codec;
	...
	bt_atomic_set_bit(ag->flags, BT_HFP_AG_CODEC_CHANGED);

	if (bt_atomic_test_and_clear_bit(ag->flags, BT_HFP_AG_CODEC_CONN)) {
...
static int bt_hfp_ag_cind_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
...
static void bt_hfp_ag_update_call_flags(struct bt_hfp_ag_call *call, enum bt_hfp_ag_call_dir dir)
{
	...
	bt_atomic_set_bit_to(call->flags, BT_HFP_AG_CALL_INCOMING, dir == BT_HFP_AG_CALL_DIR_INCOMING);

	if (call_count > 1) {
		if (dir == BT_HFP_AG_CALL_DIR_INCOMING) {
			bt_atomic_set_bit(call->flags, BT_HFP_AG_CALL_INCOMING_3WAY);
		} else {
			bt_atomic_set_bit(call->flags, BT_HFP_AG_CALL_OUTGOING_3WAY);
		}
	}
}
...
{
	if (bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_INCOMING) ||
		bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_INCOMING_3WAY)) {
					     ...
					     struct bt_hfp_ag_ongoing_call *ongoing_call)
{
	...
	bt_hfp_ag_set_call_state(call, BT_HFP_CALL_ACTIVE);
	bt_atomic_set_bit(call->flags, BT_HFP_AG_CALL_INCOMING_HELD);
	bt_hfp_ag_update_call_flags(call, ongoing_call->dir);
	bt_hfp_ag_notify_new_call(ag, call);
...
static struct bt_hfp_ag_call *get_call_with_flag(struct bt_hfp_ag *ag, int bit)
{
	...
	ARRAY_FOR_EACH(ag->calls, i) {
		...
		if (bt_atomic_test_bit(call->flags, bit)) {
			return call;
		}
	}
						    ...
						    bt_hfp_call_state_t state)
{
	...
	ARRAY_FOR_EACH(ag->calls, i) {
		...
		if ((call->call_state & state) && bt_atomic_test_bit(call->flags, bit)) {
			return call;
		}
	}
...
static void hfp_ag_sco_connected(struct bt_sco_chan *chan)
{
	...
	if (bt_atomic_ptr_cas(&ag->sco_conn, NULL, chan->sco)) {
...
static struct bt_conn *bt_hfp_ag_create_sco(struct bt_hfp_ag *ag)
{
	...
	sco = bt_atomic_ptr_get(&ag->sco_conn);
	if (sco != NULL) {
		return sco;
	}
...
static int hfp_ag_open_sco(struct bt_hfp_ag *ag, struct bt_hfp_ag_call *call)
{
	...
	if (bt_atomic_test_bit(ag->flags, BT_HFP_AG_CREATING_SCO)) {
		LOG_WRN("SCO connection is creating!");
		return 0;
	}

	hfp_ag_lock(ag);
	create_sco = bt_atomic_ptr_get(&ag->sco_conn) == NULL ? true : false;
	if (create_sco) {
		bt_atomic_set_bit(ag->flags, BT_HFP_AG_CREATING_SCO);
	}
	hfp_ag_unlock(ag);
	...
	if (create_sco) {
		...
		bt_atomic_clear_bit(ag->flags, BT_HFP_AG_CREATING_SCO);
		bt_atomic_clear_bit(ag->flags, BT_HFP_AG_AUDIO_CONN);
		...
		bt_atomic_set_bit_to(ag->flags, BT_HFP_AG_AUDIO_CONN, call == NULL);
	...
	if ((call->call_state == BT_HFP_CALL_INCOMING) ||
	    bt_atomic_test_and_clear_bit(call->flags, BT_HFP_AG_CALL_ALERTING)) {
...
static int bt_hfp_ag_create_audio_connection(struct bt_hfp_ag *ag, struct bt_hfp_ag_call *call)
{
	...
	if ((hf_codec_ids != 0) && bt_atomic_test_bit(ag->flags, BT_HFP_AG_CODEC_CHANGED)) {
		err = bt_hfp_ag_codec_select(ag);
		bt_atomic_set_bit_to(ag->flags, BT_HFP_AG_CODEC_CONN, err == 0);
		if (call) {
			bt_atomic_set_bit_to(call->flags, BT_HFP_AG_CALL_OPEN_SCO, err == 0);
		}
	} else {
	...
	for (size_t index = 0; index < ag->ongoing_call_count; index++) {
		...
		case BT_HFP_AG_CALL_STATUS_ALERTING:
			...
			if (err != 0) {
				...
				if (call != NULL) {
					bt_atomic_set_bit(call->flags, BT_HFP_AG_CALL_ALERTING);
				}
				call_setup_value = BT_HFP_CALL_SETUP_REMOTE_ALERTING;
			} else {
				call_setup_value = BT_HFP_CALL_SETUP_OUTGOING;
...
static void bt_hfp_ag_set_in_band_ring(struct bt_hfp_ag *ag)
{
	...
	if (is_inband_ringtone && !bt_atomic_test_bit(ag->flags, BT_HFP_AG_INBAND_RING)) {
		int err = hfp_ag_send_data(ag, NULL, NULL, "\r\n+BSIR:1\r\n");

		bt_atomic_set_bit_to(ag->flags, BT_HFP_AG_INBAND_RING, err == 0);
	}
}
...
static int bt_hfp_ag_cmer_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	uint32_t number;
	int err;
	...
	if (number == 1) {
		bt_atomic_set_bit(ag->flags, BT_HFP_AG_CMER_ENABLE);
		...
		bt_atomic_set_bit(ag->flags, BT_HFP_AG_SLC_CONNECTED);
		return 0;
	} else if (number == 0) {
		bt_atomic_clear_bit(ag->flags, BT_HFP_AG_CMER_ENABLE);
	} else {
		return -ENOTSUP;
	}
...
static int chld_deactivate_calls_and_accept_other_call(struct bt_hfp_ag *ag, bool release)
{
	...
	if (!none_active_call) {
		...
		if (!(bt_atomic_test_bit(none_active_call->flags, BT_HFP_AG_CALL_INCOMING_3WAY) ||
		      bt_atomic_test_bit(none_active_call->flags, BT_HFP_AG_CALL_INCOMING))) {
			return -ENOTSUP;
		}
	}
...
static int bt_hfp_ag_chld_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
#if defined(CONFIG_BT_HFP_AG_3WAY_CALL)
	uint32_t value;
	int err;
...
static int bt_hfp_ag_bind_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	uint32_t indicator;
...
static int bt_hfp_ag_cmee_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	uint32_t cmee;
	int err;
	...
	if (BT_HFP_AG_FEATURE_EXT_ERR_ENABLE) {
		bt_atomic_set_bit_to(ag->flags, BT_HFP_AG_CMEE_ENABLE, cmee == 1);
		return 0;
	}
...
static int bt_hfp_ag_chup_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	bt_hfp_call_state_t call_state;
	struct bt_hfp_ag_call *call;
...
static uint8_t bt_hfp_get_call_state(struct bt_hfp_ag_call *call)
{
	...
	case BT_HFP_CALL_TERMINATE:
		...
		break;
	case BT_HFP_CALL_OUTGOING:
		if (bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_ALERTING)) {
	...
	case BT_HFP_CALL_ALERTING:
		...
		break;
	case BT_HFP_CALL_ACTIVE:
		if (bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_INCOMING_HELD)) {
...
static int bt_hfp_ag_clcc_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	struct bt_hfp_ag_call *call;
	uint8_t dir;
	...
	for (size_t index = 0; index < ARRAY_SIZE(ag->calls); index++) {
		...
		dir = bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_INCOMING) ?
...
static int bt_hfp_ag_bia_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	uint32_t number;
	int err;
	int index = 0;
...
static void bt_hfp_ag_call_setup_none_cb(struct bt_hfp_ag *ag, void *user_data)
{
	...
	if (bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_OUTGOING_3WAY)) {
		err = hfp_ag_update_indicator(ag, BT_HFP_AG_CALL_HELD_IND,
...
static int bt_hfp_ag_ata_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	int call_count;
	struct bt_hfp_ag_call *call;
...
static int bt_hfp_ag_cops_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	uint32_t number;
	...
	if (is_char(buf, '=')) {
		...
		bt_atomic_set_bit(ag->flags, BT_HFP_AG_COPS_SET);
		return 0;
	}

	if (!bt_atomic_test_bit(ag->flags, BT_HFP_AG_COPS_SET)) {
		return -ENOTSUP;
	}
...
static int bt_hfp_ag_bcc_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	...
	if (bt_atomic_ptr_get(&ag->sco_conn) != NULL) {
		hfp_ag_unlock(ag);
		return -ECONNREFUSED;
	}
	hfp_ag_unlock(ag);
...
static int bt_hfp_ag_bcs_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	uint32_t number;
	bool codec_conn;
	...
	if (!bt_atomic_test_bit(ag->flags, BT_HFP_AG_CODEC_CONN)) {
		return -ESRCH;
	}
	...
	codec_conn = bt_atomic_test_and_clear_bit(ag->flags, BT_HFP_AG_CODEC_CONN);
	bt_atomic_clear_bit(ag->flags, BT_HFP_AG_CODEC_CHANGED);
...
static void bt_hfp_ag_outgoing_cb(struct bt_hfp_ag *ag, void *user_data)
{
	...
	if (bt_atomic_test_bit(ag->flags, BT_HFP_AG_INBAND_RING) ||
	    bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_OUTGOING_3WAY)) {
		int err;
	...
	if (bt_atomic_test_and_clear_bit(call->flags, BT_HFP_AG_CALL_ALERTING)) {
		bt_hfp_ag_set_call_state(call, BT_HFP_CALL_ALERTING);
...
static int bt_hfp_ag_outgoing_call(struct bt_hfp_ag *ag, const char *number, uint8_t type)
{
	...
	bt_atomic_clear_bit(call->flags, BT_HFP_AG_CALL_INCOMING);
	bt_hfp_ag_set_call_state(call, BT_HFP_CALL_OUTGOING);
...
static int bt_hfp_ag_atd_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	char *number = NULL;
	uint8_t *data;
...
static int bt_hfp_ag_bldn_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
...
static int bt_hfp_ag_clip_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	uint32_t clip;
	...
	bt_atomic_set_bit_to(ag->flags, BT_HFP_AG_CLIP_ENABLE, clip == 1);

	return err;
}

static int bt_hfp_ag_vgm_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	uint32_t vgm;
...
static int bt_hfp_ag_vgs_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	uint32_t vgs;
...
static int bt_hfp_ag_nrec_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	uint32_t disable;
...
static int bt_hfp_ag_btrh_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	uint32_t action;
	struct bt_hfp_ag_call *call;
	...
	if (action == BT_HFP_BTRH_ON_HOLD) {
		...
		call = get_call_with_flag_and_state(ag, BT_HFP_AG_CALL_INCOMING_HELD,
						    BT_HFP_CALL_ACTIVE);
		if (call) {
			bt_atomic_clear_bit(call->flags, BT_HFP_AG_CALL_INCOMING_HELD);
...
static int bt_hfp_ag_ccwa_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	uint32_t value;
	...
	bt_atomic_set_bit_to(ag->flags, BT_HFP_AG_CCWA_ENABLE, value);
	return 0;
}
...
static void bt_hfp_ag_vr_activate(struct bt_hfp_ag *ag, void *user_data)
{
	...
	bt_atomic_set_bit_to(ag->flags, BT_HFP_AG_VRE_R2A, !feature);

#if defined(CONFIG_BT_HFP_AG_ENH_VOICE_RECG)
	if (bt_atomic_test_bit(ag->flags, BT_HFP_AG_VRE_R2A)) {
...
{
	bt_atomic_set_bit(ag->flags, BT_HFP_AG_VRE_R2A);
...
static int bt_hfp_ag_bvra_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	uint32_t value;
	...
	case BT_HFP_BVRA_DEACTIVATION:
		if (!bt_atomic_test_and_clear_bit(ag->flags, BT_HFP_AG_VRE_ACTIVATE)) {
	...
	case BT_HFP_BVRA_ACTIVATION:
		if (bt_atomic_test_and_set_bit(ag->flags, BT_HFP_AG_VRE_ACTIVATE)) {
			LOG_WRN("VR has been activated");
			return -ENOTSUP;
		}
		bt_atomic_clear_bit(ag->flags, BT_HFP_AG_VRE_R2A);
	...
	case BT_HFP_BVRA_READY_TO_ACCEPT:
		...
		if (!bt_atomic_test_bit(ag->flags, BT_HFP_AG_VRE_ACTIVATE)) {
...
static int bt_hfp_ag_binp_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	uint32_t value;
...
static int bt_hfp_ag_vts_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
	char code = 0;
...
static int bt_hfp_ag_cnum_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	int err;
...
static int bt_hfp_ag_biev_handler(struct bt_hfp_ag *ag, struct bt_buf *buf)
{
	uint32_t indicator;
	uint32_t value;
...
static struct bt_ag_tx *ag_get_tx(struct bt_hfp_ag *ag, bt_slist_t *list)
{
	bt_snode_t *node;

	hfp_ag_lock(ag);
	node = bt_slist_get(list);
	hfp_ag_unlock(ag);
	if (node == NULL) {
		return NULL;
	}
...
static void hfp_ag_disconnected(struct bt_rfcomm_dlc *dlc)
{
	...
	bt_work_cancel_delayable(&ag->tx_work);

	tx = ag_get_tx(ag, &ag->tx_pending);
	while (tx != NULL) {
		if ((tx->buf != NULL) &&
		    !bt_atomic_test_and_clear_bit(ag->flags, BT_HFP_AG_TX_ONGOING)) {
			bt_buf_unref(tx->buf);
		}
		tx->err = -ESHUTDOWN;
		bt_ag_tx_notify(tx);
...
{
	bt_atomic_set_bit(ag->flags, BT_HFP_AG_AT_PROCESS);
}
...
static void hfp_ag_postprocess_at_cmd(struct bt_hfp_ag *ag)
{
	...
	if (!bt_atomic_test_and_clear_bit(ag->flags, BT_HFP_AG_AT_PROCESS)) {
		LOG_WRN("No AT CMD is processing");
	}

	hfp_ag_lock(ag);
	node = bt_slist_get(&ag->tx_submit_pending);
	while (node != NULL) {
		bt_slist_append(&ag->tx_pending, node);
		node = bt_slist_get(&ag->tx_submit_pending);
	}
	hfp_ag_unlock(ag);
...
static int hfp_ag_at_cmd_ack(struct bt_hfp_ag *ag, int err)
{
	...
	if ((err != 0) && bt_atomic_test_bit(ag->flags, BT_HFP_AG_CMEE_ENABLE)) {
		cme_err = bt_hfp_ag_get_cme_err(err);
		err = hfp_ag_send_data(ag, NULL, NULL, "\r\n+CME ERROR:%d\r\n", (uint32_t)cme_err);
	} else {
		bt_hfp_ag_tx_cb_t cb;

		cb = bt_atomic_test_and_clear_bit(ag->flags, BT_HFP_AG_SLC_CONNECTED)
...
static void hfp_ag_recv(struct bt_rfcomm_dlc *dlc, struct bt_buf *buf)
{
	struct bt_hfp_ag *ag = CONTAINER_OF(dlc, struct bt_hfp_ag, rfcomm_dlc);
	...
	for (uint32_t index = 0; index < ARRAY_SIZE(cmd_handlers); index++) {
		...
		if (NULL != cmd_handlers[index].handler) {
			(void)bt_buf_pull(buf, strlen(cmd_handlers[index].cmd));
	...
	if (!bt_atomic_test_and_set_bit(ag->flags, BT_HFP_AG_1ST_AT_RECV)) {
...
static void hfp_ag_sent(struct bt_rfcomm_dlc *dlc, int err)
{
	...
	node = bt_slist_get(&ag->tx_pending);
	hfp_ag_unlock(ag);
	if (!node) {
...
static void bt_ag_ringing_work_cb(struct bt_hfp_ag *ag, void *user_data)
{
	...
	if (call->call_state == BT_HFP_CALL_ALERTING) {
		...
		if (err) {
			...
			if (bt_atomic_test_bit(ag->flags, BT_HFP_AG_CLIP_ENABLE)) {
				err = hfp_ag_send_data(ag, NULL, NULL, "\r\n+CLIP:\"%s\",%d\r\n",
						       call->number, 0);
				if (err) {
...
static void bt_ag_ongoing_call_work(struct bt_work *work)
{
	struct bt_work_delayable *dwork = bt_work_delayable_from_work(work);
	...
	if (!bt_atomic_test_and_clear_bit(ag->flags, BT_HGP_AG_ONGOING_CALLS)) {
		return;
	}
...
static void bt_ag_slc_work(struct bt_work *work)
{
	struct bt_hfp_ag *ag = CONTAINER_OF(work, struct bt_hfp_ag, slc_work);
	int err;

	if (!bt_atomic_test_bit(ag->flags, BT_HFP_AG_DISCOVER_DONE)) {
		return;
	}

	if (!bt_atomic_test_bit(ag->flags, BT_HFP_AG_1ST_AT_RECV)) {
		return;
	}

	if (bt_atomic_test_and_set_bit(ag->flags, BT_HFP_AG_FEAT_UPDATED)) {
		return;
	}

	if (bt_atomic_test_bit(ag->flags, BT_HFP_AG_RECORD_FOUND)) {
				     ...
				     const struct bt_sdp_discover_params *params)
{
	...
	bt_atomic_set_bit(ag->flags, BT_HFP_AG_RECORD_FOUND);
failed:
	bt_atomic_set_bit(ag->flags, BT_HFP_AG_DISCOVER_DONE);
	bt_work_submit(&ag->slc_work);
...
static struct bt_hfp_ag *hfp_ag_create(struct bt_conn *conn)
{
	...
	bt_slist_init(&ag->tx_pending);
	bt_slist_init(&ag->tx_submit_pending);

	os_sem_init(&ag->lock, 1, 1);
	...
	bt_work_init(&ag->slc_work, bt_ag_slc_work);

	/* Init delay work */
	bt_work_init_delayable(&ag->tx_work, bt_ag_tx_work);
	/* Init delay work */
	bt_work_init_delayable(&ag->ongoing_call_work, bt_ag_ongoing_call_work);
...
static void hfp_ag_init(void)
{
	...
	bt_fifo_init(&ag_tx_free);
	bt_fifo_init(&ag_tx_notify);

	ARRAY_FOR_EACH(ag_tx, index) {
		bt_fifo_put(&ag_tx_free, &ag_tx[index]);
	}
}
...
int bt_hfp_ag_remote_incoming(struct bt_hfp_ag *ag, const char *number)
{
	...
	if (call_count) {
		...
		if (!bt_atomic_test_bit(ag->flags, BT_HFP_AG_CCWA_ENABLE)) {
	...
	bt_atomic_set_bit(call->flags, BT_HFP_AG_CALL_INCOMING);
	bt_hfp_ag_set_call_state(call, BT_HFP_CALL_INCOMING);
...
int bt_hfp_ag_reject(struct bt_hfp_ag_call *call)
{
	...
	if ((call_state == BT_HFP_CALL_ACTIVE) &&
	    bt_atomic_test_and_clear_bit(call->flags, BT_HFP_AG_CALL_INCOMING_HELD)) {
		err = hfp_ag_send_data(ag, btrh_reject_cb, call, "\r\n+BTRH:%d\r\n",
				       BT_HFP_BTRH_REJECTED);
		if (err) {
			LOG_ERR("Fail to send err :(%d)", err);
...
int bt_hfp_ag_accept(struct bt_hfp_ag_call *call)
{
	...
	if (bt_atomic_test_and_clear_bit(call->flags, BT_HFP_AG_CALL_INCOMING_HELD) &&
	    (call_state == BT_HFP_CALL_ACTIVE)) {
		err = hfp_ag_send_data(ag, btrh_accept_cb, call, "\r\n+BTRH:%d\r\n",
				       BT_HFP_BTRH_ACCEPTED);
		if (err) {
...
int bt_hfp_ag_terminate(struct bt_hfp_ag_call *call)
{
	...
	if (bt_atomic_test_and_clear_bit(call->flags, BT_HFP_AG_CALL_INCOMING_HELD) &&
	    bt_atomic_test_bit(call->flags, BT_HFP_AG_CALL_INCOMING) &&
	    (call_state == BT_HFP_CALL_ACTIVE)) {
		err = hfp_ag_send_data(ag, btrh_reject_cb, call, "\r\n+BTRH:%d\r\n",
				       BT_HFP_BTRH_REJECTED);
		if (err) {
...
int bt_hfp_ag_inband_ringtone(struct bt_hfp_ag *ag, bool inband)
{
	...
	bt_atomic_set_bit_to(ag->flags, BT_HFP_AG_INBAND_RING, inband);
	return 0;
}
...
int bt_hfp_ag_voice_recognition(struct bt_hfp_ag *ag, bool activate)
{
	...
	if (activate && bt_atomic_test_bit(ag->flags, BT_HFP_AG_VRE_ACTIVATE)) {
		LOG_WRN("VR has been activated");
		return -ENOTSUP;
	} else if (!activate && !bt_atomic_test_bit(ag->flags, BT_HFP_AG_VRE_ACTIVATE)) {
		LOG_WRN("VR is not activated");
		return -ENOTSUP;
	}
	...
	bt_atomic_set_bit_to(ag->flags, BT_HFP_AG_VRE_ACTIVATE, activate);
	bt_atomic_clear_bit(ag->flags, BT_HFP_AG_VRE_R2A);
	return 0;
#else
	return -ENOTSUP;
...
int bt_hfp_ag_vre_state(struct bt_hfp_ag *ag, uint8_t state)
{
	...
	if ((state & BT_HFP_BVRA_STATE_SEND_AUDIO) &&
		!bt_atomic_test_bit(ag->flags, BT_HFP_AG_VRE_R2A)) {
...
failed:
	bt_work_cancel_delayable(&ag->ongoing_call_work);
	bt_hfp_ag_notify_cind_value(ag);
	bt_ag_send_ok_code(ag);
	return err;
}
