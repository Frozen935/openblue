...
BT_BUF_POOL_FIXED_DEFINE(friend_buf_pool, FRIEND_BUF_COUNT, BT_MESH_ADV_DATA_SIZE,
			  sizeof(struct friend_adv), NULL);

static struct friend_adv adv_pool[FRIEND_BUF_COUNT];

#define FRIEND_ADV(buf) (*(struct friend_adv **)bt_buf_user_data(buf))
...
static void purge_buffers(bt_slist_t *list)
{
	struct bt_buf *buf;

	while ((buf = (void *)bt_buf_slist_get(list))) {
		bt_buf_unref(buf);
	}
}
...
static void friend_clear(struct bt_mesh_friend *frnd)
{
	...
	(void)bt_work_cancel_delayable(&frnd->timer);
...
int bt_mesh_friend_clear(struct bt_mesh_net_rx *rx, struct bt_buf_simple *buf)
{
...
static struct bt_buf *create_friend_pdu(struct bt_mesh_friend *frnd,
					 struct friend_pdu_info *info,
					 struct bt_buf_simple *sdu)
{
	struct bt_buf *buf;


	buf = bt_buf_alloc(&friend_buf_pool, OS_TIMEOUT_NO_WAIT);
	if (!buf) {
		return NULL;
	}

	FRIEND_ADV(buf) = adv_alloc(bt_buf_id(buf));

	bt_buf_add_u8(buf, (info->iv_index & 1) << 7); /* Will be reset in encryption */

	if (info->ctl) {
		bt_buf_add_u8(buf, info->ttl | 0x80);
	} else {
		bt_buf_add_u8(buf, info->ttl);
	}

	bt_buf_add_mem(buf, info->seq, sizeof(info->seq));

	bt_buf_add_be16(buf, info->src);
	bt_buf_add_be16(buf, info->dst);

	bt_buf_add_mem(buf, sdu->data, sdu->len);
				 ...
				 const struct unseg_app_sdu_meta *meta)
{
	 ...
	 */
	bt_buf_simple_clone(&buf->b, &in);
	bt_buf_simple_pull(&in, BT_MESH_NET_HDR_LEN);
	bt_buf_simple_pull(&in, 1);
	in.len -= BT_MESH_MIC_SHORT;

	bt_buf_simple_clone(&in, &out);
				 ...
				 const struct unseg_app_sdu_meta *meta)
{
	...
	bt_buf_simple_clone(&buf->b, &sdu);
	bt_buf_simple_pull(&sdu, BT_MESH_NET_HDR_LEN);
	bt_buf_simple_pull(&sdu, 1);
...
static int encrypt_friend_pdu(struct bt_mesh_friend *frnd, struct bt_buf *buf,
			      bool flooding_cred)
{
...
static struct bt_buf *encode_friend_ctl(struct bt_mesh_friend *frnd,
					 uint8_t ctl_op,
					 struct bt_buf_simple *sdu)
{
	struct friend_pdu_info info;
	...
	bt_buf_simple_push_u8(sdu, TRANS_CTL_HDR(ctl_op, 0));
...
static struct bt_buf *encode_update(struct bt_mesh_friend *frnd, uint8_t md)
{
	struct bt_mesh_ctl_friend_update *upd;
	BT_BUF_SIMPLE_DEFINE(sdu, 1 + sizeof(*upd));

	__ASSERT_NO_MSG(friend_is_allocated(frnd));

	LOG_DBG("lpn 0x%04x md 0x%02x", frnd->lpn, md);

	bt_buf_simple_reserve(&sdu, 1);

	upd = bt_buf_simple_add(&sdu, sizeof(*upd));
...
static void enqueue_sub_cfm(struct bt_mesh_friend *frnd, uint8_t xact)
{
	...
	BT_BUF_SIMPLE_DEFINE(sdu, 1 + sizeof(*cfm));
	struct bt_buf *buf;
	...
	cfm = bt_buf_simple_add(&sdu, sizeof(*cfm));
	cfm->xact = xact;
...
static void friend_recv_delay(struct bt_mesh_friend *frnd)
{
	...
	frnd->pending_req = 1U;
	bt_work_reschedule(&frnd->timer, OS_MSEC(delay));
			   ...
			   struct bt_buf_simple *buf)
{
	...
	xact = bt_buf_simple_pull_u8(buf);

	while (buf->len >= 2U) {
		friend_sub_add(frnd, bt_buf_simple_pull_be16(buf));
	}
			   ...
			   struct bt_buf_simple *buf)
{
	...
	while (buf->len >= 2U) {
		friend_sub_rem(frnd, bt_buf_simple_pull_be16(buf));
	}
...
static void enqueue_buf(struct bt_mesh_friend *frnd, struct bt_buf *buf)
{
	bt_buf_slist_put(&frnd->queue, buf);
	frnd->queue_size++;
}
...
int bt_mesh_friend_poll(struct bt_mesh_net_rx *rx, struct bt_buf_simple *buf)
{
	...
	if (msg->fsn == frnd->fsn && frnd->last) {
		...
		if (bt_slist_is_empty(&frnd->queue)) {
...
static void friend_clear_sent(int err, void *user_data)
{
	...
	bt_work_reschedule(&frnd->clear.timer,
			  OS_SECONDS(frnd->clear.repeat_sec));
...
static void clear_timeout(struct bt_work *work)
{
			     ...
			     struct bt_buf_simple *buf)
{
	...
	(void)bt_work_cancel_delayable(&frnd->clear.timer);
	frnd->clear.frnd = BT_MESH_ADDR_UNASSIGNED;
...
static void enqueue_offer(struct bt_mesh_friend *frnd, int8_t rssi)
{
	...
	BT_BUF_SIMPLE_DEFINE(sdu, 1 + sizeof(*off));
	struct bt_buf *buf;
	...
	off = bt_buf_simple_add(&sdu, sizeof(*off));
...
int bt_mesh_friend_req(struct bt_mesh_net_rx *rx, struct bt_buf_simple *buf)
{
...
{
	struct bt_buf *buf = (void *)bt_slist_peek_head(&seg->queue);
	...
	bt_buf_skip(buf, 5);   /* skip IVI, NID, CTL, TTL, SEQ */
	buf_src = bt_buf_pull_be16(buf);
	bt_buf_skip(buf, 3);   /* skip DST, OP/AID */
	buf_seq_zero = ((bt_buf_pull_be16(buf) >> 2) & TRANS_SEQ_ZERO_MASK);
					  ...
					  uint8_t seg_count)
{
	...
	for (i = 0; i < ARRAY_SIZE(frnd->seg); i++) {
		...
		if (!unassigned && !bt_slist_peek_head(&seg->queue)) {
			unassigned = seg;
		}
	}
			       ...
			       struct bt_buf *buf)
{
	...
	bt_buf_slist_put(&seg->queue, buf);

	if (type == BT_MESH_FRIEND_PDU_COMPLETE) {
		bt_slist_merge_slist(&frnd->queue, &seg->queue);
...
static void buf_send_end(int err, void *user_data)
{
	...
	if (frnd->established) {
		...
		bt_work_reschedule(&frnd->timer, OS_MSEC(frnd->poll_to));
		LOG_DBG("Waiting %u ms for next poll", frnd->poll_to);
	} else {
		/* Friend offer timeout is 1 second */
		bt_work_reschedule(&frnd->timer, OS_SECONDS(1));
		LOG_DBG("Waiting for first poll");
	}
}

static void update_overwrite(struct bt_buf *buf, uint8_t md)
{
	struct bt_buf_simple_state state;
	struct bt_mesh_ctl_friend_update *upd;
	...
	bt_buf_skip(buf, 1); /* skip IVI, NID */

	if (!(bt_buf_pull_u8(buf) >> 7)) {
		goto end;
	}

	bt_buf_skip(buf, 7); /* skip seqnum src dec*/

	if (TRANS_CTL_OP((uint8_t *) bt_buf_pull_mem(buf, 1))
			!= TRANS_CTL_OP_FRIEND_UPDATE) {
		goto end;
	}

	upd = bt_buf_pull_mem(buf, sizeof(*upd));
...
static void friend_timeout(struct bt_work *work)
{
	struct bt_work_delayable *dwork = bt_work_delayable_from_work(work);
	struct bt_mesh_friend *frnd = CONTAINER_OF(dwork, struct bt_mesh_friend,
						   timer);
	...
	frnd->last = (void *)bt_buf_slist_get(&frnd->queue);
	if (!frnd->last) {
	...
	md = (uint8_t)(bt_slist_peek_head(&frnd->queue) != NULL);
...
send_last:
	adv = bt_mesh_adv_create(BT_MESH_ADV_DATA, BT_MESH_ADV_TAG_FRIEND,
				 FRIEND_XMIT, OS_TIMEOUT_NO_WAIT);
	if (!adv) {
		LOG_ERR("Unable to allocate friend adv");
		return;
	}

	bt_buf_simple_add_mem(&adv->b, frnd->last->data, frnd->last->len);
...
int bt_mesh_friend_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_mesh.frnd); i++) {
		...
		bt_slist_init(&frnd->queue);

		bt_work_init_delayable(&frnd->timer, friend_timeout);
		bt_work_init_delayable(&frnd->clear.timer, clear_timeout);

		for (j = 0; j < ARRAY_SIZE(frnd->seg); j++) {
			bt_slist_init(&frnd->seg[j].queue);
		}
	}
...
static bool is_segack(struct bt_buf *buf, const uint64_t *seqauth, uint16_t src)
{
	struct bt_buf_simple_state state;
	bool found = false;
	...
	bt_buf_pull(buf, 3); /* skip SEQNUM */

	if (src != bt_buf_pull_be16(buf)) {
		goto end;
	}

	bt_buf_skip(buf, 2); /* skip dst */

	if (TRANS_CTL_OP((uint8_t *) bt_buf_pull_mem(buf, 1)) != TRANS_CTL_OP_ACK) {
		goto end;
	}

	found = ((bt_buf_pull_be16(buf) >> 2) & TRANS_SEQ_ZERO_MASK) ==
...
{
	bt_snode_t *cur, *prev = NULL;

	LOG_DBG("SeqAuth %llx src 0x%04x", *seq_auth, src);

	for (cur = bt_slist_peek_head(&frnd->queue);
	     cur != NULL; prev = cur, cur = bt_slist_peek_next(cur)) {
		struct bt_buf *buf = (void *)cur;

		if (is_segack(buf, seq_auth, src)) {
			LOG_DBG("Removing old ack from Friend Queue");

			bt_slist_remove(&frnd->queue, prev, cur);
			frnd->queue_size--;
				   ...
				   uint16_t src)
{
	...
	for (cur = bt_slist_peek_head(&frnd->queue); cur != NULL;
	     cur = bt_slist_peek_next(cur)) {
		struct bt_buf *buf = (void *)cur;
				       ...
				       const uint64_t *seq_auth, uint8_t seg_count)
{
	...
	while (pending_segments || avail_space < seg_count) {
		struct bt_buf *buf = (void *)bt_buf_slist_get(&frnd->queue);
