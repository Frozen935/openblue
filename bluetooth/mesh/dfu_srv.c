			   ...
			   struct bt_buf_simple *buf)
{
	...
	limit = bt_buf_simple_pull_u8(buf);

	LOG_DBG("from %u (limit: %u)", idx, limit);

	BT_BUF_SIMPLE_DEFINE(rsp, BT_MESH_TX_SDU_MAX);
	bt_mesh_model_msg_init(&rsp, BT_MESH_DFU_OP_UPDATE_INFO_STATUS);
	bt_buf_simple_add_u8(&rsp, srv->img_count);
	bt_buf_simple_add_u8(&rsp, idx);
	...
	for (; idx < srv->img_count && limit > 0; ++idx) {
		...
		if (bt_buf_simple_tailroom(&rsp) + BT_MESH_MIC_SHORT < entry_len) {
			break;
		}

		bt_buf_simple_add_u8(&rsp, srv->imgs[idx].fwid_len);
		bt_buf_simple_add_mem(&rsp, srv->imgs[idx].fwid,
				       srv->imgs[idx].fwid_len);

		if (srv->imgs[idx].uri) {
			size_t len = strlen(srv->imgs[idx].uri);

			bt_buf_simple_add_u8(&rsp, len);
			bt_buf_simple_add_mem(&rsp, srv->imgs[idx].uri, len);
		} else {
				 ...
				 struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_add_u8(&rsp, (status & BIT_MASK(3)) | (effect << 3));
	bt_buf_simple_add_u8(&rsp, idx);
			      ...
			      const struct bt_mesh_send_cb *send_cb)
{
	...
	bt_buf_simple_add_u8(&buf, ((status & BIT_MASK(3)) |
				     (srv->update.phase << 5)));

	if (srv->update.phase != BT_MESH_DFU_PHASE_IDLE) {
		bt_buf_simple_add_u8(&buf, srv->update.ttl);
		bt_buf_simple_add_u8(&buf, srv->update.effect);
		bt_buf_simple_add_le16(&buf, srv->update.timeout_base);
		bt_buf_simple_add_le64(&buf, srv->blob.state.xfer.id);
		bt_buf_simple_add_u8(&buf, srv->update.idx);
