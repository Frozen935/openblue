...
static void handle_read_preset_rsp(struct bt_has_client *inst, struct bt_buf_simple *buf)
{
...
static void handle_generic_update(struct bt_has_client *inst, struct bt_buf_simple *buf,
				  bool is_last)
{
...
static void handle_preset_deleted(struct bt_has_client *inst, struct bt_buf_simple *buf,
				  bool is_last)
{
	...
	client_cb->preset_deleted(&inst->has, bt_buf_simple_pull_u8(buf), is_last);
}

static void handle_preset_availability(struct bt_has_client *inst, struct bt_buf_simple *buf,
				       bool available, bool is_last)
{
	...
	client_cb->preset_availability(&inst->has, bt_buf_simple_pull_u8(buf), available,
				       is_last);
}

static void handle_preset_changed(struct bt_has_client *inst, struct bt_buf_simple *buf)
{
...
static int cp_write(struct bt_has_client *inst, struct bt_buf_simple *buf,
...
static int read_presets_req(struct bt_has_client *inst, uint8_t start_index, uint8_t num_presets)
{
	...
	BT_BUF_SIMPLE_DEFINE(buf, sizeof(*hdr) + sizeof(*req));
...
static int preset_set_next_or_prev(struct bt_has_client *inst, uint8_t opcode)
{
	...
	BT_BUF_SIMPLE_DEFINE(buf, sizeof(*hdr));
...
static uint8_t active_index_update(struct bt_has_client *inst, const void *data, uint16_t len)
{
	...
	inst->has.active_index = bt_buf_simple_pull_u8(&buf);
...
static void features_update(struct bt_has_client *inst, const void *data, uint16_t len)
{
	...
	inst->has.features = bt_buf_simple_pull_u8(&buf);
