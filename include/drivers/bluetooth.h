...
#ifndef __INCLUDE_BLUETOOTH_DRIVERS_BLUETOOTH_H_
#define __INCLUDE_BLUETOOTH_DRIVERS_BLUETOOTH_H_
...
struct bt_hci_transport {
	const char *name;
	/** The bus type used by the HCI driver. */
	enum bt_hci_bus bus;

	/** The API used by the HCI driver to communicate with the host. */
	const struct bt_hci_driver_api *api;

	void *user_data;

	bool (*is_ready)(const struct bt_hci_transport *transport);
};

typedef int (*bt_hci_recv_t)(const struct bt_hci_transport *transport, struct bt_buf *buf);

struct bt_hci_driver_api {
	int (*open)(const struct bt_hci_transport *transport, bt_hci_recv_t recv);
 ...
 */
static inline int bt_hci_open(const struct bt_hci_transport *transport, bt_hci_recv_t recv)
{
	const struct bt_hci_driver_api *api = transport->api;

	return api->open(transport, recv);
}
 ...
 * @return 0 on success or negative POSIX error number on failure.
 */
static inline int bt_hci_close(const struct bt_hci_transport *transport)
{
	const struct bt_hci_driver_api *api = transport->api;
 ...
 * for eventually calling @ref bt_buf_unref on it.
 *
 ...
 * @return 0 on success or negative POSIX error number on failure.
 */
static inline int bt_hci_send(const struct bt_hci_transport *transport, struct bt_buf *buf)
{
	const struct bt_hci_driver_api *api = transport->api;

	return api->send(transport, buf);
}
...
static inline int bt_hci_setup(const struct bt_hci_transport *transport, struct bt_hci_setup_params *params)
{
	...
	return api->setup(transport, params);
}
#endif

static inline bool bt_hci_is_ready(const struct bt_hci_transport *transport)
{
	return transport->is_ready(transport);
}
 ...
 */
int bt_hci_transport_setup(const struct bt_hci_transport *transport);
 ...
 * @return 0 on success, negative error value on failure
 */
int bt_hci_transport_teardown(const struct bt_hci_transport *transport);
 ...
 * @return Newly allocated buffer.
 */
struct bt_buf *bt_hci_cmd_complete_create(uint16_t op, uint8_t plen);
 ...
 * @return Newly allocated buffer.
 */
struct bt_buf *bt_hci_cmd_status_create(uint16_t op, uint8_t status);

#ifdef __cplusplus
}
#endif

#endif /* __INCLUDE_BLUETOOTH_DRIVERS_BLUETOOTH_H_ */
