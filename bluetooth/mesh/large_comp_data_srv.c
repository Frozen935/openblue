				      ...
				      struct bt_buf_simple *buf)
{
	...
	if (bt_atomic_test_bit(bt_mesh.flags, BT_MESH_COMP_DIRTY) && page < 128) {
		...
		bt_buf_simple_add_le16(&rsp, temp_buf.len);
		if (offset > temp_buf.len) {
			return 0;
		}

		msg_space = bt_buf_simple_tailroom(&rsp) - BT_MESH_MIC_SHORT;
		bt_buf_simple_add_mem(
		...
		bt_buf_simple_add_le16(&rsp, total_size);
