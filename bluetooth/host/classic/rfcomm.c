				       ...
				       uint8_t credits)
{
	...
	while (credits--) {
		os_sem_give(&dlc->tx_credits);
		rfcomm_dlc_tx_trigger(dlc);
	}

	LOG_DBG("dlc %p updated credits %u", dlc, os_sem_count_get(&dlc->tx_credits));
}
...
static void rfcomm_dlc_destroy(struct bt_rfcomm_dlc *dlc)
{
	...
	bt_work_cancel_delayable(&dlc->rtx_work);
	bt_work_cancel(&dlc->tx_work);
...
struct bt_buf *bt_rfcomm_create_pdu(struct bt_buf_pool *pool)
{
...
static int rfcomm_send_cb(struct bt_rfcomm_session *session, struct bt_buf *buf,
...
static int rfcomm_send(struct bt_rfcomm_session *session, struct bt_buf *buf)
{
...
static struct bt_buf *rfcomm_make_uih_msg(struct bt_rfcomm_session *session,
					   ...
					   uint8_t len)
{
	...
	msg_hdr = bt_buf_add(buf, sizeof(*msg_hdr));
...
static void rfcomm_disconnected(struct bt_l2cap_chan *chan)
{
	...
	bt_work_cancel_delayable(&session->rtx_work);
	rfcomm_session_disconnected(session);
	session->state = BT_RFCOMM_STATE_IDLE;
}

static void rfcomm_dlc_rtx_timeout(struct bt_work *work)
{
			    ...
			    bt_rfcomm_role_t role)
{
	...
	if (dlc->tx_work.handler != NULL) {
		...
		bt_work_cancel_sync(&dlc->tx_work, &sync);
	...
	dlc->role = role;
	bt_work_init_delayable(&dlc->rtx_work, rfcomm_dlc_rtx_timeout);
...
static bool rfcomm_check_fc(struct bt_rfcomm_dlc *dlc)
{
	...
	err = os_sem_take(&dlc->tx_credits, OS_TIMEOUT_NO_WAIT);
	if (err != 0) {
	...
	err = os_sem_take(&dlc->session->fc, OS_TIMEOUT_NO_WAIT);
	if (err != 0) {
		os_sem_give(&dlc->tx_credits);
...
static void bt_rfcomm_tx_destroy(struct bt_rfcomm_dlc *dlc, struct bt_buf *buf)
{
...
static void rfcomm_dlc_tx_worker(struct bt_work *work)
{
	...
	if (dlc->state == BT_RFCOMM_STATE_CONNECTED ||
		...
		if (bt_fifo_is_empty(&dlc->tx_queue) == true) {
		...
		buf = bt_fifo_get(&dlc->tx_queue, OS_TIMEOUT_NO_WAIT);
		...
		if (bt_fifo_is_empty(&dlc->tx_queue) == false) {
			rfcomm_dlc_tx_trigger(dlc);
			return;
		}
...
disconnect:
	...
	while ((buf = bt_fifo_get(&dlc->tx_queue, OS_TIMEOUT_NO_WAIT))) {
		bt_rfcomm_tx_destroy(dlc, buf);
		bt_buf_unref(buf);
	}
			   ...
			   struct bt_rfcomm_rpn *rpn)
{
	...
	bt_buf_add_mem(buf, rpn, sizeof(*rpn));
			    ...
			    uint8_t *pattern, uint8_t len)
{
	...
	bt_buf_add_mem(buf, pattern, len);
...
static void rfcomm_dlc_connected(struct bt_rfcomm_dlc *dlc)
{
	...
	if (dlc->session->cfc == BT_RFCOMM_CFC_NOT_SUPPORTED) {
		...
		os_sem_init(&dlc->tx_credits, 0, 1);
	}

	/* Cancel conn timer */
	bt_work_cancel_delayable(&dlc->rtx_work);

	bt_fifo_init(&dlc->tx_queue);
	bt_work_init(&dlc->tx_work, rfcomm_dlc_tx_worker);
			      ...
			      struct bt_buf *buf, uint8_t cr)
{
	...
	if (dlc->session->cfc == BT_RFCOMM_CFC_NOT_SUPPORTED) {
		...
		if (BT_RFCOMM_GET_FC(msc->v24_signal)) {
			 ...
			 * Take the semaphore with timeout OS_TIMEOUT_NO_WAIT so that
			 * dlc thread will be blocked when it tries sem_take
			 * before sending the data. OS_TIMEOUT_NO_WAIT timeout will make
			 * sure that RX thread will not be blocked while taking
			 * the semaphore.
			 */
			os_sem_take(&dlc->tx_credits, OS_TIMEOUT_NO_WAIT);
		} else {
			     ...
			     struct bt_buf *buf, uint8_t cr)
{
	...
	if (!dlc) {
		...
		if (pn->flow_ctrl == BT_RFCOMM_PN_CFC_CMD) {
			...
			os_sem_init(&dlc->tx_credits, 0, OS_SEM_MAX_LIMIT);
			      ...
			      struct bt_buf *buf)
{
	...
	case BT_RFCOMM_FCOFF:
		...
		/* Take the semaphore with timeout OS_TIMEOUT_NO_WAIT so that all the
		 * dlc threads in this session will be blocked when it tries
		 * sem_take before sending the data. OS_TIMEOUT_NO_WAIT timeout will
		 * make sure that RX thread will not be blocked while taking
		 * the semaphore.
		 */
		os_sem_take(&session->fc, OS_TIMEOUT_NO_WAIT);
...
static void rfcomm_handle_data(struct bt_rfcomm_session *session,
			       struct bt_buf *buf, uint8_t dlci, uint8_t pf)

{
	...
	if (pf == BT_RFCOMM_PF_UIH_CREDIT) {
		...
		rfcomm_dlc_tx_give_credits(dlc, bt_buf_pull_u8(buf));
	}
...
int bt_rfcomm_dlc_send(struct bt_rfcomm_dlc *dlc, struct bt_buf *buf)
{
	uint8_t fcs, cr;
	...
	LOG_DBG("dlc %p tx credit %d", dlc, os_sem_count_get(&dlc->tx_credits));
	...
	if (buf->len > BT_RFCOMM_MAX_LEN_8) {
		/* Length is 2 byte */
		bt_buf_push_le16(buf, BT_RFCOMM_SET_LEN_16(buf->len));
	} else {
		bt_buf_push_u8(buf, BT_RFCOMM_SET_LEN_8(buf->len));
	}

	/* control */
	bt_buf_push_u8(buf, BT_RFCOMM_SET_CTRL(BT_RFCOMM_UIH,
					BT_RFCOMM_PF_UIH_NO_CREDIT));
	/* address */
	cr = BT_RFCOMM_UIH_CR(dlc->session->role);
	bt_buf_push_u8(buf, BT_RFCOMM_SET_ADDR(dlc->dlci, cr));

	fcs = rfcomm_calc_fcs(BT_RFCOMM_FCS_LEN_UIH, buf->data);
	bt_buf_add_u8(buf, fcs);

	bt_fifo_put(&dlc->tx_queue, buf);
	rfcomm_dlc_tx_trigger(dlc);

	return buf->len;
}

static int rfcomm_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
	struct bt_rfcomm_session *session = RFCOMM_SESSION(chan);
...
static void rfcomm_session_rtx_timeout(struct bt_work *work)
{
...
static struct bt_rfcomm_session *rfcomm_session_new(bt_rfcomm_role_t role)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_rfcomm_pool); i++) {
		...
		bt_work_init_delayable(&session->rtx_work,
				      rfcomm_session_rtx_timeout);
		os_sem_init(&session->fc, 0, 1);
