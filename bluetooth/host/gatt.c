...
#define DB_HASH_TIMEOUT	OS_MSEC(10)
 ...
 * Invariant: (bt_slist_is_empty(subscriptions[x].list))
 *              <=> (subscriptions[x].peer == BT_ADDR_LE_ANY).
 */
static struct gatt_sub subscriptions[SUB_MAX];
static bt_slist_t callback_list = BT_SLIST_STATIC_INIT(&callback_list);

#if defined(CONFIG_BT_GATT_DYNAMIC_DB)
static bt_slist_t db;
...
static void db_hash_process(struct bt_work *work)
{
	do_db_hash();
}
...
static void gatt_insert(struct bt_gatt_service *svc, uint16_t last_handle)
{
	...
	if (last_handle == 0 || svc->attrs[0].handle > last_handle) {
		bt_slist_append(&db, &svc->node);
		return;
	}

	/* DB shall always have its service in ascending order */
	BT_SLIST_FOR_EACH_CONTAINER(&db, tmp, node) {
		if (tmp->attrs[0].handle > svc->attrs[0].handle) {
			if (prev) {
				bt_slist_insert(&db, &prev->node, &svc->node);
			} else {
				bt_slist_prepend(&db, &svc->node);
			}
			return;
		}
...
static int gatt_register(struct bt_gatt_service *svc)
{
	...
	if (bt_slist_is_empty(&db)) {
		handle = last_static_handle;
		last_handle = 0;
		goto populate;
	}

	last = BT_SLIST_PEEK_TAIL_CONTAINER(&db, last, node);
...
static inline void sc_work_submit(os_timeout_t timeout)
{
#if defined(CONFIG_BT_GATT_SERVICE_CHANGED)
...
static void delayed_store(struct bt_work *work)
{
	struct ds_peer *el;
	struct bt_work_delayable *dwork = bt_work_delayable_from_work(work);
...
#if defined(CONFIG_BT_GATT_CACHING)
	bt_work_init_delayable(&db_hash.work, db_hash_process);
...
#if defined(CONFIG_BT_GATT_SERVICE_CHANGED)
	bt_work_init_delayable(&gatt_sc.work, sc_process);
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
...
#if defined(CONFIG_BT_SETTINGS_DELAYED_STORE)
	bt_work_init_delayable(&gatt_delayed_store.work, delayed_store);
#endif
...
{
	if (!bt_slist_find_and_remove(&db, &svc->node)) {
		return -ENOENT;
	}
...
bool bt_gatt_service_is_registered(const struct bt_gatt_service *svc)
{
	...
	os_sched_lock();
	BT_SLIST_FOR_EACH_NODE(&db, node) {
		if (&svc->node == node) {
			registered = true;
			break;
		}
				    ...
				    bt_gatt_attr_func_t func, void *user_data)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&db, svc, node) {
		struct bt_gatt_service *next;

		next = BT_SLIST_PEEK_NEXT_CONTAINER(svc, node);
		if (next) {
...
static struct bt_buf *nfy_mult[CONFIG_BT_MAX_CONN];

static int gatt_notify_mult_send(struct bt_conn *conn, struct bt_buf *buf)
{
	int ret;
	...
	if (buf->len ==
		 ...
		 */
		(void)bt_buf_pull(buf, 1 + sizeof(struct bt_att_notify_mult));

		/* Add back an ATT_HANDLE_VALUE_NTF opcode and attr handle */
		/* PDU structure is now [Opcode (1)] [Handle (1)] [Value] */
		bt_buf_push_le16(buf, handle);
		bt_buf_push_u8(buf, BT_ATT_OP_NOTIFY);
...
static void notify_mult_process(struct bt_work *work)
{
	int i;

	/* Send to any connection with an allocated buffer */
	for (i = 0; i < ARRAY_SIZE(nfy_mult); i++) {
		struct bt_buf **buf = &nfy_mult[i];
...
BT_WORK_DELAYABLE_DEFINE(nfy_mult_work, notify_mult_process);
...
{
	int err = 0;
	struct bt_buf **buf = &nfy_mult[bt_conn_index(conn)];
...
static void gatt_add_nfy_to_buf(struct bt_buf *buf,
				uint16_t handle,
				struct bt_gatt_notify_params *params)
{
	struct bt_att_notify_mult *nfy;

	nfy = bt_buf_add(buf, sizeof(*nfy) + params->len);
			    ...
			    struct bt_gatt_notify_params *params)
{
	 ...
	 */
	if (*buf && ((bt_buf_tailroom(*buf) < sizeof(struct bt_att_notify_mult) + params->len) ||
	...
	bt_work_schedule(&nfy_mult_work,
			OS_MSEC(CONFIG_BT_GATT_NOTIFY_MULTIPLE_FLUSH_MS));
			 ...
			 struct bt_gatt_indicate_params *params)
{
	...
	ind = bt_buf_add(buf, sizeof(*ind) + params->len);
...
{
	return bt_slist_is_empty(&sub->list);
}
			    ...
			    struct bt_gatt_subscribe_params *params)
{
	if (params) {
		...
		bt_slist_remove(&sub->list, prev, &params->node);
		/* Notify removal */
			       ...
			       uint16_t length)
{
	...
	while (buf.len > sizeof(*nfy)) {
		...
		nfy = bt_buf_simple_pull_mem(&buf, sizeof(*nfy));
...
static int gatt_exchange_mtu_encode(struct bt_buf *buf, size_t len,
				    void *user_data)
{
...
static int gatt_find_type_encode(struct bt_buf *buf, size_t len,
				 void *user_data)
{
	struct bt_gatt_discover_params *params = user_data;
	...
	case BT_UUID_TYPE_16:
		bt_buf_add_le16(buf, BT_UUID_16(params->uuid)->val);
		break;
	case BT_UUID_TYPE_128:
		bt_buf_add_mem(buf, BT_UUID_128(params->uuid)->val, 16);
		break;
	}
...
static int read_included_uuid_encode(struct bt_buf *buf, size_t len,
				     void *user_data)
{
	struct bt_gatt_discover_params *params = user_data;
...
static int gatt_read_type_encode(struct bt_buf *buf, size_t len,
				 void *user_data)
{
	struct bt_gatt_discover_params *params = user_data;
	...
	case BT_GATT_DISCOVER_INCLUDE:
		bt_buf_add_le16(buf, BT_UUID_GATT_INCLUDE_VAL);
		break;
	case BT_GATT_DISCOVER_CHARACTERISTIC:
		bt_buf_add_le16(buf, BT_UUID_GATT_CHRC_VAL);
		break;
	default:
...
static int gatt_read_group_encode(struct bt_buf *buf, size_t len,
				  void *user_data)
{
	struct bt_gatt_discover_params *params = user_data;
	...
	if (params->type == BT_GATT_DISCOVER_PRIMARY) {
		bt_buf_add_le16(buf, BT_UUID_GATT_PRIMARY_VAL);
	} else {
		bt_buf_add_le16(buf, BT_UUID_GATT_SECONDARY_VAL);
	}
...
static int gatt_find_info_encode(struct bt_buf *buf, size_t len,
				 void *user_data)
{
	struct bt_gatt_discover_params *params = user_data;
...
static int gatt_read_blob_encode(struct bt_buf *buf, size_t len,
				 void *user_data)
{
	struct bt_gatt_read_params *params = user_data;
...
static int gatt_read_uuid_encode(struct bt_buf *buf, size_t len,
				 void *user_data)
{
	struct bt_gatt_read_params *params = user_data;
	struct bt_att_read_type_req *req;
	...
	if (params->by_uuid.uuid->type == BT_UUID_TYPE_16) {
		bt_buf_add_le16(buf, BT_UUID_16(params->by_uuid.uuid)->val);
	} else {
		bt_buf_add_mem(buf, BT_UUID_128(params->by_uuid.uuid)->val, 16);
	}
...
static int gatt_read_mult_encode(struct bt_buf *buf, size_t len,
				 void *user_data)
{
	struct bt_gatt_read_params *params = user_data;
	uint8_t i;
	...
	for (i = 0U; i < params->handle_count; i++) {
		bt_buf_add_le16(buf, params->multiple.handles[i]);
	}
				  ...
				  void *user_data)
{
	...
	bt_buf_simple_init_with_data(&buf, (void *)pdu, length);
...
static int gatt_read_mult_vl_encode(struct bt_buf *buf, size_t len,
				    void *user_data)
{
	struct bt_gatt_read_params *params = user_data;
	uint8_t i;
...
static int gatt_read_encode(struct bt_buf *buf, size_t len, void *user_data)
{
	struct bt_gatt_read_params *params = user_data;
	struct bt_att_read_req *req;
...
static int gatt_exec_encode(struct bt_buf *buf, size_t len, void *user_data)
{
...
static int gatt_cancel_encode(struct bt_buf *buf, size_t len, void *user_data)
{
	struct bt_att_exec_write_req *req;
...
static int gatt_prepare_write_encode(struct bt_buf *buf, size_t len,
				     void *user_data)
{
	struct bt_gatt_write_params *params = user_data;
	...
	write = bt_buf_append_bytes(buf, len - sizeof(*req),
				     (uint8_t *)params->data, OS_TIMEOUT_NO_WAIT, NULL,
				     NULL);
...
static int gatt_write_encode(struct bt_buf *buf, size_t len, void *user_data)
{
	struct bt_gatt_write_params *params = user_data;
	...
	write = bt_buf_append_bytes(buf, params->length, params->data,
				     OS_TIMEOUT_NO_WAIT, NULL, NULL);
			       ...
			       void *user_data)
{
	...
	if (err) {
		...
		BT_SLIST_FOR_EACH_NODE_SAFE(&sub->list, node, tmp) {
			if (node == &params->node) {
				gatt_sub_remove(conn, sub, prev, params);
				break;
			}
...
static int gatt_write_ccc_buf(struct bt_buf *buf, size_t len, void *user_data)
{
	struct bt_gatt_subscribe_params *params = user_data;
	struct bt_att_write_req *write_req;

	write_req = bt_buf_add(buf, sizeof(*write_req));
		      ...
		      struct bt_gatt_subscribe_params *params)
{
	...
	/* Lookup existing subscriptions */
	BT_SLIST_FOR_EACH_CONTAINER(&sub->list, tmp, node) {
	 ...
	 */
	bt_slist_prepend(&sub->list, &params->node);
			...
			struct bt_gatt_subscribe_params *params)
{
	...
	bt_slist_find_and_remove(&sub->list, &params->node);
...
static void bt_gatt_clear_subscriptions(uint8_t id, const bt_addr_le_t *addr)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&sub->list, params, tmp,
					  node) {
		params->value = 0U;
