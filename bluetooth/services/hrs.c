...
static bt_slist_t hrs_cbs = BT_SLIST_STATIC_INIT(&hrs_cbs);
...
static void hrmc_ccc_cfg_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&hrs_cbs, listener, _node) {
...
int bt_hrs_cb_register(struct bt_hrs_cb *cb)
{
	...
	bt_slist_append(&hrs_cbs, &cb->_node);
...
int bt_hrs_cb_unregister(struct bt_hrs_cb *cb)
{
	...
	if (!bt_slist_find_and_remove(&hrs_cbs, &cb->_node)) {
		return -ENOENT;
	}
