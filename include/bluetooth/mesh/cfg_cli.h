...
struct bt_mesh_cfg_cli_cb {
	 ...
	 *        @ref bt_buf_simple_pull_le16 in iteration, as long
	...
	void (*app_key_list)(struct bt_mesh_cfg_cli *cli, uint16_t addr, uint8_t status,
			     uint16_t net_idx, struct bt_buf_simple *buf);
...
struct bt_mesh_comp_p1_elem *bt_mesh_comp_p1_elem_pull(
	struct bt_buf_simple *buf, struct bt_mesh_comp_p1_elem *elem);
...
struct bt_mesh_comp_p2_record {
	...
	struct bt_buf_simple *elem_buf;
	/** Additional data buffer. */
	struct bt_buf_simple *data_buf;
};
 ...
 */
int bt_mesh_key_idx_unpack_list(struct bt_buf_simple *buf, uint16_t *dst_arr, size_t *dst_cnt);
...
#endif /* __INCLUDE_BLUETOOTH_MESH_CFG_CLI_H__ */
