...
{
	if (err) {
		bt_shell_error("Local player instance failed (%d)", err);
		return;
	}

	local_player = plr;
	bt_shell_print("Local player instance: %p", local_player);
...
{
	if (err) {
		bt_shell_error("Discover player failed (%d)", err);
		return;
	}

	remote_player = plr;
	bt_shell_print("Discovered player instance: %p", remote_player);
...
{
	if (err) {
		bt_shell_error("Player: %p, Player name failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Player name: %s", plr, name);
}
...
static void icon_id_cb(struct media_player *plr, int err, uint64_t id)
{
	...
	if (err) {
		bt_shell_error("Player: %p, Icon ID failed (%d)", plr, err);
		return;
	}

	(void)bt_ots_obj_id_to_str(id, str, sizeof(str));
	bt_shell_print("Player: %p, Icon Object ID: %s", plr, str);
}

static void icon_url_cb(struct media_player *plr, int err, const char *url)
{
	if (err) {
		bt_shell_error("Player: %p, Icon URL failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Icon URL: %s", plr, url);
}

static void track_changed_cb(struct media_player *plr, int err)
{
	if (err) {
		bt_shell_error("Player: %p, Track change failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Track changed", plr);
}

static void track_title_cb(struct media_player *plr, int err, const char *title)
{
	if (err) {
		bt_shell_error("Player: %p, Track title failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Track title: %s", plr, title);
}

static void track_duration_cb(struct media_player *plr, int err, int32_t duration)
{
	if (err) {
		bt_shell_error("Player: %p, Track duration failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Track duration: %d", plr, duration);
}

static void track_position_recv_cb(struct media_player *plr, int err, int32_t position)
{
	if (err) {
		bt_shell_error("Player: %p, Track position receive failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Track Position received: %d", plr, position);
}

static void track_position_write_cb(struct media_player *plr, int err, int32_t position)
{
	if (err) {
		bt_shell_error("Player: %p, Track position write failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Track Position write: %d", plr, position);
}

static void playback_speed_recv_cb(struct media_player *plr, int err, int8_t speed)
{
	if (err) {
		bt_shell_error("Player: %p, Playback speed receive failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Playback speed received: %d", plr, speed);
}

static void playback_speed_write_cb(struct media_player *plr, int err, int8_t speed)
{
	if (err) {
		bt_shell_error("Player: %p, Playback speed write failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Playback speed write: %d", plr, speed);
}

static void seeking_speed_cb(struct media_player *plr, int err, int8_t speed)
{
	if (err) {
		bt_shell_error("Player: %p, Seeking speed failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Seeking speed: %d", plr, speed);
}
...
static void track_segments_id_cb(struct media_player *plr, int err, uint64_t id)
{
	...
	if (err) {
		bt_shell_error("Player: %p, Track segments ID failed (%d)", plr, err);
		return;
	}

	(void)bt_ots_obj_id_to_str(id, str, sizeof(str));
	bt_shell_print("Player: %p, Track Segments Object ID: %s", plr, str);
}
...
static void current_track_id_cb(struct media_player *plr, int err, uint64_t id)
{
	...
	if (err) {
		bt_shell_error("Player: %p, Current track ID failed (%d)", plr, err);
		return;
	}

	(void)bt_ots_obj_id_to_str(id, str, sizeof(str));
	bt_shell_print("Player: %p, Current Track Object ID: %s", plr, str);
}
...
static void next_track_id_cb(struct media_player *plr, int err, uint64_t id)
{
	...
	if (err) {
		bt_shell_error("Player: %p, Next track ID failed (%d)", plr, err);
		return;
	}

	if (id == MPL_NO_TRACK_ID) {
		bt_shell_print("Player: %p, Next Track Object ID is empty", plr);
	} else {
		(void)bt_ots_obj_id_to_str(id, str, sizeof(str));
		bt_shell_print("Player: %p, Next Track Object ID: %s", plr, str);
	}
}
...
static void current_group_id_cb(struct media_player *plr, int err, uint64_t id)
{
	...
	if (err) {
		bt_shell_error("Player: %p, Current group ID failed (%d)", plr, err);
		return;
	}

	(void)bt_ots_obj_id_to_str(id, str, sizeof(str));
	bt_shell_print("Player: %p, Current Group Object ID: %s", plr, str);
}
...
static void parent_group_id_cb(struct media_player *plr, int err, uint64_t id)
{
	...
	if (err) {
		bt_shell_error("Player: %p, Parent group ID failed (%d)", plr, err);
		return;
	}

	(void)bt_ots_obj_id_to_str(id, str, sizeof(str));
	bt_shell_print("Player: %p, Parent Group Object ID: %s", plr, str);
}
#endif /* CONFIG_BT_OTS */
...
{
	if (err) {
		bt_shell_error("Player: %p, Playing order receive_failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Playing received: %u", plr, order);
}

static void playing_order_write_cb(struct media_player *plr, int err, uint8_t order)
{
	if (err) {
		bt_shell_error("Player: %p, Playing order write_failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Playing written: %u", plr, order);
}

static void playing_orders_supported_cb(struct media_player *plr, int err, uint16_t orders)
{
	if (err) {
		bt_shell_error("Player: %p, Playing orders supported failed (%d)",
			    plr, err);
		return;
	}

	bt_shell_print("Player: %p, Playing orders supported: %u", plr, orders);
...
{
	if (err) {
		bt_shell_error("Player: %p, Media state failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Media State: %u", plr, state);
...
{
	if (err) {
		bt_shell_error("Player: %p, Command send failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Command opcode sent: %u", plr, cmd->opcode);
}

static void command_recv_cb(struct media_player *plr, int err, const struct mpl_cmd_ntf *cmd_ntf)
{
	if (err) {
		bt_shell_error("Player: %p, Command failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Command opcode: %u, result: %u",
...
{
	if (err) {
		bt_shell_error("Player: %p, Commands supported failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Command opcodes supported: %u", plr, opcodes);
...
{
	if (err) {
		bt_shell_error("Player: %p, Search send failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Search sent with len %u", plr, search->len);
}

static void search_recv_cb(struct media_player *plr, int err, uint8_t result_code)
{
	if (err) {
		bt_shell_error("Player: %p, Search failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Search result code: %u", plr, result_code);
}
...
static void search_results_id_cb(struct media_player *plr, int err, uint64_t id)
{
	...
	if (err) {
		bt_shell_error("Player: %p, Search results ID failed (%d)", plr, err);
		return;
	}

	if (id == 0) {
		bt_shell_print("Player: %p, Search result not available", plr);
	}

	(void)bt_ots_obj_id_to_str(id, str, sizeof(str));
	bt_shell_print("Player: %p, Search Results Object ID: %s", plr, str);
}
#endif /* CONFIG_BT_OTS */
...
{
	if (err) {
		bt_shell_error("Player: %p, Content control ID failed (%d)", plr, err);
		return;
	}

	bt_shell_print("Player: %p, Content Control ID: %u", plr, ccid);
}

static int cmd_media_init(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err;
	...
	if (err) {
		bt_shell_error(sh, "Could not register media shell as controller");
	}

	return err;
}

static int cmd_media_set_player(const struct bt_shell *sh, size_t argc, char *argv[])
{
	if (!strcmp(argv[1], "local")) {
		if (local_player) {
			current_player = local_player;
			bt_shell_print(sh, "Current player set to local player: %p",
				    current_player);
			return 0;
		}

		bt_shell_print(sh, "No local player");
		return -EOPNOTSUPP;

	} else if (!strcmp(argv[1], "remote")) {
		if (remote_player) {
			current_player = remote_player;
			bt_shell_print(sh, "Current player set to remote player: %p",
				    current_player);
			return 0;
		}

		bt_shell_print(sh, "No remote player");
		return -EOPNOTSUPP;

	} else {
		bt_shell_error(sh, "Input argument must be either \"local\" or \"remote\"");
		return -EINVAL;
	}
}

static int cmd_media_show_players(const struct bt_shell *sh, size_t argc, char *argv[])
{
	bt_shell_print(sh, "Local player: %p", local_player);
	bt_shell_print(sh, "Remote player: %p", remote_player);

	if (current_player == NULL) {
		bt_shell_print(sh, "Current player is not set");
	} else if (current_player == local_player) {
		bt_shell_print(sh, "Current player is set to local player: %p", current_player);
	} else if (current_player == remote_player) {
		bt_shell_print(sh, "Current player is set to remote player: %p", current_player);
	} else {
		bt_shell_print(sh, "Current player is not set to valid player");
	}
...
#ifdef CONFIG_MCTL_REMOTE_PLAYER_CONTROL
static int cmd_media_discover_player(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_discover_player(default_conn);

	if (err) {
		bt_shell_error(sh, "Discover player failed (%d)", err);
	}
...
static int cmd_media_read_player_name(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_player_name(current_player);

	if (err) {
		bt_shell_error(sh, "Player name get failed (%d)", err);
	}
...
#ifdef CONFIG_BT_OTS
static int cmd_media_read_icon_obj_id(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_icon_id(current_player);

	if (err) {
		bt_shell_error(sh, "Icon ID get failed (%d)", err);
	}
...
static int cmd_media_read_icon_url(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_icon_url(current_player);

	if (err) {
		bt_shell_error(sh, "Icon URL get failed (%d)", err);
	}

	return err;
}

static int cmd_media_read_track_title(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_track_title(current_player);

	if (err) {
		bt_shell_error(sh, "Track title get failed (%d)", err);
	}

	return err;
}

static int cmd_media_read_track_duration(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_track_duration(current_player);

	if (err) {
		bt_shell_error(sh, "Track duration get failed (%d)", err);
	}

	return err;
}

static int cmd_media_read_track_position(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_track_position(current_player);

	if (err) {
		bt_shell_error(sh, "Track position get failed (%d)", err);
	}

	return err;
}

static int cmd_media_set_track_position(const struct bt_shell *sh, size_t argc,
			       char *argv[])
{
	long position;
	int err = 0;

	position = bt_shell_strtol(argv[1], 0, &err);
	if (err != 0) {
		bt_shell_error(sh, "Could not parse position: %d", err);

		return -ENOEXEC;
	}

	if (sizeof(long) != sizeof(int32_t) && !IN_RANGE(position, INT32_MIN, INT32_MAX)) {
		bt_shell_error(sh, "Invalid position: %ld", position);
	...
	if (err) {
		bt_shell_error(sh, "Track position set failed (%d)", err);
	}

	return err;
}

static int cmd_media_read_playback_speed(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_playback_speed(current_player);

	if (err) {
		bt_shell_error(sh, "Playback speed get failed (%d)", err);
	}

	return err;
}

static int cmd_media_set_playback_speed(const struct bt_shell *sh, size_t argc, char *argv[])
{
	long speed;
	int err = 0;

	speed = bt_shell_strtol(argv[1], 0, &err);
	if (err != 0) {
		bt_shell_error(sh, "Could not parse speed: %d", err);

		return -ENOEXEC;
	}

	if (!IN_RANGE(speed, INT8_MIN, INT8_MAX)) {
		bt_shell_error(sh, "Invalid speed: %ld", speed);
	...
	if (err) {
		bt_shell_error(sh, "Playback speed set failed (%d)", err);
	}

	return err;
}

static int cmd_media_read_seeking_speed(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_seeking_speed(current_player);

	if (err) {
		bt_shell_error(sh, "Seeking speed get failed (%d)", err);
	}
...
#ifdef CONFIG_BT_OTS
static int cmd_media_read_track_segments_obj_id(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_track_segments_id(current_player);

	if (err) {
		bt_shell_error(sh, "Track segments ID get failed (%d)", err);
	}

	return err;
}

static int cmd_media_read_current_track_obj_id(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_current_track_id(current_player);

	if (err) {
		bt_shell_error(sh, "Current track ID get failed (%d)", err);
	}
...
static int cmd_media_read_next_track_obj_id(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_next_track_id(current_player);

	if (err) {
		bt_shell_error(sh, "Next track ID get failed (%d)", err);
	}

	return err;
}

static int cmd_media_read_current_group_obj_id(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_current_group_id(current_player);

	if (err) {
		bt_shell_error(sh, "Current group ID get failed (%d)", err);
...
static int cmd_media_read_parent_group_obj_id(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_parent_group_id(current_player);

	if (err) {
		bt_shell_error(sh, "Parent group ID get failed (%d)", err);
	}
...
static int cmd_media_read_playing_order(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_playing_order(current_player);

	if (err) {
		bt_shell_error(sh, "Playing order get failed (%d)", err);
	}

	return err;
}

static int cmd_media_set_playing_order(const struct bt_shell *sh, size_t argc, char *argv[])
{
	unsigned long order;
	int err = 0;

	order = bt_shell_strtoul(argv[1], 0, &err);
	if (err != 0) {
		bt_shell_error(sh, "Could not parse order: %d", err);

		return -ENOEXEC;
	}

	if (order > UINT8_MAX) {
		bt_shell_error(sh, "Invalid order: %ld", order);
	...
	if (err) {
		bt_shell_error(sh, "Playing order set failed (%d)", err);
	}

	return err;
}

static int cmd_media_read_playing_orders_supported(const struct bt_shell *sh, size_t argc,
						   char *argv[])
{
...
static int cmd_media_read_media_state(const struct bt_shell *sh, size_t argc, char *argv[])
{
...
static int cmd_media_play(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (err != 0) {
		bt_shell_error(sh, "Media Controller play failed: %d", err);
	}

	return err;
}

static int cmd_media_pause(const struct bt_shell *sh, size_t argc, char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller pause failed: %d", err);
	}

	return err;
}

static int cmd_media_fast_rewind(const struct bt_shell *sh, size_t argc,
				 char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller fast rewind failed: %d", err);
	}

	return err;
}

static int cmd_media_fast_forward(const struct bt_shell *sh, size_t argc,
				  char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller fast forward failed: %d",
			    err);
	}
...
static int cmd_media_stop(const struct bt_shell *sh, size_t argc, char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller stop failed: %d", err);
	}

	return err;
}

static int cmd_media_move_relative(const struct bt_shell *sh, size_t argc,
				   char *argv[])
{
	struct mpl_cmd cmd = {
	...
	if (err != 0) {
		bt_shell_error(sh, "Failed to parse offset: %d", err);

		return err;
	}

	if (sizeof(long) != sizeof(int32_t) && !IN_RANGE(offset, INT32_MIN, INT32_MAX)) {
		bt_shell_error(sh, "Invalid offset: %ld", offset);
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller move relative failed: %d",
			    err);
	}
...
static int cmd_media_prev_segment(const struct bt_shell *sh, size_t argc,
				  char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller previous segment failed: %d",
			    err);
	}
...
static int cmd_media_next_segment(const struct bt_shell *sh, size_t argc,
				  char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller next segment failed: %d",
			    err);
	}
...
static int cmd_media_first_segment(const struct bt_shell *sh, size_t argc,
				   char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller first segment failed: %d",
			    err);
	}
...
static int cmd_media_last_segment(const struct bt_shell *sh, size_t argc,
				  char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller last segment failed: %d",
			    err);
	}
...
static int cmd_media_goto_segment(const struct bt_shell *sh, size_t argc,
				  char *argv[])
{
	struct mpl_cmd cmd = {
	...
	if (err != 0) {
		bt_shell_error(sh, "Failed to parse segment: %d", err);

		return err;
	}

	if (sizeof(long) != sizeof(int32_t) && !IN_RANGE(segment, INT32_MIN, INT32_MAX)) {
		bt_shell_error(sh, "Invalid segment: %ld", segment);
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller goto segment failed: %d",
			    err);
	}
...
static int cmd_media_prev_track(const struct bt_shell *sh, size_t argc,
				char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller previous track failed: %d",
			    err);
	}
...
static int cmd_media_next_track(const struct bt_shell *sh, size_t argc,
				char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller next track failed: %d",
			    err);
	}
...
static int cmd_media_first_track(const struct bt_shell *sh, size_t argc,
				 char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller first track failed: %d",
			    err);
	}
...
static int cmd_media_last_track(const struct bt_shell *sh, size_t argc,
				char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller last track failed: %d", err);
	}

	return err;
}

static int cmd_media_goto_track(const struct bt_shell *sh, size_t argc,
				char *argv[])
{
	struct mpl_cmd cmd = {
	...
	if (err != 0) {
		bt_shell_error(sh, "Failed to parse track: %d", err);

		return err;
	}

	if (sizeof(long) != sizeof(int32_t) && !IN_RANGE(track, INT32_MIN, INT32_MAX)) {
		bt_shell_error(sh, "Invalid track: %ld", track);
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller goto track failed: %d",
			    err);
	}
...
static int cmd_media_prev_group(const struct bt_shell *sh, size_t argc,
				char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller previous group failed: %d",
			    err);
	}
...
static int cmd_media_next_group(const struct bt_shell *sh, size_t argc,
				char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller next group failed: %d", err);
	}

	return err;
}

static int cmd_media_first_group(const struct bt_shell *sh, size_t argc,
				 char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller first group failed: %d", err);
	}

	return err;
}

static int cmd_media_last_group(const struct bt_shell *sh, size_t argc,
				char *argv[])
{
	const struct mpl_cmd cmd = {
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller last group failed: %d", err);
	}

	return err;
}

static int cmd_media_goto_group(const struct bt_shell *sh, size_t argc,
				char *argv[])
{
	struct mpl_cmd cmd = {
	...
	if (err != 0) {
		bt_shell_error(sh, "Failed to parse group: %d", err);

		return err;
	}

	if (sizeof(long) != sizeof(int32_t) && !IN_RANGE(group, INT32_MIN, INT32_MAX)) {
		bt_shell_error(sh, "Invalid group: %ld", group);
	...
	err = media_proxy_ctrl_send_command(current_player, &cmd);
	if (err != 0) {
		bt_shell_error(sh, "Media Controller goto group failed: %d",
			    err);
	}
...
static int cmd_media_read_commands_supported(const struct bt_shell *sh, size_t argc, char *argv[])
{
	int err = media_proxy_ctrl_get_commands_supported(current_player);

	if (err) {
		bt_shell_error(sh, "Commands supported read failed (%d)", err);
	}
...
#ifdef CONFIG_BT_OTS
static int cmd_media_set_search(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (len > sizeof(search.search)) {
		bt_shell_print(sh, "Fail: Invalid argument");
		return -EINVAL;
	}
	...
	if (err) {
		bt_shell_error(sh, "Search send failed (%d)", err);
	}

	return err;
}

static int cmd_media_read_search_results_obj_id(const struct bt_shell *sh, size_t argc,
						char *argv[])
{
	int err = media_proxy_ctrl_get_search_results_id(current_player);

	if (err) {
		bt_shell_error(sh, "Search results ID get failed (%d)", err);
	}
...
static int cmd_media_read_content_control_id(const struct bt_shell *sh, size_t argc,
					     char *argv[])
{
	int err = media_proxy_ctrl_get_content_ctrl_id(current_player);

	if (err) {
		bt_shell_error(sh, "Content control ID get failed (%d)", err);
	}

	return err;
}

static int cmd_media(const struct bt_shell *sh, size_t argc, char **argv)
{
...
BT_SHELL_STATIC_SUBCMD_SET_CREATE(media_cmds,
	BT_SHELL_CMD_ARG(init, NULL,
		      "Initialize media player",
		      cmd_media_init, 1, 0),
	BT_SHELL_CMD_ARG(set_player, NULL, "Set current player [local || remote]",
		      cmd_media_set_player, 2, 0),
	BT_SHELL_CMD_ARG(show_players, NULL, "Show local, remote and current player",
		      cmd_media_show_players, 1, 0),
#ifdef CONFIG_BT_MCC
	BT_SHELL_CMD_ARG(discover_player, NULL, "Discover remote media player",
	...
	BT_SHELL_CMD_ARG(play, NULL, "Send the play command", cmd_media_play, 1,
		      0),
	...
	BT_SHELL_CMD_ARG(stop, NULL, "Send the stop command", cmd_media_stop, 1,
		      0),
	...
	BT_SHELL_CMD_ARG(read_commands_supported, NULL, "Read Commands Supported",
		      cmd_media_read_commands_supported, 1, 0),
#ifdef CONFIG_BT_OTS
	BT_SHELL_CMD_ARG(set_search, NULL, "Set search <search control item sequence>",
...
BT_SHELL_CMD_ARG_REGISTER(media, &media_cmds, "Media commands",
		       cmd_media, 1, 1);
