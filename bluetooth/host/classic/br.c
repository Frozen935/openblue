...
static bt_slist_t discovery_cbs = BT_SLIST_STATIC_INIT(&discovery_cbs);
...
void bt_hci_conn_complete(struct bt_buf *buf)
{
	...
	bt_atomic_set_bit_to(conn->flags, BT_CONN_BR_BONDABLE, bt_get_bondable());
...
static void report_discovery_results(void)
{
	...
	bt_atomic_clear_bit(bt_dev.flags, BT_DEV_INQUIRY);

	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&discovery_cbs, listener, next, node) {
		if (listener->timeout) {
...
void bt_hci_inquiry_result_with_rssi(struct bt_buf *buf)
{
	...
	if (!bt_atomic_test_bit(bt_dev.flags, BT_DEV_INQUIRY)) {
		return;
	}
...
static void read_buffer_size_complete(struct bt_buf *buf)
{
	...
	os_sem_init(&bt_dev.br.pkts, pkts, pkts);
}
...
int bt_br_init(void)
{
	...
	ssp_cp = bt_buf_add(buf, sizeof(*ssp_cp));
	ssp_cp->mode = 0x01;
	...
	inq_cp = bt_buf_add(buf, sizeof(*inq_cp));
	inq_cp->mode = 0x02;
	...
	bt_buf_add_le24(buf, CONFIG_BT_COD);
	...
	bt_buf_add_le16(buf, CONFIG_BT_PAGE_TIMEOUT);
	...
	if (BT_FEAT_SC(bt_dev.features)) {
		...
		sc_cp = bt_buf_add(buf, sizeof(*sc_cp));
	...
	if (should_enable != is_enabled) {
		...
		policy_cp = bt_buf_add(buf, sizeof(*policy_cp));
			  ...
			  struct bt_br_discovery_result *results, size_t cnt)
{
	...
	bt_atomic_set_bit(bt_dev.flags, BT_DEV_INQUIRY);
...
{
	bt_slist_append(&discovery_cbs, &cb->node);
}

void bt_br_discovery_cb_unregister(struct bt_br_discovery_cb *cb)
{
	bt_slist_find_and_remove(&discovery_cbs, &cb->node);
}
...
static int write_scan_enable(uint8_t scan)
{
	...
	bt_buf_add_u8(buf, scan);
	err = bt_hci_cmd_send_sync(BT_HCI_OP_WRITE_SCAN_ENABLE, buf, NULL);
	if (err) {
		return err;
	}

	bt_atomic_set_bit_to(bt_dev.flags, BT_DEV_ISCAN, (scan & BT_BREDR_SCAN_INQUIRY));
	bt_atomic_set_bit_to(bt_dev.flags, BT_DEV_PSCAN, (scan & BT_BREDR_SCAN_PAGE));
...
{
	if (enable) {
		if (bt_atomic_test_bit(bt_dev.flags, BT_DEV_PSCAN)) {
			return -EALREADY;
		}
...
static int bt_br_write_current_iac_lap(bool limited)
{
	...
	iac_lap = bt_buf_add(buf, param_len);
...
static void bt_br_limited_discoverable_timeout_handler(struct bt_work *work)
{
	int err;

	if (!bt_atomic_test_bit(bt_dev.flags, BT_DEV_LIMITED_DISCOVERABLE_MODE)) {
...
int bt_br_set_discoverable(bool enable, bool limited)
{
	...
	if (enable) {
		if (bt_atomic_test_bit(bt_dev.flags, BT_DEV_ISCAN)) {
			return -EALREADY;
		}
	...
	if (bt_atomic_test_bit(bt_dev.flags, BT_DEV_LIMITED_DISCOVERABLE_MODE)) {
		...
		bt_atomic_clear_bit(bt_dev.flags, BT_DEV_LIMITED_DISCOVERABLE_MODE);
		bt_work_cancel_delayable(&bt_br_limited_discoverable_timeout);
	}
...
int bt_br_write_local_name(const char *name)
{
	...
	if (bt_buf_tailroom(buf) < sizeof(*name_cp)) {
		bt_buf_unref(buf);
		return -ENOMEM;
	}

	name_cp = bt_buf_add(buf, sizeof(*name_cp));
