...
BT_BUF_POOL_FIXED_DEFINE(data_tx_pool, 1, BT_L2CAP_SDU_BUF_SIZE(DATA_MTU),
			  CONFIG_BT_CONN_TX_USER_DATA_SIZE, NULL);
BT_BUF_POOL_FIXED_DEFINE(data_rx_pool, 1, DATA_MTU, 8, NULL);
...
static BT_FIFO_DEFINE(l2cap_recv_fifo);
struct l2ch {
	struct bt_work_delayable recv_work;
	struct bt_l2cap_le_chan ch;
};
#define L2CH_CHAN(_chan) CONTAINER_OF(_chan, struct l2ch, ch.chan)
#define L2CH_WORK(_work) CONTAINER_OF(bt_work_delayable_from_work(_work), \
...
static void l2cap_recv_cb(struct bt_work *work)
{
	struct l2ch *c = L2CH_WORK(work);
	struct bt_buf *buf;

	while ((buf = bt_fifo_get(&l2cap_recv_fifo, OS_TIMEOUT_NO_WAIT))) {
...
static int l2cap_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
	...
	if (l2cap_recv_delay_ms > 0) {
		...
		if (bt_fifo_is_empty(&l2cap_recv_fifo)) {
			bt_shell_print("Delaying response in %u ms...",
				       l2cap_recv_delay_ms);
		}

		bt_fifo_put(&l2cap_recv_fifo, buf);
		bt_work_schedule(&l2ch->recv_work, OS_MSEC(l2cap_recv_delay_ms));
...
static void l2cap_connected(struct bt_l2cap_chan *chan)
{
	...
	bt_work_init_delayable(&c->recv_work, l2cap_recv_cb);
...
static struct bt_buf *l2cap_alloc_buf(struct bt_l2cap_chan *chan)
{
	...
	return bt_buf_alloc(&data_rx_pool, OS_TIMEOUT_FOREVER);
}
...
static int cmd_send(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	while (count--) {
		...
		bt_buf_reserve(buf, BT_L2CAP_SDU_CHAN_SEND_RESERVE);
