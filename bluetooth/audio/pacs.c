...
static bt_slist_t src_pac_list = BT_SLIST_STATIC_INIT(&src_pac_list);
...
static bt_slist_t snk_pac_list = BT_SLIST_STATIC_INIT(&snk_pac_list);
...
static OS_SEM_DEFINE(read_buf_sem, 1, 1);
...
static bool build_pac_records(const struct bt_pacs_cap *cap, void *user_data)
{
	...
	struct bt_buf_simple *buf = data->buf;
	struct bt_buf_simple_state state;
	struct bt_pac_codec *pac_codec;

	bt_buf_simple_save(buf, &state);

	if (bt_buf_simple_tailroom(buf) < sizeof(*pac_codec)) {
		goto fail;
	}

	pac_codec = bt_buf_simple_add(buf, sizeof(*pac_codec));
	pac_codec->id = codec_cap->id;
	pac_codec->cid = sys_cpu_to_le16(codec_cap->cid);
	pac_codec->vid = sys_cpu_to_le16(codec_cap->vid);

	if (bt_buf_simple_tailroom(buf) < (sizeof(struct bt_pac_ltv_data) + codec_cap->data_len)) {
		goto fail;
	}

	bt_buf_simple_add_u8(buf, codec_cap->data_len);
	bt_buf_simple_add_mem(buf, codec_cap->data, codec_cap->data_len);

	if (bt_buf_simple_tailroom(buf) < (sizeof(struct bt_pac_ltv_data) + codec_cap->meta_len)) {
		goto fail;
	}

	bt_buf_simple_add_u8(buf, codec_cap->meta_len);
	bt_buf_simple_add_mem(buf, codec_cap->meta, codec_cap->meta_len);
...
static void foreach_cap(bt_slist_t *list, bt_pacs_cap_foreach_func_t func,
			void *user_data)
{
	struct bt_pacs_cap *cap;

	BT_SLIST_FOR_EACH_CONTAINER(list, cap, _node) {
...
static void get_pac_records(bt_slist_t *list, struct bt_buf_simple *buf)
{
	...
	data.rsp = bt_buf_simple_add(buf, sizeof(*data.rsp));
	data.rsp->num_pac = 0;
	data.buf = buf;
			...
			void *buf, uint16_t len, uint16_t offset)
{
	...
	err = os_sem_take(&read_buf_sem, READ_BUF_SEM_TIMEOUT);
	if (err != 0) {
...
static bt_slist_t *pacs_get_pac(enum bt_audio_dir dir)
{
	switch (dir) {
#if defined(CONFIG_BT_PAC_SNK)
	case BT_AUDIO_DIR_SINK:
...
#define PACS_SNK_PAC_CHAR_ATTR_COUNT UTIL_COND_CODE(IS_ENABLED(CONFIG_BT_PAC_SNK_NOTIFIABLE), (3), (2))
#else
#define BT_PAC_SNK
...
#define BT_PACS_SNK_LOC                                                                            \
	BT_AUDIO_CHRC(BT_UUID_PACS_SNK_LOC, BT_PACS_SNK_LOC_PROP, BT_PACS_SNK_LOC_PERM,            \
		      snk_loc_read,                                                                \
		      UTIL_COND_CODE(CONFIG_BT_PAC_SNK_LOC_WRITEABLE, (snk_loc_write), (NULL)), NULL),\
	IF_ENABLED(CONFIG_BT_PAC_SNK_LOC_NOTIFIABLE, (BT_AUDIO_CCC(snk_loc_cfg_changed),))

/* declaration + value [+ cccd] */
#define PACS_SNK_PAC_LOC_CHAR_ATTR_COUNT                                                           \
	UTIL_COND_CODE(IS_ENABLED(CONFIG_BT_PAC_SNK_LOC_NOTIFIABLE), (3), (2))
#else
#define BT_PACS_SNK_LOC
...
/* declaration + value [+ cccd] */
#define PACS_SRC_PAC_CHAR_ATTR_COUNT UTIL_COND_CODE(IS_ENABLED(CONFIG_BT_PAC_SRC_NOTIFIABLE), (3), (2))
#else
#define BT_PAC_SRC
...
#define BT_PACS_SRC_LOC                                                                            \
	BT_AUDIO_CHRC(BT_UUID_PACS_SRC_LOC, BT_PACS_SRC_LOC_PROP, BT_PACS_SRC_LOC_PERM,            \
		      src_loc_read,                                                                \
		      UTIL_COND_CODE(CONFIG_BT_PAC_SRC_LOC_WRITEABLE, (src_loc_write), (NULL)), NULL),\
	IF_ENABLED(CONFIG_BT_PAC_SRC_LOC_NOTIFIABLE, (BT_AUDIO_CCC(src_loc_cfg_changed),))

/* declaration + value [+ cccd] */
#define PACS_SRC_PAC_LOC_CHAR_ATTR_COUNT                                                           \
	UTIL_COND_CODE(IS_ENABLED(CONFIG_BT_PAC_SRC_LOC_NOTIFIABLE), (3), (2))
#else
#define BT_PACS_SRC_LOC
...
static int pac_notify(struct bt_conn *conn, enum bt_audio_dir dir)
{
	...
	err = os_sem_take(&read_buf_sem, OS_TIMEOUT_NO_WAIT);
	if (err != 0) {
		LOG_DBG("Failed to take read_buf_sem: %d", err);
...
int bt_pacs_cap_unregister(enum bt_audio_dir dir, struct bt_pacs_cap *cap)
{
	...
	if (!bt_slist_find_and_remove(pac, &cap->_node)) {
		return -ENOENT;
	}
