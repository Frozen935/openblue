...
#define FRIEND_REQ_RETRY_TIMEOUT  OS_SECONDS(CONFIG_BT_MESH_LPN_RETRY_TIMEOUT)
...
static void friend_clear_sent(int err, void *user_data)
{
	...
	bt_work_reschedule(&lpn->timer, OS_MSEC(FRIEND_REQ_TIMEOUT));
}
...
static void clear_friendship(bool force, bool disable)
{
	...
	(void)bt_work_cancel_delayable(&lpn->timer);
	...
	if (!disable) {
		...
		bt_work_reschedule(&lpn->timer, FRIEND_REQ_RETRY_TIMEOUT);
...
static void friend_req_send_end(int err, void *user_data)
{
	...
	if (IS_ENABLED(CONFIG_BT_MESH_LPN_ESTABLISHMENT)) {
		bt_work_reschedule(&lpn->timer,
				  OS_MSEC(FRIEND_REQ_WAIT - (int32_t)lpn->adv_duration));
...
static void req_send_end(int err, void *user_data)
{
	...
	if (lpn->established || IS_ENABLED(CONFIG_BT_MESH_LPN_ESTABLISHMENT)) {
		 ...
		 */
		bt_work_reschedule(&lpn->timer,
				  OS_MSEC(LPN_RECV_DELAY - SCAN_LATENCY - RX_DELAY_CORRECTION(lpn)));
	} else {
		lpn_set_state(BT_MESH_LPN_WAIT_UPDATE);
		bt_work_reschedule(&lpn->timer, OS_MSEC(LPN_RECV_DELAY + lpn->recv_win));
	}
...
static void friend_response_received(struct bt_mesh_lpn *lpn)
{
	...
	bt_work_reschedule(&lpn->timer, OS_MSEC(timeout));
	bt_mesh_scan_disable();
}
...
void bt_mesh_lpn_msg_received(struct bt_mesh_net_rx *rx)
{
	...
	if (lpn->state == BT_MESH_LPN_TIMER) {
		...
		bt_work_reschedule(&lpn->timer, OS_MSEC(LPN_AUTO_TIMEOUT));
		return;
	}
...
static void update_timeout(struct bt_mesh_lpn *lpn)
{
	if (lpn->established) {
		...
		lpn_set_state(BT_MESH_LPN_ESTABLISHED);
		bt_work_reschedule(&lpn->timer, OS_MSEC(POLL_RETRY_TIMEOUT));
		bt_mesh_scan_disable();
	} else {
		if (IS_ENABLED(CONFIG_BT_MESH_LPN_ESTABLISHMENT)) {
			bt_mesh_scan_disable();
...
static void lpn_timeout(struct bt_work *work)
{
	struct bt_mesh_lpn *lpn = &bt_mesh.lpn;
	...
	case BT_MESH_LPN_ENABLED:
		...
		break;
	case BT_MESH_LPN_REQ_WAIT:
		bt_work_reschedule(&lpn->timer, OS_MSEC(lpn->adv_duration + FRIEND_REQ_SCAN));
		lpn_set_state(BT_MESH_LPN_WAIT_OFFER);
		bt_mesh_scan_enable();
		break;
	case BT_MESH_LPN_WAIT_OFFER:
	...
	case BT_MESH_LPN_RECV_DELAY:
		bt_work_reschedule(&lpn->timer,
				  OS_MSEC(SCAN_LATENCY + lpn->recv_win + RX_DELAY_CORRECTION(lpn)));
		lpn_set_state(BT_MESH_LPN_WAIT_UPDATE);
		bt_mesh_scan_enable();
		break;
	case BT_MESH_LPN_WAIT_UPDATE:
...
int bt_mesh_lpn_init(void)
{
	...
	bt_work_init_delayable(&lpn->timer, lpn_timeout);
