...
BT_BUF_POOL_FIXED_DEFINE(reassembly_buf_pool, CONFIG_BT_CHANNEL_SOUNDING_REASSEMBLY_BUFFER_CNT,
			  CONFIG_BT_CHANNEL_SOUNDING_REASSEMBLY_BUFFER_SIZE,
			  sizeof(struct reassembly_buf_meta_data), NULL);

static bt_slist_t reassembly_bufs = BT_SLIST_STATIC_INIT(&reassembly_bufs);
 ...
 * @return struct bt_buf* Reassembly buffer, NULL if allocation fails
 */
static struct bt_buf *alloc_reassembly_buf(uint16_t conn_handle)
{
	struct bt_buf *buf = bt_buf_alloc(&reassembly_buf_pool, OS_TIMEOUT_NO_WAIT);
	...
	bt_buf_slist_put(&reassembly_bufs, buf);
 ...
 */
static void free_reassembly_buf(struct bt_buf **buf)
{
	if (!buf) {
	...
	if (!bt_slist_find_and_remove(&reassembly_bufs, &(*buf)->node)) {
 ...
 * @return struct bt_buf* Reassembly buffer, NULL if it doesn't exist or failed when allocating new
 */
static struct bt_buf *get_reassembly_buf(uint16_t conn_handle, bool allocate)
{
	bt_snode_t *node;

	BT_SLIST_FOR_EACH_NODE(&reassembly_bufs, node) {
		struct bt_buf *buf = CONTAINER_OF(node, struct bt_buf, node);
		struct reassembly_buf_meta_data *buf_meta_data =
			(struct reassembly_buf_meta_data *)(buf->user_data);
 ...
 */
static bool add_reassembly_data(struct bt_buf *reassembly_buf, const uint8_t *data,
				uint16_t data_len)
{
	if (data_len > bt_buf_tailroom(reassembly_buf)) {
		LOG_ERR("Not enough reassembly buffer space for subevent result");
		return false;
	}

	bt_buf_add_mem(reassembly_buf, data, data_len);
	return true;
}
 ...
 * @return struct bt_buf* Pointer to reassembly buffer, NULL if fails to allocate or insert data
 */
static struct bt_buf *start_reassembly(uint16_t conn_handle, const uint8_t *steps,
					uint16_t step_data_len)
{
	struct bt_buf *reassembly_buf = get_reassembly_buf(conn_handle, true);
 ...
 * @param steps Step data
 * @param step_data_len Step data length
 * @return struct bt_buf* Pointer to reassembly buffer, NULL if fails to insert data
 */
static struct bt_buf *continue_reassembly(uint16_t conn_handle, const uint8_t *steps,
					   uint16_t step_data_len)
{
	struct bt_buf *reassembly_buf = get_reassembly_buf(conn_handle, false);
...
static void clear_on_disconnect(struct bt_conn *conn, uint8_t reason)
{
	struct bt_buf *buf = get_reassembly_buf(conn->handle, false);
...
int bt_le_cs_start_test(const struct bt_le_cs_test_param *params)
{
	...
	if (params->override_config & BT_HCI_OP_LE_CS_TEST_OVERRIDE_CONFIG_0_MASK) {
		...
		bt_buf_add_u8(buf, num_channels);
		override_parameters_length++;
		bt_buf_add_mem(buf, params->override_config_0.set.channels, num_channels);
		override_parameters_length += num_channels;
	} else {
		bt_buf_add_mem(buf, params->override_config_0.not_set.channel_map,
				sizeof(params->override_config_0.not_set.channel_map));
		bt_buf_add_u8(buf, params->override_config_0.not_set.channel_selection_type);
		bt_buf_add_u8(buf, params->override_config_0.not_set.ch3c_shape);
		bt_buf_add_u8(buf, params->override_config_0.not_set.ch3c_jump);
	...
	if (params->override_config & BT_HCI_OP_LE_CS_TEST_OVERRIDE_CONFIG_2_MASK) {
		bt_buf_add_mem(buf, &params->override_config_2, sizeof(params->override_config_2));
	...
	if (params->override_config & BT_HCI_OP_LE_CS_TEST_OVERRIDE_CONFIG_3_MASK) {
		bt_buf_add_mem(buf, &params->override_config_3, sizeof(params->override_config_3));
	...
	if (params->override_config & BT_HCI_OP_LE_CS_TEST_OVERRIDE_CONFIG_4_MASK) {
		bt_buf_add_mem(buf, &params->override_config_4, sizeof(params->override_config_4));
	...
	if (params->override_config & BT_HCI_OP_LE_CS_TEST_OVERRIDE_CONFIG_5_MASK) {
		bt_buf_add_le32(buf, params->override_config_5.cs_sync_aa_initiator);
		bt_buf_add_le32(buf, params->override_config_5.cs_sync_aa_reflector);
	...
	if (params->override_config & BT_HCI_OP_LE_CS_TEST_OVERRIDE_CONFIG_6_MASK) {
		bt_buf_add_mem(buf, &params->override_config_6, sizeof(params->override_config_6));
	...
	if (params->override_config & BT_HCI_OP_LE_CS_TEST_OVERRIDE_CONFIG_7_MASK) {
		bt_buf_add_mem(buf, &params->override_config_7, sizeof(params->override_config_7));
	...
	if (params->override_config & BT_HCI_OP_LE_CS_TEST_OVERRIDE_CONFIG_8_MASK) {
		bt_buf_add_mem(buf, &params->override_config_8, sizeof(params->override_config_8));
...
void bt_hci_le_cs_subevent_result(struct bt_buf *buf)
{
	struct bt_conn *conn = NULL;
	struct bt_hci_evt_le_cs_subevent_result *evt;
	struct bt_conn_le_cs_subevent_result result;
	struct bt_conn_le_cs_subevent_result *p_result = &result;
	struct bt_buf_simple step_data_buf;
	struct bt_buf *reassembly_buf = NULL;
	...
	if (evt->subevent_done_status != BT_HCI_LE_CS_SUBEVENT_DONE_STATUS_PARTIAL) {
		...
		if (evt->num_steps_reported) {
			bt_buf_simple_init_with_data(&step_data_buf, evt->steps, buf->len);
		...
		p_result->step_data_buf = (struct bt_buf_simple *)&reassembly_buf->data;
	}
...
void bt_hci_le_cs_subevent_result_continue(struct bt_buf *buf)
{
	struct bt_conn *conn = NULL;
...
void bt_hci_le_cs_procedure_enable_complete(struct bt_buf *buf)
{
	struct bt_conn *conn;
	...
	if (evt->state == BT_HCI_OP_LE_CS_PROCEDURES_DISABLED) {
		struct bt_buf *reassembly_buf = get_reassembly_buf(conn->handle, false);
...
void bt_hci_le_cs_test_end_complete(struct bt_buf *buf)
{
	...
	struct bt_buf *reassembly_buf = get_reassembly_buf(BT_HCI_LE_CS_TEST_CONN_HANDLE, false);
...
void bt_le_cs_step_data_parse(struct bt_buf_simple *step_data_buf,
			      ...
			      void *user_data)
{
	...
	while (step_data_buf->len > 1) {
		...
		step.mode = bt_buf_simple_pull_u8(step_data_buf);
		step.channel = bt_buf_simple_pull_u8(step_data_buf);
		step.data_len = bt_buf_simple_pull_u8(step_data_buf);
		...
		bt_buf_simple_pull(step_data_buf, step.data_len);
	}
}
