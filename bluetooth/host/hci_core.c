...
#define HCI_CMD_TIMEOUT      OS_SECONDS(10)

/* Stacks for the threads */
static void rx_work_handler(struct bt_work *work);
static BT_WORK_DEFINE(rx_work, rx_work_handler);
#if defined(CONFIG_BT_RECV_WORKQ_BT)
static struct bt_work_q bt_workq;
#endif /* CONFIG_BT_RECV_WORKQ_BT */

static void init_work(struct bt_work *work);

struct bt_dev bt_dev = {
	.init          = BT_WORK_INITIALIZER(init_work),
...
#define cmd(buf) (&cmd_data[bt_buf_id(buf)])
#define acl(buf) ((struct bt_conn_rx *)bt_buf_user_data(buf))
...
BT_BUF_POOL_FIXED_DEFINE(hci_cmd_pool, BT_BUF_CMD_TX_COUNT, CMD_BUF_SIZE, 0, NULL);

struct event_handler {
	uint8_t event;
	uint8_t min_len;
	void (*handler)(struct bt_buf *buf);
};
...
static int handle_event_common(uint8_t event, struct bt_buf *buf,
...
static void handle_event(uint8_t event, struct bt_buf *buf, const struct event_handler *handlers,
			 size_t num_handlers)
{
	int err;
...
static void handle_vs_event(uint8_t event, struct bt_buf *buf,
			    const struct event_handler *handlers, size_t num_handlers)
{
	int err;
...
__weak void bt_testing_trace_event_acl_pool_destroy(struct bt_buf *buf)
{
}
#endif

#if defined(CONFIG_BT_HCI_ACL_FLOW_CONTROL)
void bt_hci_host_num_completed_packets(struct bt_buf *buf)
{
	struct bt_hci_cp_host_num_completed_packets *cp;
...
struct bt_buf *bt_hci_cmd_alloc(os_timeout_t timeout)
{
	struct bt_buf *buf;

	buf = bt_buf_alloc(&hci_cmd_pool, timeout);
	if (!buf) {
	...
	bt_buf_reserve(buf, sizeof(uint8_t) + sizeof(struct bt_hci_cmd_hdr));
...
int bt_hci_cmd_send(uint16_t opcode, struct bt_buf *buf)
{
	...
	if (buf != NULL) {
		 ...
		 * bt_hci_cmd_alloc().
		 */
		if (bt_buf_headroom(buf) < sizeof(uint8_t) + sizeof(*hdr)) {
			return -EINVAL;
		}
	} else {
		buf = bt_hci_cmd_alloc(OS_TIMEOUT_FOREVER);
	...
	bt_fifo_put(&bt_dev.cmd_tx_queue, buf);
	bt_tx_irq_raise();

	return 0;
}

static bool process_pending_cmd(os_timeout_t timeout);
			 ...
			 struct bt_buf **rsp)
{
	 ...
	 */
	(void)os_sem_init(&sync_sem, 0, 1);
	cmd(buf)->sync = &sync_sem;

	err = bt_hci_cmd_send(opcode, bt_buf_ref(buf));
	if (err) {
		bt_buf_unref(buf);
		return err;
	}
	...
	if (!IS_ENABLED(CONFIG_BT_TX_PROCESSOR_THREAD) &&
	    os_thread_self() == bt_work_queue_thread_get(bt_work_main_work_queue())) {
		/* drain the command queue until we get to send the command of interest. */
		struct bt_buf *cmd = NULL;

		do {
			cmd = bt_fifo_peek_head(&bt_dev.cmd_tx_queue);
	...
	err = os_sem_take(&sync_sem, HCI_CMD_TIMEOUT);
	BT_ASSERT_MSG(err == 0,
...
static void hci_num_completed_packets(struct bt_buf *buf)
{
	...
	for (i = 0; i < evt->num_handles; i++) {
		...
		while (count--) {
			...
			bt_slist_append(&conn->tx_complete, node);

			/* align the `pending` value */
			__ASSERT_NO_MSG(bt_atomic_get(&conn->in_ll));
...
static void hci_data_buf_overflow(struct bt_buf *buf)
{
...
int bt_le_create_conn_ext(const struct bt_conn *conn)
{
	...
	if (IS_ENABLED(CONFIG_BT_FILTER_ACCEPT_LIST)) {
		use_filter = bt_atomic_test_bit(conn->flags, BT_CONN_AUTO_CONNECT);
	}
...
static void hci_disconn_complete_prio(struct bt_buf *buf)
{
...
static void hci_disconn_complete(struct bt_buf *buf)
{
	...
	if (!bt_conn_is_le(conn)) {
		...
		if (bt_conn_is_br(conn) && conn->br.link_key != NULL) {
			 ...
			 */
			if (bt_atomic_test_and_clear_bit(conn->flags, BT_CONN_BR_NOBOND)) {
	...
	if (bt_atomic_test_bit(conn->flags, BT_CONN_AUTO_CONNECT)) {
...
int bt_hci_read_remote_version(struct bt_conn *conn)
{
	...
	if (bt_atomic_test_bit(conn->flags, BT_CONN_AUTO_VERSION_INFO)) {
		return 0;
	}
...
static void le_conn_complete_cancel(uint8_t err)
{
	...
	if (bt_atomic_test_bit(conn->flags, BT_CONN_AUTO_CONNECT)) {
		...
		int busy_status = bt_work_delayable_busy_get(&conn->deferred_work);

		if (!(busy_status & (BT_WORK_QUEUED | BT_WORK_DELAYED))) {
...
static void le_conn_complete_adv_timeout(void)
{
	if (!(IS_ENABLED(CONFIG_BT_EXT_ADV) &&
		...
		bt_atomic_clear_bit(adv->flags, BT_ADV_ENABLED);
...
void bt_hci_le_enh_conn_complete(struct bt_hci_evt_le_enh_conn_complete *evt)
{
	...
	if (IS_ENABLED(CONFIG_BT_CENTRAL) &&
		 ...
		 */
		bt_atomic_clear_bit(bt_dev.flags, BT_DEV_INITIATING);
	}
	...
	if (IS_ENABLED(CONFIG_BT_PERIPHERAL) &&
		...
		if (!(IS_ENABLED(CONFIG_BT_EXT_ADV) &&
			...
			if (IS_ENABLED(CONFIG_BT_PRIVACY) &&
			    !bt_atomic_test_bit(adv->flags, BT_ADV_USE_IDENTITY)) {
...
static void le_enh_conn_complete(struct bt_buf *buf)
{
...
static void le_enh_conn_complete_v2(struct bt_buf *buf)
{
...
static void le_legacy_conn_complete(struct bt_buf *buf)
{
...
static void le_remote_feat_complete(struct bt_buf *buf)
{
	...
	bt_atomic_set_bit(conn->flags, BT_CONN_LE_FEATURES_EXCHANGED);
...
static void le_read_all_remote_feat_complete(struct bt_buf *buf)
{
...
static void le_frame_space_update_complete(struct bt_buf *buf)
{
...
static void le_data_len_change(struct bt_buf *buf)
{
...
static void le_phy_update_complete(struct bt_buf *buf)
{
...
static void le_conn_param_req(struct bt_buf *buf)
{
...
static void le_conn_update_complete(struct bt_buf *buf)
{
	...
	if (evt->status == BT_HCI_ERR_UNSUPP_REMOTE_FEATURE &&
	    conn->role == BT_HCI_ROLE_PERIPHERAL &&
	    !bt_atomic_test_and_set_bit(conn->flags,
...
static int set_flow_control(void)
{
	...
	bt_buf_add_u8(buf, BT_HCI_CTL_TO_HOST_FLOW_ENABLE);
...
static void unpair(uint8_t id, const bt_addr_le_t *addr)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&bt_auth_info_cbs, listener,
					  next, node) {
...
static void hci_encrypt_change(struct bt_buf *buf)
{
	...
	if (bt_conn_is_br(conn)) {
		...
		if (IS_ENABLED(CONFIG_BT_SMP)) {
			 ...
			 * central on the link
			 */
			if (bt_atomic_test_bit(conn->flags, BT_CONN_BR_PAIRED) &&
...
static void bt_hci_evt_read_remote_version_complete(struct bt_buf *buf)
{
	struct bt_hci_evt_remote_version_info *evt;
	struct bt_conn *conn;
	uint16_t handle;

	evt = bt_buf_pull_mem(buf, sizeof(*evt));
	handle = sys_le16_to_cpu(evt->handle);
	conn = bt_conn_lookup_handle(handle, BT_CONN_TYPE_ALL);
	if (!conn) {
		LOG_ERR("No connection for handle %u", handle);
	...
	bt_atomic_set_bit(conn->flags, BT_CONN_AUTO_VERSION_INFO);

	if (IS_ENABLED(CONFIG_BT_REMOTE_INFO) &&
	    bt_atomic_test_bit(conn->flags, BT_CONN_LE_FEATURES_EXCHANGED)) {
...
static void hci_hardware_error(struct bt_buf *buf)
{
...
static void hci_reset_complete(void)
{
	...
	flags = (bt_atomic_get(bt_dev.flags) & BT_DEV_PERSISTENT_FLAGS);
	bt_atomic_set(bt_dev.flags, flags);
}

static void hci_cmd_done(uint16_t opcode, uint8_t status, struct bt_buf *evt_buf)
{
	...
	buf = bt_atomic_ptr_clear((bt_atomic_ptr_t *)&bt_dev.sent_cmd);
	...
	if (cmd(buf)->opcode != opcode) {
		LOG_ERR("OpCode 0x%04x completed instead of expected 0x%04x", opcode,
			cmd(buf)->opcode);
		buf = bt_atomic_ptr_set((bt_atomic_ptr_t *)&bt_dev.sent_cmd, buf);
		__ASSERT_NO_MSG(!buf);
		goto exit;
	}
	...
	if (evt_buf != buf) {
		bt_buf_reset(buf);
		bt_buf_add_mem(buf, evt_buf->data, evt_buf->len);
	}

	if (cmd(buf)->state && !status) {
		struct bt_hci_cmd_state_set *update = cmd(buf)->state;

		bt_atomic_set_bit_to(update->target, update->bit, update->val);
	}
...
static void hci_cmd_complete(struct bt_buf *buf)
{
	...
	if (ncmd) {
		(void)os_sem_give(&bt_dev.ncmd_sem);
		bt_tx_irq_raise();
	}
}
...
{
	if (!bt_atomic_test_bit(adv->flags, BT_ADV_CREATED)) {
		return -EINVAL;
	}
...
{
	if (!bt_atomic_test_bit(sync->flags, BT_PER_ADV_SYNC_CREATED)) {
		return -EINVAL;
	}
...
void bt_hci_le_transmit_power_report(struct bt_buf *buf)
{
...
void bt_hci_le_path_loss_threshold_event(struct bt_buf *buf)
{
...
#if defined(CONFIG_BT_SUBRATING)
void bt_hci_le_subrate_change_event(struct bt_buf *buf)
{
...
void bt_hci_le_conn_rate_change_event(struct bt_buf *buf)
{
...
static void hci_vendor_event(struct bt_buf *buf)
{
	bool handled = false;
...
static void hci_core_send_cmd(void)
{
	...
	/* Get next command */
	LOG_DBG("fetch cmd");
	buf = bt_fifo_get(&bt_dev.cmd_tx_queue, OS_TIMEOUT_NO_WAIT);
	BT_ASSERT(buf);

	/* Clear out any existing sent command */
	if (bt_dev.sent_cmd) {
		LOG_ERR("Uncleared pending sent_cmd");
		bt_buf_unref(bt_dev.sent_cmd);
		bt_dev.sent_cmd = NULL;
	}

	bt_dev.sent_cmd = bt_buf_ref(buf);
...
static void read_le_all_supported_features_complete(struct bt_buf *buf)
{
...
static void read_buffer_size_complete(struct bt_buf *buf)
{
	...
	(void)os_sem_init(&bt_dev.le.acl_pkts, pkts, pkts);
}
...
static void le_read_buffer_size_complete(struct bt_buf *buf)
{
	...
	(void)os_sem_init(&bt_dev.le.acl_pkts, rp->le_max_num, rp->le_max_num);
#endif /* CONFIG_BT_CONN */
}

static void read_buffer_size_v2_complete(struct bt_buf *buf)
{
#if defined(CONFIG_BT_ISO)
	...
	if (acl_mtu && rp->acl_max_num) {
		...
		(void)os_sem_init(&bt_dev.le.acl_pkts, rp->acl_max_num, rp->acl_max_num);
	...
	(void)os_sem_init(&bt_dev.le.iso_pkts, rp->iso_max_num, rp->iso_max_num);
...
static void read_supported_commands_complete(struct bt_buf *buf)
{
...
static void le_read_supp_states_complete(struct bt_buf *buf)
{
...
#if defined(CONFIG_BT_BROADCASTER)
static void le_read_maximum_adv_data_len_complete(struct bt_buf *buf)
{
...
#if defined(CONFIG_BT_SMP)
static void le_read_resolving_list_size_complete(struct bt_buf *buf)
{
...
static int le_set_event_mask(void)
{
	...
	cp_mask = bt_buf_add(buf, sizeof(*cp_mask));
...
static int le_init(void)
{
	...
	if (BT_FEAT_BREDR(bt_dev.features)) {
		...
		cp_le = bt_buf_add(buf, sizeof(*cp_le));
...
static void bt_dev_show_info(void)
{
	...
	LOG_INF("HCI transport: %s", bt_dev.hci->name);
...
static void le_legacy_conn_complete_prio(struct bt_buf *buf)
{
	struct bt_hci_evt_le_conn_complete *evt = (void *)buf->data;
...
static void le_enh_conn_complete_prio(struct bt_buf *buf)
{
	struct bt_hci_evt_le_enh_conn_complete *evt =
		(struct bt_hci_evt_le_enh_conn_complete *)buf->data;
...
static void hci_le_meta_prio_event(struct bt_buf *buf)
{
	struct bt_hci_evt_le_meta_event *evt;
...
static void hci_event_prio(struct bt_buf *buf)
{
	struct bt_buf_simple_state state;
	struct bt_hci_evt_hdr *hdr;
	uint8_t evt_flags;
...
static void rx_queue_put(struct bt_buf *buf)
{
	bt_buf_slist_put(&bt_dev.rx_queue, buf);

#if defined(CONFIG_BT_RECV_WORKQ_SYS)
	const int err = bt_work_submit(&rx_work);
#elif defined(CONFIG_BT_RECV_WORKQ_BT)
	const int err = bt_work_submit_to_queue(&bt_workq, &rx_work);
...
static int bt_recv_unsafe(struct bt_buf *buf)
{
...
{
	bt_atomic_set_bit(bt_dev.flags, BT_DEV_READY);
...
static int bt_init(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		...
		bt_atomic_set_bit(bt_dev.flags, BT_DEV_PRESET_ID);
	}
...
static void rx_work_handler(struct bt_work *work)
{
	...
	LOG_DBG("Getting bt_buf from queue");
	buf = bt_buf_slist_get(&bt_dev.rx_queue);
	if (!buf) {
		return;
	}

	type = bt_buf_pull_u8(buf);
	 ...
	 * we used a while() loop with an explicit yield.
	 */
	if (!bt_slist_is_empty(&bt_dev.rx_queue)) {

#if defined(CONFIG_BT_RECV_WORKQ_SYS)
		err = bt_work_submit(&rx_work);
#elif defined(CONFIG_BT_RECV_WORKQ_BT)
		err = bt_work_submit_to_queue(&bt_workq, &rx_work);
#endif
		if (err < 0) {
			LOG_ERR("Could not submit rx_work: %d", err);
		}
...
#if defined(CONFIG_BT_TESTING)
os_tid_t bt_testing_tx_tid_get(void)
{
	return bt_work_queue_thread_get(bt_work_main_work_queue());
}
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	bt_atomic_clear_bit(bt_dev.flags, BT_DEV_DISABLE);

	if (bt_atomic_test_and_set_bit(bt_dev.flags, BT_DEV_ENABLE)) {
		return -EALREADY;
	}
	...
	if (!IS_ENABLED(CONFIG_BT_WAIT_NOP)) {
		(void)os_sem_init(&bt_dev.ncmd_sem, 1, 1);
	} else {
		(void)os_sem_init(&bt_dev.ncmd_sem, 0, 1);
	}
	bt_fifo_init(&bt_dev.cmd_tx_queue);

#if defined(CONFIG_BT_RECV_WORKQ_BT)
	/* RX thread */
	bt_work_queue_init(&bt_workq);
	bt_work_queue_start(&bt_workq, CONFIG_BT_RX_STACK_SIZE,
			    OS_PRIORITY(CONFIG_BT_RX_PRIO), NULL);
	(void)os_thread_name_set(&bt_workq.thread, "BT RX WQ");
#endif
...
int bt_disable(void)
{
	...
	if (bt_atomic_test_and_set_bit(bt_dev.flags, BT_DEV_DISABLE)) {
		return -EALREADY;
	}

	/* Clear BT_DEV_READY before disabling HCI link */
	bt_atomic_clear_bit(bt_dev.flags, BT_DEV_READY);
...
#if defined(CONFIG_BT_PRIVACY)
	bt_work_cancel_delayable(&bt_dev.rpa_update);
...
#if defined(CONFIG_BT_RECV_WORKQ_BT)
	/* Abort RX thread */
	(void)bt_work_queue_stop(&bt_workq, OS_TIMEOUT_FOREVER);
#endif
	 ...
	 * completed.
	 */
	bt_atomic_clear_bit(bt_dev.flags, BT_DEV_ENABLE);
...
{
	return bt_atomic_test_bit(bt_dev.flags, BT_DEV_READY);
}
...
int bt_set_name(const char *name)
{
	...
	if (IS_ENABLED(CONFIG_BT_CLASSIC) && bt_atomic_test_bit(bt_dev.flags, BT_DEV_READY)) {
...
int bt_le_get_local_features(struct bt_le_local_features *remote_info)
{
	...
	remote_info->acl_mtu = UTIL_COND_CODE(CONFIG_BT_CONN, (bt_dev.le.acl_mtu), (0));
	remote_info->acl_pkts = UTIL_COND_CODE(CONFIG_BT_CONN, (bt_dev.le.acl_pkts.limit), (0));
	remote_info->iso_mtu = UTIL_COND_CODE(CONFIG_BT_ISO, (bt_dev.le.iso_mtu), (0));
	remote_info->iso_pkts = UTIL_COND_CODE(CONFIG_BT_ISO, (bt_dev.le.iso_limit), (0));
	remote_info->rl_size = UTIL_COND_CODE(CONFIG_BT_SMP, (bt_dev.le.rl_size), (0));
	remote_info->max_adv_data_len =
		UTIL_COND_CODE(CONFIG_BT_BROADCASTER, (bt_dev.le.max_adv_data_len), (0));
...
int bt_le_set_rpa_timeout(uint16_t new_rpa_timeout)
{
	...
	bt_atomic_set_bit(bt_dev.flags, BT_DEV_RPA_TIMEOUT_CHANGED);
...
static bool process_pending_cmd(os_timeout_t timeout)
{
	if (!bt_fifo_is_empty(&bt_dev.cmd_tx_queue)) {
		if (os_sem_take(&bt_dev.ncmd_sem, timeout) == 0) {
			hci_core_send_cmd();
			return true;
		}
	}
...
static void tx_processor(struct bt_work *item)
{
	...
	if (process_pending_cmd(OS_TIMEOUT_NO_WAIT)) {
 ...
 */
static BT_WORK_DEFINE(tx_work, tx_processor);

#if defined(CONFIG_BT_TX_PROCESSOR_THREAD)
/**
 * This work queue shall never be stopped, drained or plugged.
 */
static struct bt_work_q bt_tx_processor_workq;

static int bt_tx_processor_init(void)
{
	struct bt_work_queue_config cfg = {};

	if (IS_ENABLED(CONFIG_THREAD_NAME)) {
		cfg.name = "bt_tx_processor";
	}

	bt_work_queue_start(&bt_tx_processor_workq, CONFIG_BT_TX_PROCESSOR_STACK_SIZE,
			    OS_PRIORITY(CONFIG_BT_TX_PROCESSOR_THREAD_PRIO), &cfg);
 ...
 */
STACK_INIT(bt_tx_processor_init, STACK_BASE_INIT, 999);
...
void bt_tx_irq_raise(void)
{
	...
	LOG_DBG("kick TX");
#if defined(CONFIG_BT_TX_PROCESSOR_THREAD)
	err = bt_work_submit_to_queue(&bt_tx_processor_workq, &tx_work);
