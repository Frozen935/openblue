...
#define AVDTP_KWORK(_work)                                                                         \
	CONTAINER_OF(CONTAINER_OF(_work, struct bt_work_delayable, work), struct bt_avdtp,          \
		     timeout_work)
...
BT_BUF_POOL_DEFINE(avdtp_pool, CONFIG_BT_MAX_CONN * 2,
 ...
 */
BT_BUF_POOL_DEFINE(avdtp_frag_pool, 1,
		    BT_L2CAP_BUF_SIZE(CONFIG_BT_L2CAP_TX_MTU),
		    CONFIG_BT_CONN_TX_USER_DATA_SIZE, NULL);

/* tx list for packets */
bt_slist_t avdtp_tx_list = BT_SLIST_STATIC_INIT(&avdtp_tx_list);
static void avdtp_tx_processor(struct bt_work *item);
static BT_WORK_DEFINE(avdtp_tx_work, avdtp_tx_processor);
...
{
	if (sep != NULL) {
		os_sem_take(&sep->sem_lock, OS_TIMEOUT_FOREVER);
	}
}
...
{
	bt_work_cancel_delayable(&sep->_delay_work);
}

static void avdtp_media_disconnect_work(struct bt_work *work)
{
...
int bt_avdtp_media_l2cap_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
...
static struct bt_buf *avdtp_create_pdu(uint8_t msg_type, uint8_t sig_id, uint8_t tid)
{
	struct bt_buf *buf;
	struct bt_avdtp_single_sig_hdr *hdr;
	...
	__ASSERT_NO_MSG(bt_buf_tailroom(buf) >= sizeof(*hdr));
...
{
	if (!bt_slist_is_empty(&avdtp_tx_list)) {
		LOG_DBG("kick TX");
...
static void avdtp_tx_remove(struct bt_buf *buf)
{
	os_sem_take(&avdtp_sem_lock, OS_TIMEOUT_FOREVER);
	bt_slist_find_and_remove(&avdtp_tx_list, &buf->node);
	os_sem_give(&avdtp_sem_lock);
}

static void avdtp_tx_rel_buf(struct bt_buf *buf, struct bt_buf *frag)
{
	avdtp_tx_remove(buf);
	bt_buf_unref(buf);
	bt_buf_unref(frag);
...
static void avdtp_tx_signal(struct bt_avdtp *session, struct bt_buf *buf)
{
	int err;
...
static void avdtp_tx_frags(struct bt_avdtp *session, struct bt_buf *buf,
			   struct avdtp_buf_user_data *user_data)
{
	...
	while (user_data->frag_count > user_data->current_frag && buf->len > 0) {
		...
		frag = bt_l2cap_create_pdu_timeout(NULL, 0, OS_TIMEOUT_NO_WAIT);
		if (frag == NULL) {
			frag = bt_l2cap_create_pdu_timeout(&avdtp_frag_pool, 0, OS_TIMEOUT_NO_WAIT);
		}

		if (frag == NULL) {
			LOG_DBG("No Buff available, wait tx cb to trigger this work again");
			/* Do NOT call `avdtp_tx_raise` here.
			 * When there is no idle bt_buf available while AVDTP TX is still pending,
			 ...
			 * the CPU continuously because TX is pending but no idle bt_buf exists.
			 *
			 * `avdtp_tx_raise` should only be triggered when at least one idle bt_buf
			 * is available. After the previously transmitted avdtp bt_buf is released,
		...
		if (user_data->current_frag == 1) {
			...
			sig_hdr = bt_buf_pull_mem(buf, sizeof(*sig_hdr));
			user_data->hdr = *sig_hdr;

			__ASSERT_NO_MSG(bt_buf_tailroom(frag) >= sizeof(*start_hdr));
			start_hdr = bt_buf_add(frag, sizeof(*start_hdr));
			...
			bt_buf_add_mem(frag, bt_buf_pull_mem(buf, len), len);
		} else {
			...
			__ASSERT_NO_MSG(bt_buf_tailroom(frag) >= sizeof(*cont_hdr));
			cont_hdr = bt_buf_add(frag, sizeof(*cont_hdr));
			/* use same transaction label and message type */
...
static void avdtp_tx_processor(struct bt_work *item)
{
	struct bt_avdtp *session;
	struct avdtp_buf_user_data *user_data;
	const bt_snode_t *node;
	struct bt_buf *buf;

	os_sem_take(&avdtp_sem_lock, OS_TIMEOUT_FOREVER);
	node = bt_slist_peek_head(&avdtp_tx_list);
	if (node == NULL) {
		os_sem_give(&avdtp_sem_lock);
	...
	user_data = bt_buf_user_data(buf);
...
static void avdtp_buf_init_user_data(struct bt_avdtp *session, struct bt_buf *buf)
{
	struct avdtp_buf_user_data *user_data;
...
static void avdtp_send_common(struct bt_avdtp *session, struct bt_buf *buf)
{
	avdtp_buf_init_user_data(session, buf);
	os_sem_take(&avdtp_sem_lock, OS_TIMEOUT_FOREVER);
	bt_slist_append(&avdtp_tx_list, &buf->node);
	os_sem_give(&avdtp_sem_lock);

	avdtp_tx_raise();
}

static int avdtp_send_rsp(struct bt_avdtp *session, struct bt_buf *buf)
{
...
static void avdtp_set_status(struct bt_avdtp_req *req, struct bt_buf *buf, uint8_t msg_type)
{
	if (msg_type == BT_AVDTP_ACCEPT) {
		req->status = BT_AVDTP_SUCCESS;
	} else if (msg_type == BT_AVDTP_REJECT) {
		if (buf->len >= sizeof(req->status)) {
			req->status = bt_buf_pull_u8(buf);
...
static void avdtp_discover_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	int err;
	...
	if (err) {
		...
		__ASSERT_NO_MSG(bt_buf_tailroom(rsp_buf) >= sizeof(avdtp_err_code));
		bt_buf_add_u8(rsp_buf, avdtp_err_code);
	} else {
		struct bt_avdtp_sep_data sep_data;

		BT_SLIST_FOR_EACH_CONTAINER(&seps, sep, _node) {
			...
			if (bt_buf_tailroom(rsp_buf) >= sizeof(sep_data)) {
				bt_buf_add_mem(rsp_buf, &sep_data, sizeof(sep_data));
			}
		}
	}
...
static void avdtp_discover_rsp(struct bt_avdtp *session, struct bt_buf *buf, uint8_t msg_type)
{
	struct bt_avdtp_req *req = session->req;
	...
	bt_work_cancel_delayable(&session->timeout_work);
...
static struct bt_avdtp_sep *avdtp_get_cmd_sep(struct bt_buf *buf, uint8_t *error_code,
					      uint8_t *seid)
{
	struct bt_avdtp_sep *sep;
	uint8_t id;
	...
	id = AVDTP_SEID_GET(bt_buf_pull_u8(buf));
...
static void avdtp_get_caps_cmd_internal(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid,
					bool get_all_caps)
{
	int err = 0;
	struct bt_buf *rsp_buf;
...
static void avdtp_get_capabilities_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	avdtp_get_caps_cmd_internal(session, buf, tid, false);
}

static void avdtp_get_all_capabilities_cmd(struct bt_avdtp *session,
					   struct bt_buf *buf, uint8_t tid)
{
	avdtp_get_caps_cmd_internal(session, buf, tid, true);
}

static void avdtp_get_capabilities_rsp(struct bt_avdtp *session, struct bt_buf *buf,
				       uint8_t msg_type)
{
	struct bt_avdtp_req *req = session->req;
...
struct bt_avdtp_service_category_handler {
	...
	uint8_t err_code;
	uint8_t (*handler)(struct bt_buf *buf);
};

uint8_t bt_avdtp_check_media_recovery_type(struct bt_buf *buf)
{
...
uint8_t bt_avdtp_check_service_category(struct bt_buf *buf, uint8_t *service_category,
					bool reconfig)
{
...
static void avdtp_process_configuration_cmd(struct bt_avdtp *session, struct bt_buf *buf,
					    uint8_t tid, bool reconfig)
{
	...
	if (sep == NULL) {
		...
		if (!(sep->state & expected_state)) {
			...
			if (!reconfig) {
				...
				int_seid = AVDTP_SEID_GET(bt_buf_pull_u8(buf));
			}
			bt_buf_simple_save(&buf->b, &state);
	...
	if (err) {
		...
		__ASSERT_NO_MSG(bt_buf_tailroom(rsp_buf) >=
				(sizeof(service_category) + sizeof(avdtp_err_code)));
		/* error Service Category*/
		bt_buf_add_u8(rsp_buf, service_category);
		/* ERROR CODE */
		bt_buf_add_u8(rsp_buf, avdtp_err_code);
	}
...
static void avdtp_process_configuration_rsp(struct bt_avdtp *session, struct bt_buf *buf,
...
static void avdtp_set_configuration_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	avdtp_process_configuration_cmd(session, buf, tid, false);
}

static void avdtp_set_configuration_rsp(struct bt_avdtp *session, struct bt_buf *buf,
					uint8_t msg_type)
{
	avdtp_process_configuration_rsp(session, buf, msg_type, false);
}

static void avdtp_re_configure_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	avdtp_process_configuration_cmd(session, buf, tid, true);
}

static void avdtp_re_configure_rsp(struct bt_avdtp *session, struct bt_buf *buf, uint8_t msg_type)
{
	avdtp_process_configuration_rsp(session, buf, msg_type, true);
}

static void avdtp_open_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	int err = 0;
	struct bt_avdtp_sep *sep;
	struct bt_buf *rsp_buf;
...
static void avdtp_open_rsp(struct bt_avdtp *session, struct bt_buf *buf, uint8_t msg_type)
{
	struct bt_avdtp_req *req = session->req;
...
static void avdtp_handle_reject_with_acp_seid(struct bt_buf *buf, struct bt_avdtp_req *req)
{
	if (buf->len >= sizeof(uint8_t)) {
		uint8_t acp_seid;

		acp_seid = bt_buf_pull_u8(buf);
...
static void avdtp_start_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	int err = 0;
	struct bt_avdtp_sep *sep;
	struct bt_buf *rsp_buf;
	...
	if (err) {
		...
		bt_buf_add_u8(rsp_buf, acp_seid);
		bt_buf_add_u8(rsp_buf, avdtp_err_code);
	}
...
static void avdtp_start_rsp(struct bt_avdtp *session, struct bt_buf *buf, uint8_t msg_type)
{
	struct bt_avdtp_req *req = session->req;
...
static void avdtp_close_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	int err = 0;
	struct bt_avdtp_sep *sep;
	struct bt_buf *rsp_buf;
...
static void avdtp_close_rsp(struct bt_avdtp *session, struct bt_buf *buf, uint8_t msg_type)
{
	struct bt_avdtp_req *req = session->req;
...
static void avdtp_suspend_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	int err = 0;
	struct bt_avdtp_sep *sep;
	struct bt_buf *rsp_buf;
...
static void avdtp_suspend_rsp(struct bt_avdtp *session, struct bt_buf *buf, uint8_t msg_type)
{
	struct bt_avdtp_req *req = session->req;
...
static void avdtp_abort_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	int err = 0;
	struct bt_avdtp_sep *sep;
	struct bt_buf *rsp_buf;
...
static void avdtp_abort_rsp(struct bt_avdtp *session, struct bt_buf *buf, uint8_t msg_type)
{
	struct bt_avdtp_req *req = session->req;
...
static void avdtp_delay_report_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	int err = 0;
	struct bt_avdtp_sep *sep;
	struct bt_buf *rsp_buf;
...
static void avdtp_delay_report_rsp(struct bt_avdtp *session, struct bt_buf *buf, uint8_t msg_type)
{
	struct bt_avdtp_req *req = session->req;
...
static void avdtp_get_configuration_cmd(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid)
{
	int err = 0;
	struct bt_buf *rsp_buf;
	struct bt_avdtp_sep *sep;
...
static void avdtp_get_configuration_rsp(struct bt_avdtp *session, struct bt_buf *buf,
					uint8_t msg_type)
{
	struct bt_avdtp_req *req = session->req;
...
/* Timeout handler */
static void avdtp_timeout(struct bt_work *work)
{
...
static int avdtp_send_cmd(struct bt_avdtp *session, struct bt_buf *buf, struct bt_avdtp_req *req)
{
	struct bt_avdtp_single_sig_hdr *hdr;
	...
	bt_work_init_delayable(&session->timeout_work, avdtp_timeout);
	/* Start timeout work */
...
static void avdtp_release_work(struct bt_work *work)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&seps, sep, next, _node) {
...
{
	struct bt_buf *buf, *next;
	struct avdtp_buf_user_data *user_data;

	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&avdtp_tx_list, buf, next, node) {
		user_data = bt_buf_user_data(buf);
...
void bt_avdtp_l2cap_disconnected(struct bt_l2cap_chan *chan)
{
	...
	if (session->reasm_buf != NULL) {
		bt_buf_unref(session->reasm_buf);
	...
	bt_work_submit(&session->_release_work);
}

void (*cmd_handler[])(struct bt_avdtp *session, struct bt_buf *buf, uint8_t tid) = {
...
void (*rsp_handler[])(struct bt_avdtp *session, struct bt_buf *buf, uint8_t msg_type) = {
...
static int bt_avdtp_l2cap_frags_recv(struct bt_avdtp *session, struct bt_buf *buf,
				     uint8_t pack_type, bool *finish)
{
	struct bt_buf *sdu_buf;
	...
	*finish = false;
	...
	if (pack_type == BT_AVDTP_PACKET_TYPE_START) {
		...
		start_len = buf->len;
		start_hdr = bt_buf_pull_mem(buf, sizeof(*start_hdr));
		...
		session->reasm_buf = bt_buf_alloc(&avdtp_pool, OS_TIMEOUT_FOREVER);
		if (session->reasm_buf == NULL) {
		...
		if (total_len > bt_buf_tailroom(sdu_buf)) {
			LOG_ERR("Not enough buffer space");
			return avdtp_rel_and_return(session);
		}

		if (bt_buf_tailroom(sdu_buf) < sizeof(*single_hdr)) {
			return avdtp_rel_and_return(session);
		}

		single_hdr = bt_buf_add(sdu_buf, sizeof(*single_hdr));
		 ...
		 * sdu_buf too.
		 */
		bt_buf_add_mem(sdu_buf, buf->data, buf->len);
	...
	cont_hdr = bt_buf_pull_mem(buf, sizeof(*cont_hdr));
	...
	if (buf->len < bt_buf_tailroom(sdu_buf)) {
		bt_buf_add_mem(sdu_buf, buf->data, buf->len);
	} else {
		LOG_ERR("Not enough buffer space");
		return avdtp_rel_and_return(session);
...
int bt_avdtp_l2cap_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
	struct bt_avdtp_single_sig_hdr *single_hdr;
	struct bt_avdtp *session = AVDTP_CHAN(chan);
	...
	single_hdr = bt_buf_pull_mem(buf, sizeof(*single_hdr));
...
int bt_avdtp_connect(struct bt_conn *conn, struct bt_avdtp *session)
{
	...
	os_sem_init(&session->sem_lock, 1, 1);
	bt_work_init(&session->_release_work, avdtp_release_work);
...
int bt_avdtp_register_sep(uint8_t media_type, uint8_t sep_type, struct bt_avdtp_sep *sep)
{
	...
	os_sem_take(&avdtp_sem_lock, OS_TIMEOUT_FOREVER);
	if (bt_slist_find(&seps, &sep->_node, NULL)) {
		os_sem_give(&avdtp_sem_lock);
	...
	/* Locking semaphore initialized to 1 (unlocked) */
	os_sem_init(&sep->sem_lock, 1, 1);
	bt_work_init_delayable(&sep->_delay_work, avdtp_media_disconnect_work);
	bt_avdtp_set_state_lock(sep, AVDTP_IDLE);

	bt_slist_append(&seps, &sep->_node);
	os_sem_give(&avdtp_sem_lock);
...
void bt_avdtp_init(void)
{
	...
	err = os_sem_init(&avdtp_sem_lock, 1, 1);
	if (err) {
		LOG_ERR("AVDTP semaphore init failed %d", err);
		return;
	}
...
int bt_avdtp_parse_sep(struct bt_buf *buf, struct bt_avdtp_sep_info *sep_info)
{
	struct bt_avdtp_sep_data *sep_data;

	if ((sep_info != NULL) && (buf != NULL)) {
		if (buf->len >= sizeof(*sep_data)) {
			sep_data = bt_buf_pull_mem(buf, sizeof(*sep_data));
			      ...
			      struct bt_avdtp_get_capabilities_params *param)
{
	...
	__ASSERT_NO_MSG(bt_buf_tailroom(buf) >= sizeof(uint8_t));
	/* Body of the message */
	bt_buf_add_u8(buf, AVDTP_SEID_PREP(param->stream_endpoint_id));

	return avdtp_send_cmd(session, buf, &param->req);
}

int bt_avdtp_parse_capability_codec(struct bt_buf *buf, uint8_t *codec_type,
	...
	*codec_info_element_len = 0;
	...
	while (buf->len) {
		...
		case BT_AVDTP_SERVICE_MULTIPLEXING:
			...
			length = bt_buf_pull_u8(buf);
			if (length > 0) {
				if (buf->len < length) {
					return -EINVAL;
				}

				bt_buf_pull_mem(buf, length);
			}
			break;
		...
		case BT_AVDTP_SERVICE_MEDIA_CODEC:
			...
			if (length > 3) {
				...
				if (data == BT_AVDTP_AUDIO) {
					...
					*codec_info_element =
						bt_buf_pull_mem(buf, (*codec_info_element_len));
					return 0;
				}
			}
			break;
					   ...
					   struct bt_avdtp_set_configuration_params *param)
{
	...
	__ASSERT_NO_MSG(bt_buf_tailroom(buf) >= sizeof(uint8_t));
	bt_buf_add_u8(buf, AVDTP_SEID_PREP(param->acp_stream_ep_id));

	if (cmd == BT_AVDTP_SET_CONFIGURATION) {
		if (bt_buf_tailroom(buf) < sizeof(*cap) + sizeof(uint8_t)) {
			goto unref_and_return;
		}

		/* INT Stream Endpoint ID */
		bt_buf_add_u8(buf, AVDTP_SEID_PREP(param->int_stream_endpoint_id));

		/* Service Category: Media Transport */
		cap = bt_buf_add(buf, sizeof(*cap));
		cap->service_category = BT_AVDTP_SERVICE_MEDIA_TRANSPORT;
		cap->losc = 0;
	}

	if (bt_buf_tailroom(buf) < param->codec_specific_ie_len + sizeof(*cap) +
	    sizeof(*media_cap)) {
		goto unref_and_return;
	}
...
int bt_avdtp_delay_report(struct bt_avdtp *session, struct bt_avdtp_delay_report_params *param)
{
	...
	__ASSERT_NO_MSG(bt_buf_tailroom(buf) >= (sizeof(uint8_t) + sizeof(uint16_t)));
	bt_buf_add_u8(buf, AVDTP_SEID_PREP(param->acp_stream_ep_id));
	bt_buf_add_be16(buf, param->delay_report);

	return avdtp_send_cmd(session, buf, &param->req);
}

int bt_avdtp_send_media_data(struct bt_avdtp_sep *sep, struct bt_buf *buf)
{
	int err;
