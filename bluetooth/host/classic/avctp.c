...
/* L2CAP Server list */
static bt_slist_t avctp_l2cap_server = BT_SLIST_STATIC_INIT(&avctp_l2cap_server);
static os_sem_t avctp_lock;

/* tx list for packets */
static bt_slist_t avctp_tx_list = BT_SLIST_STATIC_INIT(&avctp_tx_list);
static struct bt_work_delayable avctp_tx_work;
...
{
	if (bt_slist_is_empty(&avctp_tx_list)) {
...
static void bt_avctp_clear_tx(struct bt_avctp *session)
{
	...
	const bt_snode_t *head = bt_slist_peek_head(&avctp_tx_list);

	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&avctp_tx_list, buf, next, node) {
		...
		bt_slist_find_and_remove(&avctp_tx_list, &buf->node);
		bt_buf_unref(buf);

		if (is_head) {
			avctp_tx_raise(0);
			head = bt_slist_peek_head(&avctp_tx_list);
		}
	}
}
...
static void avctp_l2cap_disconnected(struct bt_l2cap_chan *chan)
{
	...
	if (session->reassembly_buf != NULL) {
		bt_buf_unref(session->reassembly_buf);
		session->reassembly_buf = NULL;
	}

	os_sem_take(&avctp_lock, OS_TIMEOUT_FOREVER);
	bt_avctp_clear_tx(session);
	os_sem_give(&avctp_lock);
...
static void avctp_tx_remove(struct bt_buf *buf)
{
	os_sem_take(&avctp_lock, OS_TIMEOUT_FOREVER);
	bt_slist_find_and_remove(&avctp_tx_list, &buf->node);
	os_sem_give(&avctp_lock);
...
static int send_single_avctp(struct bt_avctp *avctp, uint8_t tid, bt_avctp_cr_t cr, uint8_t ipid,
			     uint16_t pid, struct bt_buf *buf)
{
	...
	if (bt_buf_headroom(buf) < BT_AVCTP_HDR_SIZE_SINGLE) {
		LOG_ERR("Not enough headroom in buffer for AVCTP SINGLE header");
		return -ENOMEM;
	}

	avctp_hdr = bt_buf_push(buf, BT_AVCTP_HDR_SIZE_SINGLE);
				 ...
				 bt_avctp_pkt_type_t pkt_type, uint8_t *data, uint16_t data_len)
{
	...
	case BT_AVCTP_PKT_TYPE_START:
		...
		avctp_hdr_start = bt_buf_push(buf, BT_AVCTP_HDR_SIZE_START);
	...
	case BT_AVCTP_PKT_TYPE_END:
		...
		avctp_hdr_continue_end = bt_buf_push(buf, BT_AVCTP_HDR_SIZE_CONTINUE_END);
	...
	if (data_len > 0U) {
		if (bt_buf_tailroom(buf) < data_len) {
...
static void avctp_tx_processor(struct bt_work *item)
{
	struct bt_avctp *session;
	struct avctp_buf_user_data *user_data;
	...
	os_sem_take(&avctp_lock, OS_TIMEOUT_FOREVER);
	node = bt_slist_peek_head(&avctp_tx_list);
	if (node == NULL) {
		os_sem_give(&avctp_lock);
...
static int bt_avctp_send_common(struct bt_avctp *session, struct bt_buf *buf,
				...
				uint8_t num_packet, uint8_t ipid)
{
	...
	os_sem_take(&avctp_lock, OS_TIMEOUT_FOREVER);
	bt_slist_append(&avctp_tx_list, &buf->node);
	os_sem_give(&avctp_lock);
...
static int dispatch_avctp_packet(struct bt_avctp *session, struct bt_buf *buf,
...
static int avctp_recv_fragmented(struct bt_avctp *avctp, struct bt_buf *buf)
{
	...
	if (pkt_type == BT_AVCTP_PKT_TYPE_START) {
		...
		if (avctp->reassembly_buf != NULL) {
			...
			bt_buf_unref(avctp->reassembly_buf);
		...
		avctp->reassembly_buf = bt_buf_alloc(avctp->rx_pool, OS_TIMEOUT_FOREVER);
		if (avctp->reassembly_buf == NULL) {
		...
		hdr_reassembly = bt_buf_user_data(avctp->reassembly_buf);
		memcpy(hdr_reassembly, bt_buf_pull_mem(buf, sizeof(*hdr_reassembly)),
		       sizeof(*hdr_reassembly));

		if (bt_buf_tailroom(avctp->reassembly_buf) < buf->len) {
			LOG_WRN("Not enough tailroom for start fragment");
			goto failed;
		}
		bt_buf_add_mem(avctp->reassembly_buf, buf->data, buf->len);
	...
	if ((BT_AVCTP_HDR_GET_TRANSACTION_LABLE(&hdr_reassembly->common) == tid) &&
		...
		hdr_common = bt_buf_pull_mem(buf, BT_AVCTP_HDR_SIZE_CONTINUE_END);
...
static int avctp_l2cap_recv(struct bt_l2cap_chan *chan, struct bt_buf *buf)
{
	...
	hdr_single = bt_buf_pull_mem(buf, BT_AVCTP_HDR_SIZE_SINGLE);
...
struct bt_buf *bt_avctp_create_pdu(struct bt_buf_pool *pool)
{
	struct bt_buf *buf;
...
int bt_avctp_send(struct bt_avctp *session, struct bt_buf *buf, bt_avctp_cr_t cr, uint8_t tid)
{
	uint8_t num_packet;
	uint16_t max_payload_size;
			      ...
			      struct bt_l2cap_chan **chan)
{
	...
	if (!bt_slist_find(&avctp_l2cap_server, &avctp_server->node, NULL)) {
...
int bt_avctp_server_register(struct bt_avctp_server *server)
{
	...
	if (bt_slist_find(&avctp_l2cap_server, &server->node, NULL)) {
	...
	bt_slist_append(&avctp_l2cap_server, &server->node);
