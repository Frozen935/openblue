...
#include <bluetooth/services/ans.h>
...
static os_mutex_t new_alert_mutex = OS_MUTEX_INITIALIZER;
static os_mutex_t unread_mutex = OS_MUTEX_INITIALIZER;
...
static int notify_new_alert_category(struct bt_conn *conn, uint8_t cat)
{
	...
	ret = os_mutex_lock(&new_alert_mutex, OS_TIMEOUT_NO_WAIT);
	if (ret != 0) {
	...
	ret = os_mutex_unlock(&new_alert_mutex);
...
static int notify_unread_alert_category(struct bt_conn *conn, uint8_t cat)
{
	...
	ret = os_mutex_lock(&unread_mutex, OS_TIMEOUT_NO_WAIT);
	if (ret != 0) {
		LOG_ERR("Unable to lock mutex (err: %d)", ret);
		return -EAGAIN;
	}
	...
	ret = os_mutex_unlock(&unread_mutex);
	__ASSERT(ret == 0, "Unable to unlock mutex (err: %d)", ret);
			    ...
			    const char *text)
{
	...
	ret = os_mutex_lock(&new_alert_mutex, OS_TIMEOUT_FOREVER);
...
int bt_ans_set_unread_count(struct bt_conn *conn, enum bt_ans_cat category, uint8_t unread)
{
	...
	/* Update the saved value */
	ret = os_mutex_lock(&unread_mutex, OS_TIMEOUT_FOREVER);
	__ASSERT(ret == 0, "Unable to lock mutex (err: %d)", ret);
