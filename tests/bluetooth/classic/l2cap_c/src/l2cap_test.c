...
BT_BUF_POOL_FIXED_DEFINE(l2cap_c_data_tx_pool, 1, BT_L2CAP_SDU_BUF_SIZE(DATA_POOL_SIZE),
			 CONFIG_BT_CONN_TX_USER_DATA_SIZE, NULL);
BT_BUF_POOL_FIXED_DEFINE(l2cap_c_data_rx_pool, 1, DATA_POOL_SIZE, 8, NULL);
...
static struct bt_buf *l2cap_alloc_buf(struct bt_l2cap_chan *chan)
{
	...
	return bt_buf_alloc(&l2cap_c_data_rx_pool, OS_TIMEOUT_NO_WAIT);
}
...
static int cmd_l2cap_send(const struct bt_shell *sh, size_t argc, char *argv[])
{
	...
	if (br_l2cap[id].active == true) {
		buf = bt_buf_alloc(&l2cap_c_data_tx_pool, OS_MSEC(2000));
		if (buf == NULL) {
