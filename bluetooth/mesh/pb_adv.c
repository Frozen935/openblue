...
#define RETRANSMIT_TIMEOUT  OS_MSEC(CONFIG_BT_MESH_PB_ADV_RETRANS_TIMEOUT)
#define BUF_TIMEOUT         OS_MSEC(400)
...
struct pb_adv {
	...
	struct {
		...
		/* Retransmit timer */
		struct bt_work_delayable retransmit;
	...
	/* Protocol timeout */
	struct bt_work_delayable prot_timer;
};
...
BT_BUF_SIMPLE_DEFINE_STATIC(rx_buf, RX_BUFFER_MAX);

static struct pb_adv link = { .rx = { .buf = &rx_buf } };

static void gen_prov_ack_send(uint8_t xact_id);
static void link_open(struct prov_rx *rx, struct bt_buf_simple *buf);
static void link_ack(struct prov_rx *rx, struct bt_buf_simple *buf);
static void link_close(struct prov_rx *rx, struct bt_buf_simple *buf);
static void prov_link_close(enum prov_bearer_link_status status);
static void close_link(enum prov_bearer_link_status status);

static void tx_work_handler(struct bt_work *work);
static BT_WORK_DELAYABLE_DEFINE(tx_work, tx_work_handler);
...
static void tx_schedule(void)
{
	...
	(void)bt_work_schedule(&tx_work, OS_MSEC(random_delay));
}
...
static void tx_work_handler(struct bt_work *work)
{
	...
	if (link.tx.next == ARRAY_SIZE(link.tx.adv) || link.tx.adv[link.tx.next] == NULL) {
		...
		bt_work_reschedule(&link.tx.retransmit, RETRANSMIT_TIMEOUT);
	}
}
...
static void prov_clear_tx(void)
{
	 ...
	 * exit early.
	 */
	(void)bt_work_cancel_delayable(&link.tx.retransmit);
...
static void gen_prov_ack_send(uint8_t xact_id)
{
	...
	bt_buf_simple_add_be32(&adv->b, link.id);
	bt_buf_simple_add_u8(&adv->b, xact_id);
	bt_buf_simple_add_u8(&adv->b, GPC_ACK);
...
static void gen_prov_cont(struct prov_rx *rx, struct bt_buf_simple *buf)
{
...
static void gen_prov_ack(struct prov_rx *rx, struct bt_buf_simple *buf)
{
...
static void gen_prov_start(struct prov_rx *rx, struct bt_buf_simple *buf)
{
	...
	bt_buf_simple_reset(link.rx.buf);
	link.rx.buf->len = bt_buf_simple_pull_be16(buf);
	link.rx.id = rx->xact_id;
	link.rx.fcs = bt_buf_simple_pull_u8(buf);
...
static void gen_prov_ctl(struct prov_rx *rx, struct bt_buf_simple *buf)
{
...
static const struct {
	void (*func)(struct prov_rx *rx, struct bt_buf_simple *buf);
	bool require_link;
	uint8_t min_len;
} gen_prov[] = {
...
static void gen_prov_recv(struct prov_rx *rx, struct bt_buf_simple *buf)
{
...
static void prov_retransmit(struct bt_work *work)
{
	LOG_DBG("");
					  ...
					  uint8_t retransmits)
{
	...
	bt_buf_simple_add_u8(&adv->b, 0x00);
	bt_buf_simple_add_u8(&adv->b, GPC_CTL(op));
	bt_buf_simple_add_mem(&adv->b, data, data_len);
...
static int prov_send_adv(struct bt_buf_simple *msg,
			 prov_bearer_send_complete_t cb, void *cb_data)
{
	...
	bt_buf_simple_add_be32(&start->b, link.id);
	bt_buf_simple_add_u8(&start->b, link.tx.id);

	bt_buf_simple_add_u8(&start->b, GPC_START(last_seg(msg->len)));
	bt_buf_simple_add_be16(&start->b, msg->len);
	bt_buf_simple_add_u8(&start->b, bt_mesh_fcs_calc(msg->data, msg->len));
	...
	bt_buf_simple_add_mem(&start->b, msg->data, seg_len);
	bt_buf_simple_pull(msg, seg_len);
	...
	for (seg_id = 1U; msg->len > 0; seg_id++) {
		...
		bt_buf_simple_add_be32(&adv->b, link.id);
		bt_buf_simple_add_u8(&adv->b, link.tx.id);
		bt_buf_simple_add_u8(&adv->b, GPC_CONT(seg_id));
		bt_buf_simple_add_mem(&adv->b, msg->data, seg_len);
		bt_buf_simple_pull(msg, seg_len);
	}
...
static void link_close(struct prov_rx *rx, struct bt_buf_simple *buf)
{
	...
	close_link(bt_buf_simple_pull_u8(buf));
}
...
void bt_mesh_pb_adv_recv(struct bt_buf_simple *buf)
{
	struct prov_rx rx;
	...
	rx.link_id = bt_buf_simple_pull_be32(buf);
...
{
	bt_work_init_delayable(&link.prot_timer, protocol_timeout);
	bt_work_init_delayable(&link.tx.retransmit, prov_retransmit);
}
