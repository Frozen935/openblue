...
static void notify_work_handler(struct bt_work *work)
{
	...
	if (err == -ENOMEM &&
	    bt_work_reschedule(d_work, OS_USEC(BT_AUDIO_NOTIFY_RETRY_DELAY_US)) >= 0) {
			  ...
			  uint8_t flags)
{
	...
	if (*val != micp_inst.mute) {
		...
		err = bt_work_reschedule(&micp_inst.notify_work, OS_TIMEOUT_NO_WAIT);
		if (err < 0) {
...
int bt_micp_mic_dev_register(struct bt_micp_mic_dev_register_param *param)
{
	...
	bt_work_init_delayable(&micp_inst.notify_work, notify_work_handler);
