			  ...
			  struct bt_audio_codec_cfg *codec_cfg)
{
	...
	if (conn != NULL) {
		__ASSERT_MSG(stream->conn == NULL || stream->conn == conn,
...
static int bap_stream_send(struct bt_bap_stream *stream, struct bt_buf *buf, uint16_t seq_num,
...
int bt_bap_stream_send(struct bt_bap_stream *stream, struct bt_buf *buf, uint16_t seq_num)
{
	return bap_stream_send(stream, buf, seq_num, 0, false);
}

int bt_bap_stream_send_ts(struct bt_bap_stream *stream, struct bt_buf *buf, uint16_t seq_num,
			  uint32_t ts)
{
...
static uint8_t conn_get_role(const struct bt_conn *conn)
{
	...
	__ASSERT_MSG(err == 0, "Failed to get conn info");
			 ...
			 struct bt_audio_codec_cfg *codec_cfg)
{
	...
	__ASSERT_MSG(ep->iso == NULL, "endpoint %p already bound to iso %p", ep, ep->iso);
...
int bt_bap_stream_qos(struct bt_conn *conn, struct bt_bap_unicast_group *group)
{
	...
	if (bt_slist_is_empty(&group->streams)) {
