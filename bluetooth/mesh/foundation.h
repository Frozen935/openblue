...
static inline void key_idx_pack_pair(struct bt_buf_simple *buf, uint16_t idx1, uint16_t idx2)
{
	bt_buf_simple_add_le16(buf, idx1 | ((idx2 & 0x00f) << 12));
	bt_buf_simple_add_u8(buf, idx2 >> 4);
}

static inline void key_idx_unpack_pair(struct bt_buf_simple *buf, uint16_t *idx1, uint16_t *idx2)
{
