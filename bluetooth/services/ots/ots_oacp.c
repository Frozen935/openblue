...
static enum bt_gatt_ots_oacp_res_code oacp_checksum_proc_validate(
	...
	struct bt_ots *ots,
	struct bt_gatt_ots_oacp_proc *proc,
	struct bt_buf_simple *resp_param)
{
	...
	if (ots->cb->obj_cal_checksum) {
		...
		bt_buf_simple_add_le32(resp_param, checksum);
			       ...
			       struct bt_gatt_ots_oacp_proc *proc)
{
	...
	proc->type = bt_buf_simple_pull_u8(&bt_buf);
	switch (proc->type) {
#if defined(CONFIG_BT_OTS_OACP_CREATE_SUPPORT)
	case BT_GATT_OTS_OACP_PROC_CREATE:
		if (bt_buf.len < BT_GATT_OTS_OACP_CREATE_GENERIC_PARAMS_SIZE) {
			return -EBADMSG;
		}
		proc->create_params.size = bt_buf_simple_pull_le32(&bt_buf);
		if (!bt_uuid_create(&proc->create_params.type.uuid, bt_buf.data,
				    bt_buf.len)) {
			return -EBADMSG;
		}
		bt_buf_simple_pull_mem(&bt_buf, bt_buf.len);
...
#endif
	case BT_GATT_OTS_OACP_PROC_CHECKSUM_CALC:
		if (bt_buf.len != BT_GATT_OTS_OACP_CS_CALC_PARAMS_SIZE) {
			return -EBADMSG;
		}
	...
	case BT_GATT_OTS_OACP_PROC_EXECUTE:
		...
		return 0;
	case BT_GATT_OTS_OACP_PROC_READ:
		if (bt_buf.len != BT_GATT_OTS_OACP_READ_PARAMS_SIZE) {
			return -EBADMSG;
		}
...
#if defined(CONFIG_BT_OTS_OACP_WRITE_SUPPORT)
	case BT_GATT_OTS_OACP_PROC_WRITE:
		if (bt_buf.len != BT_GATT_OTS_OACP_WRITE_PARAMS_SIZE) {
			return -EBADMSG;
		}
			 ...
			 struct bt_buf_simple *resp_param)
{
	...
	if (oacp_proc.type == BT_GATT_OTS_OACP_PROC_CHECKSUM_CALC) {
		sys_put_le32(bt_buf_simple_pull_le32(resp_param), (oacp_res + oacp_res_len));
	...
	bt_work_submit(&ots->oacp_ind.work);
}
				   ...
				   uint16_t offset, uint8_t flags)
{
	...
	BT_BUF_SIMPLE_DEFINE(resp_param, sizeof(uint32_t));
	...
	if (bt_work_is_pending(&ots->oacp_ind.work)) {
