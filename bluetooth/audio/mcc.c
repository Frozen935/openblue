...
BT_BUF_SIMPLE_DEFINE_STATIC(otc_obj_buf, CONFIG_BT_MCC_OTC_OBJ_BUF_SIZE);
				     ...
				     struct bt_gatt_discover_params *params)
{
	...
	if (attr) {
		...
		__ASSERT_MSG(params->type == BT_GATT_DISCOVER_INCLUDE,
			 "Wrong type");
		     ...
		     struct bt_conn *conn, int result)
{
	...
	bt_buf_simple_reset(&otc_obj_buf);
		    ...
		    bool is_complete)
{
	...
	if (len > bt_buf_simple_tailroom(&otc_obj_buf)) {
		LOG_WRN("Can not fit whole object");
		cb_err = -EMSGSIZE;
	}

	bt_buf_simple_add_mem(&otc_obj_buf, data_p,
			       MIN(bt_buf_simple_tailroom(&otc_obj_buf), len));
...
static void decode_track_segments(struct bt_buf_simple *buff,
				  struct track_segs_t *track_segs)
{
	uint16_t i;
	struct track_seg_t *seg;
	uint8_t *name;
	struct bt_buf_simple tmp_buf;

	/* Copy the buf, to not consume the original in this debug function */
	bt_buf_simple_clone(buff, &tmp_buf);
	...
	while (tmp_buf.len &&
		...
		seg->name_len =  bt_buf_simple_pull_u8(&tmp_buf);
		...
		if (seg->name_len) {
			...
			name = bt_buf_simple_pull_mem(&tmp_buf, seg->name_len);
		...
		track_segs->segs[i].pos = (int32_t)bt_buf_simple_pull_le32(&tmp_buf);
	}
}
...
static void decode_group(struct bt_buf_simple *buff,
			 struct id_list_t *ids)
{
	...
	while ((tmp_buf.len) && (ids->cnt < CONFIG_BT_MCC_GROUP_RECORDS_MAX)) {
		ids->ids[ids->cnt].type = bt_buf_simple_pull_u8(&tmp_buf);
		ids->ids[ids->cnt++].id = bt_buf_simple_pull_le48(&tmp_buf);
	}
}
#endif /* CONFIG_BT_MCC_LOG_LEVEL_DBG */
