...
static bt_slist_t sink_cbs = BT_SLIST_STATIC_INIT(&sink_cbs);
				       ...
				       enum bt_bap_ep_state state)
{
	...
	if (bt_slist_is_empty(&sink->streams)) {
		LOG_DBG("Sink does not have any streams");

		return state == BT_BAP_EP_STATE_IDLE;
	}

	BT_SLIST_FOR_EACH_CONTAINER(&sink->streams, stream, _node) {
					    ...
					    uint8_t reason)
{
	...
	if (sink == NULL) {
		...
		if (!bt_slist_find_and_remove(&sink->streams, &stream->_node)) {
...
static bool pa_decode_base(struct bt_data *data, void *user_data)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER(&sink_cbs, listener, _node) {
...
int bt_bap_broadcast_sink_register_cb(struct bt_bap_broadcast_sink_cb *cb)
{
	...
	if (bt_slist_find(&sink_cbs, &cb->_node, NULL)) {
	...
	bt_slist_append(&sink_cbs, &cb->_node);
...
static void broadcast_sink_cleanup_streams(struct bt_bap_broadcast_sink *sink)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&sink->streams, stream, next, _node) {
		if (stream->ep != NULL) {
		...
		bt_slist_remove(&sink->streams, NULL, &stream->_node);
	}
			       ...
			       const uint8_t broadcast_code[BT_ISO_BROADCAST_CODE_SIZE])
{
	...
	for (size_t i = 0; i < stream_count; i++) {
		...
		bt_slist_append(&sink->streams, &stream->_node);
		sink->stream_count++;
