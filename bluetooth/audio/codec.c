...
static int ltv_set_val(struct bt_buf_simple *buf, uint8_t type, const uint8_t *data,
		       size_t data_len)
{
	size_t new_buf_len;
	...
	if (new_buf_len <= buf->size) {
		bt_buf_simple_add_u8(buf, data_len + sizeof(type)); /* len */
		bt_buf_simple_add_u8(buf, type); /* type */
		if (data_len > 0) {
			bt_buf_simple_add_mem(buf, data, data_len); /* value */
		}
	} else {
...
static int ltv_unset_val(struct bt_buf_simple *buf, uint8_t type)
{
	for (uint16_t i = 0U; i < buf->len;) {
...
static void init_bt_buf_simple_from_codec_cfg(struct bt_buf_simple *buf,
			       ...
			       size_t data_len)
{
	...
	init_bt_buf_simple_from_codec_cfg(&buf, codec_cfg);
...
static void init_bt_buf_simple_from_meta(struct bt_buf_simple *buf, uint8_t meta[],
			      ...
			      size_t data_len)
{
	...
	init_bt_buf_simple_from_meta(&buf, meta, meta_len, meta_size);
...
static void init_bt_buf_simple_from_codec_cap(struct bt_buf_simple *buf,
					       struct bt_audio_codec_cap *codec_cap)
{
			       ...
			       size_t data_len)
{
	...
	init_bt_buf_simple_from_codec_cap(&buf, codec_cap);
